...
use swc_atoms::Atom;
use swc_common::{BytePos, DUMMY_SP, EqIgnoreSpan, Span, Spanned, SyntaxContext, ast_node, util::take::Take};

use crate::{
	ArrayPat, BindingIdent, ComputedPropName, Id, IdentName, ImportPhase, Invalid, KeyValueProp, Number, ObjectPat,
	PropName, Str,
	class::Class,
	function::Function,
	...
	stmt::BlockStmt,
	typescript::{
		TsAsExpr, TsConstAssertion, TsInstantiation, TsNonNullExpr, TsSatisfiesExpr, TsTypeAnn, TsTypeAssertion,
...
impl Expr {
	...
	pub fn unwrap_parens_mut(&mut self) -> &mut Expr {
		self.unwrap_mut_with(|e| if let Expr::Paren(expr) = e { Some(&mut expr.expr) } else { None })
	}
...
impl Take for ArrayLit {
	fn dummy() -> Self {
		ArrayLit { span: DUMMY_SP, elems: Default::default() }
	}
}
...
impl ImportWith {
	pub fn get(&self, key: &str) -> Option<&Str> {
		self.values
			.iter()
			.find_map(|item| if item.key.sym == key { Some(&item.value) } else { None })
	}
}
...
impl Take for ObjectLit {
	fn dummy() -> Self {
		ObjectLit { span: DUMMY_SP, props: Default::default() }
	}
}
...
impl Take for SpreadElement {
	fn dummy() -> Self {
		SpreadElement { dot3_token: DUMMY_SP, expr: Take::dummy() }
	}
}
...
impl Take for UnaryExpr {
	fn dummy() -> Self {
		UnaryExpr { span: DUMMY_SP, op: op!("!"), arg: Take::dummy() }
	}
}
...
impl Take for ClassExpr {
	fn dummy() -> Self {
		ClassExpr { ident: None, class: Take::dummy() }
	}
}
...
impl Take for MemberExpr {
	fn dummy() -> Self {
		MemberExpr { span: DUMMY_SP, obj: Take::dummy(), prop: Take::dummy() }
	}
}
...
impl Take for SeqExpr {
	fn dummy() -> Self {
		SeqExpr { span: DUMMY_SP, exprs: Take::dummy() }
	}
}
...
impl Take for YieldExpr {
	fn dummy() -> Self {
		YieldExpr { span: DUMMY_SP, arg: Take::dummy(), delegate: false }
	}
}
...
impl Take for Tpl {
	fn dummy() -> Self {
		Tpl { span: DUMMY_SP, exprs: Take::dummy(), quasis: Take::dummy() }
	}
}
...
impl Take for ParenExpr {
	fn dummy() -> Self {
		ParenExpr { span: DUMMY_SP, expr: Take::dummy() }
	}
}
...
impl Take for Import {
	fn dummy() -> Self {
		Import { span: DUMMY_SP, phase: ImportPhase::default() }
	}
}
...
impl SimpleAssignTarget {
	pub fn leftmost(&self) -> Option<Cow<Ident>> {
		match self {
			SimpleAssignTarget::Ident(i) => Some(Cow::Owned(Ident::new(i.sym.clone(), i.span, i.ctxt))),
