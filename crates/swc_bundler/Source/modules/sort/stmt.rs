...
pub(super) fn sort_stmts(
	...
	let orders = iter(&mut id_graph, &same_module_ranges, free_range, &module_starts, &stmts).collect::<Vec<_>>();
...
fn iter<'a>(
	...
	from_fn(move || {
		...
		'main: while let Some(idx) = stack.pop_front() {
			...
			let current_range = same_module_ranges.iter().find(|range| range.contains(&idx)).cloned();

			// dbg!(&current_range);

			let can_ignore_deps = match &stmts[idx] {
				ModuleItem::ModuleDecl(ModuleDecl::ExportDecl(ExportDecl { decl: Decl::Fn(..), .. }))
				| ModuleItem::Stmt(Stmt::Decl(Decl::Fn(..))) => true,

				ModuleItem::ModuleDecl(ModuleDecl::ExportDecl(ExportDecl { decl: Decl::Class(cls), .. }))
				| ModuleItem::Stmt(Stmt::Decl(Decl::Class(cls)))
			...
			let can_ignore_weak_deps = can_ignore_deps
				|| matches!(
					&stmts[idx],
					ModuleItem::ModuleDecl(ModuleDecl::ExportDecl(ExportDecl { decl: Decl::Class(..), .. }))
				...
				if !deps.is_empty() {
					...
					for dep in deps.iter().rev().copied() {
						...
						let can_ignore_dep = can_ignore_deps
							|| (can_ignore_weak_deps && graph.edge_weight(idx, dep) == Some(Required::Maybe));
			...
			Callee::Expr(callee) => {
				if let Expr::Member(callee) = &**callee {
					if callee.obj.is_ident_ref_to("Object") {
						match &callee.prop {
							MemberProp::Ident(IdentName { sym: prop_sym, .. }) if *prop_sym == *"assign" => {
	...
	fn visit_member_expr(&mut self, e: &MemberExpr) {
		...
		if !self.in_rhs || self.in_object_assign {
			if let Expr::Ident(obj) = &*e.obj {
				match &e.prop {
					MemberProp::Ident(IdentName { sym: prop_sym, .. }) if *prop_sym == *"prototype" => {
...
fn calc_deps(new: &[ModuleItem]) -> StmtDepGraph {
	...
	for (uninit_id, start_idx) in uninitialized_ids {
		for (idx, item) in new.iter().enumerate().filter(|(idx, _)| *idx > start_idx) {
			let mut finder = InitializerFinder { ident: uninit_id.clone(), found: false, in_complex: false };
				...
				for &declarator_index in declarator_indexes {
					if declarator_index != idx {
						...
						if cfg!(debug_assertions) {
							assert!(graph.neighbors_directed(idx, Dependencies).any(|x| x == declarator_index));
						}
					}
				}
			}
			...
			for i in 0..module.body.len() {
				...
				for dep in deps {
					...
					print_hygiene(
						"first item",
						&t.cm,
						&Module { span: DUMMY_SP, body: vec![module.body[dep].clone()], shebang: None },
					);
					print_hygiene(
						"second item",
						&t.cm,
					...
					assert!(!graph.has_a_path(dep, i), "{} and {} is dependant to each other", dep, i,);
				}
			}
