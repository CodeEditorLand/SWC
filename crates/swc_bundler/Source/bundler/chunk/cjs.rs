	...
	R: Resolve,
{
	...
	pub(super) fn replace_cjs_require_calls(&self, info: &TransformedModule, module: &mut Modules, is_entry: bool) {
		if !self.config.require {
			return;
		}

		let mut v = RequireReplacer { is_entry, base: info, bundler: self, replaced: false };
	...
	pub(super) fn wrap_cjs_module(
		...
		module.visit_mut_with(&mut DefaultHandler { local_ctxt: info.local_ctxt() });
		...
		let wrapped = Modules::from(
			info.id,
			Module { span: DUMMY_SP, body: vec![stmt], shebang: None },
			self.injected_ctxt,
		);
...
struct Remapper {
	vars: FxHashMap<Id, SyntaxContext>,
}
