...
use crate::{Bundle, bundler::chunk::merge::Ctx, load::Load, resolve::Resolve, util::IntoParallelIterator};
	...
	R: Resolve,
{
	...
	pub(super) fn chunk(&self, entries: FxHashMap<String, TransformedModule>) -> Result<Vec<Bundle>, Error> {
		...
		if cfg!(debug_assertions) {
			for (i, id1) in plan.all.iter().enumerate() {
				for (j, id2) in plan.all.iter().enumerate() {
					...
					debug_assert_ne!(id1, id2, "Dependency analysis created duplicate entries: {:?}", id1)
				}
			}
		}
			...
			entries
				...
				.map(|(id, module)| {
					let kind = plan
						.entries
						.get(&id)
						.unwrap_or_else(|| unreachable!("Plan does not contain bundle kind for {:?}", id))
						.clone();
					Bundle { kind, id, module: module.into() }
				})
				.collect()
		} else {
			entries
...
mod tests {
	...
	fn cjs_chunk() {
		suite()
			...
			.run(|t| {
				let module = t.bundler.load_transformed(&FileName::Real("main.js".into()))?.unwrap();
