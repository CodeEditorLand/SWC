...
impl Ctx {
	pub fn is_exported_ctxt(&self, ctxt_to_check: SyntaxContext, entry_export_ctxt: SyntaxContext) -> bool {
...
where
	...
	R: Resolve,
{
	pub(super) fn get_for_merging(&self, ctx: &Ctx, id: ModuleId, is_entry: bool) -> Result<Modules, Error> {
		self.run(|| {
			let info = self
				.scope
				.get_module(id)
	...
	pub(super) fn merge_into_entry(
		...
		self.run(|| {
			...
			let all_deps_of_entry = self.collect_all_deps(&ctx.graph, entry_id, &mut Default::default());
			...
			let deps = all_deps_of_entry.iter().map(|id| {
				...
				all.remove(id).unwrap_or_else(|| {
					unreachable!("failed to merge into {}: module {} does not exist in the map", entry_id, id)
				})
			});
	...
	pub(crate) fn apply_hooks(&self, module_id: ModuleId, is_entry: bool) -> Result<Modules, Error> {
		self.run(|| {
	...
	fn inject_reexports(&self, ctx: &Ctx, _entry_id: ModuleId, entry: &mut Modules) {
...

		{
			...
			fn add_var(
				...
				add_var(injected_ctxt, vars, declared, map, module_id, reexported.clone());
				...
				vars.push((
					module_id,
					id.assign_to(reexported).into_module_item(injected_ctxt, "export_star_replacer"),
				));
			}
											...
											ModuleExportName::Ident(orig) => {
												vars.push((
													...
													orig.clone()
														...
														.into_module_item(injected_ctxt, "finalize -> export to var"),
												));
											},
											ModuleExportName::Str(..) => {
												unimplemented!("module string names unimplemented")
						...
						for id in ids {
							...
							if ctxts.contains(&id.ctxt()) {
								additional_props
									.entry(module_id)
									.or_default()
									.push(PropOrSpread::Prop(Box::new(Prop::Shorthand(id.into_ident()))));
							}
						}
					}
				}
				...
				for (_, stmt) in entry.iter_mut() {
					let var = match stmt {
						ModuleItem::Stmt(Stmt::Decl(Decl::Var(var)))
							if matches!(&**var, VarDecl { kind: VarDeclKind::Const, .. }) =>
						{
							var
						},
						_ => continue,
		...
		entry.retain_mut(|_, item| {
			match item {
				...
				ModuleItem::ModuleDecl(ModuleDecl::ExportNamed(export)) => {
					...
					export.specifiers.retain(|s| !matches!(s, ExportSpecifier::Namespace(_)));
	...
	fn remove_wrong_exports(&self, ctx: &Ctx, info: &TransformedModule, module: &mut Modules) {
		...
		module.retain_mut(|_, item| {
			if let ModuleItem::ModuleDecl(ModuleDecl::ExportNamed(NamedExport { specifiers, .. })) = item {
				specifiers.retain(|s| match s {
					ExportSpecifier::Named(ExportNamedSpecifier { exported: Some(exported), .. }) => {
						let exported = match exported {
							ModuleExportName::Ident(ident) => ident,
							ModuleExportName::Str(..) => {
								unimplemented!("module string names unimplemented")
	...
	fn handle_imports_and_exports(&self, ctx: &Ctx, info: &TransformedModule, module: &mut Modules) {
		let injected_ctxt = self.injected_ctxt;
			...
			for item in items {
				match item {
					ModuleItem::ModuleDecl(ModuleDecl::Import(mut import)) => {
						...
						if let Some((src, _)) =
							info.imports.specifiers.iter().find(|s| s.0.src.value == import.src.value)
						{
									...
									if let Some(imported) = &s.imported {
										...
										new.push(
											imported
												...
												.into_module_item(injected_ctxt, "prepare -> named import -> aliased"),
										);
									}
								},
								ImportSpecifier::Default(s) => {
									new.push(
										Ident::new(
											"default".into(),
											import.span,
											ExportMetadata::decode(import.with.as_deref()).export_ctxt.unwrap(),
										)
										.assign_to(s.local.clone())
										.into_module_item(injected_ctxt, "prepare -> default import"),
									);
								},
										...
										info.imports.specifiers.iter().find(|s| s.0.src.value == import.src.value)
									{
										...
										new.push(
											esm_id
												.clone()
												.assign_to(s.local.clone())
												.into_module_item(injected_ctxt, "prepare -> import -> namespace"),
										);
									}
								},
							}
					...
					ModuleItem::ModuleDecl(ModuleDecl::ExportDefaultDecl(export)) => {
						...
						match export.decl {
							DefaultDecl::Class(c) => {
								...
								match c.ident {
									Some(ident) => {
										new.push(
											ClassDecl { ident: ident.clone(), class: c.class, declare: false }.into(),
										);
							...
							DefaultDecl::Fn(f) => {
								...
								match f.ident {
									Some(ident) => {
										new.push(
											FnDecl { ident: ident.clone(), function: f.function, declare: false }
												.into(),
										);
						...
						tracing::trace!("Exporting `default` with `export default decl` ({})", local.sym);
						...
						extra.push(
							NamedExport {
								...
								src: None,
								type_only: false,
								with: Some(ExportMetadata { injected: true, ..Default::default() }.into_with()),
							}
							.into(),
						);
					},
					...
					ModuleItem::ModuleDecl(ModuleDecl::ExportDecl(export)) => {
							...
							Decl::Var(v) => {
								...
								let export = NamedExport {
									...
									specifiers: ids
										.into_iter()
										.map(|id| {
											let exported = Ident::new(id.sym.clone(), id.span, info.export_ctxt());

											tracing::trace!("Exporting `{}{:?}` with `export decl`", id.sym, id.ctxt);
						...
						let exported = Ident::new(local.sym.clone(), local.span, info.export_ctxt());
					...
					ModuleItem::ModuleDecl(ModuleDecl::ExportNamed(NamedExport {
						ref specifiers, ref src, ..
					})) => {
						if let Some(export_src) = src {
							if let Some((src, _)) =
								info.exports.reexports.iter().find(|s| s.0.src.value == export_src.value)
							{
								...
								if !dep.is_es6 {
									...
									vars.push(VarDeclarator {
										...
										init: Some(
											CallExpr {
												span: DUMMY_SP,
												callee: Ident::new("load".into(), DUMMY_SP, dep.export_ctxt())
													.as_callee(),
												..Default::default()
											}
											.into(),
										...
										match s {
											...
											ExportSpecifier::Default(s) => {
												vars.push(VarDeclarator {
													...
													init: Some(
														mod_var.clone().make_member(quote_ident!("default")).into(),
													),
													definite: Default::default(),
												});
											},
											ExportSpecifier::Named(s) => {
												...
												vars.push(VarDeclarator {
													...
													init: Some(mod_var.clone().make_member(orig.clone().into()).into()),
													definite: Default::default(),
												});
											},
										}
								...
								ExportSpecifier::Named(ExportNamedSpecifier {
									...
									new.push(
										orig_ident
											...
											.into_module_item(injected_ctxt, "prepare -> export named -> aliased"),
									);
								},
										...
										.find(|s| s.0.src.value == src.as_ref().unwrap().value)
									{
										match &ns.name {
											ModuleExportName::Ident(name) => {
												...
												let wrapped_esm_id = self.scope.wrapped_esm_id(src.module_id);
												match wrapped_esm_id {
													Some(module_var) => {
														...
														let specifier = ExportSpecifier::Named(ExportNamedSpecifier {
															span: ns.span,
															orig: ModuleExportName::Ident(module_var.into()),
						...
						match specifier {
								...
								if let Some(imported) = &named.imported {
									...
									vars.push((
										...
										imporeted_ident
											...
											.into_module_item(injected_ctxt, "from_replace_import_specifiers"),
									));
									continue;
								}
							},
							ImportSpecifier::Default(default) => {
								if let Some((src, _)) =
									info.imports.specifiers.iter().find(|s| s.0.src.value == import.src.value)
								{
									let imported = Ident::new("default".into(), DUMMY_SP, src.export_ctxt);
									vars.push((
										module_id,
										imported
...
impl VisitMut for ImportMetaHandler<'_, '_> {
	fn visit_mut_expr(&mut self, e: &mut Expr) {
		...
		if let Expr::MetaProp(MetaPropExpr { kind: MetaPropKind::ImportMeta, .. }) = e {
	...
	fn visit_mut_module(&mut self, n: &mut Module) {
		...
		if self.occurred {
			match self.hook.get_import_meta_props(
				n.span,
				&ModuleRecord { file_name: self.file.to_owned(), is_entry: self.is_entry },
			) {
				Ok(key_value_props) => {
					prepend_stmt(
						&mut n.body,
