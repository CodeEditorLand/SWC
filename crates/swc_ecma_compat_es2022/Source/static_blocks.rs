...
impl ClassStaticBlock {
	fn transform_static_block(&mut self, mut static_block: StaticBlock, private_id: Atom) -> PrivateProp {
		...
		PrivateProp {
			span,
			is_static: true,
			key: PrivateName { span: PLACEHOLDER_SP, name: private_id },
			value,
			..Default::default()
		}
	}
	...
	fn visit_mut_class(&mut self, class: &mut Class) {
		...
		for member in class.body.iter_mut() {
			if let ClassMember::StaticBlock(static_block) = member {
				...
				*member = self.transform_static_block(static_block.take(), static_block_private_id).into();
			};
		}
	}
}
