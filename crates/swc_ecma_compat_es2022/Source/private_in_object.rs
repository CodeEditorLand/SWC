...
use swc_atoms::Atom;
use swc_common::{DUMMY_SP, Mark, Spanned, SyntaxContext, pass::CompilerPass, util::take::Take};
use swc_ecma_ast::*;
use swc_ecma_utils::{ExprFactory, default_constructor_with_span, prepend_stmt, private_ident, quote_ident};
...
#[derive(Debug)]
enum Mode {
	ClassExpr { vars: Vec<VarDeclarator>, init_exprs: Vec<Box<Expr>> },
	ClassDecl { vars: Vec<VarDeclarator> },
}

impl Default for Mode {
	fn default() -> Self {
		Self::ClassDecl { vars: Default::default() }
	}
}
		...
		match self {
			Mode::ClassExpr { vars, init_exprs } => {
				...
				if let Some(init) = init {
					init_exprs.push(AssignExpr { span: DUMMY_SP, op: op!("="), left: n.into(), right: init }.into());
				}
			},
			Mode::ClassDecl { vars } => {
				vars.push(VarDeclarator { span: DUMMY_SP, name: n.into(), init, definite: Default::default() });
			},
		}
	}
}
...
impl VisitMut for PrivateInObject {
	...
	fn visit_mut_class(&mut self, n: &mut Class) {
		...
		if !self.cls.constructor_exprs.is_empty() {
			let has_constructor = n.body.iter().any(|m| matches!(m, ClassMember::Constructor(_)));

			if !has_constructor {
				let has_super = n.super_class.is_some();
				n.body
					.push(ClassMember::Constructor(default_constructor_with_span(has_super, n.span)));
			}

			for m in &mut n.body {
				if let ClassMember::Constructor(Constructor { body: Some(body), .. }) = m {
					for expr in take(&mut self.cls.constructor_exprs) {
						body.stmts.push(ExprStmt { span: DUMMY_SP, expr }.into());
					}
				}
			}
		}
	...
	fn visit_mut_class_decl(&mut self, n: &mut ClassDecl) {
		...
		self.cls.vars = Mode::ClassDecl { vars: Default::default() };
	...
	fn visit_mut_class_expr(&mut self, n: &mut ClassExpr) {
		...
		self.cls.vars = Mode::ClassExpr { vars: Default::default(), init_exprs: Default::default() };
	...
	fn visit_mut_assign_pat(&mut self, p: &mut AssignPat) {
...

		{
			...
			let mut v = ClassAnalyzer { brand_check_names: &mut buf, ignore_class: false };
			p.right.visit_with(&mut v);

			if buf.is_empty() {
				p.right.visit_mut_with(self);
			} else {
				let mut bs = BlockStmt { span: DUMMY_SP, stmts: Vec::new(), ..Default::default() };
				bs.stmts.push(ReturnStmt { span: DUMMY_SP, arg: Some(p.right.take()) }.into());
	...
	fn visit_mut_expr(&mut self, e: &mut Expr) {
		...
		if !prepend_exprs.is_empty() {
			match e {
				...
				_ => {
					...
					*e = SeqExpr { span: DUMMY_SP, exprs: prepend_exprs }.into();
				},
			}
			return;
		}
		...
		match e {
			Expr::Bin(BinExpr { span, op: op!("in"), left, right }) if left.is_private_name() => {
				...
				if let Some(cls_ident) = self.cls.ident.clone() {
					if is_static && is_method {
						*e =
							BinExpr { span: *span, op: op!("==="), left: cls_ident.into(), right: right.take() }.into();
						return;
					}
				}
				...
				if self.cls.privates.contains(&left.name) && self.injected_vars.insert(var_name.to_id()) {
					...
					if is_method {
						self.cls.constructor_exprs.push(
							CallExpr {
								span: DUMMY_SP,
								callee: var_name.clone().make_member(quote_ident!("add")).as_callee(),
		...
		if self.cls.names_used_for_brand_checks.contains(&n.key.name) {
			...
			match &mut n.value {
				Some(init) => {
					...
					let assign =
						AssignExpr { span: DUMMY_SP, op: op!("="), left: tmp.clone().into(), right: init.take() }
							.into();
					...
					*init =
						SeqExpr { span: init_span, exprs: vec![assign, add_to_checker, Box::new(tmp.into())] }.into();
				},
				None => {
					n.value = Some(
						UnaryExpr {
