...
use swc_ecma_ast::*;
use swc_ecma_visit::{
	Visit, VisitMut, VisitMutWith, VisitWith, noop_visit_mut_type, noop_visit_type, visit_mut_obj_and_computed,
...
pub use self::{
	...
	value::{
		Merge,
		Type::{
			self, Bool as BoolType, Null as NullType, Num as NumberType, Obj as ObjectType, Str as StringType,
...
pub trait StmtExt {
	...
	fn terminates(&self) -> bool {
		fn terminates_many(stmts: &[Stmt], allow_break: bool, allow_throw: bool) -> bool {
			stmts.iter().rev().any(|s| terminates(s, allow_break, allow_throw))
		}
	...
	fn may_have_side_effects(&self, ctx: ExprCtx) -> bool {
		fn may_have_side_effects(stmt: &Stmt, ctx: ExprCtx) -> bool {
			match stmt {
				Stmt::Block(block_stmt) => block_stmt.stmts.iter().any(|stmt| stmt.may_have_side_effects(ctx)),
				Stmt::Empty(_) => false,
...
pub fn num_from_str(s: &str) -> Value<f64> {
	...
	if (s.starts_with('-') || s.starts_with('+')) && (s[1..].starts_with("0x") || s[1..].starts_with("0X")) {
...
pub fn is_simple_pure_expr(expr: &Expr, pure_getters: bool) -> bool {
	match expr {
		...
		Expr::Unary(UnaryExpr { op: op!("void") | op!("!"), arg, .. }) => is_simple_pure_expr(arg, pure_getters),
...
pub fn is_simple_pure_member_expr(m: &MemberExpr, pure_getters: bool) -> bool {
	match &m.prop {
		MemberProp::Ident(..) | MemberProp::PrivateName(..) => is_simple_pure_expr(&m.obj, pure_getters),
...
fn sym_for_expr(expr: &Expr) -> Option<String> {
	match expr {
		...
		Expr::OptChain(OptChainExpr { base, .. }) => match &**base {
			...
			OptChainBase::Member(MemberExpr { prop: MemberProp::Ident(ident), obj, .. }) => {
				Some(format!("{}_{}", sym_for_expr(obj).unwrap_or_default(), ident.sym))
			},

			OptChainBase::Member(MemberExpr {
				prop: MemberProp::Computed(ComputedPropName { expr, .. }), obj, ..
			}) => Some(format!(
				"{}_{}",
		...
		Expr::SuperProp(SuperPropExpr { prop: SuperProp::Ident(ident), .. }) => Some(format!("super_{}", ident.sym)),

		Expr::SuperProp(SuperPropExpr { prop: SuperProp::Computed(ComputedPropName { expr, .. }), .. }) => {
		...
		Expr::Member(MemberExpr { prop: MemberProp::Computed(ComputedPropName { expr, .. }), obj, .. }) => {
			Some(format!(
				"{}_{}",
				sym_for_expr(obj).unwrap_or_default(),
				sym_for_expr(expr).unwrap_or_default()
...
pub fn prop_name_to_member_prop(prop_name: PropName) -> MemberProp {
	match prop_name {
		...
		PropName::BigInt(b) => MemberProp::Computed(ComputedPropName { span: DUMMY_SP, expr: b.into() }),
	}
}
...
pub fn opt_chain_test(left: Box<Expr>, right: Box<Expr>, span: Span, no_document_all: bool) -> Expr {
	if no_document_all {
		...
		BinExpr {
			...
			.into(),
			op: op!("||"),
			right: BinExpr { span: DUMMY_SP, left: right, op: op!("==="), right: Expr::undefined(DUMMY_SP) }.into(),
		}
		.into()
	}
}
...
impl ExprCtx {
	pub fn consume_depth(self) -> Option<Self> {
		...
		Some(Self { remaining_depth: self.remaining_depth - 1, ..self })
	}
	...
	pub fn extract_side_effects_to(self, to: &mut Vec<Box<Expr>>, expr: Expr) {
		match expr {
			...
			Expr::Update(_) | Expr::Assign(_) | Expr::Yield(_) | Expr::Await(_) => to.push(Box::new(expr)),
			...
			Expr::Seq(SeqExpr { exprs, .. }) => exprs.into_iter().for_each(|e| self.extract_side_effects_to(to, *e)),
...
where
	...
	N: VisitWith<BindingCollector<I>>,
{
	let mut v = BindingCollector { only: Some(ctxt), bindings: Default::default(), is_pat_decl: false };
	n.visit_with(&mut v);
	v.bindings
}
...
impl Visit for TopLevelAwait {
	...
	fn visit_class_member(&mut self, prop: &ClassMember) {
		match prop {
			...
			| ClassMember::Method(ClassMethod { key: PropName::Computed(computed), .. }) => computed.visit_children_with(self),
			_ => (),
		};
	}
	...
	fn visit_prop(&mut self, prop: &Prop) {
		match prop {
			...
			| Prop::Method(MethodProp { key: PropName::Computed(computed), .. }) => computed.visit_children_with(self),
			_ => {},
		}
	}
				...
				match p.value.take() {
					Some(default) => {
						*i = ObjectPatProp::KeyValue(KeyValuePatProp {
							...
							value: AssignPat { span: DUMMY_SP, left: p.key.clone().into(), right: default }.into(),
						});
					},
					None => {
						*i = ObjectPatProp::KeyValue(KeyValuePatProp {
	...
	T: QueryRef,
{
	...
	fn visit_mut_callee(&mut self, n: &mut Callee) {
		match n {
			Callee::Expr(e) if e.as_ident().map(|ident| self.query.should_fix_this(ident)).unwrap_or_default() => {
...
fn is_immutable_value(expr: &Expr) -> bool {
	...
	match *expr {
		Expr::Lit(Lit::Bool(..)) | Expr::Lit(Lit::Str(..)) | Expr::Lit(Lit::Num(..)) | Expr::Lit(Lit::Null(..)) => true,
...
fn is_str(expr: &Expr) -> bool {
	match expr {
		...
		Expr::Bin(BinExpr { op: op!(bin, "+"), left, right, .. }) => left.is_str() || right.is_str(),
...
fn is_void(expr: &Expr) -> bool {
	matches!(*expr, Expr::Unary(UnaryExpr { op: op!("void"), .. }))
}
...
fn cast_to_bool(expr: &Expr, ctx: ExprCtx) -> (Purity, BoolValue) {
	...
	let val = match expr {
		...
		Expr::Fn(..) | Expr::Class(..) | Expr::New(..) | Expr::Array(..) | Expr::Object(..) => Known(true),
		...
		Expr::Lit(ref lit) => {
			return (
				Pure,
				Known(match *lit {
					Lit::Num(Number { value: n, .. }) => !matches!(n.classify(), FpCategory::Nan | FpCategory::Zero),
					Lit::BigInt(ref v) => v.value.to_string().contains(|c: char| matches!(c, '1'..='9')),
					Lit::Bool(b) => b.value,
...
fn cast_to_number(expr: &Expr, ctx: ExprCtx) -> (Purity, Value<f64>) {
	...
	let v = match expr {
		...
		Expr::Unary(UnaryExpr { op: op!(unary, "-"), arg, .. }) => match arg.cast_to_number(ctx) {
			(Pure, Known(v)) => -v,
			_ => return (MayBeImpure, Unknown),
		},
		Expr::Unary(UnaryExpr { op: op!("!"), ref arg, .. }) => match arg.cast_to_bool(ctx) {
			(Pure, Known(v)) => {
				if v {
					0.0
				} else {
...
fn as_pure_string(expr: &Expr, ctx: ExprCtx) -> Value<Cow<'_, str>> {
	...
	match *expr {
		...
		Expr::Ident(Ident { ref sym, ctxt, .. }) => match &**sym {
			"undefined" | "Infinity" | "NaN" if ctxt == ctx.unresolved_ctxt => Known(Cow::Borrowed(&**sym)),
			_ => Unknown,
		},
		Expr::Unary(UnaryExpr { op: op!("void"), .. }) => Known(Cow::Borrowed("undefined")),
		Expr::Unary(UnaryExpr { op: op!("!"), ref arg, .. }) => Known(Cow::Borrowed(match arg.as_pure_bool(ctx) {
			Known(v) => {
				if v {
					"false"
				} else {
...
fn get_type(expr: &Expr, ctx: ExprCtx) -> Value<Type> {
	...
	match expr {
		Expr::Assign(AssignExpr { ref right, op: op!("="), .. }) => right.get_type(ctx),

		Expr::Member(MemberExpr { obj, prop: MemberProp::Ident(IdentName { sym: length, .. }), .. })
			if &**length == "length" =>
		{
			match &**obj {
				Expr::Array(ArrayLit { .. }) | Expr::Lit(Lit::Str(..)) => Known(Type::Num),
				Expr::Ident(Ident { sym: arguments, .. }) if &**arguments == "arguments" => Known(Type::Num),
				_ => Unknown,
			}
		},

		Expr::Seq(SeqExpr { ref exprs, .. }) => {
			exprs.last().expect("sequence expression should not be empty").get_type(ctx)
		},

		Expr::Bin(BinExpr { ref left, op: op!("&&"), ref right, .. })
		| Expr::Bin(BinExpr { ref left, op: op!("||"), ref right, .. })
		| Expr::Cond(CondExpr { cons: ref left, alt: ref right, .. }) => and(left.get_type(ctx), right.get_type(ctx)),
		...
		Expr::Unary(UnaryExpr { op: op!("typeof"), .. }) | Expr::Lit(Lit::Str { .. }) | Expr::Tpl(..) => {
			Known(StringType)
		},
	...
	match expr {
		Expr::Member(MemberExpr { obj, prop: MemberProp::Ident(prop), .. }) => {
			obj.is_global_ref_to(ctx, "Math")
				|| match &**obj {
					// Allow dummy span
					Expr::Ident(Ident { ctxt, sym: math, .. }) => &**math == "Math" && *ctxt == SyntaxContext::empty(),
					...
					Expr::Lit(Lit::Str(..)) => match &*prop.sym {
						...
						| "localeCompare" | "slice" | "split" | "startsWith" | "substr" | "substring"
...
fn may_have_side_effects(expr: &Expr, ctx: ExprCtx) -> bool {
	...
	match expr {
		...
		Expr::Bin(BinExpr { left, right, .. }) => left.may_have_side_effects(ctx) || right.may_have_side_effects(ctx),
					...
					let can_have_side_effect = |prop: &PropOrSpread| match prop {
						...
						PropOrSpread::Prop(prop) => match prop.as_ref() {
							...
							| Prop::KeyValue(KeyValueProp {
								key: PropName::Ident(IdentName { sym, .. }) | PropName::Str(Str { value: sym, .. }),
								..
							}) => &**sym == "__proto__",
							Prop::KeyValue(KeyValueProp { key: PropName::Computed(_), .. }) => true,
							_ => false,
						},
					};
		...
		Expr::Await(_) | Expr::Yield(_) | Expr::Member(_) | Expr::SuperProp(_) | Expr::Update(_) | Expr::Assign(_) => {
			true
		},

		Expr::OptChain(OptChainExpr { base, .. }) if matches!(&**base, OptChainBase::Member(_)) => true,

		// TODO
		Expr::New(_) => true,

		Expr::Call(CallExpr { callee: Callee::Expr(callee), ref args, .. }) if callee.is_pure_callee(ctx) => {
		...
		Expr::Cond(CondExpr { test, cons, alt, .. }) => {
			test.may_have_side_effects(ctx) || cons.may_have_side_effects(ctx) || alt.may_have_side_effects(ctx)
		},
...
mod tests {
	...
	fn parse_module(text: &str) -> Module {
		...
		let mut p = Parser::new(syntax, StringInput::new(text, BytePos(0), BytePos(text.len() as u32)), None);
