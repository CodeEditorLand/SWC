...
pub trait ExprFactory: Into<Box<Expr>> {
	...
	fn as_arg(self) -> ExprOrSpread {
		ExprOrSpread { expr: self.into(), spread: None }
	}

	/// Creates an expression statement with `self`.
	#[cfg_attr(not(debug_assertions), inline(always))]
	fn into_stmt(self) -> Stmt {
		ExprStmt { span: DUMMY_SP, expr: self.into() }.into()
	}

	/// Creates a statement whcih return `self`.
	#[cfg_attr(not(debug_assertions), inline(always))]
	fn into_return_stmt(self) -> ReturnStmt {
		ReturnStmt { span: DUMMY_SP, arg: Some(self.into()) }
	}
	...
	fn into_lazy_arrow(self, params: Vec<Pat>) -> ArrowExpr {
		ArrowExpr { params, body: Box::new(BlockStmtOrExpr::Expr(self.into())), ..Default::default() }
	}
	...
	fn into_var_decl(self, kind: VarDeclKind, name: Pat) -> VarDecl {
		let var_declarator = VarDeclarator { span: DUMMY_SP, name, init: Some(self.into()), definite: false };
	...
	fn as_fn_decl(self) -> Option<FnDecl> {
		match *self.into() {
			Expr::Fn(FnExpr { ident: Some(ident), function }) => Some(FnDecl { ident, declare: false, function }),
			_ => None,
		}
	}
	...
	fn as_class_decl(self) -> Option<ClassDecl> {
		match *self.into() {
			Expr::Class(ClassExpr { ident: Some(ident), class }) => Some(ClassDecl { ident, declare: false, class }),
			_ => None,
		}
	}
...
impl IntoIndirectCall for TaggedTpl {
	...
	fn into_indirect(mut self) -> Self {
		Self {
			tag: Box::new(SeqExpr { span: DUMMY_SP, exprs: vec![0f64.into(), self.tag.take()] }.into()),
			..self
		}
	}
}
...
pub trait FunctionFactory: Into<Box<Function>> {
	...
	fn into_fn_expr(self, ident: Option<Ident>) -> FnExpr {
		FnExpr { ident, function: self.into() }
	}
