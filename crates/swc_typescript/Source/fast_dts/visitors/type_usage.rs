...
use swc_ecma_ast::{
	Accessibility, ArrowExpr, Class, ClassMember, Decl, ExportDecl, ExportDefaultDecl, ExportDefaultExpr, Function, Id,
	Ident, ModuleExportName, ModuleItem, NamedExport, TsEntityName, TsExportAssignment, TsExprWithTypeArgs,
...
impl TypeUsageAnalyzer<'_> {
	pub fn analyze(module_items: &Vec<ModuleItem>, internal_annotations: Option<&FxHashSet<BytePos>>) -> FxHashSet<Id> {
		// Create a fake entry
	...
	pub fn with_source<F: FnMut(&mut TypeUsageAnalyzer)>(&mut self, id: Option<NodeIndex>, mut f: F) {
		// If id is None, we use the nearest scope
		let old_source = self.source.replace(id.unwrap_or(*self.scope_entries.last().expect("No scope")));
		f(self);
...
impl Visit for TypeUsageAnalyzer<'_> {
		...
		match node {
			...
			Decl::TsModule(ts_module_decl) => {
				if ts_module_decl.global || ts_module_decl.id.is_str() {
					...
					self.with_source(Some(self.scope_entries[0]), |this| ts_module_decl.body.visit_with(this));
	...
	fn visit_module_items(&mut self, node: &[ModuleItem]) {
		for item in node {
			...
			if item.as_stmt().map_or(false, |stmt| !stmt.is_decl()) || self.has_internal_annotation(item.span_lo()) {
				continue;
			}
