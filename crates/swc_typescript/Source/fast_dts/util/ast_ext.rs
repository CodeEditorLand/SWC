...
use swc_atoms::Atom;
use swc_ecma_ast::{BindingIdent, Expr, Ident, Lit, MemberProp, ObjectPatProp, Pat, PropName, TsTypeAnn};
		...
		match self {
			...
			Pat::Object(object_pat) => {
				for pat in &object_pat.props {
					match pat {
						ObjectPatProp::KeyValue(key_value_pat_prop) => key_value_pat_prop.value.bound_names(f),
	...
	fn static_name(&self) -> Option<&Atom> {
		match self {
			...
			MemberProp::Computed(computed_prop_name) => match computed_prop_name.expr.as_ref() {
				...
				Expr::Tpl(tpl) if tpl.quasis.len() == 1 && tpl.exprs.is_empty() => Some(&tpl.quasis[0].raw),
				_ => None,
			},
