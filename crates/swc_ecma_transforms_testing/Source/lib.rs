...
use testing::{CARGO_TARGET_DIR, CARGO_WORKSPACE_ROOT, NormalizedOutput, assert_eq, find_executable};
...
impl Tester<'_> {
		...
		let out = ::testing::run_test(false, |cm, handler| {
			HANDLER.set(handler, || {
				HELPERS.set(&Default::default(), || {
					...
					swc_common::comments::COMMENTS.set(&c, || op(&mut Tester { cm, handler, comments }))
				})
			})
		});
		...
		F: FnOnce(&mut Tester<'_>) -> Result<T, ()>,
	{
		...
		let output = ::testing::Tester::new().print_errors(|cm, handler| -> Result<(), _> {
			HANDLER.set(&handler, || {
				HELPERS.set(&Default::default(), || {
					let result = op(&mut Tester { cm, handler: &handler, comments: Default::default() });
		...
		let output = output.err().unwrap_or_else(|| NormalizedOutput::from(String::from("")));
	...
	pub fn apply_transform<T: Pass>(
		...
		let program = self.with_parser(name, syntax, src, |parser: &mut Parser<Lexer>| match is_module {
...
impl VisitMut for RegeneratorHandler {
	...
	fn visit_mut_module_item(&mut self, item: &mut ModuleItem) {
		if let ModuleItem::ModuleDecl(ModuleDecl::Import(import)) = item {
			...
			let decl = VarDeclarator { span: DUMMY_SP, name: s.into(), init: Some(init), definite: Default::default() };
			*item = VarDecl {
				span: import.span,
...
#[track_caller]
pub fn test_transform<F, P>(syntax: Syntax, is_module: Option<bool>, tr: F, input: &str, expected: &str)
where
	F: FnOnce(&mut Tester) -> P,
	P: Pass,
{
	Tester::run(|tester| {
		let expected = tester.apply_transform(swc_ecma_utils::DropSpan, "output.js", syntax, is_module, expected)?;
		...
		match ::std::env::var("PRINT_HYGIENE") {
			Ok(ref s) if s == "1" => {
				let hygiene_src =
					tester.print(&actual.clone().fold_with(&mut HygieneVisualizer), &tester.comments.clone());
				println!("----- Hygiene -----\n{}", hygiene_src);
			},
			_ => {},
		}
...

		{
			...
			if actual == expected && *actual_leading == *expected_leading && *actual_trailing == *expected_trailing {
				return Ok(());
			}
		}
...
#[doc(hidden)]
#[track_caller]
pub fn test_inline_input_output<F, P>(syntax: Syntax, is_module: Option<bool>, tr: F, input: &str, output: &str)
where
	F: FnOnce(&mut Tester) -> P,
	P: Pass,
{
	...
	let expected_src = Tester::run(|tester| {
		let expected_program = tester.apply_transform(noop_pass(), "expected.js", syntax, is_module, expected)?;

		let expected_src = tester.print(&expected_program, &Default::default());

		println!("----- {} -----\n{}", Color::Green.paint("Expected"), expected_src);
	...
	let actual_src = Tester::run_captured(|tester| {
		...
		match ::std::env::var("PRINT_HYGIENE") {
			Ok(ref s) if s == "1" => {
				let hygiene_src = tester.print(&actual.clone().fold_with(&mut HygieneVisualizer), &Default::default());
				println!("----- {} -----\n{}", Color::Green.paint("Hygiene"), hygiene_src);
			},
			_ => {},
		}
	...
	assert_eq!(expected_src, actual_src, "Exepcted:\n{expected_src}\nActual:\n{actual_src}\n",);
}

/// NOT A PUBLIC API. DO NOT USE.
#[doc(hidden)]
#[track_caller]
pub fn test_inlined_transform<F, P>(test_name: &str, syntax: Syntax, module: Option<bool>, tr: F, input: &str)
where
	F: FnOnce(&mut Tester) -> P,
	P: Pass,
{
	...
	test_fixture_inner(
		...
		FixtureTestConfig { module, ..Default::default() },
	)
}
...
fn test_inline_should_fail() {
	test_inline_input_output(Default::default(), None, |_| noop_pass(), "class Foo {}", "");
}
...
macro_rules! test {
	...
	(script, $syntax:expr, $tr:expr, $test_name:ident, $input:expr) => {
		#[test]
		fn $test_name() {
			$crate::test_inlined_script_transform(stringify!($test_name), $syntax, Some(false), $tr, $input)
		}
	};
	...
	P: Pass,
{
	Tester::run(|tester| {
		...
		match ::std::env::var("PRINT_HYGIENE") {
			Ok(ref s) if s == "1" => {
				let hygiene_src =
					tester.print(&program.clone().fold_with(&mut HygieneVisualizer), &tester.comments.clone());
				println!("----- Hygiene -----\n{}", hygiene_src);
			},
			_ => {},
		}

		let mut program = program.apply(hygiene::hygiene()).apply(fixer::fixer(Some(&tester.comments)));
...
fn stdout_of(code: &str) -> Result<String, Error> {
	exec_node_js(code, JsExecOptions { cache: true, module: false, ..Default::default() })
}
...
impl Fold for HygieneTester {
	fn fold_ident(&mut self, ident: Ident) -> Ident {
		Ident { sym: format!("{}__{}", ident.sym, ident.ctxt.as_u32()).into(), ..ident }
	}
	...
	P: Pass,
{
	...
	test_fixture_inner(syntax, Box::new(|tester| Box::new(tr(tester))), &input, output, config);
}
...
fn test_fixture_inner<'a>(
	...
	let expected_src = Tester::run(|tester| {
		let expected_program = tester.apply_transform(noop_pass(), "expected.js", syntax, config.module, &expected)?;
	...
	let mut src_map = if config.sourcemap { Some(Vec::new()) } else { None };
	...
	let (actual_src, stderr) = Tester::run_captured(|tester| {
		...
		let actual_src = {
			...
			if let Some(src_map) = &mut src_map {
				sourcemap = Some(tester.cm.build_source_map_with_config(src_map, None, SourceMapConfigImpl));
			}
	...
	if config.allow_error {
		stderr.compare_to_file(output.with_extension("stderr")).unwrap();
	...
	if let Some(actual_src) = actual_src {
		...
		if actual_src != expected_src {
			NormalizedOutput::from(actual_src).compare_to_file(output).unwrap();
		}
	}
