...
pub(super) fn prepare_vars(
	...
	for var in vars {
		...
		let pos = match var.ty {
			Some(syn::Type::Path(syn::TypePath { qself: None, path: syn::Path { leading_colon: None, segments } })) => {
			...
			None => VarPos::Ident,
			_ => {
				panic!("Var type should be one of: Ident, Expr, Pat; got {:?}", var.ty)
			},
		};
		...
		let old = init_map.entry(pos).or_default().insert(
			ident_str.clone(),
			VarData { pos, is_counting: true, clone: Default::default(), ident: var_ident.clone() },
		);
