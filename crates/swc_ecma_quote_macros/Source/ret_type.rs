			...
			match &*ident.to_string() {
				...
				"AssignTarget" => {
					return parse(input_str, &mut |p| {
						Ok(AssignTarget::try_from(p.parse_pat()?).expect("failed to parse AssignTarget"))
					});
				},
...
fn parse<T>(input_str: &str, op: &mut dyn FnMut(&mut Parser<Lexer>) -> PResult<T>) -> Result<BoxWrapper, Error>
where
	T: ToCode,
{
	...
	let lexer = Lexer::new(Default::default(), EsVersion::Es2020, StringInput::from(&*fm), None);
