	...
	I: ParserInput,
{
	...
	pub(super) fn parse_generic_value(&mut self) -> PResult<ComponentValue> {
		...
		match cur!(self) {
			...
			Token::Function { value, .. } => match &*value.to_ascii_lowercase() {
				...
				| "device-cmyk" | "color-mix" | "color-contrast" => {
			...
			tok!("[") | tok!("(") | tok!("{") => {
				...
				block.value = match self.parse_according_to_grammar(&locv, |parser| parser.parse_generic_values()) {
					Ok(values) => values,
					Err(err) => {
	...
	pub fn parse_function_values(&mut self, function_name: &FunctionName) -> PResult<Vec<ComponentValue>> {
		let function_name = match function_name {
			...
			FunctionName::DashedIdent(_) => return Err(Error::new(Default::default(), ErrorKind::Ignore)),
		};
		...
		match &*lower_fname {
			"calc" | "-moz-calc" | "-webkit-calc" | "sin" | "cos" | "tan" | "asin" | "acos" | "atan" | "sqrt"
			| "exp" | "abs" | "sign" => {
				self.input.skip_ws();
				...
				match &*lower_fname {
					"rgb" | "rgba" => {
						let percentage_or_number_or_none = self.try_parse_variable_function(
							|parser, has_variable_before| match cur!(parser) {
								tok!("percentage") => Ok(Some(ComponentValue::Percentage(parser.parse()?))),
					...
					"hsl" | "hsla" => {
						let hue_or_none = self.try_parse_variable_function(
							|parser, has_variable_before| match cur!(parser) {
								tok!("number") | tok!("dimension") => Ok(Some(ComponentValue::Hue(parser.parse()?))),
								tok!("ident") => {
									let ident: Box<Ident> = parser.parse()?;
						...
						|parser, has_variable_before| match cur!(parser) {
							...
							_ => {
								if !has_variable_before {
									Err(Error::new(
										parser.input.cur_span(),
										ErrorKind::Expected("percentage, function (math functions) or number token"),
									))
								} else {
									Ok(None)
								}
						...
						|parser, has_variable_before| match cur!(parser) {
							...
							tok!("ident") => {
								...
								if ident.value.eq_ignore_ascii_case("none") {
									...
									Err(Error::new(ident.span, ErrorKind::Expected("'none' value of an ident token")))
								}
							},
							_ => {
								if !has_variable_before {
			...
			"hwb" | "lab" | "lch" | "oklab" | "oklch" | "device-cmyk" => {
				...
				match cur!(self) {
					Token::Ident { value, .. }
						if matches_eq_ignore_ascii_case!(value, "from") && lower_fname != "device-cmyk" =>
					{
						values.push(ComponentValue::Ident(self.parse()?));
			...
			"color" => {
				...
				let ident = self.try_parse_variable_function(
					|parser, _| match cur!(parser) {
						...
						_ => Err(Error::new(parser.input.cur_span(), ErrorKind::Expected("ident token"))),
					},
					&mut has_variable,
				)?;
				...
				let number_or_percentage_or_none = self.try_parse_variable_function(
					|parser, has_variable_before| match cur!(parser) {
						tok!("number") => Ok(Some(ComponentValue::Number(parser.parse()?))),
						tok!("percentage") if !is_xyz => Ok(Some(ComponentValue::Percentage(parser.parse()?))),
						Token::Function { value, .. } if is_math_function(value) => {
							Ok(Some(ComponentValue::Function(parser.parse()?)))
						},
						tok!("ident") => {
						...
						let number_or_percentage_or_none = match cur!(self) {
							...
							Token::Function { value, .. }
								if is_math_function(value)
									|| matches_eq_ignore_ascii_case!(value, "var", "env", "constant") =>
							{
				...
				let value = match self.try_parse(|p| p.parse_generic_value()) {
					...
					None => {
						if is_one_of!(self, ";", ":") || (self.config.legacy_ie && is!(self, "=")) {
							...
							return Err(Error::new(self.input.cur_span(), ErrorKind::Expected("Declaration value")));
						}
					},
				};
		...
		F: FnMut(&mut Parser<I>, bool) -> PResult<Option<ComponentValue>>,
	{
		...
		match cur!(self) {
			Token::Function { value, .. } if matches_eq_ignore_ascii_case!(value, "var", "env", "constant") => {
	...
	I: ParserInput,
{
	fn parse(&mut self) -> PResult<Delimiter> {
		...
		if !is_one_of!(self, ",", "/", ";") {
			return Err(Error::new(span, ErrorKind::Expected("',', '/' or ';' delim token")));
		}
		...
		match cur!(self) {
			tok!(",") => {
				...
				return Ok(Delimiter { span: span!(self, span.lo), value: DelimiterValue::Comma });
			},
			tok!("/") => {
				bump!(self);

				return Ok(Delimiter { span: span!(self, span.lo), value: DelimiterValue::Solidus });
			},
			tok!(";") => {
				bump!(self);

				return Ok(Delimiter { span: span!(self, span.lo), value: DelimiterValue::Semicolon });
			},
			_ => {
				unreachable!();
			},
	...
	I: ParserInput,
{
	fn parse(&mut self) -> PResult<Number> {
		...
		match value {
			Token::Number { value, raw, .. } => Ok(Number { span, value, raw: Some(raw) }),
			_ => {
				unreachable!()
			},
		}
	...
	fn parse(&mut self) -> PResult<CustomIdent> {
		...
		match bump!(self) {
			Token::Ident { value, raw, .. } => {
				if matches_eq_ignore_ascii_case!(value, "initial", "inherit", "unset", "revert", "default") {
					return Err(Error::new(span, ErrorKind::InvalidCustomIdent(value)));
				}

				Ok(CustomIdent { span, value, raw: Some(raw) })
			},
			_ => {
				unreachable!()
			},
	...
	I: ParserInput,
{
	fn parse(&mut self) -> PResult<DashedIdent> {
		...
		match bump!(self) {
			Token::Ident { value, raw, .. } => {
				if !value.starts_with("--") {
					return Err(Error::new(span, ErrorKind::Expected("'--' at the start of dashed-ident")));
				}
				...
				Ok(DashedIdent { span, value: self.input.atom(&value[2..]), raw: Some(raw) })
			},
			_ => {
				unreachable!()
			},
	...
	fn parse(&mut self) -> PResult<CustomPropertyName> {
		...
		match bump!(self) {
			Token::Ident { value, raw, .. } => {
				...
				Ok(CustomPropertyName { span, value, raw: Some(raw) })
			},
			_ => {
				unreachable!()
			},
	...
	I: ParserInput,
{
	fn parse(&mut self) -> PResult<Ident> {
		...
		match bump!(self) {
			Token::Ident { value, raw, .. } => Ok(Ident { span, value, raw: Some(raw) }),
			_ => {
				unreachable!()
			},
		}
	...
	I: ParserInput,
{
	fn parse(&mut self) -> PResult<Length> {
		...
		match bump!(self) {
			Token::Dimension(dimension_token) => {
				let DimensionToken { value, unit, raw_value, raw_unit, .. } = *dimension_token;
		...
		match bump!(self) {
			Token::Dimension(dimension_token) => {
				...
				if !is_angle_unit(&unit) {
					return Err(Error::new(span, ErrorKind::Expected("'deg', 'grad', 'rad' or 'turn' units")));
				}
		...
		match bump!(self) {
			Token::Dimension(dimension_token) => {
				...
				if !is_resolution_unit(&unit) {
					return Err(Error::new(span, ErrorKind::Expected("'dpi', 'dpcm', 'dppx' or 'x' units")));
				}
	...
	I: ParserInput,
{
	fn parse(&mut self) -> PResult<Color> {
		...
		match cur!(self) {
			...
			Token::Ident { value, .. }
				if value.as_ref().eq_ignore_ascii_case("currentcolor") || is_system_color(value) =>
			{
		...
		match bump!(self) {
			Token::Hash { value, raw, .. } => {
				if value.chars().any(|x| !x.is_ascii_hexdigit()) {
					return Err(Error::new(span, ErrorKind::Unexpected("character in hex color")));
				}

				Ok(HexColor { span, value, raw: Some(raw) })
			},
			_ => {
				unreachable!()
			},
	...
	I: ParserInput,
{
	fn parse(&mut self) -> PResult<AlphaValue> {
		if !is_one_of!(self, "percentage", "number") {
			...
			return Err(Error::new(span, ErrorKind::Expected("percentage or number token")));
		}
	...
	I: ParserInput,
{
	fn parse(&mut self) -> PResult<Hue> {
		if !is_one_of!(self, "number", "dimension") {
			...
			return Err(Error::new(span, ErrorKind::Expected("number or dimension token")));
		}
	...
	I: ParserInput,
{
	fn parse(&mut self) -> PResult<CmykComponent> {
		if !is_one_of!(self, "number", "percentage", "function") {
			...
			return Err(Error::new(span, ErrorKind::Expected("number, function or percentage token")));
		}
	...
	fn parse(&mut self) -> PResult<Percentage> {
		...
		match bump!(self) {
			Token::Percentage { value, raw } => {
				let value = Number { span: Span::new(span.lo, span.hi - BytePos(1)), value, raw: Some(raw) };
	...
	I: ParserInput,
{
	fn parse(&mut self) -> PResult<Str> {
		...
		match bump!(self) {
			Token::String { value, raw } => Ok(Str { span, value, raw: Some(raw) }),
			_ => {
				unreachable!()
			},
		}
	...
	fn parse(&mut self) -> PResult<Url> {
		...
		match bump!(self) {
			Token::Url { value, raw } => {
				...
				Ok(Url { span: span!(self, span.lo), name, value, modifiers: None })
			},
			Token::Function { value: function_name, raw: raw_function_name } => {
				if !matches_eq_ignore_ascii_case!(function_name, "url", "src") {
					return Err(Error::new(span, ErrorKind::Expected("'url' or 'src' name of a function token")));
				}
				...
				Ok(Url { span: span!(self, span.lo), name, value, modifiers: Some(modifiers) })
			},
			_ => {
				unreachable!()
			},
	...
	I: ParserInput,
{
	fn parse(&mut self) -> PResult<UnicodeRange> {
		...
		match cur!(self) {
			...
			tok!("dimension") => {
				let raw = match bump!(self) {
					Token::Dimension(dimension_token) => (dimension_token.raw_value, dimension_token.raw_unit),
					_ => {
						unreachable!();
					},
				};
			...
			_ => {
				return Err(Error::new(span, ErrorKind::Expected("dimension, number or '?' delim token")));
			},
		}
		...
		if len == 0 || len > 6 {
			return Err(Error::new(
				span,
				ErrorKind::Expected("valid length (minimum 1 or maximum 6 hex digits) in the end of unicode range"),
			));
		}
	...
	I: ParserInput,
{
	fn parse(&mut self) -> PResult<CalcSum> {
		...
		Ok(CalcSum { span: Span::new(start, end), expressions })
	}
}
	...
	I: ParserInput,
{
	fn parse(&mut self) -> PResult<CalcProduct> {
		...
		Ok(CalcProduct { span: Span::new(start, end), expressions })
	}
}
	...
	fn parse(&mut self) -> PResult<CalcOperator> {
		...
		match cur!(self) {
			tok!("+") => {
				...
				Ok(CalcOperator { span: span!(self, span.lo), value: CalcOperatorType::Add })
			},
			tok!("-") => {
				bump!(self);

				Ok(CalcOperator { span: span!(self, span.lo), value: CalcOperatorType::Sub })
			},
			tok!("*") => {
				bump!(self);

				Ok(CalcOperator { span: span!(self, span.lo), value: CalcOperatorType::Mul })
			},
			tok!("/") => {
				bump!(self);

				Ok(CalcOperator { span: span!(self, span.lo), value: CalcOperatorType::Div })
			},
			_ => {
				let span = self.input.cur_span();

				return Err(Error::new(span, ErrorKind::Expected("'+', '-', '*' or '/' delim tokens")));
			},
		}
	}
}
	...
	fn parse(&mut self) -> PResult<CalcValue> {
		match cur!(self) {
			...
			_ => {
				...
				return Err(Error::new(
					span,
					ErrorKind::Expected("'number', 'dimension', 'percentage', 'ident', '(' or 'function' tokens"),
				));
			},
		}
	}
	...
	fn parse(&mut self) -> PResult<FamilyName> {
		match cur!(self) {
			...
			_ => {
				...
				return Err(Error::new(span, ErrorKind::Expected("'string' or 'ident' tokens")));
			},
		}
	}
}
...
fn is_length_unit(unit: &Atom) -> bool {
	matches_eq_ignore_ascii_case!(
		...
		"vw", "svw", "lvw", "dvw", "vh", "svh", "lvh", "dvh", "vi", "svi", "lvi", "dvi", "vb", "svb", "lvb", "dvb",
		"vmin", "svmin", "lvmin", "dvmin", "vmax", "svmax", "lvmax", "dvmax", // Absolute lengths
