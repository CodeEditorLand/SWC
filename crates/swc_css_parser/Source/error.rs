...
impl Error {
	...
	pub fn message(&self) -> Cow<'static, str> {
		match &self.inner.1 {
			...
			// Parser
			ErrorKind::EofButExpected(s) => format!("Unexpected end of file, but expected {}", s).into(),
			...
			ErrorKind::InvalidKeyframesName(s) => format!("{} is not valid name for keyframes", s).into(),
