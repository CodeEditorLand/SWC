use anyhow::Context;
use swc_atoms::Atom;
use swc_common::{DUMMY_SP, Mark, SourceMap, Span, SyntaxContext, source_map::PURE_SP, sync::Lrc, util::take::Take};
use swc_ecma_ast::*;
use swc_ecma_transforms_base::{feature::FeatureFlag, helper_expr};
use swc_ecma_utils::{ExprFactory, IsDirective, is_valid_prop_ident, private_ident, quote_ident, quote_str};
...
impl VisitMut for Umd {
	...
	fn visit_mut_module(&mut self, module: &mut Module) {
		...
		*module_items = vec![
			adapter_fn_expr
				.as_call(DUMMY_SP, vec![ThisExpr { span: DUMMY_SP }.as_arg(), factory_fn_expr.as_arg()])
				.into_stmt()
				.into(),
		]
	}
...
impl Umd {
	...
	fn adapter(&mut self, module_span: Span, is_export_assign: bool) -> (FnExpr, Vec<Param>) {
		macro_rules! js_typeof {
			($test:expr =>! $type:expr) => {
				Expr::Unary(UnaryExpr { span: DUMMY_SP, op: op!("typeof"), arg: Box::new(Expr::from($test)) })
		...
		let define = quote_ident!(SyntaxContext::empty().apply_mark(self.unresolved_mark), "define");
		let global_this = quote_ident!(SyntaxContext::empty().apply_mark(self.unresolved_mark), "globalThis");
		let js_self = quote_ident!(SyntaxContext::empty().apply_mark(self.unresolved_mark), "self");
		...
		self.dep_list.take().into_iter().for_each(|(ident, src_path, src_span)| {
			...
			cjs_args.push(
				require
					.clone()
					.as_call(DUMMY_SP, vec![quote_str!(src_span.0, src_path.clone()).as_arg()])
					.as_arg(),
			);
		...
		let cjs_if_test =
			js_typeof!(module => "object").make_bin(op!("&&"), js_typeof!(module_exports.clone() => "object"));
		...
		let amd_if_body = define.as_call(
			DUMMY_SP,
			vec![
				ArrayLit { span: DUMMY_SP, elems: amd_dep_list }.as_arg(),
