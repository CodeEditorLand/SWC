...
use swc_ecma_ast::*;
use swc_ecma_utils::{
	ExprFactory, FunctionFactory, IsDirective, is_valid_prop_ident, member_expr, private_ident, quote_ident, quote_str,
};
...
impl From<bool> for ImportInterop {
	fn from(no_interop: bool) -> Self {
		if no_interop { ImportInterop::None } else { ImportInterop::Swc }
	}
}
...
impl Config {
	...
	pub fn import_interop(&self) -> ImportInterop {
		self.import_interop.unwrap_or_else(|| self.no_interop.into())
	}
...
pub(super) fn local_name_for_src(src: &Atom) -> Atom {
	...
	let src = src.strip_suffix(".js").or_else(|| src.strip_suffix(".mjs")).unwrap_or(src);
	...
	if !id.starts_with('_') { format!("_{}", id).into() } else { id.into() }
}
...
///  });
/// ```
pub(super) fn object_define_property(target: ExprOrSpread, prop_name: ExprOrSpread, descriptor: ExprOrSpread) -> Expr {
...
pub(crate) fn object_define_enumerable(target: ExprOrSpread, prop_name: ExprOrSpread, prop: PropOrSpread) -> Expr {
	object_define_property(
		target,
		prop_name,
		ObjectLit {
			...
			props: vec![
				PropOrSpread::Prop(Box::new(
					KeyValueProp { key: quote_ident!("enumerable").into(), value: Box::new(true.into()) }.into(),
				)),
				prop,
			],
		}
...
pub(crate) fn esm_export() -> Function {
	...
	let for_in_stmt: Stmt = ForInStmt {
		...
		left: VarDecl {
			...
			declare: false,
			decls: vec![VarDeclarator { span: DUMMY_SP, name: name.into(), init: None, definite: false }],
			..Default::default()
		}
		.into(),
	...
	Function {
		...
		span: DUMMY_SP,
		body: Some(BlockStmt { stmts: vec![for_in_stmt], ..Default::default() }),
		is_generator: false,
		is_async: false,
		..Default::default()
	}
...
pub(crate) fn emit_export_stmts(exports: Ident, mut prop_list: Vec<ExportKV>) -> Vec<Stmt> {
	match prop_list.len() {
		0 | 1 => prop_list
			...
			.collect(),
		_ => {
			let props = prop_list.into_iter().map(prop_function).map(From::from).collect();
			let obj_lit = ObjectLit { span: DUMMY_SP, props };

			let esm_export_ident = private_ident!("_export");

			vec![
				Stmt::Decl(Decl::Fn(esm_export().into_fn_decl(esm_export_ident.clone()))),
				esm_export_ident
...
impl From<IdentOrStr> for MemberProp {
	fn from(val: IdentOrStr) -> Self {
		match val {
			IdentOrStr::Ident(i) => Self::Ident(i),
			IdentOrStr::Str(s) => Self::Computed(ComputedPropName { span: DUMMY_SP, expr: s.into() }),
		}
	}
}
