...
impl Resolver {
	...
	pub(crate) fn make_require_call(&self, unresolved_mark: Mark, src: Atom, src_span: Span) -> Expr {
		let src = self.resolve(src);

		CallExpr {
			span: DUMMY_SP,
			callee: quote_ident!(SyntaxContext::empty().apply_mark(unresolved_mark), "require").as_callee(),
			args: vec![Lit::Str(Str { span: src_span, raw: None, value: src }).as_arg()],
			..Default::default()
		}
		.into()
	}
	...
	R: Resolve,
{
	fn to_specifier(&self, mut target_path: PathBuf, orig_filename: Option<&str>) -> Atom {
		...
		if let Some(orig_filename) = orig_filename {
			let is_resolved_as_index = if let Some(stem) = target_path.file_stem() { stem == "index" } else { false };
	...
	fn try_resolve_import(&self, base: &FileName, module_specifier: &str) -> Result<Atom, Error> {
		...
		let Resolution { filename: target, slug } = target;
		...
		let mut target = match target {
			...
			_ => {
				unreachable!("Node path provider does not support using `{:?}` as a target file name", target)
			},
		};
		let mut base = match base {
			FileName::Real(v) => Cow::Borrowed(v.parent().ok_or_else(|| anyhow!("failed to get parent of {:?}", v))?),
			...
			_ => {
				unreachable!("Node path provider does not support using `{:?}` as a base file name", base)
			},
		};
...
{
	fn resolve_import(&self, base: &FileName, module_specifier: &str) -> Result<Atom, Error> {
		self.try_resolve_import(base, module_specifier).or_else(|err| {
