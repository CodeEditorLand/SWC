...
use swc_common::{DUMMY_SP, Mark, Span, Spanned, SyntaxContext, source_map::PURE_SP, util::take::Take};
use swc_ecma_ast::*;
use swc_ecma_transforms_base::{feature::FeatureFlag, helper_expr};
use swc_ecma_utils::{ExprFactory, FunctionFactory, IsDirective, member_expr, private_ident, quote_expr, quote_ident};
use swc_ecma_visit::{VisitMut, VisitMutWith, noop_visit_mut_type, visit_mut_pass};

pub use super::util::Config;
use crate::{
	module_decl_strip::{Export, ExportKV, Link, LinkFlag, LinkItem, LinkSpecifierReducer, ModuleDeclStrip},
	module_ref_rewriter::{ImportMap, rewrite_import_bindings},
	path::Resolver,
	top_level_this::top_level_this,
	util::{
		ImportInterop, VecStmtLike, define_es_module, emit_export_stmts, local_name_for_src, prop_name, use_strict,
	},
};
...
impl VisitMut for Cjs {
	...
	fn visit_mut_module(&mut self, n: &mut Module) {
		...
		// Collect directives
		stmts.extend(&mut n.body.iter_mut().take_while(|i| i.directive_continue()).map(|i| i.take()));
		...
		n.body.iter_mut().for_each(|item| {
			if let ModuleItem::ModuleDecl(module_decl) = item {
				*item = self.handle_ts_import_equals(module_decl.take(), &mut module_map, &mut has_ts_import_equals);
			}
		});
		...
		let ModuleDeclStrip { link, export, export_assign, has_module_decl, .. } = strip;
		...
		stmts.extend(
			self.handle_import_export(&mut module_map, &mut lazy_record, link, export, is_export_assign)
				.map(From::from),
		);
	...
	fn visit_mut_expr(&mut self, n: &mut Expr) {
		match n {
			Expr::Call(CallExpr {
				span,
				callee: Callee::Import(Import { span: import_span, phase: ImportPhase::Evaluation }),
				args,
				..
						...
						.unwrap_or_default() =>
			{
				...
				match p {
					"url" => {
						let require = quote_ident!(SyntaxContext::empty().apply_mark(self.unresolved_mark), "require");
					...
					"filename" => {
						*n = quote_ident!(SyntaxContext::empty().apply_mark(self.unresolved_mark), *span, "__filename")
							.into();
					},
					"dirname" => {
						*n = quote_ident!(SyntaxContext::empty().apply_mark(self.unresolved_mark), *span, "__dirname")
							.into();
					},
					_ => {},
				}
...
impl Cjs {
	fn handle_import_export(
		...
		let lexer_reexport = if export_interop_annotation { self.emit_lexer_reexport(&link) } else { None };

		link.into_iter()
			.for_each(|(src, LinkItem(src_span, link_specifier_set, mut link_flag))| {
				...
				// require("mod");
				let import_expr = self.resolver.make_require_call(self.unresolved_mark, src, src_span.0);

				// _export_star(require("mod"), exports);
				let import_expr = if link_flag.export_star() {
					helper_expr!(export_star).as_call(DUMMY_SP, vec![import_expr.as_arg(), self.exports().as_arg()])
				} else {
					import_expr
				};
				...
				if decl_mod_ident {
					let stmt = if is_lazy {
						...
						import_expr.into_var_decl(self.const_var_kind, mod_ident.into()).into()
					};
	...
	fn handle_ts_import_equals(
		...
		match module_decl {
			...
			{
				let TsImportEqualsDecl { span, is_export, id, module_ref, .. } = *v;
				let Str { span: src_span, value: src, .. } = module_ref.expect_ts_external_module_ref().expr;

				*has_ts_import_equals = true;

				let require = self.resolver.make_require_call(self.unresolved_mark, src, src_span);
	...
	fn emit_lexer_exports_init(&mut self, export_id_list: &[ExportKV]) -> Option<Stmt> {
		match export_id_list.len() {
			...
			1 => {
				...
				let expr = BinExpr { span: DUMMY_SP, op: op!("&&"), left: 0.into(), right: Box::new(expr) };
			...
			_ => {
				...
				let module_exports_assign = ObjectLit { span: DUMMY_SP, props }.make_assign_to(
					op!("="),
					member_expr!(
						SyntaxContext::empty().apply_mark(self.unresolved_mark),
						Default::default(),
	...
	fn emit_lexer_reexport(&self, link: &Link) -> Option<Stmt> {
		link.iter()
			...
			.map(|(src, ..)| {
				let import_expr = self.resolver.make_require_call(self.unresolved_mark, src.clone(), DUMMY_SP);

				quote_ident!("__export").as_call(DUMMY_SP, vec![import_expr.as_arg()])
			})
			.reduce(|left, right| {
				BinExpr { span: DUMMY_SP, op: op!("&&"), left: left.into(), right: right.into() }.into()
			})
			.map(|expr| BinExpr { span: DUMMY_SP, op: op!("&&"), left: 0.into(), right: expr.into() }.into_stmt())
	}
}
...
pub(crate) fn cjs_dynamic_import(
	...
	let import_expr = {
		...
		match import_interop {
			...
			ImportInterop::Swc => helper_expr!(interop_require_wildcard).as_call(PURE_SP, vec![require.as_arg()]),
			ImportInterop::Node => {
				helper_expr!(interop_require_wildcard).as_call(PURE_SP, vec![require.as_arg(), true.as_arg()])
			},
		}
	};

	then.as_call(span, vec![import_expr.into_lazy_auto(callback_params, support_arrow).as_arg()])
}
...
fn cjs_import_meta_url(span: Span, require: Ident, unresolved_mark: Mark) -> Expr {
	require
		...
		.as_call(
			DUMMY_SP,
			vec![quote_ident!(SyntaxContext::empty().apply_mark(unresolved_mark), "__filename").as_arg()],
		)
