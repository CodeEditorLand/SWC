...
use swc_ecma_ast::*;
use swc_ecma_utils::{ExprFactory, member_expr, private_ident, quote_ident, quote_str, var::VarCollector};
...
impl SystemJs {
	...
	fn fold_module_name_ident(&mut self, ident: Ident) -> Expr {
		if &*ident.sym == "__moduleName" && ident.ctxt.outer() == self.unresolved_mark {
			return self.context_ident.clone().make_member(quote_ident!("id")).into();
		}
		ident.into()
	}
	...
	fn build_export_call(&mut self, export_names: &mut Vec<Atom>, export_values: &mut Vec<Box<Expr>>) -> Vec<Stmt> {
			...
			_ => {
				...
				vec![
					CallExpr {
						span: DUMMY_SP,
						callee: self.export_ident.clone().as_callee(),
						args: vec![ObjectLit { span: DUMMY_SP, props }.as_arg()],
						..Default::default()
					}
					.into_stmt(),
				]
	...
	fn build_module_item_export_all(&mut self, mut meta: ModuleItemMeta) -> Vec<Stmt> {
		if !meta.has_export_all {
					...
					body: Box::new(Stmt::Block(BlockStmt {
						...
						stmts: vec![Stmt::If(IfStmt {
							...
							test: Box::new(Expr::Bin(BinExpr {
								span: DUMMY_SP,
								op: op!("&&"),
								left: Box::new(key_ident.clone().make_bin(op!("!=="), quote_str!("default"))),
								right: Box::new(key_ident.clone().make_bin(op!("!=="), quote_str!("__esModule"))),
							})),
			...
			for (sym, value) in meta.export_names.drain(..).zip(meta.export_values.drain(..)) {
				meta.setter_fn_stmts.push(
					AssignExpr {
						span: DUMMY_SP,
						op: op!("="),
	...
	fn add_module_item_meta(&mut self, mut m: ModuleItemMeta) {
		match self.module_item_meta_list.iter().position(|i| m.src.eq(&i.src)) {
			Some(index) => {
	...
	fn hoist_var_decl(&mut self, var_decl: Box<VarDecl>) -> Option<Expr> {
		...
		for var_declarator in var_decl.decls {
			...
			for (sym, ctxt) in tos {
				...
				self.declare_var_idents.push(Ident::new(sym, DUMMY_SP, ctxt));
			}
		...
		match exprs.len() {
			0 => None,
			_ => Some(SeqExpr { span: DUMMY_SP, exprs }.into()),
		}
	}
		...
		if let ForHead::VarDecl(mut var_decl) = var_decl_or_pat {
			if var_decl.kind == VarDeclKind::Var {
				...
				for to in tos {
					...
					self.declare_var_idents.push(Ident::new(to.0, DUMMY_SP, to.1));
				}
	...
	fn hoist_variables(&mut self, stmt: Stmt) -> Stmt {
		match stmt {
			...
			Stmt::For(for_stmt) => {
				if let Some(init) = for_stmt.init {
					if let VarDeclOrExpr::VarDecl(var_decl) = init {
						if var_decl.kind == VarDeclKind::Var {
							ForStmt {
								init: self.hoist_var_decl(var_decl).map(|expr| VarDeclOrExpr::Expr(Box::new(expr))),
								..for_stmt
							}
							.into()
						} else {
							ForStmt { init: Some(VarDeclOrExpr::VarDecl(var_decl)), ..for_stmt }.into()
						}
					} else {
						ForStmt { init: Some(init), ..for_stmt }.into()
					}
				} else {
					for_stmt.into()
				}
			...
			Stmt::ForIn(for_in_stmt) => {
				ForInStmt { left: self.hoist_for_var_decl(for_in_stmt.left), ..for_in_stmt }.into()
			},
			Stmt::ForOf(for_of_stmt) => {
				ForOfStmt { left: self.hoist_for_var_decl(for_of_stmt.left), ..for_of_stmt }.into()
			},
			_ => stmt,
		}
	}
...
impl Fold for SystemJs {
	...
	fn fold_call_expr(&mut self, expr: CallExpr) -> CallExpr {
		...
		match expr.callee {
			Callee::Import(_) => CallExpr {
				callee: self.context_ident.clone().make_member(quote_ident!("import")).as_callee(),
				..expr
			},
			_ => expr,
		}
	...
	fn fold_expr(&mut self, expr: Expr) -> Expr {
		...
		match expr {
			...
			Expr::Assign(assign) => {
				let assign_expr = AssignExpr {
					right: match *assign.right {
						...
						Expr::Assign(AssignExpr { op: AssignOp::Assign, .. }) => {
							return self.replace_assign_expr(AssignExpr { right: assign.right, ..assign });
						},
						_ => assign.right,
					},
					..assign
			...
			Expr::MetaProp(meta_prop_expr) => match meta_prop_expr.kind {
				MetaPropKind::ImportMeta => self.context_ident.clone().make_member(quote_ident!("meta")).into(),
	...
	fn fold_module(&mut self, module: Module) -> Module {
				...
				ModuleItem::ModuleDecl(decl) => match decl {
								...
								ImportSpecifier::Named(specifier) => {
									...
									setter_fn_stmts.push(
										AssignExpr {
											...
											right: MemberExpr {
												span: DUMMY_SP,
												obj: Box::new(quote_ident!(source_alias.clone()).into()),
												prop: match specifier.imported {
													Some(m) => get_module_export_member_prop(&m),
													None => MemberProp::Ident(specifier.local.into()),
												},
											}
											.into(),
										}
							...
							for specifier in decl.specifiers {
								...
								match specifier {
									...
									ExportSpecifier::Namespace(specifier) => {
										...
										export_values.push(quote_ident!(source_alias.clone()).into());
									},
								}
							...
							for specifier in decl.specifiers {
								if let ExportSpecifier::Named(specifier) = specifier {
									...
									if self.root_fn_decl_idents.iter().any(|i| i.sym == id.0) {
										...
										self.export_values.push(Box::new(get_module_export_expr(&specifier.orig)));
									}
					...
					ModuleDecl::ExportDecl(decl) => {
						match decl.decl {
							Decl::Class(class_decl) => {
								...
								execute_stmts.push(
									AssignExpr {
										...
										right: ClassExpr { ident: Some(ident.clone()), class: class_decl.class }.into(),
									}
									.into_stmt(),
								);
							},
							...
							Decl::Var(var_decl) => {
								let mut decl = VarDecl { decls: Vec::new(), ..*var_decl };
								for var_declarator in var_decl.decls {
									let mut tos: Vec<Id> = Vec::new();
									var_declarator.visit_with(&mut VarCollector { to: &mut tos });
									for to in tos {
							...
							DefaultDecl::Fn(fn_expr) => {
								if let Some(ident) = &fn_expr.ident {
									...
									before_body_stmts.push(
										FnDecl { ident: ident.clone(), declare: false, function: fn_expr.function }
											.into(),
									);
								} else {
									self.export_names.push("default".into());
					...
					ModuleDecl::ExportDefaultExpr(expr) => {
						execute_stmts.push(self.export_call("default".into(), expr.span, *expr.expr).into_stmt());
					},
						...
						Decl::Class(class_decl) => {
							...
							execute_stmts.push(
								AssignExpr {
									...
									right: ClassExpr { ident: Some(class_decl.ident.clone()), class: class_decl.class }
										.into(),
								}
								.into_stmt(),
							);
		...
		before_body_stmts = before_body_stmts.into_iter().map(|s| s.fold_with(self)).collect();
		...
		let mut setters = ArrayLit { span: DUMMY_SP, elems: Vec::new() };

		let mut dep_module_names = ArrayLit { span: DUMMY_SP, elems: Vec::new() };

		let module_item_meta_list: Vec<ModuleItemMeta> = self.module_item_meta_list.drain(..).collect();
		for meta in module_item_meta_list {
			dep_module_names.elems.push(Some(quote_str!(meta.src.clone()).as_arg()));
		...
		let execute = Box::new(Function {
			...
			decorators: Default::default(),
			span: DUMMY_SP,
			body: Some(BlockStmt { stmts: execute_stmts, ..Default::default() }),
			is_generator: false,
			is_async: self.tla,
			..Default::default()
		});
				...
				ObjectLit {
					...
					props: vec![
						...
						PropOrSpread::Prop(Box::new(Prop::KeyValue(KeyValueProp {
							...
							value: Box::new(Expr::Fn(FnExpr { ident: None, function: execute })),
						}))),
					],
				}
				.into(),
			...
			function_stmts.push(
				VarDecl {
					...
					decls: self
						.declare_var_idents
						.iter()
						.map(|i| VarDeclarator { span: i.span, name: i.clone().into(), init: None, definite: false })
						.collect(),
					..Default::default()
				}
				.into(),
		...
		let function = Box::new(Function {
			...
			decorators: Default::default(),
			span: DUMMY_SP,
			body: Some(BlockStmt { span: DUMMY_SP, stmts: function_stmts, ..Default::default() }),
			is_generator: false,
			is_async: false,
			..Default::default()
		});
		...
		Module {
			body: vec![
				CallExpr {
					span: DUMMY_SP,
					callee: member_expr!(Default::default(), Default::default(), System.register).as_callee(),
					args: vec![dep_module_names.as_arg(), FnExpr { ident: None, function }.as_arg()],
					..Default::default()
				}
				.into_stmt()
				.into(),
...
fn get_module_export_member_prop(module_export_name: &ModuleExportName) -> MemberProp {
	match &module_export_name {
		...
		ModuleExportName::Str(s) => {
			MemberProp::Computed(ComputedPropName { span: s.span, expr: Lit::Str(quote_str!(s.value.clone())).into() })
		},
	}
}
