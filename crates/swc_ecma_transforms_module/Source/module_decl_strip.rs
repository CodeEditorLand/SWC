...
impl VisitMut for ModuleDeclStrip {
				...
				ModuleItem::ModuleDecl(mut module_decl) => {
					...
					match module_decl {
						...
						ModuleDecl::ExportDefaultDecl(ExportDefaultDecl { decl, .. }) => match decl {
							DefaultDecl::Class(class_expr) => list.extend(class_expr.as_class_decl().map(From::from)),
							DefaultDecl::Fn(fn_expr) => list.extend(fn_expr.as_fn_decl().map(From::from)),
							DefaultDecl::TsInterfaceDecl(_) => continue,
						},
						ModuleDecl::ExportDefaultExpr(..) => list.extend(self.export_default.take().map(From::from)),
	...
	fn visit_mut_import_decl(&mut self, n: &mut ImportDecl) {
		...
		let ImportDecl { specifiers, src, .. } = n.take();
	...
	fn visit_mut_export_decl(&mut self, n: &mut ExportDecl) {
		match &n.decl {
			Decl::Class(ClassDecl { ident, .. }) | Decl::Fn(FnDecl { ident, .. }) => {
				self.export
					.insert(ident.sym.clone(), ExportItem::new((ident.span, ident.ctxt), ident.clone()));
			},
	...
	fn visit_mut_named_export(&mut self, n: &mut NamedExport) {
		...
		let NamedExport { specifiers, src, .. } = n.take();
				...
				ExportSpecifier::Named(ExportNamedSpecifier { orig, exported, .. }) => {
					...
					if let Some(exported) = exported {
						let (export_name, export_name_span) = match exported {
							ModuleExportName::Ident(Ident { ctxt, span, sym, .. }) => (sym, (span, ctxt)),
							ModuleExportName::Str(Str { span, value, .. }) => (value, (span, Default::default())),
						};

						(export_name, ExportItem::new(export_name_span, orig))
					} else {
						(orig.sym.clone(), ExportItem::new((orig.span, orig.ctxt), orig))
					}
				},
			}))
		}
	...
	fn visit_mut_export_default_decl(&mut self, n: &mut ExportDefaultDecl) {
		match &mut n.decl {
			DefaultDecl::Class(class_expr) => {
				...
				self.export
					.insert("default".into(), ExportItem::new((n.span, Default::default()), ident));
			},
			DefaultDecl::Fn(fn_expr) => {
				let ident = fn_expr.ident.get_or_insert_with(|| private_ident!(n.span, "_default")).clone();
	...
	fn visit_mut_export_default_expr(&mut self, n: &mut ExportDefaultExpr) {
		...
		self.export
			.insert("default".into(), ExportItem::new((n.span, Default::default()), ident.clone()));

		self.export_default = Some(n.expr.take().into_var_decl(self.const_var_kind, ident.into()).into());
	}
	...
	fn visit_mut_export_all(&mut self, n: &mut ExportAll) {
		let Str { value: src_key, span: src_span, .. } = *n.take().src;
	...
	fn visit_mut_ts_import_equals_decl(&mut self, n: &mut TsImportEqualsDecl) {
		...
		let TsImportEqualsDecl { id, module_ref, is_export, .. } = n;

		if let TsModuleRef::TsExternalModuleRef(TsExternalModuleRef {
			expr: Str { span, value: src_key, .. }, ..
		}) = module_ref
		{
			if *is_export {
				self.export
					.insert(id.sym.clone(), ExportItem::new((id.span, id.ctxt), id.clone()));
			}
...
pub enum LinkSpecifier {
	...
	ExportNamed { orig: (Atom, SpanCtx), exported: Option<(Atom, SpanCtx)> },
...
impl From<ImportSpecifier> for LinkSpecifier {
	fn from(i: ImportSpecifier) -> Self {
		match i {
			ImportSpecifier::Namespace(ImportStarAsSpecifier { local, .. }) => Self::ImportStarAs(local.to_id()),

			ImportSpecifier::Default(ImportDefaultSpecifier { local, .. }) => Self::ImportDefault(local.to_id()),
			ImportSpecifier::Named(ImportNamedSpecifier {
				is_type_only: false,
				local,
				imported:
					Some(ModuleExportName::Ident(Ident { sym: s, .. })) | Some(ModuleExportName::Str(Str { value: s, .. })),
				..
			}) if &*s == "default" => Self::ImportDefault(local.to_id()),

			ImportSpecifier::Named(ImportNamedSpecifier { is_type_only: false, local, imported, .. }) => {
				let imported = imported.map(|e| match e {
					ModuleExportName::Ident(Ident { sym, .. }) => sym,
					ModuleExportName::Str(Str { value, .. }) => value,
				});

				Self::ImportNamed { local: local.to_id(), imported }
			},
			_ => Self::Empty,
		}
	}
	...
	fn from(e: ExportSpecifier) -> Self {
		match e {
			ExportSpecifier::Namespace(ExportNamespaceSpecifier {
				name:
					ModuleExportName::Ident(Ident { span, sym, .. }) | ModuleExportName::Str(Str { span, value: sym, .. }),
				..
			}) => Self::ExportStarAs(sym, (span, SyntaxContext::empty())),

			ExportSpecifier::Default(ExportDefaultSpecifier { exported }) => {
				// https://github.com/tc39/proposal-export-default-from
				Self::ExportDefaultAs((exported.span, exported.ctxt), exported.sym, (exported.span, exported.ctxt))
			},

			ExportSpecifier::Named(ExportNamedSpecifier { is_type_only: false, orig, exported, .. }) => {
				let orig = match orig {
					ModuleExportName::Ident(Ident { span, sym, .. })
					| ModuleExportName::Str(Str { span, value: sym, .. }) => {
...
impl From<&ImportSpecifier> for LinkFlag {
	fn from(i: &ImportSpecifier) -> Self {
		match i {
			...
			ImportSpecifier::Named(ImportNamedSpecifier { is_type_only: false, .. }) => Self::NAMED,
	...
	fn from(e: &ExportSpecifier) -> Self {
		match e {
			...
			ExportSpecifier::Named(ExportNamedSpecifier {
				is_type_only: false,
				orig: ModuleExportName::Ident(Ident { sym: s, .. }) | ModuleExportName::Str(Str { value: s, .. }),
				..
			}) if &**s == "default" => Self::DEFAULT,

			ExportSpecifier::Named(ExportNamedSpecifier { is_type_only: false, .. }) => Self::NAMED,
		...
		self.into_iter().for_each(|s| match s {
			LinkSpecifier::ImportNamed { imported, local } => {
				*ref_to_mod_ident = true;
				...
				import_map.insert(local.clone(), (mod_ident.clone(), imported.or(Some(local.0))));
			},
			LinkSpecifier::ImportDefault(id) => {
				*ref_to_mod_ident = true;

				import_map.insert(id, (mod_ident.clone(), (!default_nowrap).then(|| "default".into())));
			},
			...
			LinkSpecifier::ExportNamed { orig, exported } => {
				*ref_to_mod_ident = true;
				...
				// foo -> mod.foo
				import_map.insert((orig.0.clone(), orig.1.1), (mod_ident.clone(), Some(orig.0.clone())));
			...
			LinkSpecifier::ExportDefaultAs(_, key, span) => {
				*ref_to_mod_ident = true;
				...
				// foo -> mod.default
				import_map.insert((key.clone(), span.1), (mod_ident.clone(), Some("default".into())));

				export_obj_prop_list.push((key.clone(), ExportItem::new(span, quote_ident!(span.1, span.0, key))));
			},
			...
			LinkSpecifier::ImportEqual(id) => {
				*ref_to_mod_ident = true;
				...
				import_map.insert(id, (raw_mod_ident.clone().unwrap_or_else(|| mod_ident.clone()), None));
			},
			LinkSpecifier::Empty => {},
		})
	}
