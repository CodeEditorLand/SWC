...
impl QueryRef for ImportQuery {
	fn query_ref(&self, ident: &Ident) -> Option<Box<Expr>> {
		self.import_map.get(&ident.to_id()).map(|(mod_ident, mod_prop)| -> Box<Expr> {
			...
			if let Some(imported_name) = mod_prop {
				...
				MemberExpr { obj: Box::new(mod_expr), span, prop }.into()
			} else {
				mod_expr.into()
			}
		})
...
pub(crate) fn rewrite_import_bindings<V>(node: &mut V, import_map: ImportMap, lazy_record: FxHashSet<Id>)
where
