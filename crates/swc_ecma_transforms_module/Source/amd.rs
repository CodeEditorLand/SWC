...
use swc_ecma_utils::{ExprFactory, FunctionFactory, IsDirective, member_expr, private_ident, quote_ident, quote_str};
...
use crate::{
	...
	util::{ImportInterop, VecStmtLike, define_es_module, emit_export_stmts, local_name_for_src, use_strict},
};
	...
	C: Comments,
{
	...
	visit_mut_pass(Amd {
		...
		require: quote_ident!(SyntaxContext::empty().apply_mark(unresolved_mark), "require"),
		exports: None,
		module: None,
	...
	C: Comments,
{
	...
	fn visit_mut_module(&mut self, n: &mut Module) {
		...
		if let Some(export_assign) = export_assign {
			let return_stmt = ReturnStmt { span: DUMMY_SP, arg: Some(export_assign) };
		...
		self.dep_list.take().into_iter().for_each(|(ident, src_path, src_span)| {
		...
		amd_call_args.push(ArrayLit { span: DUMMY_SP, elems }.as_arg());
		...
		n.body = vec![
			quote_ident!(SyntaxContext::empty().apply_mark(self.unresolved_mark), "define")
	...
	fn visit_mut_expr(&mut self, n: &mut Expr) {
		match n {
			Expr::Call(CallExpr {
				...
				*n = amd_dynamic_import(*span, args.take(), require, self.config.import_interop(), self.support_arrow);
			},
					...
					"resolve" => {
						...
						*obj = require.into();
						...
						match prop {
							...
							MemberProp::Computed(ComputedPropName { expr, .. }) => match &mut **expr {
								Expr::Lit(Lit::Str(s)) => {
									s.value = "toUrl".into();
									s.raw = None;
								},
	...
	C: Comments,
{
		...
		link.into_iter()
			.for_each(|(src, LinkItem(src_span, link_specifier_set, mut link_flag))| {
				...
				let mut import_expr: Expr = if need_re_export {
					helper_expr!(export_star)
						.as_call(DUMMY_SP, vec![mod_ident.clone().as_arg(), self.exports().as_arg()])
				} else {
				...
				if need_new_var {
					let stmt: Stmt = import_expr.into_var_decl(self.const_var_kind, new_var_ident.into()).into();

					stmts.push(stmt)
				} else if need_interop {
					let stmt = import_expr.make_assign_to(op!("="), mod_ident.into()).into_stmt();
					stmts.push(stmt);
	...
	fn module(&mut self) -> Ident {
		self.module.get_or_insert_with(|| private_ident!("module")).clone()
	}

	fn exports(&mut self) -> Ident {
		self.exports.get_or_insert_with(|| private_ident!("exports")).clone()
	}
	...
	fn get_amd_module_id_from_comments(&self, span: Span) -> Option<String> {
		...
		let amd_module_re =
			Regex::new(r#"(?i)^/\s*<amd-module.*?name\s*=\s*(?:(?:'([^']*)')|(?:"([^"]*)")).*?/>"#).unwrap();
...
pub(crate) fn amd_dynamic_import(
	...
	let resolved_module: Expr = match import_interop {
		...
		ImportInterop::Swc => helper_expr!(interop_require_wildcard).as_call(PURE_SP, vec![module.clone().as_arg()]),
		ImportInterop::Node => {
			helper_expr!(interop_require_wildcard).as_call(PURE_SP, vec![module.clone().as_arg(), true.as_arg()])
		},
	};
	...
	let require_call = require.as_call(
		DUMMY_SP,
		vec![
			ArrayLit { span: DUMMY_SP, elems: arg }.as_arg(),
			resolve_callback.as_arg(),
			reject.clone().as_arg(),
		],
	);

	let promise_executer = require_call.into_lazy_auto(vec![resolve.into(), reject.into()], support_arrow);
