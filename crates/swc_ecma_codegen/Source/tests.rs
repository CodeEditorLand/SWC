...
impl Builder {
		...
		Ret: 'static,
	{
		...
		{
			let mut e = Emitter { cfg: self.cfg, cm: self.cm.clone(), wr: writer, comments: Some(&self.comments) };
...
fn test_get_quoted_utf16() {
	...
	fn es2020(src: &str, expected: &str) {
		assert_eq!(combine(super::get_quoted_utf16(src, true, EsVersion::Es2020)), expected)
	}
	...
	fn es5(src: &str, expected: &str) {
		assert_eq!(combine(super::get_quoted_utf16(src, true, EsVersion::Es5)), expected)
	}
...
#[test]
fn issue_1619_3() {
	assert_eq!(&*get_quoted_utf16("\x00\x31", true, EsVersion::Es3).1, "\\x001");
}
...
fn test_all(src: &str, expected: &str, expected_minified: &str, config: Config) {
	{
		let out = parse_then_emit(src, Config { minify: false, omit_last_semi: true, ..config }, Syntax::default());
	...
	{
		eprintln!("> minified");
		let out = parse_then_emit(src, Config { minify: true, omit_last_semi: true, ..config }, Syntax::default());
