...
pub fn to_code_default(cm: Lrc<SourceMap>, comments: Option<&dyn Comments>, node: &impl Node) -> String {
	...
	let mut matched_indexes = chars
		...
		.filter(|(index, byte)| {
			byte == &&b'<'
				...
				&& matches!(chars[index + pattern_len], b'>' | b' ' | b'\t' | b'\n' | b'\x0C' | b'\r')
		})
		.map(|(index, _)| index)
		.peekable();
	...
	S: SourceMapperExt,
{
	...
	fn emit_import(&mut self, n: &ImportDecl) -> Result {
		...
		if specifiers.is_empty() {
			...
			punct!("{");
			self.emit_list(n.span(), Some(&specifiers), ListFormat::NamedImportsOrExportsElements)?;
			punct!("}");
			formatting_space!();
	...
	fn emit_named_export(&mut self, node: &NamedExport) -> Result {
		...
		let Specifiers { has_namespace_spec, namespace_spec, has_named_specs, named_specs } =
		...
		if has_named_specs || !has_namespace_spec {
			punct!("{");
			self.emit_list(node.span, Some(&named_specs), ListFormat::NamedImportsOrExportsElements)?;
			punct!("}");
		}
	...
	fn emit_str_lit(&mut self, node: &Str) -> Result {
		...
		{
			self.wr.write_str_lit(DUMMY_SP, node.raw.as_ref().unwrap())?;
	...
	fn emit_num_lit_internal(&mut self, num: &Number, mut detect_dot: bool) -> std::result::Result<bool, io::Error> {
		self.wr.commit_pending_semi()?;
	...
	fn emit_bin_expr_trailing(&mut self, node: &BinExpr) -> Result {
		...
		let need_pre_space = if self.cfg.minify {
			if is_kwd_op {
				...
				match *node.left {
					Expr::Update(UpdateExpr { prefix: false, op, .. }) => {
						matches!((op, node.op), (op!("--"), op!(">") | op!(">>") | op!(">>>") | op!(">=")))
					},
					_ => false,
				}
			}
	...
	fn emit_class_trailing(&mut self, node: &Class) -> Result {
		...
		if !node.implements.is_empty() {
			...
			self.emit_list(node.span, Some(&node.implements), ListFormat::ClassHeritageClauses)?;
		}
	...
	fn emit_bool(&mut self, n: &Bool) -> Result {
		...
		if n.value { keyword!(n.span, "true") } else { keyword!(n.span, "false") }
	}
	...
	fn emit_class_method(&mut self, n: &ClassMethod) -> Result {
		...
		punct!("(");
		self.emit_list(n.function.span, Some(&n.function.params), ListFormat::CommaListElements)?;
			...
			PropName::Ident(ident) => {
				...
				if self.cfg.ascii_only {
					if self.wr.can_ignore_invalid_unicodes() {
						self.wr
							.write_symbol(DUMMY_SP, &get_ascii_only_ident(&ident.sym, true, self.cfg.target))?;
					} else {
	...
	fn emit_quasi(&mut self, node: &TplElement) -> Result {
		...
		if self.cfg.minify || (self.cfg.ascii_only && !node.raw.is_ascii()) {
			...
			for ((offset_gen, _), mat) in v.match_indices('\n').zip(NEW_LINE_TPL_REGEX.find_iter(&raw)) {
				...
				if offset_gen != 0 {
					self.wr.add_srcmap(span.lo + BytePos(last_offset_origin as u32))?;
				}

				self.wr.write_str_lit(DUMMY_SP, &v[last_offset_gen..=offset_gen])?;
	...
	fn emit_yield_expr(&mut self, node: &YieldExpr) -> Result {
		...
		if let Some(ref arg) = node.arg {
			let need_paren = node.arg.as_deref().map(|expr| self.has_leading_comment(expr)).unwrap_or(false);
			if need_paren {
				punct!("(")
	...
	fn emit_expr_or_spreads(&mut self, parent_node: Span, nodes: &[ExprOrSpread], format: ListFormat) -> Result {
	...
	fn emit_object_lit(&mut self, node: &ObjectLit) -> Result {
		...
		let emit_new_line =
			!self.cfg.minify && !(node.props.is_empty() && is_empty_comments(&node.span(), &self.comments));

		if emit_new_line {
			self.wr.write_line()?;
		}

		let mut list_format = ListFormat::ObjectLiteralExpressionProperties | ListFormat::CanSkipTrailingComma;
	...
	fn emit_ident_like(&mut self, span: Span, sym: &Atom, optional: bool) -> Result {
		...
		if self.cfg.ascii_only {
			...
			self.wr.write_symbol(DUMMY_SP, &handle_invalid_unicodes(sym))?;
		}
	...
	fn emit_list<N: Node>(&mut self, parent_node: Span, children: Option<&[N]>, format: ListFormat) -> Result {
		self.emit_list5(parent_node, children, format, 0, children.map(|c| c.len()).unwrap_or(0))
	}
	...
	fn emit_pre_child_for_list5(
		...
		if let Some(previous_sibling) = previous_sibling {
			...
			if self
				.cm
				.should_write_separating_line_terminator(Some(previous_sibling), Some(child), format)
			{
	...
	fn emit_list_finisher_of_list5(
					...
					match self.cm.span_to_snippet(parent_node) {
						Ok(snippet) => {
							if snippet.len() < 3 {
								...
								snippet[..snippet.len() - last_char.len_utf8()].trim().ends_with(',')
							}
						},
						_ => false,
					}
		...
		if self.cm.should_write_closing_line_terminator(parent_node, last_child, format) {
			if !self.cfg.minify {
				self.wr.write_line()?;
			}
	...
	fn emit_list5<N: Node>(
		...
		if let Some(result) = self.emit_first_of_list5(parent_node, children.map(|v| v.len()), format, start, count) {
			return result;
		}
		...
		if is_empty {
			...
			if self
				.cm
				.should_write_leading_line_terminator(parent_node, children.first().map(|v| v.span()), format)
			{
				if !self.cfg.minify {
					self.wr.write_line()?;
				}
			...
			self.emit_list_finisher_of_list5(parent_node, format, previous_sibling, children.last().map(|v| v.span()))?;
		}
	...
	S: SourceMapperExt,
{
	...
	fn emit_block_stmt_inner(&mut self, node: &BlockStmt, skip_first_src_map: bool) -> Result {
		...
		let emit_new_line =
			!self.cfg.minify && !(node.stmts.is_empty() && is_empty_comments(&node.span(), &self.comments));
	...
	fn simple_assign_target_has_leading_comment(&self, arg: &SimpleAssignTarget) -> bool {
		match arg {
			SimpleAssignTarget::Ident(i) => span_has_leading_comment(self.comments.as_ref().unwrap(), i.span),
	...
	fn emit_return_stmt(&mut self, n: &ReturnStmt) -> Result {
		...
		if let Some(arg) = n.arg.as_deref() {
			let need_paren = n.arg.as_deref().map(|expr| self.has_leading_comment(expr)).unwrap_or(false);
			if need_paren {
				punct!("(");
	...
	fn emit_switch_case(&mut self, n: &SwitchCase) -> Result {
		...
		let emit_as_single_stmt = n.cons.len() == 1 && {
			...
			n.is_synthesized()
				...
				|| self.cm.is_on_same_line(n.span().lo(), n.cons[0].span().lo())
		};
...
fn should_emit_whitespace_before_operand(node: &UnaryExpr) -> bool {
	match *node {
		UnaryExpr { op: op!("void"), .. }
		| UnaryExpr { op: op!("typeof"), .. }
		| UnaryExpr { op: op!("delete"), .. } => return node.arg.starts_with_alpha_num(),
		_ => {},
	}

	match &*node.arg {
		Expr::Update(UpdateExpr { op: op!("++"), prefix: true, .. })
...
fn get_template_element_from_raw(s: &str, ascii_only: bool) -> String {
	fn read_escaped(radix: u32, len: Option<usize>, buf: &mut String, iter: impl Iterator<Item = char>) {
		let mut v = 0;
		...
		match unescape {
			...
			Some(c) => {
				if !ascii_only || c.is_ascii() {
					...
					buf.extend(c.escape_unicode().map(|c| if c == 'u' { c } else { c.to_ascii_uppercase() }));
				}
			},
			None => {},
		}
						...
						if let Some(c @ 'D' | c @ 'd') = next {
							...
							for _ in 0..3 {
								match inner_iter.next() {
									Some(c @ '0'..='9') | Some(c @ 'a'..='f') | Some(c @ 'A'..='F') => {
										inner_buf.push(c);
									},
									_ => {
										is_valid = false;
							...
							let range = if is_curly { 3..(inner_buf.len() - 1) } else { 2..6 };
...
fn require_space_before_rhs(rhs: &Expr, op: &BinaryOp) -> bool {
	match rhs {
		...
		Expr::Update(UpdateExpr { prefix: true, op: update, .. }) => {
			matches!((op, update), (op!(bin, "-"), op!("--")) | (op!(bin, "+"), op!("++")))
		},

		// space is mandatory to avoid outputting <!--
		Expr::Unary(UnaryExpr { op: op!("!"), arg, .. }) if *op == op!("<") || *op == op!("<<") => {
		...
		Expr::Unary(UnaryExpr { op: unary, .. }) => {
			matches!((op, unary), (op!(bin, "-"), op!(unary, "-")) | (op!(bin, "+"), op!(unary, "+")))
		},
...
fn minify_number(num: f64, detect_dot: &mut bool) -> String {
	...
	'hex: {
		if num.fract() == 0.0 && num.abs() <= u64::MAX as f64 {
			...
			*detect_dot = false;
			return format!("{}{:#x}", if num.is_sign_negative() { "-" } else { "" }, int);
		}
	}
	...
	if num.ends_with("000") {
		*detect_dot = false;
		...
		let cnt = num.as_bytes().iter().rev().skip(3).take_while(|&&c| c == b'0').count() + 3;
