...
pub use preset_env_base::{BrowserData, Versions, query::Targets, version::Version};
	...
	C: Comments + Clone,
{
		...
		($feature:ident, $default:expr) => {{
			...
			!exclude.contains(&f)
				&& (c.force_all_transforms
					|| (is_any_target || include.contains(&f) || f.should_enable(&targets, c.bugfixes, $default)))
		}};
	}
	...
	// ES2021
	let pass = add!(pass, LogicalAssignmentOperators, es2021::logical_assignments());
	...
	let pass = add!(pass, Spread, es2015::spread(es2015::spread::Config { loose }), true);
	...
	let pass = add!(
		...
		Parameters,
		es2015::parameters(
			es2015::parameters::Config { ignore_function_length: loose || assumptions.ignore_function_length },
			unresolved_mark
		)
	);
	...
	let pass = add!(
		pass,
		ForOf,
		es2015::for_of(es2015::for_of::Config { loose, assume_array: loose || assumptions.iterable_is_array }),
		true
	);
	let pass = add!(
		pass,
	...
	let pass = add!(pass, BlockScoping, es2015::block_scoping(unresolved_mark), true);
	let pass = add!(pass, Regenerator, generator(unresolved_mark, comments), true);
	...
	let pass = add!(pass, MemberExpressionLiterals, es3::member_expression_literals());
	...
	let pass = add!(pass, BugfixAsyncArrowsInClass, bugfixes::async_arrows_in_class(unresolved_mark));
	let pass = add!(pass, BugfixTaggedTemplateCaching, bugfixes::template_literal_caching());
	let pass = add!(
		pass,
	...
	(
		...
		visit_mut_pass(Polyfills {
			...
			corejs: c.core_js.unwrap_or(Version { major: 3, minor: 0, patch: 0 }),
...
impl Polyfills {
			...
			+ VisitMutWith<corejs3::Entry>,
	{
		let required = match self.mode {
			...
			Some(Mode::Usage) => {
				let mut r = match self.corejs {
					...
					Version { major: 3, .. } => {
						let mut v =
							corejs3::UsageVisitor::new(self.targets.clone(), self.shipped_proposals, self.corejs);
						m.visit_with(&mut v);
						v.required
					},
			...
			Some(Mode::Entry) => match self.corejs {
				...
				Version { major: 3, .. } => {
					let mut v = corejs3::Entry::new(self.targets.clone(), self.corejs, !self.regenerator);
					m.visit_mut_with(&mut v);
					v.imports
				},
		...
		required
			.iter()
			.filter(|s| !s.starts_with("esnext") || !required.contains(&s.replace("esnext", "es").as_str()))
			...
			prepend_stmts(
				...
				v.into_iter().map(|src| {
					ImportDecl {
						span,
						specifiers: Vec::new(),
						src: Str { span: DUMMY_SP, raw: None, value: src }.into(),
						type_only: false,
						with: None,
				...
				v.into_iter().map(|src| {
					ExprStmt {
						...
						expr: CallExpr {
							...
							.as_callee(),
							args: vec![Str { span: DUMMY_SP, value: src, raw: None }.as_arg()],
							type_args: None,
							..Default::default()
						}
						.into(),
