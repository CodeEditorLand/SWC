...
pub(crate) static FEATURES: Lazy<FxHashMap<Feature, BrowserData<Option<Version>>>> = Lazy::new(|| {
		...
		.map(|(feature, version)| {
			(
				...
				version.map_value(|version| {
					...
					version.map(|v| {
						v.parse()
							.unwrap_or_else(|err| panic!("failed to parse `{v}` as a version: {err:?}"))
					})
				}),
			)
		})
...
pub(crate) static BUGFIX_FEATURES: Lazy<FxHashMap<Feature, BrowserData<Option<Version>>>> = Lazy::new(|| {
	let map: FxHashMap<Feature, BrowserData<Option<String>>> =
		serde_json::from_str(include_str!("../data/@babel/compat-data/data/plugin-bugfixes.json"))
			.expect("failed to parse json");
	...
	FEATURES
		...
		.chain(
			map.into_iter()
				.map(|(feature, version)| (feature, version.map_value(|version| version.map(|v| v.parse().unwrap())))),
		)
		.collect()
});
...
mod tests {
	...
	fn arrow() {
		assert!(Feature::ArrowFunctions.should_enable(
			&BrowserData { ie: Some("11.0.0".parse().unwrap()), ..Default::default() },
			false,
			false
		));
	}
	...
	fn tpl_lit() {
		assert!(!Feature::TemplateLiterals.should_enable(
			&BrowserData { chrome: Some("71.0.0".parse().unwrap()), ..Default::default() },
			false,
			true
		));
	}
	...
	fn tpl_lit_bugfixes() {
		...
		assert!(Feature::TemplateLiterals.should_enable(
			&BrowserData { safari: Some("9.0.0".parse().unwrap()), ..Default::default() },
			false,
			false
		));

		assert!(!Feature::BugfixTaggedTemplateCaching.should_enable(
			&BrowserData { safari: Some("10.0.0".parse().unwrap()), ..Default::default() },
			false,
			false
		));
		...
		assert!(!Feature::BugfixTaggedTemplateCaching.should_enable(
			&BrowserData { safari: Some("13.0.0".parse().unwrap()), ..Default::default() },
			true,
			false
		));
	}
	...
	fn edge_default_param_bug() {
		...
		assert!(Feature::Parameters.should_enable(
			&BrowserData { edge: Some("17.0.0".parse().unwrap()), ..Default::default() },
			false,
			false
		));
		...
		assert!(!Feature::BugfixEdgeDefaultParam.should_enable(
			&BrowserData { edge: Some("18.0.0".parse().unwrap()), ..Default::default() },
			true,
			false
		));
	}
	...
	fn async_arrows_in_class_bug() {
		...
		assert!(Feature::AsyncToGenerator.should_enable(
			&BrowserData { safari: Some("10.1.0".parse().unwrap()), ..Default::default() },
			false,
			false
		));
		...
		assert!(!Feature::BugfixAsyncArrowsInClass.should_enable(
			&BrowserData { safari: Some("11.1.0".parse().unwrap()), ..Default::default() },
			true,
			false
		));
	}
