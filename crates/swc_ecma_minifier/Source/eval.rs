...
impl Evaluator {
	...
	pub fn eval(&mut self, e: &Expr) -> Option<EvalResult> {
		match e {
			...
			Expr::Lit(
				l @ Lit::Num(..) | l @ Lit::Str(..) | l @ Lit::BigInt(..) | l @ Lit::Bool(..) | l @ Lit::Null(..),
			...
			Expr::TaggedTpl(t) => {
				...
				match &*t.tag {
					Expr::Member(MemberExpr { obj: tag_obj, prop: MemberProp::Ident(prop), .. })
						if tag_obj.is_global_ref_to(self.expr_ctx, "String") && prop.sym == *"raw" =>
					{
			...
			// "foo".length
			Expr::Member(MemberExpr { obj, prop, .. }) if obj.is_lit() && prop.is_ident_with("length") => {},

			Expr::Unary(UnaryExpr { op: op!("void"), .. }) => return Some(EvalResult::Undefined),

			Expr::Unary(UnaryExpr { op: op!("!"), arg, .. }) => {
				let arg = self.eval(arg)?;

				if is_truthy(&arg)? {
					return Some(EvalResult::Lit(Lit::Bool(Bool { span: DUMMY_SP, value: false })));
				} else {
					return Some(EvalResult::Lit(Lit::Bool(Bool { span: DUMMY_SP, value: true })));
				}
			},
	...
	fn eval_as_expr(&mut self, e: &Expr) -> Option<Box<Expr>> {
		match e {
			...
			Expr::Member(MemberExpr { span, obj, prop, .. }) if !prop.is_computed() => {
				let obj = self.eval_as_expr(obj)?;

				let mut e: Expr = MemberExpr { span: *span, obj, prop: prop.clone() }.into();

				e.visit_mut_with(&mut pure_optimizer(
					&Default::default(),
					self.marks,
					PureOptimizerConfig { enable_join_vars: false, force_str_for_tpl: self.data.force_str_for_tpl() },
				));
	...
	pub fn eval_tpl(&mut self, q: &Tpl) -> Option<EvalResult> {
		...
		let mut e: Box<Expr> = Tpl { span: q.span, exprs, quasis: q.quasis.clone() }.into();
