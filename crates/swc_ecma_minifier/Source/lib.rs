...
use crate::{
	...
	pass::{
		global_defs, mangle_names::idents_to_preserve, mangle_props::mangle_properties, merge_exports::merge_exports,
...
pub fn optimize(
	...
	if let Some(c) = &options.compress {
		{
			...
			n.mutate(&mut compressor(marks, c, options.mangle.as_ref(), &Minification))
		}
		...
		n.visit_mut_with(&mut pure_optimizer(
			c,
			marks,
			PureOptimizerConfig { force_str_for_tpl: Minification.force_str_for_tpl(), enable_join_vars: true },
		));
	}
	...
	if let Some(mangle) = &options.mangle {
		...
		let chars = if !mangle.disable_char_freq {
			CharFreq::compute(&n, &preserved, SyntaxContext::empty().apply_mark(marks.unresolved_mark)).compile()
		} else {
