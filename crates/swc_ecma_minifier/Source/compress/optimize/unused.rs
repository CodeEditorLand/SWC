...
impl Optimizer<'_> {
	...
	pub(super) fn drop_unused_var_declarator(
		...
		match &mut var.init {
			Some(init) => match &**init {
				...
				Expr::Fn(FnExpr { function, .. }) if matches!(&**function, Function { is_async: true, .. }) => {},
				_ => {
	...
	pub(super) fn drop_unused_vars(&mut self, name: &mut Pat, init: Option<&mut Expr>) {
		...
		if let Some(scope) = self.data.scopes.get(&self.ctx.scope) {
			if scope.has_eval_call || scope.has_with_stmt {
				log_abort!("unused: Preserving `{}` because of usages", dump(&*name, false));
				return;
			}
		}
	...
	fn take_ident_of_pat_if_unused(&mut self, i: &mut Ident, init: Option<&mut Expr>) {
		...
		if let Some(v) = self.data.vars.get(&i.to_id()) {
			if v.ref_count == 0 && v.usage_count == 0 && !v.reassigned && v.property_mutation_count == 0 {
				self.changed = true;
				report_change!("unused: Dropping a variable '{}{:?}' because it is not used", i.sym, i.ctxt);
			...
			log_abort!("unused: Cannot drop ({}) because it's used", dump(&*i, false));
		}
	}

	pub(crate) fn should_preserve_property_access(&self, e: &Expr, opts: PropertyAccessOpts) -> bool {
	...
	#[cfg_attr(feature = "debug", tracing::instrument(skip_all))]
	pub(super) fn take_pat_if_unused(&mut self, name: &mut Pat, mut init: Option<&mut Expr>, is_var_decl: bool) {
		...
		match name {
			...
			Pat::Array(arr) => {
				for (idx, arr_elem) in arr.elems.iter_mut().enumerate() {
					if let Some(p) = arr_elem {
						let elem = init.as_mut().and_then(|expr| self.access_numeric_property(expr, idx));
			...
			Pat::Object(obj) => {
				...
				if obj.props.iter().any(|p| matches!(p, ObjectPatProp::Rest(_))) {
					return;
				}
		...
		match decl {
			Decl::Class(ClassDecl { ident, class, .. }) => {
				...
				let may_have_side_effect = class.body.iter().any(|m| match m {
					ClassMember::ClassProp(ClassProp { is_static: true, value: Some(_), .. })
					| ClassMember::PrivateProp(PrivateProp { is_static: true, value: Some(_), .. }) => true,
					ClassMember::StaticBlock(StaticBlock { body: BlockStmt { stmts, .. }, .. })
						if !stmts.is_empty() =>
					{
						true
					},
				...
				{
					self.changed = true;
					report_change!("unused: Dropping a decl '{}{:?}' because it is not used", ident.sym, ident.ctxt);
					// This will remove the declaration.
					let class = decl.take().class().unwrap();
					let mut side_effects = extract_class_side_effect(self.ctx.expr_ctx, *class.class);
			...
			Decl::Fn(FnDecl { ident, .. }) => {
				...
				if !self.may_remove_ident(ident) {
					log_abort!("unused: Preserving function `{}` because it's top-level", ident.sym);
					return;
				}
		...
		if let Expr::Ident(arg) = &*update.arg {
			if let Some(var) = self.data.vars.get(&arg.to_id()) {
				...
				if var.declared && var.is_fn_local && var.usage_count == 1 {
					self.changed = true;
					report_change!("unused: Dropping an update '{}{:?}' because it is not used", arg.sym, arg.ctxt);
	...
	pub(super) fn drop_unused_assignments(&mut self, e: &mut Expr) {
		...
		trace_op!("unused: drop_unused_assignments: Target: `{}`", dump(&assign.left, false));
		...
		if let AssignTarget::Simple(SimpleAssignTarget::Ident(i)) = &mut assign.left {
			...
			if let Some(var) = self.data.vars.get(&i.to_id()) {
				...
				if !var.inline_prevented
					&& !var.exported
					&& var.usage_count == 0
					&& var.declared && (!var.declared_as_fn_param || !used_arguments || self.ctx.expr_ctx.in_strict)
				{
					report_change!(
					...
					if self.ctx.is_this_aware_callee {
						*e = SeqExpr { span: DUMMY_SP, exprs: vec![0.into(), assign.right.take()] }.into()
					} else {
						*e = *assign.right.take();
					}
				} else {
	...
	pub(super) fn remove_name_if_not_used(&mut self, name: &mut Option<Ident>) {
		...
		if let Some(i) = &name {
			let can_remove_ident = self
				...
				.vars
				.get(&i.to_id())
				.map(|v| (!v.used_recursively && v.ref_count == 0 && v.usage_count == 0) || v.var_kind.is_some())
				.unwrap_or(false);
			...
			if d.init.is_none() {
				if let Pat::Ident(name) = &d.name {
					if let Some(usage) = self.data.vars.get_mut(&name.to_id()) {
						if usage.is_fn_local && usage.declared_as_fn_param && usage.declared_count >= 2 {
							d.name.take();
							usage.declared_count -= 1;

							report_change!("Removing a variable statement because it's a function parameter");
							self.changed = true;
						}
					}
				}
	...
	pub(super) fn remove_duplicate_name_of_function(&mut self, v: &mut VarDeclarator) {
		...
		if let Some(Expr::Fn(f)) = v.init.as_deref_mut() {
			...
			self.changed = true;
			report_change!("unused: Removing the name of a function expression because it's not used by it'");
			f.ident = None;
		}
	}
	...
	pub(super) fn drop_unused_properties(&mut self, v: &mut VarDeclarator) -> Option<()> {
		...
		if obj.props.iter().any(|p| match p {
			...
			PropOrSpread::Prop(p) => match &**p {
				...
				Prop::KeyValue(p) => p.key.is_computed() || p.value.may_have_side_effects(self.ctx.expr_ctx),
				Prop::Assign(_) => true,
