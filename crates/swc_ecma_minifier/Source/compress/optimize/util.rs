...
impl<'b> Optimizer<'b> {
	...
	pub(super) fn access_numeric_property<'e>(&mut self, _expr: &'e mut Expr, _idx: usize) -> Option<&'e mut Expr> {
		None
	}
		...
		if self.ctx.scope != scope_ctxt {
			...
			#[cfg(debug_assertions)]
			{
				self.data
					.scopes
					.get(&scope_ctxt)
					.unwrap_or_else(|| panic!("scope not found: {:?}; {:#?}", scope_ctxt, self.data.scopes));
			}
		}
	...
	fn visit_mut_prop(&mut self, p: &mut Prop) {
		...
		if let Prop::Shorthand(i) = p {
			if let Some(value) = self.var(&i.to_id()) {
				...
				*p =
					Prop::KeyValue(KeyValueProp { key: PropName::Ident(IdentName::new(i.sym.clone(), i.span)), value });
			}
		}
	}
