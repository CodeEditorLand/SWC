...
use swc_ecma_utils::{ExprExt, Remapper, class_has_side_effect, collect_decls, find_pat_ids};
...
use crate::{
	...
	util::{idents_captured_by, idents_used_by, idents_used_by_ignoring_nested, size::SizeWithCtxt},
};
...
impl Optimizer<'_> {
	...
	pub(super) fn store_var_for_inlining(&mut self, ident: &mut Ident, init: &mut Expr, can_drop: bool) {
		...
		if let Some(usage) = self.data.vars.get(&ident.to_id()) {
			...
			let is_inline_enabled = self.options.reduce_vars || self.options.collapse_vars || self.options.inline != 0;
				...
				&& !usage.used_as_ref
			{
				if let Expr::Array(arr) = init {
					if arr.elems.len() < 32
						...
						report_change!("inline: Decided to store '{}{:?}' for array access", ident.sym, ident.ctxt);
						self.vars.lits_for_array_access.insert(ident.to_id(), Box::new(init.clone()));
					}
				}
			}
			...
			if !usage.is_fn_local {
				match init {
					...
					Expr::Unary(UnaryExpr { op: op!("!"), arg, .. }) if matches!(&**arg, Expr::Lit(..)) => {},

					Expr::Fn(FnExpr { function, .. }) if matches!(&**function, Function { body: Some(..), .. }) => {
			...
			if is_inline_enabled
				...
				&& match init {
					...
					Expr::Ident(id) if !id.eq_ignore_span(ident) => {
						if !usage.assigned_fn_local {
							...
							if u.declared_as_for_init && !usage.is_fn_local {
								should_inline &= !matches!(u.var_kind, Some(VarDeclKind::Let | VarDeclKind::Const))
							}

							if u.declared_as_fn_decl || u.declared_as_fn_expr {
								if self.options.keep_fnames || self.mangle_options.map_or(false, |v| v.keep_fn_names) {
									should_inline = false
								}
							}
					...
					Expr::Lit(lit) => match lit {
						Lit::Str(s) => {
							if ref_count == 1 || s.value.len() <= 3 {
								...
								self.vars.lits_for_cmp.insert(ident.to_id(), init.clone().into());
								false
							}
						},
					...
					Expr::Unary(UnaryExpr { op: op!("!"), arg, .. }) => arg.is_lit(),
				...
				if self.options.inline != 0
					&& may_remove && match init {
					Expr::Arrow(..) => self.options.unused,
					_ => true,
				} {
					self.changed = true;

					report_change!("inline: Decided to inline '{}{:?}' because it's simple", ident.sym, ident.ctxt);
				...
				&& ref_count == 1
			{
				match init {
					Expr::Fn(FnExpr { function: f, .. })
						if matches!(&**f, Function { is_async: true, .. } | Function { is_generator: true, .. }) =>
					{
						return;
					},
					Expr::Arrow(ArrowExpr { is_async: true, .. })
					| Expr::Arrow(ArrowExpr { is_generator: true, .. }) => return,
					...
					_ => {
						for id in idents_used_by(init) {
							if let Some(v_usage) = self.data.vars.get(&id) {
								if v_usage.reassigned
									|| v_usage.property_mutation_count > usage.property_mutation_count
				...
				report_change!("inline: Decided to inline var '{}' because it's used only once", ident);
				self.changed = true;

				self.vars.vars_for_inlining.insert(ident.take().to_id(), init.take().into());
			}
		}
	}

	/// Check if the body of a function is simple enough to inline.
	fn is_fn_body_simple_enough_to_inline(&self, body: &BlockStmt, param_count: usize, usage: &VarUsageInfo) -> bool {
		...
		if body.stmts.len() == 1 {
			match &body.stmts[0] {
				Stmt::Expr(ExprStmt { expr, .. }) if expr.size(self.ctx.expr_ctx.unresolved_ctxt) < cost_limit => {
					return true;
				},
		...
		if let Some(usage) = self.data.vars.get(&i.to_id()) {
					...
					if let Some(body) = &f.function.body {
                            ...
                            )
						{
							...
							for i in collect_infects_from(
								&f.function,
								AliasConfig::default().marks(Some(self.marks)).need_all(true),
							) {
							...
							self.vars
								.simple_functions
								.insert(i.to_id(), FnExpr { ident: None, function: f.function.clone() }.into());
			...
			if (self.options.reduce_vars || self.options.collapse_vars || self.options.inline != 0)
				...
				let e = match decl.take() {
					Decl::Class(c) => ClassExpr { ident: Some(c.ident), class: c.class }.into(),
					Decl::Fn(f) => FnExpr {
						ident: if usage.used_recursively { Some(f.ident) } else { None },
						function: f.function,
					}
					.into(),
					_ => {
		...
		match e {
			...
			Expr::Ident(i) => {
					...
					.cloned()
				{
					...
					for id in bindings {
						let new_ctxt = cache.entry(id.1).or_insert_with(|| id.1.apply_mark(new_mark));
...
fn is_arrow_body_simple_enough_for_copy(e: &Expr) -> Option<u8> {
	match e {
		...
		Expr::Bin(b) => {
			return Some(
				is_arrow_body_simple_enough_for_copy(&b.left)? + is_arrow_body_simple_enough_for_copy(&b.right)? + 2,
			);
		},
		_ => {},
	}
...
fn is_block_stmt_of_fn_simple_enough_for_copy(b: &BlockStmt) -> Option<u8> {
	if b.stmts.len() == 1 {
		if let Stmt::Return(ret) = &b.stmts[0] {
			return ret.arg.as_deref().map_or(Some(0), is_arrow_body_simple_enough_for_copy);
		}
	}
