...
impl Optimizer<'_> {
	pub(super) fn optimize_expr_in_str_ctx_unsafely(&mut self, e: &mut Expr) {
		...
		if let Expr::Call(CallExpr { callee: Callee::Expr(callee), args, .. }) = e {
			if args.iter().any(|arg| arg.expr.may_have_side_effects(self.ctx.expr_ctx)) {
				return;
			}
	...
	pub(super) fn optimize_expr_in_str_ctx(&mut self, n: &mut Expr) {
		...
		if let Known(value) = value {
			...
			self.changed = true;
			report_change!("strings: Converted an expression into a string literal (in string context)");
			*n = Lit::Str(Str { span, raw: None, value: value.into() }).into();
			return;
		}
		...
		match n {
			Expr::Lit(Lit::Num(v)) => {
				...
				*n = Lit::Str(Str { span: v.span, raw: None, value: value.into() }).into();
			},
			...
			Expr::Bin(BinExpr { span, op: op!("/"), left, right }) => {
