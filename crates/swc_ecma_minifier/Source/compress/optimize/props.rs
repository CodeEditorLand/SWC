...
use super::{Optimizer, unused::PropertyAccessOpts};
use crate::util::deeply_contains_this_expr;

/// Methods related to the option `hoist_props`.
impl Optimizer<'_> {
	pub(super) fn hoist_props_of_var(&mut self, n: &mut VarDeclarator) -> Option<Vec<VarDeclarator>> {
		...
		if let Pat::Ident(name) = &mut n.name {
			if name.id.ctxt == self.marks.top_level_ctxt && self.options.top_retain.contains(&name.id.sym) {
			...
			if usage.accessed_props.is_empty() {
				log_abort!("hoist_props: Variable `{}` is not accessed with known keys", name.id);
				return None;
			}
			...
			self.changed = true;
			report_change!("hoist_props: Hoisting properties of a variable `{}`", name.id.sym);
			...
			for prop in &mut object.props {
				...
				let (key, suffix) = match &**prop {
					Prop::KeyValue(p) => match &p.key {
						...
						PropName::Str(s) => (
							s.value.clone(),
							s.value.clone().replace(|c: char| !Ident::is_valid_continue(c), "$").into(),
						),
						_ => unreachable!(),
					},
				...
				self.vars.hoisted_props.insert((name.to_id(), key), new_var_name);
	...
	pub(super) fn replace_props(&mut self, e: &mut Expr) {
		...
		if let Expr::Ident(obj) = &*member.obj {
			...
			if let Some(value) = self.vars.hoisted_props.get(&(obj.to_id(), sym.clone())).cloned() {
	...
	pub(super) fn handle_property_access(&mut self, e: &mut Expr) {
		...
		if obj.props.iter().any(|prop| match prop {
			...
			PropOrSpread::Prop(p) => match &**p {
				...
				Prop::Assign(p) => {
					p.value.may_have_side_effects(self.ctx.expr_ctx) || deeply_contains_this_expr(&p.value)
				},
				...
				PropOrSpread::Prop(p) => match &**p {
					...
					Prop::KeyValue(p) => {
						if prop_name_eq(&p.key, &key.sym) {
							report_change!("properties: Inlining a key-value property `{}`", key.sym);
							self.changed = true;
							*e = *p.value.clone();
							return;
						}
