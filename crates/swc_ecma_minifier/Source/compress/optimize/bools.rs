...
impl Optimizer<'_> {
	...
	pub(super) fn optimize_cmp_with_null_or_undefined(&mut self, e: &mut BinExpr) {
		if e.op == op!("||") || e.op == op!("&&") {
			{
				let res = self.optimize_cmp_with_null_or_undefined_inner(e.span, e.op, &mut e.left, &mut e.right);
				if let Some(res) = res {
					self.changed = true;
			...
			if let (Expr::Bin(left), right) = (&mut *e.left, &mut *e.right) {
				if e.op == left.op {
					...
					if let Some(res) = res {
						self.changed = true;
						report_change!("bools: Optimizing `=== null || === undefined` to `== null`");
						*e = BinExpr { span: e.span, op: e.op, left: left.left.take(), right: res.into() };
					}
				}
			}
		}
	...
	fn optimize_cmp_with_null_or_undefined_inner(
		...
		let (cmp, op, left, right) = match &mut *e_left {
			Expr::Bin(left_bin) => {
				...
				match &*left_bin.right {
					...
					Expr::Member(MemberExpr { obj, prop: MemberProp::Ident(..), .. }) => {
						if self.should_preserve_property_access(
							obj,
							super::unused::PropertyAccessOpts { allow_getter: false, only_ident: false },
						) {
							return None;
						}
					},
				...
				(&mut left_bin.right, left_bin.op, &mut *left_bin.left, &mut *right)
			},
			_ => return None,
		};
