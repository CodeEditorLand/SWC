...
use super::Optimizer;
use crate::{DISABLE_BUGGY_PASSES, compress::util::eval_as_number, maybe_par};
...
impl Optimizer<'_> {
	...
	fn eval_fn_props(&mut self, e: &mut Expr) -> Option<()> {
		...
		if let Expr::Member(MemberExpr { span, obj, prop: MemberProp::Ident(prop), .. }) = e {
				...
				if self.options.unsafe_passes {
					match &*prop.sym {
						"length" => {
							...
							*e = Lit::Num(Number { span: *span, value: metadata.len as _, raw: None }).into();
							self.changed = true;
						},
						...
						"name" => {
							...
							*e = Lit::Str(Str { span: *span, value: obj.sym.clone(), raw: None }).into();
							self.changed = true;
						},
	...
	fn eval_global_vars(&mut self, e: &mut Expr) {
		...
		if self.ctx.is_delete_arg || self.ctx.is_update_arg || self.ctx.is_lhs_of_assign || self.ctx.in_with_stmt {
			return;
		}

		// We should not convert used-defined `undefined` to `void 0`.
		if let Expr::Ident(i) = e {
			if self.data.vars.get(&i.to_id()).map(|var| var.declared).unwrap_or(false) {
				return;
			}
		}
		...
		match e {
			...
			Expr::Ident(Ident { span, sym, .. }) if &**sym == "Infinity" => {
				...
				*e = BinExpr {
					span: *span,
					op: op!("/"),
					left: Lit::Num(Number { span: DUMMY_SP, value: 1.0, raw: None }).into(),
					right: Lit::Num(Number { span: DUMMY_SP, value: 0.0, raw: None }).into(),
				}
				.into();
			},
	...
	fn eval_known_static_method_call(&mut self, e: &mut Expr) {
		...
		let (span, callee, args) = match e {
			Expr::Call(CallExpr { span, callee: Callee::Expr(callee), args, .. }) => (span, callee, args),
			_ => return,
		};
		let span = *span;
				...
				match args.len() {
					...
					1 => {
						if let Expr::Lit(Lit::Str(exp)) = &*args[0].expr {
							self.changed = true;
							report_change!("evaluate: Converting RegExpr call into a regexp literal `/{}/`", exp.value);

							*e = Lit::Regex(Regex { span, exp: exp.value.as_ref().into(), flags: atom!("") }).into();
						}
					},
					_ => {
						if let (Expr::Lit(Lit::Str(exp)), Expr::Lit(Lit::Str(flags))) = (&*args[0].expr, &*args[1].expr)
						{
							self.changed = true;
							report_change!(
					...
					if &*prop.sym == "fromCharCode" {
						...
						if let Known(char_code) = args[0].expr.as_pure_number(self.ctx.expr_ctx) {
							...
							if let Some(v) = char::from_u32(v) {
								...
								self.changed = true;
								report_change!("evaluate: Evaluated `String.charCodeAt({})` as `{}`", char_code, v);
								...
								PropOrSpread::Prop(p) => match &**p {
									Prop::Shorthand(p) => {
										keys.push(Some(ExprOrSpread {
											spread: None,
											expr: Lit::Str(Str { span: p.span, raw: None, value: p.sym.clone() })
												.into(),
										}));
									},
	...
	fn eval_numbers(&mut self, e: &mut Expr) {
		...
		if let Expr::Call(..) = e {
			if let Some(value) = eval_as_number(self.ctx.expr_ctx, e) {
				...
				*e = Lit::Num(Number { span: e.span(), value, raw: None }).into();
				return;
			}
		}
				...
				if let Known(l) = l {
					if let Known(r) = r {
						...
						if l.is_nan() || r.is_nan() {
							...
							*e = Lit::Num(Number { span: bin.span, value: l.powf(r), raw: None }).into();
						};
					}
				}
			},
