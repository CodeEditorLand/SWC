...
/// `if_return` is false.
impl Optimizer<'_> {
	pub(super) fn merge_if_returns(&mut self, stmts: &mut Vec<Stmt>, terminates: bool, is_fn_body: bool) {
	...
	fn merge_if_returns_inner(&mut self, stmts: &mut Vec<Stmt>, should_preserve_last_return: bool) {
		...
		let idx_of_not_mergable = stmts.iter().enumerate().rposition(|(idx, stmt)| match stmt.as_stmt() {
...

		{
			...
			let if_return_count = stmts
				.iter()
				.filter(|s| match s {
					Stmt::If(IfStmt { cons, alt: None, .. }) => always_terminates_with_return_arg(cons),
					_ => false,
				})
				.count();

			if stmts.len() >= 2 {
				match (&stmts[stmts.len() - 2].as_stmt(), &stmts[stmts.len() - 1].as_stmt()) {
					(_, Some(Stmt::If(IfStmt { alt: None, .. }) | Stmt::Expr(..))) if if_return_count <= 1 => {
						log_abort!("if_return: [x] Aborting because last stmt is a not return stmt");
						return;
					},

					(Some(Stmt::If(IfStmt { cons, alt: None, .. })), Some(Stmt::Return(..))) => match &**cons {
...

		{
			...
			let ends_with_mergable = stmts
				...
				.map(|stmt| match stmt.as_stmt() {
					Some(Stmt::If(IfStmt { alt: None, .. })) if self.ctx.is_nested_if_return_merging => false,
			...
			let can_merge = stmts.iter().enumerate().skip(skip).all(|(idx, stmt)| match stmt.as_stmt() {
				...
				Expr::Cond(v) => match &mut cur {
					Some(cur) => match &mut **cur {
						...
						Expr::Seq(prev_seq) => {
							...
							*cur = CondExpr {
								span: DUMMY_SP,
								test: Box::new(SeqExpr { span: prev_seq.span, exprs }.into()),
								cons: v.cons,
								alt: v.alt,
							}
							.into();
			...
			match &*cur {
							...
							.unwrap_or(true) =>
				{
					...
					if let Some(cur) = expr {
						new.push(ExprStmt { span: DUMMY_SP, expr: Box::new(cur) }.into())
					} else {
						trace_op!("if_return: Ignoring return value");
					}
				},
				_ => {
					new.push(ReturnStmt { span: DUMMY_SP, arg: Some(cur) }.into());
				},
			}
		}
	...
	fn merge_if_returns_to(&mut self, stmt: Stmt, mut exprs: Vec<Box<Expr>>) -> Expr {
		...
		match stmt {
			...
			Stmt::If(IfStmt { span, test, cons, alt, .. }) => {
				...
				CondExpr { span, test: SeqExpr { span: DUMMY_SP, exprs }.into(), cons, alt }.into()
			},
			Stmt::Expr(stmt) => {
				exprs.push(UnaryExpr { span: DUMMY_SP, op: op!("void"), arg: stmt.expr }.into());
	...
	fn can_merge_stmt_as_if_return(&self, s: &Stmt, is_last: bool) -> bool {
		...
		match s {
			Stmt::Expr(..) => true,
			Stmt::Return(..) => is_last,
			Stmt::Block(s) => s.stmts.len() == 1 && self.can_merge_stmt_as_if_return(&s.stmts[0], is_last),
			Stmt::If(stmt) => {
				matches!(&*stmt.cons, Stmt::Return(..))
					&& matches!(stmt.alt.as_deref(), None | Some(Stmt::Return(..) | Stmt::Expr(..)))
			},
			_ => false,
		}
	}
	...
	match s {
		...
		Stmt::If(IfStmt { cons, alt, .. }) => {
			always_terminates_with_return_arg(cons)
				&& alt.as_deref().map(always_terminates_with_return_arg).unwrap_or(false)
		},
...
fn can_merge_as_if_return(s: &Stmt) -> bool {
	fn cost(s: &Stmt) -> Option<isize> {
		...
		match s {
			...
			Stmt::If(IfStmt { cons, alt, .. }) => Some(cost(cons)? + alt.as_deref().and_then(cost).unwrap_or(0)),
			Stmt::Block(s) => {
				let mut sum = 0;
				let mut found = false;
