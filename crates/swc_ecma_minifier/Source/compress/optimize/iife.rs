...
use swc_common::{DUMMY_SP, Span, Spanned, SyntaxContext, pass::Either, util::take::Take};
use swc_ecma_ast::*;
...
use super::{Optimizer, util::NormalMultiReplacer};
#[cfg(feature = "debug")]
...
impl Optimizer<'_> {
	...
	pub(super) fn negate_iife_ignoring_ret(&mut self, e: &mut Expr) {
		...
		if let Expr::Fn(..) = callee {
			...
			*e = UnaryExpr { span: DUMMY_SP, op: op!("!"), arg: Box::new(e.take()) }.into();
		}
	}
	...
	pub(super) fn negate_iife_in_cond(&mut self, e: &mut Expr) -> bool {
		...
		match callee {
			Expr::Fn(..) => {
				...
				cond.test = UnaryExpr { span: DUMMY_SP, op: op!("!"), arg: cond.test.take() }.into();
	...
	pub(super) fn restore_negated_iife(&mut self, cond: &mut CondExpr) {
		...
		if let Expr::Unary(UnaryExpr { op: op!("!"), arg, .. }) = &mut *cond.test {
			if let Expr::Call(CallExpr { span: call_span, callee: Callee::Expr(callee), args, .. }) = &mut **arg {
				if let Expr::Fn(..) = &**callee {
					cond.test = CallExpr {
						span: *call_span,
...
impl Optimizer<'_> {
	...
	pub(super) fn inline_args_of_iife(&mut self, e: &mut CallExpr) {
		...
		if let Expr::Fn(FnExpr { ident: Some(ident), .. }) = callee {
			if self
				.data
				.vars
				.get(&ident.to_id())
		...
		if let Some(mut params) = params {
					...
					Pat::Ident(param) => {
						...
						if let Some(arg) = arg {
							...
							if should_be_inlined {
								trace_op!("iife: Trying to inline argument ({}{:?})", param.id.sym, param.id.ctxt);
					...
					Pat::Rest(rest_pat) => {
						if let Pat::Ident(param_id) = &*rest_pat.arg {
							if let Some(usage) = self.data.vars.get(&param_id.to_id()) {
								if usage.reassigned || usage.ref_count != 1 || !usage.has_property_access {
									continue;
								}
								...
								vars.insert(
									...
									ArrayLit {
										span: param_id.span,
										elems: e.args.iter().skip(idx).map(|arg| Some(arg.clone())).collect(),
									}
									.into(),
								);
								param.take();
			...
			let ctx = Ctx { in_fn_like: true, top_level: false, ..self.ctx.clone() };
		...
		for idx in removed {
			if let Some(arg) = e.args.get_mut(idx) {
				...
				if let Some(new) = new {
					...
					arg.expr = Number { span: arg.expr.span(), value: 0.0, raw: None }.into();
				}
			} else {
				break;
			}
	...
	fn may_invoke_iife(&self, call: &mut CallExpr) -> bool {
		if self.options.inline == 0 && !(self.options.reduce_vars && self.options.reduce_fns && self.options.evaluate) {
		...
		match callee {
			...
			Expr::Fn(f) => {
				...
				if let Some(i) = &f.ident {
					if self.data.vars.get(&i.to_id()).filter(|usage| usage.used_recursively).is_some() {
						log_abort!("iife: [x] Recursive?");
						return false;
					}
				}
	...
	pub(super) fn invoke_iife(&mut self, e: &mut Expr) {
		...
		match callee {
			Expr::Arrow(f) => {
				let param_ids = f.params.iter().map(|p| p.clone().ident().unwrap().id).collect::<Vec<_>>();
			...
			Expr::Arrow(f) => {
				...
				match &mut *f.body {
					...
					BlockStmtOrExpr::BlockStmt(block_stmt) => {
						self.inline_fn_like_stmt(param_ids, block_stmt, &mut call.args, is_return, call.span)
					},
				}
			},
			Expr::Fn(f) => {
	...
	fn can_extract_param(&self, param_ids: &[Ident]) -> bool {
		...
		for pid in param_ids {
			if self.ident_reserved(&pid.sym) {
				log_abort!("iife: [x] Cannot inline because of reservation of `{}`", pid);
				return false;
			}
		}
			...
			if !param_ids.is_empty() {
				...
				for param in param_ids {
					if captured.contains(&param.to_id()) {
						log_abort!("iife: [x] Cannot inline because of the capture of `{}`", param);
						return false;
					}
				}
			}
		...
		if !body.stmts.iter().all(|stmt| {
			...
			match stmt {
				Stmt::Decl(Decl::Var(var))
					if matches!(&**var, VarDecl { kind: VarDeclKind::Var | VarDeclKind::Let, .. }) =>
				{
					for decl in &var.decls {
						match &decl.name {
							...
							Pat::Ident(id) => {
								if self.vars.has_pending_inline_for(&id.to_id()) {
									log_abort!("iife: [x] Cannot inline because pending inline of `{}`", id.id);
									return false;
								}
							},
				...
				Stmt::Return(ReturnStmt { arg, .. }) => match arg.as_deref() {
					...
					Some(Expr::Lit(Lit::Num(..))) => for_stmt || !self.ctx.in_obj_of_non_computed_member,
					_ => true,
				},
				_ => for_stmt,
			}
			...
			if let Some(arg) = arg {
				if let Some(usage) = self.data.vars.get_mut(&params[idx].to_id()) {
					if usage.ref_count == 1
						...
						&& matches!(&*arg, Expr::Lit(Lit::Num(..) | Lit::Str(..) | Lit::Bool(..) | Lit::BigInt(..)))
					{
				...
				exprs.push(AssignExpr { span: DUMMY_SP, op: op!("="), left: param.clone().into(), right: arg }.into())
			};
	...
	fn inline_fn_param_stmt(&mut self, params: &[Ident], args: &mut [ExprOrSpread]) -> Vec<VarDeclarator> {
		let mut vars = Vec::with_capacity(params.len());
		...
		for (idx, param) in params.iter().enumerate() {
			...
			if let Some(arg) = &mut arg {
				if let Some(usage) = self.data.vars.get_mut(&params[idx].to_id()) {
					...
					{
						// We don't need to create a variable in this case
						self.vars.vars_for_inlining.insert(param.to_id(), arg.take());
						continue;
					}
	...
	fn inline_fn_like(&mut self, params: &[Ident], body: &mut BlockStmt, args: &mut [ExprOrSpread]) -> Option<Expr> {
		...
		for mut stmt in body.stmts.take() {
			match stmt {
				...
				Stmt::Return(stmt) => {
					...
					let mut e = SeqExpr { span: DUMMY_SP, exprs };
	...
	fn inline_fn_like_stmt(
		...
		if decl.count
			+ (params.len().saturating_sub(
				args.iter()
					.filter(|a| a.expr.is_ident() || a.expr.as_lit().map(|l| !l.is_regex()).unwrap_or(false))
					.count(),
			)) * 2 > 4
		{
			return None;
		...
		if args.len() > params.len() {
			...
			let expr =
				Stmt::Expr(ExprStmt { span: DUMMY_SP, expr: Box::new(Expr::Seq(SeqExpr { span: DUMMY_SP, exprs })) });
		...
		Some(BlockStmt { span, ctxt: SyntaxContext::empty().apply_mark(self.marks.fake_block), stmts })
	}
	...
	fn can_be_inlined_for_iife(&self, arg: &Expr) -> bool {
		match arg {
			...
			Expr::Unary(UnaryExpr { op: op!("void"), arg, .. }) | Expr::Unary(UnaryExpr { op: op!("!"), arg, .. }) => {
				self.can_be_inlined_for_iife(arg)
			},

			Expr::Ident(..) => true,

			Expr::Member(MemberExpr { obj, prop, .. }) if !prop.is_computed() => self.can_be_inlined_for_iife(obj),

			Expr::Bin(BinExpr { op, left, right, .. }) => match op {
				op!(bin, "+") | op!("*") => self.can_be_inlined_for_iife(left) && self.can_be_inlined_for_iife(right),
				_ => false,
			},
			...
			Expr::Arrow(ArrowExpr { params, body, is_async: false, is_generator: false, .. }) if body.is_expr() => {
				params.iter().all(|p| p.is_ident()) && self.can_be_inlined_for_iife(body.as_expr().unwrap())
			},
...
fn find_params(callee: &mut Expr) -> Option<Vec<&mut Pat>> {
	match callee {
		...
		Expr::Fn(callee) => Some(callee.function.params.iter_mut().map(|param| &mut param.pat).collect()),
		_ => None,
	}
}
