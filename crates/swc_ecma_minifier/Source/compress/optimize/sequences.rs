...
use swc_ecma_utils::{ExprExt, StmtLike, Type, Value, contains_arguments, contains_this_expr, prepend_stmts};
...
use crate::{
	...
	util::{ExprOptExt, IdentUsageCollector, ModuleItemExt, idents_used_by, idents_used_by_ignoring_nested},
};
...
impl Optimizer<'_> {
		...
		T: StmtLike,
	{
		...
		{
			let can_work = stmts.windows(2).any(|stmts| match (stmts[0].as_stmt(), stmts[1].as_stmt()) {
				(Some(l @ Stmt::Expr(..)), Some(r)) => {
					...
					match r {
						...
						| Stmt::For(ForStmt { init: Some(VarDeclOrExpr::Expr(..)), .. })
						| Stmt::ForIn(..)
						| Stmt::ForOf(..) => true,

						Stmt::Decl(Decl::Var(v)) if matches!(&**v, VarDecl { kind: VarDeclKind::Var, .. }) => {
			...
			if stmts.len() == 2 {
				match stmts[1].as_stmt() {
					Some(Stmt::Decl(Decl::Var(v))) if matches!(&**v, VarDecl { kind: VarDeclKind::Var, .. },) => {
			...
			match stmt.try_into_stmt() {
				Ok(stmt) => {
					...
					match stmt {
						...
						| Stmt::For(mut stmt @ ForStmt { init: Some(VarDeclOrExpr::Expr(..)), .. }) => {
							match &mut stmt.init {
								Some(VarDeclOrExpr::Expr(e)) => {
									if exprs
										.iter()
										.all(|expr| matches!(&**expr, Expr::Assign(AssignExpr { op: op!("="), .. })))
									{
										let ids_used_by_exprs = idents_used_by_ignoring_nested(&exprs);

										let ids_used_by_first_expr =
											idents_used_by_ignoring_nested(&*e.first_expr_mut());

										let has_conflict =
											ids_used_by_exprs.iter().any(|id| ids_used_by_first_expr.contains(id));
										...
										if let Expr::Assign(AssignExpr { op: op!("="), left, right, .. }) =
											e.first_expr_mut()
										{
											if !has_conflict
												&& left.as_ident().is_some() && match &**right {
												Expr::Lit(Lit::Regex(..)) => false,
												Expr::Lit(..) => true,
												_ => false,
											} {
												let seq = e.force_seq();
												let extra = seq.exprs.drain(1..).collect::<Vec<_>>();
												seq.exprs.extend(take(&mut exprs));
												seq.exprs.extend(extra);

												if seq.exprs.len() == 1 {
													stmt.init = Some(VarDeclOrExpr::Expr(seq.exprs.pop().unwrap()));
												}
								...
								None => {
									if exprs.is_empty() {
										...
										stmt.init = Some(VarDeclOrExpr::Expr(Expr::from_exprs(take(&mut exprs))))
									}
								},
								_ => {
									unreachable!()
						...
						Stmt::Decl(Decl::Var(var))
							if matches!(&*var, VarDecl { kind: VarDeclKind::Var, .. })
						...
						_ => {
							if !exprs.is_empty() {
								new_stmts.push(T::from(
									ExprStmt { span: DUMMY_SP, expr: Expr::from_exprs(take(&mut exprs)) }.into(),
								))
							}
	...
	pub(super) fn lift_seqs_of_assign(&mut self, e: &mut Expr) {
		...
		if let Expr::Assign(AssignExpr { op: op!("="), left, right, span }) = e {
			...
			if let Expr::Seq(seq) = &mut **right {
				...
				if let Some(last) = seq.exprs.last_mut() {
					**last = AssignExpr { span: *span, op: op!("="), left: left.take(), right: last.take() }.into()
				}
	...
	fn seq_exprs_of<'a>(
		...
		Some(match s {
			Stmt::Expr(e) => {
				if self.options.sequences() || self.options.collapse_vars || self.options.side_effects {
			...
			Stmt::Return(ReturnStmt { arg: Some(arg), .. }) => Either::Right(once(Mergable::Expr(arg))),

			Stmt::If(s) if options.sequences() => Either::Right(once(Mergable::Expr(&mut s.test))),

			Stmt::Switch(s) if options.sequences() => Either::Right(once(Mergable::Expr(&mut s.discriminant))),
			...
			Stmt::ForOf(s) if options.sequences() => Either::Right(once(Mergable::Expr(&mut s.right))),

			Stmt::ForIn(s) if options.sequences() => Either::Right(once(Mergable::Expr(&mut s.right))),

			Stmt::Throw(s) if options.sequences() => Either::Right(once(Mergable::Expr(&mut s.arg))),
	...
	pub(super) fn merge_sequences_in_seq_expr(&mut self, e: &mut SeqExpr) {
		...
		let _tracing = {
			...
			Some(span!(Level::ERROR, "merge_sequences_in_seq_expr", seq_expr = &*e_str).entered())
		};
		...
		let mut exprs = e.exprs.iter_mut().map(|e| &mut **e).map(Mergable::Expr).collect();
	...
	fn merge_sequences_in_exprs(&mut self, exprs: &mut Vec<Mergable>) -> Result<(), ()> {
		#[cfg(feature = "debug")]
		let _tracing = { Some(tracing::span!(Level::TRACE, "merge_sequences_in_exprs", len = exprs.len()).entered()) };
				...
				for b_idx in (a_idx + 1)..exprs.len() {
					...
					match b {
						Mergable::Var(b) => match b.init.as_deref_mut() {
							Some(b) => {
								if !merge_seq_cache.is_top_retain(self, a, a_idx) && self.merge_sequential_expr(a, b)? {
									changed = true;
									merge_seq_cache.invalidate(a_idx);
									merge_seq_cache.invalidate(b_idx);
									break;
							...
							None => {
								if let Mergable::Expr(Expr::Assign(a_exp)) = a {
									if let (Some(a_id), Some(b_id)) = (a_exp.left.as_ident(), b.name.as_ident()) {
										if a_id.id.eq_ignore_span(&b_id.id)
											&& a_exp.op == op!("=") && self
											.data
											.vars
											.get(&a_id.id.to_id())
											.map(|u| !u.inline_prevented && !u.declared_as_fn_expr)
											.unwrap_or(false)
										{
											changed = true;
					...
					match a {
						Mergable::Var(VarDeclarator { init: Some(init), .. }) => {
	...
	fn is_simple_assign_target_skippable_for_seq(&self, a: Option<&Mergable>, e: &SimpleAssignTarget) -> bool {
		match e {
	...
	fn is_ident_skippable_for_seq(&self, a: Option<&Mergable>, e: &Ident) -> bool {
		...
		if let Some(a) = a {
			...
			if let Some(deps) = ids_used_by_a_init {
				...
				if deps.contains(&(e.to_id(), AccessKind::Reference)) || deps.contains(&(e.to_id(), AccessKind::Call)) {
					return false;
				}
			}
	...
	fn is_member_expr_skippable_for_seq(
		...
		if !self.should_preserve_property_access(obj, PropertyAccessOpts { allow_getter: false, only_ident: false }) {
	...
	fn is_skippable_for_seq(&self, a: Option<&Mergable>, e: &Expr) -> bool {
		...
		match e {
			...
			Expr::TaggedTpl(t) => {
				self.is_skippable_for_seq(a, &t.tag) && t.tpl.exprs.iter().all(|e| self.is_skippable_for_seq(a, e))
			},
				...
				if e.args.is_empty() {
					if let Callee::Expr(callee) = &e.callee {
						if let Expr::Fn(callee) = &**callee {
							...
							if ids.iter().all(|id| id.1.outer() == self.marks.unresolved_mark) {
								return true;
							}
						}
					}
			...
			Expr::Seq(SeqExpr { exprs, .. }) => exprs.iter().all(|e| self.is_skippable_for_seq(a, e)),
			...
			Expr::This(_) | Expr::Fn(_) | Expr::MetaProp(_) | Expr::Arrow(_) | Expr::PrivateName(_) => true,
			...
			Expr::OptChain(OptChainExpr { base, .. }) => match &**base {
				OptChainBase::Member(e) => {
					if !self.should_preserve_property_access(
						&e.obj,
						PropertyAccessOpts { allow_getter: false, only_ident: false },
					) {
		...
		match a {
			Mergable::Expr(a) => {
				let has_side_effect = match a {
					Expr::Assign(a) if a.is_simple_assign() => a.right.may_have_side_effects(self.ctx.expr_ctx),
	...
	fn merge_sequential_expr(&mut self, a: &mut Mergable, b: &mut Expr) -> Result<bool, ()> {
		...
		match &*b {
			...
			Expr::Assign(AssignExpr { op: op!("**="), right, .. })
			| Expr::Bin(BinExpr { op: op!("**"), right, .. }) => {
				if !right.is_lit() {
					return Ok(false);
				}
			},

			Expr::Unary(UnaryExpr { op: op!("delete"), .. }) => return Ok(false),
			_ => {},
		}
		...
		match b {
			Expr::Update(..) | Expr::Arrow(..) | Expr::Fn(..) | Expr::OptChain(..) => return Ok(false),
			...
			Expr::Member(MemberExpr { obj, prop: MemberProp::Computed(c), .. }) => {
			...
			Expr::SuperProp(SuperPropExpr { prop: SuperProp::Computed(c), .. }) => {
				trace_op!("seq: Try prop of member (computed)");
				return self.merge_sequential_expr(a, &mut c.expr);
			},
				...
				match &mut b_assign.left {
					AssignTarget::Simple(b_left) => {
						...
						if let SimpleAssignTarget::Member(..) = b_left {
							...
							let AssignTarget::Simple(SimpleAssignTarget::Member(b_left)) = &b_assign.left else {
								return Err(());
							};
							...
							if let Some(left_obj) = b_left.obj.as_ident() {
								if let Some(usage) = self.data.vars.get(&left_obj.to_id()) {
									if left_obj.ctxt != self.ctx.expr_ctx.unresolved_ctxt
										&& !usage.inline_prevented && !usage.reassigned
			...
			Expr::Call(CallExpr { callee: Callee::Expr(b_callee), args: b_args, .. }) => {
				...
				if self.merge_sequential_expr(a, b_callee)? {
					if is_this_undefined {
						if let Expr::Member(..) = &**b_callee {
							...
							*b_callee = SeqExpr { span: b_callee.span(), exprs: vec![zero, b_callee.take()] }.into();
						}
					}
			...
			Expr::New(NewExpr { callee: b_callee, args: b_args, .. }) => {
								...
								Prop::Shorthand(shorthand) => {
									...
									if self.merge_sequential_expr(a, &mut new_b)? {
										*prop = Box::new(Prop::KeyValue(KeyValueProp {
											key: Ident::new_no_ctxt(shorthand.sym.clone(), shorthand.span).into(),
	...
	fn replace_seq_update(&mut self, a: &mut Mergable, b: &mut Expr) -> Result<bool, ()> {
		...
		if let Mergable::Expr(a) = a {
			match a {
				Expr::Update(UpdateExpr { op, prefix: false, arg, .. }) => {
					if let Expr::Ident(a_id) = &**arg {
						...
						if replaced {
							self.changed = true;
							report_change!("sequences: Merged update expression into another expression",);
				...
				Expr::Update(UpdateExpr { op, prefix: true, arg, .. }) => {
					if let Expr::Ident(a_id) = &**arg {
						if let Some(usage) = self.data.vars.get(&a_id.to_id()) {
							if let Some(VarDeclKind::Const) = usage.var_kind {
								return Err(());
					...
					Expr::Assign(AssignExpr { left, right, .. }) => {
						...
						if let Some(usage) = self.data.vars.get(&left_id.to_id()) {
							...
							if usage.declared_as_fn_expr {
								log_abort!("sequences: Declared as fn expr ({}, {:?})", left_id.sym, left_id.ctxt);
								return Ok(false);
							}

							// We can remove this variable same as unused pass
							if !usage.reassigned && usage.usage_count == 1 && usage.declared && !usage.used_recursively
							{
								can_remove = true;
							}
						} else {
		...
		let take_a = |a: &mut Mergable, force_drop: bool, drop_op| {
			match a {
				...
				Mergable::FnDecl(a) => {
					...
					FnExpr { ident: Some(a.ident.take()), function: a.function.take() }.into()
				},
		...
		match b {
				...
				if let Some(b_left) = b.left.as_ident() {
					if b_left.to_id() == left_id.to_id() {
						...
						if let Some(a) = a_expr {
							b.right = SeqExpr { span: DUMMY_SP, exprs: vec![Box::new(a), b.right.take()] }.into();
						}
						return Ok(true);
					}
				}
			...
			Expr::Assign(b) => {
				if let Some(b_left) = b.left.as_ident() {
					...
					let var_type = self.data.vars.get(&left_id.to_id()).and_then(|info| info.merged_var_type);
						...
						if can_drop_op_for(a_op, b.op, var_type, a_type, b_type) {
							if b_left.to_id() == left_id.to_id() {
								if let Some(bin_op) = b.op.to_update() {
									report_change!("sequences: Merged assignment into another (op) assignment");
									self.changed = true;
									...
									b.right =
										BinExpr { span: DUMMY_SP, op: bin_op, left: to, right: b.right.take() }.into();
									return Ok(true);
								}
							}
						}
...
impl MergeSequenceCache {
	fn new(cap: usize) -> Self {
		Self { ident_usage_cache: vec![None; cap], top_retain_cache: vec![None; cap] }
	}

	fn is_ident_used_by<N: VisitWith<IdentUsageCollector>>(&mut self, ident: &Id, node: &N, node_id: usize) -> bool {
