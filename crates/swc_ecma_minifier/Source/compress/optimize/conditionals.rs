...
impl Optimizer<'_> {
	...
	pub(super) fn negate_if_stmt(&mut self, stmt: &mut IfStmt) {
		...
		if negate_cost(self.ctx.expr_ctx, &stmt.test, true, false) < 0 {
			...
			let ctx = Ctx { in_bool_ctx: true, ..self.ctx.clone() };
		...
		match &*alt {
			Stmt::Return(..) | Stmt::Continue(ContinueStmt { label: None, .. }) => {
				self.changed = true;
				report_change!("if_return: Negating an if statement because the alt is return / continue");
	...
	pub(super) fn compress_cond_to_logical_ignoring_return_value(&mut self, e: &mut Expr) {
		...
		if !cond.cons.may_have_side_effects(self.ctx.expr_ctx) {
			...
			*e = BinExpr { span: cond.span, op: op!("||"), left: cond.test.take(), right: cond.alt.take() }.into();
			return;
		}
		...
		if !cond.alt.may_have_side_effects(self.ctx.expr_ctx) {
			...
			*e = BinExpr { span: cond.span, op: op!("&&"), left: cond.test.take(), right: cond.cons.take() }.into();
		}
	}
		...
		T: StmtLike,
	{
		...
		let has_work = stmts.windows(2).any(|stmts| match (&stmts[0].as_stmt(), &stmts[1].as_stmt()) {
			(Some(Stmt::If(l @ IfStmt { alt: None, .. })), Some(Stmt::If(r @ IfStmt { alt: None, .. }))) => {
				SyntaxContext::within_ignored_ctxt(|| l.cons.eq_ignore_span(&r.cons) && l.cons.terminates())
			},
			_ => false,
		});
		if !has_work {
						...
						Stmt::If(mut stmt @ IfStmt { alt: None, .. }) => {
							...
							match &mut cur {
								Some(cur_if) => {
									...
									if SyntaxContext::within_ignored_ctxt(|| cur_if.cons.eq_ignore_span(&stmt.cons)) {
										cur_if.test = BinExpr {
											span: DUMMY_SP,
	...
	pub(super) fn compress_if_stmt_as_cond(&mut self, s: &mut Stmt) {
		...
		if let Stmt::Empty(..) = &*stmt.cons {
			if (self.options.conditionals || self.options.unused) && stmt.alt.is_none() {
				*s = ExprStmt { span: stmt.span, expr: stmt.test.take() }.into();
				self.changed = true;
		...
		if let Stmt::Empty(..) = &*stmt.cons {
			match &mut *stmt.test {
				Expr::Unary(UnaryExpr { op: op!("!"), arg, .. }) => {
					...
					let mut expr =
						BinExpr { span: DUMMY_SP, left: arg.take(), op: op!("&&"), right: Box::new(alt.take()) }.into();
					self.compress_logical_exprs_as_bang_bang(&mut expr, true);
					*s = ExprStmt { span: stmt.span, expr: expr.into() }.into();
				},
				_ => {
					report_change!("Optimizing `if (foo); else bar();` as `foo || bar();`");
		...
		if let Some(v) = new_expr {
			...
			*s = ExprStmt { span: stmt.span, expr: Box::new(v) }.into();
			return;
		}
	...
	pub(super) fn compress_cond_expr_if_similar(&mut self, e: &mut Expr) {
		...
		let compressed = self.compress_similar_cons_alt(&mut cond.test, &mut cond.cons, &mut cond.alt, false);
	...
	fn compress_similar_cons_alt(
		...
		if cons.eq_ignore_span(alt) && !matches!(&*cons, Expr::Yield(..) | Expr::Fn(..)) {
			...
			return Some(SeqExpr { span: DUMMY_SP, exprs: vec![test.take(), Box::new(cons.take())] }.into());
		}
		...
		match (cons, alt) {
			(Expr::Call(cons), Expr::Call(alt)) => {
					...
					&& alt.args.iter().all(|arg| arg.spread.is_none())
				{
					...
					return Some(
						CallExpr { span: DUMMY_SP, callee: cons.callee.take(), args, ..Default::default() }.into(),
					);
				}
			...
			(Expr::New(cons), Expr::New(alt)) => {
				...
				if cons.callee.eq_ignore_span(&alt.callee)
					...
					&& (cons.args.is_some() && cons.args.as_ref().unwrap().iter().all(|arg| arg.spread.is_none()))
					&& (alt.args.is_some() && alt.args.as_ref().unwrap().iter().all(|arg| arg.spread.is_none()))
				{
			...
			(
				...
				if self.data.ident_is_unresolved(&cons.left.as_ident().unwrap().id) {
					return None;
				}
			...
			(Expr::Cond(cons), alt) if (*cons.alt).eq_ignore_span(&*alt) => {
				...
				Some(
					CondExpr {
						span: DUMMY_SP,
						test: BinExpr { span: DUMMY_SP, left: test.take(), op: op!("&&"), right: cons.test.take() }
							.into(),
						cons: cons.cons.take(),
						alt: cons.alt.take(),
					}
			...
			(cons, Expr::Cond(alt)) if cons.eq_ignore_span(&*alt.cons) => {
				...
				Some(
					CondExpr {
						span: DUMMY_SP,
						test: BinExpr { span: DUMMY_SP, left: test.take(), op: op!("||"), right: alt.test.take() }
							.into(),
						cons: alt.cons.take(),
						alt: alt.alt.take(),
					}
			...
			(cons, Expr::Seq(alt)) if (**alt.exprs.last().unwrap()).eq_ignore_span(&*cons) => {
				...
				.into();
				Some(SeqExpr { span: DUMMY_SP, exprs: vec![first, Box::new(cons.take())] }.into())
			},
			...
			(Expr::Seq(cons), alt) if (**cons.exprs.last().unwrap()).eq_ignore_span(&*alt) => {
				...
				.into();
				Some(SeqExpr { span: DUMMY_SP, exprs: vec![first, Box::new(alt.take())] }.into())
			},
			...
			(Expr::Seq(left), Expr::Seq(right)) => {
				...
				while idx < min_len
					&& left.exprs[left_len - idx - 1].eq_ignore_span(&right.exprs[right_len - idx - 1])
					&& !matches!(&*left.exprs[left_len - idx - 1], Expr::Yield(..) | Expr::Fn(..))
				{
					idx += 1;
				}
				...
				if idx == 0 {
					...
					Some(SeqExpr { span: DUMMY_SP, exprs: seq }.into())
	...
	pub(super) fn inject_else(&mut self, stmts: &mut Vec<Stmt>) {
		...
		let pos_of_if = stmts.iter().enumerate().rposition(|(idx, s)| {
			idx != len - 1
				&& match s {
					Stmt::If(IfStmt { cons, alt: None, .. }) => match &**cons {
						Stmt::Block(b) => {
							b.stmts.len() == 2
								&& !matches!(b.stmts.first(), Some(Stmt::If(..) | Stmt::Expr(..)))
								&& matches!(b.stmts.last(), Some(Stmt::Return(ReturnStmt { arg: None, .. })))
						},
						_ => false,
					},
					_ => false,
		...
		match if_stmt {
			Stmt::If(mut s) => {
				...
				s.alt = Some(if alt.len() == 1 {
					...
					Box::new(BlockStmt { span: DUMMY_SP, stmts: alt, ..Default::default() }.into())
				});
		...
		T: StmtLike,
	{
		...
		let need_work = stmts.iter().any(|stmt| match stmt.as_stmt() {
			Some(Stmt::If(IfStmt { cons, alt: Some(..), .. })) => cons.terminates(),
			_ => false,
		});
		if !need_work {
			return;
		...
		stmts.take().into_iter().for_each(|stmt| {
			match stmt.try_into_stmt() {
				Ok(stmt) => match stmt {
					Stmt::If(IfStmt { span, mut test, mut cons, alt: Some(mut alt), .. }) if cons.terminates() => {
						if let (Stmt::Return(ReturnStmt { arg: None, .. }), Stmt::Decl(Decl::Fn(..))) = (&*cons, &*alt)
						{
						...
						new_stmts.push(T::from(IfStmt { span, test, cons, alt: None }.into()));
