...
impl Optimizer<'_> {
	...
	pub(super) fn optimize_loops_with_break(&mut self, s: &mut Stmt) {
		...
		self.prepend_stmts.extend(f.init.take().map(|init| match init {
			VarDeclOrExpr::VarDecl(var) => var.into(),
			VarDeclOrExpr::Expr(expr) => ExprStmt { span: DUMMY_SP, expr }.into(),
		}));
		self.prepend_stmts
			.extend(f.test.take().map(|expr| ExprStmt { span: DUMMY_SP, expr }.into()));
			...
			Stmt::While(w) => {
				...
				if let Known(false) = val {
					if purity.is_pure() {
						...
						if changed {
							report_change!("loops: Removing unreachable while statement without side effects");
						}
					} else {
			...
			Stmt::For(f) => {
				if let Some(test) = &mut f.test {
					...
					if let Known(false) = val {
						...
						self.prepend_stmts
							.push(ExprStmt { span: DUMMY_SP, expr: f.test.take().unwrap() }.into());
						f.update = None;
						*stmt = *f.body.take();
					} else if let Known(true) = val {
						if purity.is_pure() {
							self.changed = true;
							report_change!("loops: Removing `test` part of a for stmt as it's always true");
							f.test = None;
						}
					}
				}
