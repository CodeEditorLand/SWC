...
impl Optimizer<'_> {
			...
			op!("===") | op!("==") | op!("!==") | op!("!=") => {
				if e.left.is_ident() && e.left.eq_ignore_span(&e.right) {
					...
					if let Some(t) = self.typeofs.get(&id.to_id()) {
						match &**t {
							"object" | "function" => {
								e.left = Box::new(make_bool(e.span, e.op == op!("===") || e.op == op!("==")));
								e.right.take();
			...
			if let Known(lt) = lt {
				if let Known(rt) = rt {
					if lt == rt {
						e.op = op!("==");
						self.changed = true;
						report_change!("Reduced `===` to `==` because types of operands are identical")
					}
				}
			}
		}
		...
		if let Expr::Bin(right) = &mut *n.right {
			if right.op == n.op {
					...
					|| (n.left.is_str() && right.right.is_str())
				{
					...
					let BinExpr { op, left: rl, right: rr, .. } = right.take();
					*n.left = BinExpr { span: left.span(), op, left, right: rl }.into();
					n.right = rr;
				}
			}
		}
	...
	pub(super) fn negate(&mut self, e: &mut Expr, is_ret_val_ignored: bool) {
		negate(self.ctx.expr_ctx, e, self.ctx.in_bool_ctx, is_ret_val_ignored)
	}
	...
	pub(super) fn compress_typeofs(&mut self, e: &mut Expr) {
		...
		if let Expr::Unary(UnaryExpr { span, op: op!("typeof"), arg, .. }) = e {
			match &**arg {
				Expr::Ident(arg) => {
					if let Some(value) = self.typeofs.get(&arg.to_id()).cloned() {
						report_change!("Converting typeof of variable to literal as we know the value");
						self.changed = true;
						*e = Lit::Str(Str { span: *span, raw: None, value }).into();
					}
				},
				...
				Expr::Arrow(..) | Expr::Fn(..) | Expr::Class(..) => {
					...
					self.changed = true;
					*e = Lit::Str(Str { span: *span, raw: None, value: "function".into() }).into();
				},

				Expr::Array(..) | Expr::Object(..) => {
					report_change!("Converting typeof to 'object' as we know the value");
					self.changed = true;
					*e = Lit::Str(Str { span: *span, raw: None, value: "object".into() }).into();
				},
				_ => {},
			}
		}
