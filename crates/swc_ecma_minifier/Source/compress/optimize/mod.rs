...
use swc_ecma_utils::{ExprCtx, ExprExt, ExprFactory, IsEmpty, ModuleItemLike, StmtLike, Type, Value, prepend_stmts};
...
use crate::{
	...
	util::{ExprOptExt, ModuleItemExt, contains_eval, contains_leaping_continue_with_label, make_number},
};
...
impl Optimizer<'_> {
	fn may_remove_ident(&self, id: &Ident) -> bool {
		if let Some(VarUsageInfo { exported: true, .. }) = self.data.vars.get(&id.clone().to_id()).map(|v| &**v) {
			return false;
		}
	...
	fn handle_stmts(&mut self, stmts: &mut Vec<Stmt>, will_terminate: bool) {
		...
		self.with_ctx(self.ctx.clone()).handle_stmt_likes(stmts, will_terminate);
		...
		if self.options.bools_as_ints || self.options.bools {
			if let Lit::Bool(v) = lit {
				...
				*e = UnaryExpr {
					span: v.span,
					op: op!("!"),
					arg: Lit::Num(Number { span: v.span, value: if v.value { 0.0 } else { 1.0 }, raw: None }).into(),
				}
				.into();
			}
		}
	...
	fn ignore_return_value(&mut self, e: &mut Expr) -> Option<Expr> {
		...
		match e {
			Expr::This(_) | Expr::Invalid(_) | Expr::Lit(..) => {
				report_change!("ignore_return_value: Dropping unused expr: {}", dump(&*e, false));
			...
			Expr::Class(cls) => {
				...
				let exprs: Vec<Box<Expr>> = extract_class_side_effect(self.ctx.expr_ctx, *cls.class.take())
					.into_iter()
				...
				return Some(SeqExpr { span: cls.class.span, exprs }.into());
			},

			Expr::Paren(e) => return self.ignore_return_value(&mut e.expr),

			Expr::Bin(BinExpr { op, left, right, .. }) if op.may_short_circuit() => {
				let ctx = Ctx {
					dont_use_negated_iife: self.ctx.dont_use_negated_iife || self.options.side_effects,
					..self.ctx.clone()
				};
			...
			Expr::Unary(UnaryExpr { op: op!("delete"), .. }) => return Some(e.take()),

			Expr::Unary(UnaryExpr { op: op!("void"), .. }) if !self.options.unused => return Some(e.take()),
			...
			Expr::Bin(
				...
				return Some(SeqExpr { span, exprs: vec![Box::new(left.unwrap()), Box::new(right.unwrap())] }.into());
			},

			// Pure calls can be removed
			Expr::Call(CallExpr { callee: Callee::Expr(callee), args, .. })
				if match &**callee {
					Expr::Fn(f) => f.function.body.as_ref().map(|body| body.stmts.is_empty()).unwrap_or(false),
			...
			Expr::Call(CallExpr { callee: Callee::Expr(callee), args, .. }) => {
				if let Expr::Fn(FnExpr { ident: None, function }) = &mut **callee {
					if args.is_empty() {
						...
						if let Some(usage) = self.data.vars.get(&callee.to_id()) {
							if !usage.reassigned && usage.pure_fn {
								...
								if args.iter().any(|arg| arg.spread.is_some()) {
									...
									return Some(ArrayLit { span: callee.span, elems }.into());
								}
								...
								return Some(SeqExpr { span: callee.span, exprs: args }.into());
							}
						}
					}
				}
			...
			Expr::Assign(AssignExpr { op, left: left @ AssignTarget::Simple(_), right, .. })
				...
				if let AssignTarget::Simple(expr) = left {
					if let SimpleAssignTarget::Member(m) = expr {
						if !m.obj.may_have_side_effects(self.ctx.expr_ctx)
							&& (m.obj.is_object() || m.obj.is_fn_expr() || m.obj.is_arrow() || m.obj.is_class())
						{
							if self.should_preserve_property_access(
								&m.obj,
								PropertyAccessOpts { allow_getter: true, only_ident: false },
							) {
								return Some(e.take());
							} else {
								report_change!(
			...
			Expr::Member(MemberExpr { obj, prop, .. })
				if !prop.is_computed() && (self.options.top_level() || !self.ctx.in_top_level()) =>
			{
				if self
					.should_preserve_property_access(obj, PropertyAccessOpts { allow_getter: true, only_ident: true })
				{
					return Some(e.take());
				} else {
					return None;
			...
			Expr::Array(arr) => {
				...
				return Some(SeqExpr { span: arr.span, exprs }.into());
			},
			...
			Expr::Object(obj) => {
					...
					match prop {
						...
						PropOrSpread::Prop(prop) => match *prop {
							...
							Prop::Assign(mut prop) => {
								exprs.extend(self.ignore_return_value(&mut prop.value).map(Box::new));
							},
				...
				return Some(SeqExpr { span: obj.span, exprs }.into());
			},

			// Preserves negated iife
			Expr::Unary(UnaryExpr { op: op!("!"), arg, .. })
				if (self.options.negate_iife || self.options.reduce_vars || self.options.side_effects)
			...
			Expr::Bin(BinExpr {
				...
				let mut seq = SeqExpr { span: *span, exprs: left.into_iter().chain(right).collect() }.into();
			...
			Expr::Cond(cond) => {
				...
				let cons = self.with_ctx(ctx.clone()).ignore_return_value(&mut cond.cons).map(Box::new);
				let alt = self.with_ctx(ctx.clone()).ignore_return_value(&mut cond.alt).map(Box::new);
			...
			Expr::Seq(seq) => {
				...
				let mut exprs = seq
					...
					.filter_map(|(idx, expr)| {
						...
						let ctx = Ctx { dont_use_negated_iife: idx != 0, ..self.ctx.clone() };
				...
				if exprs.len() <= 1 {
					...
					let is_last_undefined = is_pure_undefined(self.ctx.expr_ctx, exprs.last().unwrap());
					...
					if is_last_undefined {
						...
						if let Some(last) = exprs.last_mut() {
							...
							self.changed = true;
							*last = UnaryExpr { span: DUMMY_SP, op: op!("void"), arg: last.take() }.into();
						}
					}
					...
					return Some(SeqExpr { span: seq.span, exprs }.into());
				}
			},
	...
	fn try_removing_block(&mut self, s: &mut Stmt, unwrap_more: bool, allow_fn_decl: bool) {
			...
			Stmt::Block(bs) => {
				...
				if bs.stmts.len() == 1
					&& bs.stmts.iter().all(|stmt| match stmt {
						Stmt::Decl(Decl::Var(v)) if matches!(&**v, VarDecl { kind: VarDeclKind::Var, .. }) => true,
						_ => false,
					}) {
				...
				if unwrap_more && bs.stmts.len() == 1 {
					match &bs.stmts[0] {
						...
						Stmt::Decl(Decl::Fn(..)) if allow_fn_decl && !self.ctx.expr_ctx.in_strict => {
							*s = bs.stmts[0].take();
		...
		if !self.options.conditionals && !self.options.sequences() && !self.options.join_vars && !self.options.unused {
			return;
		}
		...
		match s {
			...
			Stmt::Block(block) if block.stmts.len() == 1 && is_fine_for_if_cons(&block.stmts[0]) => {
...
impl VisitMut for Optimizer<'_> {
	...
	fn visit_mut_arrow_expr(&mut self, n: &mut ArrowExpr) {
		...
		{
			let ctx = Ctx { in_param: true, ..self.ctx.clone() };
		...
		if !self.prepend_stmts.is_empty() {
			...
			match &mut *n.body {
				...
				BlockStmtOrExpr::Expr(v) => {
					...
					stmts.push(ReturnStmt { span: DUMMY_SP, arg: Some(v.take()) }.into());
	...
	fn visit_mut_assign_expr(&mut self, e: &mut AssignExpr) {
		{
			let ctx = Ctx { is_lhs_of_assign: true, is_exact_lhs_of_assign: true, ..self.ctx.clone() };
	...
	fn visit_mut_bin_expr(&mut self, n: &mut BinExpr) {
		{
			let ctx = Ctx { in_cond: self.ctx.in_cond || n.op.may_short_circuit(), ..self.ctx.clone() };
		...
		if is_this_undefined {
			if let Callee::Expr(callee) = &mut e.callee {
				if let Expr::Member(..) = &mut **callee {
					let zero = Lit::Num(Number { span: DUMMY_SP, value: 0.0, raw: None }).into();
					self.changed = true;
					report_change!("injecting zero to preserve `this` in call");

					*callee = SeqExpr { span: callee.span(), exprs: vec![zero, callee.take()] }.into();
				}
			}
		}
	...
	fn visit_mut_class(&mut self, n: &mut Class) {
		...
		{
			let ctx = Ctx { dont_invoke_iife: true, is_update_arg: false, ..self.ctx.clone() };
...

		{
			let ctx = Ctx {
				is_update_arg: false,
				expr_ctx: ExprCtx { in_strict: true, ..self.ctx.clone().expr_ctx },
				..self.ctx.clone()
			};
	...
	fn visit_mut_do_while_stmt(&mut self, n: &mut DoWhileStmt) {
		{
			let ctx = Ctx { executed_multiple_time: true, ..self.ctx.clone() };
			n.visit_mut_children_with(&mut *self.with_ctx(ctx));
		}
	}
	...
	fn visit_mut_export_decl(&mut self, n: &mut ExportDecl) {
		...
		let ctx = Ctx { is_exported: true, ..self.ctx.clone() };
		n.visit_mut_children_with(&mut *self.with_ctx(ctx));
	}
	...
	fn visit_mut_expr(&mut self, e: &mut Expr) {
		...
		let _tracing = {
			let s = dump(&*e, true);
			tracing::span!(tracing::Level::ERROR, "visit_mut_expr", src = tracing::field::display(&s)).entered()
		};
		let ctx = Ctx { is_exported: false, is_callee: false, ..self.ctx.clone() };
		...
		match e {
			...
			Expr::Assign(AssignExpr { op: op!("="), left, right, .. }) => {
				if let Some(i) = left.as_ident_mut() {
					...
					if right.is_invalid() {
						if let Some(lit) = self.vars.lits.get(&old).or_else(|| self.vars.vars_for_inlining.get(&old)) {
							*e = (**lit).clone();
						}
					}
				}
	...
	fn visit_mut_expr_stmt(&mut self, n: &mut ExprStmt) {
		...
		if is_directive {
			if !was_directive {
				*n = ExprStmt { span: DUMMY_SP, expr: Take::dummy() };
			}
			return;
		}
		...
		if need_ignore_return_value
			...
			|| (self.options.conditionals && matches!(&*n.expr, Expr::Bin(BinExpr { op: op!("||") | op!("&&"), .. })))
		{
			// Preserve top-level negated iifes.
			if let Expr::Unary(unary @ UnaryExpr { op: op!("!"), .. }) = &*n.expr {
				if let Expr::Call(CallExpr { callee: Callee::Expr(callee), .. }) = &*unary.arg {
	...
	fn visit_mut_fn_decl(&mut self, f: &mut FnDecl) {
		#[cfg(feature = "debug")]
		let _tracing = tracing::span!(Level::ERROR, "visit_mut_fn_decl", id = tracing::field::display(&f.ident)).entered();
	...
	fn visit_mut_function(&mut self, n: &mut Function) {
		...
		{
			let ctx = Ctx { in_fn_like: true, scope: n.ctxt, top_level: false, ..self.ctx.clone() };
		...
		{
			self.with_ctx(self.ctx.clone()).optimize_usage_of_arguments(n);
		}
	...
	fn visit_mut_if_stmt(&mut self, n: &mut IfStmt) {
		...
		let ctx = Ctx { in_cond: true, ..self.ctx.clone() };
	...
	fn visit_mut_labeled_stmt(&mut self, n: &mut LabeledStmt) {
		let ctx = Ctx {
			dont_use_prepend_nor_append: contains_leaping_continue_with_label(&n.body, n.label.sym.clone()),
			..self.ctx.clone()
		};
	...
	fn visit_mut_module_item(&mut self, s: &mut ModuleItem) {
		...
		if let ModuleItem::ModuleDecl(ModuleDecl::ExportDecl(ExportDecl { decl: Decl::Var(v), .. })) = s {
			if v.decls.is_empty() {
				s.take();
			}
		}
	...
	fn visit_mut_module_items(&mut self, stmts: &mut Vec<ModuleItem>) {
		let ctx = Ctx { top_level: true, ..self.ctx.clone() };
	...
	fn visit_mut_new_expr(&mut self, n: &mut NewExpr) {
		...
		{
			let ctx = Ctx { is_exact_lhs_of_assign: false, is_lhs_of_assign: false, ..self.ctx.clone() };
	...
	fn visit_mut_param(&mut self, n: &mut Param) {
		let ctx = Ctx { var_kind: None, in_param: true, ..self.ctx.clone() };
	...
	fn visit_mut_prop(&mut self, n: &mut Prop) {
		...
		if let Prop::Shorthand(i) = n {
			if self.vars.has_pending_inline_for(&i.to_id()) {
				...
				*n = Prop::KeyValue(KeyValueProp { key: PropName::Ident(i.clone().into()), value: Box::new(e) });
			}
		}
	...
	fn visit_mut_stmt(&mut self, s: &mut Stmt) {
		...
		match s {
			Stmt::Labeled(LabeledStmt { label: Ident { sym, .. }, body, .. }) if sym.is_empty() => {
				*s = *body.take();
		...
		match s {
			...
			Stmt::Return(ReturnStmt { arg: Some(arg), .. }) => {
				if let Some(mut block) = self.invoke_iife_stmt(&mut *arg, true) {
					if !block.stmts.last().map(swc_ecma_utils::StmtExt::terminates).unwrap_or(false) {
						block.stmts.push(Stmt::Return(ReturnStmt { span: DUMMY_SP, arg: None }))
					}
					*s = Stmt::Block(block)
				}
			},
		...
		if let Stmt::Expr(ExprStmt { expr, .. }) = s {
			...
			if self.options.unused {
				let can_be_removed =
					!is_directive && !expr.is_ident() && !expr.may_have_side_effects(self.ctx.expr_ctx);
	...
	fn visit_mut_tagged_tpl(&mut self, n: &mut TaggedTpl) {
		n.tag
			.visit_mut_with(&mut *self.with_ctx(Ctx { is_this_aware_callee: true, ..self.ctx.clone() }));
	...
	fn visit_mut_tpl(&mut self, n: &mut Tpl) {
		...
		{
			let ctx = Ctx { in_tpl_expr: true, ..self.ctx.clone() };
			let mut o = self.with_ctx(ctx);
			n.visit_mut_children_with(&mut *o);
		}

		n.exprs.iter_mut().for_each(|expr| self.optimize_expr_in_str_ctx(expr));
	}

	#[cfg_attr(feature = "debug", tracing::instrument(skip_all))]
	fn visit_mut_try_stmt(&mut self, n: &mut TryStmt) {
		let ctx = Ctx { in_try_block: true, ..self.ctx.clone() };
	...
	fn visit_mut_update_expr(&mut self, n: &mut UpdateExpr) {
		let ctx = Ctx { is_update_arg: true, ..self.ctx.clone() };
	...
	fn visit_mut_using_decl(&mut self, n: &mut UsingDecl) {
		let ctx = Ctx { is_update_arg: false, has_const_ann: false, var_kind: None, ..self.ctx.clone() };
			...
			n.decls.iter_mut().for_each(|var| {
				if let Some(e) = &var.init {
					if is_pure_undefined(self.ctx.expr_ctx, e) {
						self.changed = true;
						report_change!("Dropping explicit initializer which evaluates to `undefined`");
	...
	fn visit_mut_var_declarator(&mut self, var: &mut VarDeclarator) {
		...
		if let VarDeclarator { name: Pat::Ident(id), init: Some(init), definite: false, .. } = var {
					...
					self.prepend_stmts.push(
						ExprStmt {
							...
							expr: if side_effects.len() == 1 {
								...
								SeqExpr { span: DUMMY_SP, exprs: side_effects.take() }.into()
							},
						}
						.into(),
					);
				...
				self.append_stmts.push(
					ExprStmt {
						...
						expr: if side_effects.len() == 1 {
							...
							SeqExpr { span: DUMMY_SP, exprs: side_effects }.into()
						},
					}
					.into(),
				);
	...
	fn visit_mut_with_stmt(&mut self, n: &mut WithStmt) {
		...
		{
			let ctx = Ctx { in_with_stmt: true, ..self.ctx.clone() };
			n.body.visit_mut_with(&mut *self.with_ctx(ctx));
		}
	}
