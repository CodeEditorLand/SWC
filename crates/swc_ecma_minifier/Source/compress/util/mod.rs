...
fn negate_inner(expr_ctx: ExprCtx, e: &mut Expr, in_bool_ctx: bool, is_ret_val_ignored: bool) -> bool {
	#[cfg(feature = "debug")]
	...
	match e {
		...
		Expr::Bin(BinExpr { left, right, op: op @ op!("&&"), .. }) if is_ok_to_negate_rhs(expr_ctx, right) => {
		...
		Expr::Cond(CondExpr { cons, alt, .. }) if is_ok_to_negate_for_cond(cons) && is_ok_to_negate_for_cond(alt) => {
	...
	if let Expr::Unary(UnaryExpr { op: op!("!"), arg, .. }) = &mut *arg {
		match &mut **arg {
			...
			Expr::Bin(BinExpr { op: op!("in"), .. }) | Expr::Bin(BinExpr { op: op!("instanceof"), .. }) => {
				report_change!("negate: !bool => bool");
				*e = *arg.take();
				return true;
			},
	...
	if is_ret_val_ignored {
		...
		*e = UnaryExpr { span: DUMMY_SP, op: op!("!"), arg }.into();
...
pub(crate) fn is_ok_to_negate_rhs(expr_ctx: ExprCtx, rhs: &Expr) -> bool {
	match rhs {
		Expr::Member(..) => true,
		Expr::Bin(BinExpr { op: op!("===") | op!("!==") | op!("==") | op!("!="), .. }) => true,
		...
		Expr::Unary(UnaryExpr { op: op!("!") | op!("delete"), .. }) => true,
		...
		Expr::Cond(e) => is_ok_to_negate_rhs(expr_ctx, &e.cons) && is_ok_to_negate_rhs(expr_ctx, &e.alt),
...
pub(crate) fn negate_cost(expr_ctx: ExprCtx, e: &Expr, in_bool_ctx: bool, is_ret_val_ignored: bool) -> isize {
		...
		let cost = (|| {
			match e {
				Expr::Unary(UnaryExpr { op: op!("!"), arg, .. }) => {
					...
					if let Expr::Call(CallExpr { callee: Callee::Expr(callee), .. }) = &**arg {
				...
				Expr::Bin(BinExpr { op: op!("===") | op!("!==") | op!("==") | op!("!="), .. }) => 0,

				Expr::Bin(BinExpr { op: op @ op!("||") | op @ op!("&&"), left, right, .. }) => {
					let l_cost = cost(expr_ctx, left, in_bool_ctx || is_ret_val_ignored, Some(*op), false);
				...
				Expr::Cond(..) | Expr::Update(..) | Expr::Bin(BinExpr { op: op!("in") | op!("instanceof"), .. }) => 3,
...
pub(crate) fn is_pure_undefined(expr_ctx: ExprCtx, e: &Expr) -> bool {
	match e {
		Expr::Unary(UnaryExpr { op: UnaryOp::Void, arg, .. }) if !arg.may_have_side_effects(expr_ctx) => true,
...
pub(crate) fn is_valid_identifier(s: &str, ascii_only: bool) -> bool {
	...
	s.starts_with(Ident::is_valid_start) && s.chars().skip(1).all(Ident::is_valid_continue) && !s.is_reserved()
}
				...
				match &**obj {
					Expr::Ident(obj) if &*obj.sym == "Math" => match &*prop.sym {
						...
						"max" => {
							...
							return Some(
								numbers.into_iter().max_by(|&a, &b| cmp_num(a, b)).unwrap_or(f64::NEG_INFINITY),
							);
						},
						...
						"min" => {
							...
							return Some(numbers.into_iter().min_by(|&a, &b| cmp_num(a, b)).unwrap_or(f64::INFINITY));
						},
						...
						"pow" => {
							...
							let exponent: JsNumber = eval_as_number(expr_ctx, &args[1].expr)?.into();
...
pub(super) fn is_fine_for_if_cons(s: &Stmt) -> bool {
	match s {
		Stmt::Decl(Decl::Fn(FnDecl { ident: Ident { sym, .. }, .. })) if &**sym == "undefined" => false,
	...
	T: ModuleItemExt,
{
	stmts.retain(|s| match s.as_module_decl() {
		Ok(s) => match s {
			ModuleDecl::ExportDecl(ExportDecl { decl: Decl::Var(v), .. }) => !v.decls.is_empty(),
			_ => true,
		},
		Err(s) => match s {
			Stmt::Empty(..) => false,
		...
		if v.vars.is_empty() {
			...
			*s = VarDecl {
				...
				decls: v
					...
					.map(Pat::Ident)
					.map(|name| VarDeclarator { span: DUMMY_SP, name, init: None, definite: false })
					.collect(),
				..Default::default()
			}
			.into()
...
pub(crate) fn can_absorb_negate(e: &Expr, expr_ctx: ExprCtx) -> bool {
	match e {
		...
		Expr::Unary(UnaryExpr { op: op!("!"), arg, .. }) => arg.get_type(expr_ctx) == Value::Known(Type::Bool),
		_ => false,
	}
}
