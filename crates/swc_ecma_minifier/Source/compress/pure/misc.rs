...
impl Pure<'_> {
	...
	pub(super) fn compress_bin_assignment_to_left(&mut self, e: &mut AssignExpr) {
		...
		let (op, right) = match &mut *e.right {
			Expr::Bin(BinExpr { left, op, right, .. }) => match &**left {
	...
	pub(super) fn compress_bin_assignment_to_right(&mut self, e: &mut AssignExpr) {
		...
		let (op, left) = match &mut *e.right {
			Expr::Bin(BinExpr { left, op, right, .. }) => match &**right {
	...
	pub(super) fn eval_spread_object(&mut self, e: &mut ObjectLit) {
		fn should_skip(p: &PropOrSpread, expr_ctx: ExprCtx) -> bool {
			match p {
				...
				PropOrSpread::Spread(SpreadElement { expr, .. }) => match &**expr {
					Expr::Object(ObjectLit { props, .. }) => props.iter().any(|p| should_skip(p, expr_ctx)),
					_ => false,
				},
			}
		}
		...
		if e.props.iter().any(|p| should_skip(p, self.expr_ctx))
			|| !e.props.iter().any(|p| match p {
				PropOrSpread::Spread(SpreadElement { expr, .. }) => expr.is_object() || expr.is_null(),
				_ => false,
			}) {
			return;
		}
		...
		for prop in e.props.take() {
			match prop {
				PropOrSpread::Spread(SpreadElement { expr, .. }) if expr.is_object() || expr.is_null() => match *expr {
	...
	pub(super) fn eval_spread_array_in_args(&mut self, args: &mut Vec<ExprOrSpread>) {
		if !args.iter().any(|arg| arg.spread.is_some() && arg.expr.is_array()) {
			return;
		}
		...
		for arg in args.take() {
			match arg {
				ExprOrSpread { spread: Some(spread), expr } => match *expr {
					Expr::Array(ArrayLit { elems, .. }) => {
						for elem in elems {
							match elem {
								...
								None => {
									new_args.push(ExprOrSpread { spread: None, expr: Expr::undefined(DUMMY_SP) });
								},
							}
						}
					},
					_ => {
						new_args.push(ExprOrSpread { spread: Some(spread), expr });
					},
				},
	...
	pub(super) fn eval_spread_array_in_array(&mut self, args: &mut Vec<Option<ExprOrSpread>>) {
		if !args
			.iter()
			.any(|arg| arg.as_ref().map_or(false, |arg| arg.spread.is_some() && arg.expr.is_array()))
		{
			return;
		}
		...
		for arg in args.take() {
			match arg {
				Some(ExprOrSpread { spread: Some(spread), expr }) => match *expr {
					Expr::Array(ArrayLit { elems, .. }) => {
						for elem in elems {
							match elem {
								Some(ExprOrSpread { expr, spread }) => {
								...
								None => {
									new_args.push(Some(ExprOrSpread { spread: None, expr: Expr::undefined(DUMMY_SP) }));
								},
							}
						}
					},
					_ => {
						new_args.push(Some(ExprOrSpread { spread: Some(spread), expr }));
					},
				},
				arg => new_args.push(arg),
			}
	...
	pub(super) fn compress_array_join(&mut self, e: &mut Expr) {
		...
		let arr = match callee {
			Expr::Member(MemberExpr { obj, prop: MemberProp::Ident(IdentName { sym, .. }), .. }) if *sym == *"join" => {
		...
		if arr
			.elems
			.iter()
			.any(|elem| matches!(elem, Some(ExprOrSpread { spread: Some(..), .. })))
		{
			return;
		}

		let cannot_join_as_str_lit = arr.elems.iter().filter_map(|v| v.as_ref()).any(|v| match &*v.expr {
		...
		if cannot_join_as_str_lit {
			if let Some(new_expr) = self.compress_array_join_as_tpl(arr.span, &mut arr.elems, &separator) {
				self.changed = true;
				*e = new_expr;
				return;
			}
			...
			if arr.elems.iter().filter_map(|v| v.as_ref()).any(|v| match &*v.expr {
				e if is_pure_undefined(self.expr_ctx, e) => false,
				Expr::Lit(lit) => !matches!(lit, Lit::Str(..) | Lit::Num(..) | Lit::Null(..)),
			...
			let sep: Box<Expr> = Lit::Str(Str { span: DUMMY_SP, raw: None, value: separator }).into();
			let mut res = Lit::Str(Str { span: DUMMY_SP, raw: None, value: atom!("") }).into();

			fn add(to: &mut Expr, right: Box<Expr>) {
				let lhs = to.take();
				*to = BinExpr { span: DUMMY_SP, left: Box::new(lhs), op: op!(bin, "+"), right }.into();
			}
		...
		for (last, elem) in arr.elems.iter().identify_last() {
			if let Some(elem) = elem {
				...
				match &*elem.expr {
					...
					Expr::Lit(Lit::Null(..)) => {},
					_ => {
						unreachable!("Expression {:#?} cannot be joined and it should be filtered out", elem.expr)
					},
				}
			}
		...
		self.changed = true;
		*e = Lit::Str(Str { span: call.span, raw: None, value: res.into() }).into()
	}
	...
	fn optimize_regex(&mut self, args: &mut Vec<ExprOrSpread>, span: &mut Span) -> Option<Expr> {
		...
		let (pattern, flag) = match args.as_slice() {
			...
			[
				ExprOrSpread { spread: None, expr: pattern },
				ExprOrSpread { spread: None, expr: flag },
			] => (valid_pattern(pattern)?, valid_flag(flag, self.options.ecma)?),
			_ => return None,
		};
	...
	fn optimize_array(&mut self, args: &mut Vec<ExprOrSpread>, span: &mut Span) -> Option<Expr> {
		if args.len() == 1 {
				...
				match &**expr {
					Expr::Lit(Lit::Num(num)) => {
						if num.value <= 5_f64 && num.value >= 0_f64 {
							Some(ArrayLit { span: *span, elems: vec![None; num.value as usize] }.into())
						} else {
							None
						}
					},
					Expr::Lit(_) => Some(ArrayLit { span: *span, elems: vec![args.take().into_iter().next()] }.into()),
					_ => None,
				}
			} else {
				None
			...
			Some(ArrayLit { span: *span, elems: args.take().into_iter().map(Some).collect() }.into())
		}
	}
	...
	fn optimize_object(&mut self, args: &mut Vec<ExprOrSpread>, span: &mut Span) -> Option<Expr> {
		if args.is_empty() {
			Some(ObjectLit { span: *span, props: Vec::new() }.into())
		} else {
			None
		}
	}
	...
	pub(super) fn optimize_opt_chain(&mut self, e: &mut Expr) {
		...
		if let OptChainBase::Member(base) = &mut *opt.base {
			if match &*base.obj {
				...
				*e = MemberExpr { span: opt.span, obj: base.obj.take(), prop: base.prop.take() }.into();
			}
		}
	}
	...
	pub(super) fn optimize_builtin_object(&mut self, e: &mut Expr) {
		...
		match e {
			Expr::New(NewExpr { span, callee, args: Some(args), .. })
			| Expr::Call(CallExpr { span, callee: Callee::Expr(callee), args, .. })
				if callee.is_one_of_global_ref_to(self.expr_ctx, &["Array", "Object", "RegExp"]) =>
			{
				let new_expr = match &**callee {
					Expr::Ident(Ident { sym, .. }) if &**sym == "RegExp" => self.optimize_regex(args, span),
					Expr::Ident(Ident { sym, .. }) if &**sym == "Array" => self.optimize_array(args, span),
					Expr::Ident(Ident { sym, .. }) if &**sym == "Object" => self.optimize_object(args, span),
					_ => unreachable!(),
				};

				if let Some(new_expr) = new_expr {
					report_change!("Converting Regexp/Array/Object call to native constructor into literal");
					self.changed = true;
					*e = new_expr;
					return;
				}
			...
			Expr::Call(CallExpr { span, callee: Callee::Expr(callee), args, .. })
				if callee.is_one_of_global_ref_to(self.expr_ctx, &["Boolean", "Number", "String", "Symbol"]) =>
			{
				let new_expr = match &**callee {
					Expr::Ident(Ident { sym, .. }) if &**sym == "Boolean" => match &mut args[..] {
						[] => Some(Lit::Bool(Bool { span: *span, value: false }).into()),
						[ExprOrSpread { spread: None, expr }] => Some(
							UnaryExpr {
								span: *span,
								op: op!("!"),
								arg: UnaryExpr { span: *span, op: op!("!"), arg: expr.take() }.into(),
							}
							.into(),
						),
						_ => None,
					...
					Expr::Ident(Ident { sym, .. }) if &**sym == "Number" => match &mut args[..] {
						[] => Some(Lit::Num(Number { span: *span, value: 0.0, raw: None }).into()),
						// this is indeed very unsafe in case of BigInt
						[ExprOrSpread { spread: None, expr }] if self.options.unsafe_math => {
							Some(UnaryExpr { span: *span, op: op!(unary, "+"), arg: expr.take() }.into())
						},
						_ => None,
					},
					Expr::Ident(Ident { sym, .. }) if &**sym == "String" => match &mut args[..] {
						[] => Some(Lit::Str(Str { span: *span, value: "".into(), raw: None }).into()),
						// this is also very unsafe in case of Symbol
						[ExprOrSpread { spread: None, expr }] if self.options.unsafe_passes => Some(
							BinExpr {
								span: *span,
								left: expr.take(),
								op: op!(bin, "+"),
								right: Lit::Str(Str { span: *span, value: "".into(), raw: None }).into(),
							}
							.into(),
						),
						_ => None,
		...
		match e {
			Expr::New(NewExpr { span, ctxt, callee, args, .. })
			...
			{
				self.changed = true;
				report_change!("new operator: Compressing `new Array/RegExp/..` => `Array()/RegExp()/..`");
				*e = CallExpr {
					span: *span,
					ctxt: *ctxt,
	...
	fn drop_return_value(&mut self, stmts: &mut Vec<Stmt>) -> bool {
		for s in stmts.iter_mut() {
			if let Stmt::Return(ReturnStmt { arg: arg @ Some(..), .. }) = s {
	...
	fn compress_array_join_as_tpl(
		...
		let mut new_tpl = Tpl { span, quasis: Vec::new(), exprs: Vec::new() };
	...
	fn make_ignored_expr(&mut self, span: Span, exprs: impl Iterator<Item = Box<Expr>>) -> Option<Expr> {
		let mut exprs = exprs
			.filter_map(|mut e| {
				self.ignore_return_value(
					&mut e,
					DropOpts { drop_global_refs_if_unused: true, drop_str_lit: true, drop_number: true },
				);

				if let Expr::Invalid(..) = &*e { None } else { Some(e) }
			})
			.collect::<Vec<_>>();
		...
		Some(SeqExpr { span: DUMMY_SP, exprs }.into())
	}
	...
	pub(super) fn ignore_return_value(&mut self, e: &mut Expr, opts: DropOpts) {
		...
		match e {
			...
			Expr::Call(CallExpr { span, ctxt, args, .. }) if ctxt.has_mark(self.marks.pure) => {
				report_change!("ignore_return_value: Dropping a pure call");
				self.changed = true;

				let new = self.make_ignored_expr(*span, args.take().into_iter().map(|arg| arg.expr));
			...
			Expr::TaggedTpl(TaggedTpl { span, ctxt, tpl, .. }) if ctxt.has_mark(self.marks.pure) => {
				report_change!("ignore_return_value: Dropping a pure call");
				self.changed = true;
			...
			Expr::New(NewExpr { callee, span, ctxt, args, .. })
				if callee.is_pure_callee(self.expr_ctx) || ctxt.has_mark(self.marks.pure) =>
			{
				report_change!("ignore_return_value: Dropping a pure call");
				self.changed = true;

				let new = self.make_ignored_expr(*span, args.take().into_iter().flatten().map(|arg| arg.expr));
		...
		match e {
			...
			Expr::TaggedTpl(TaggedTpl { span, tag: callee, tpl, .. }) => {
				if callee.is_pure_callee(self.expr_ctx) {
					self.changed = true;
		...
		if self.options.conditionals || self.options.expr {
			if let Expr::Cond(CondExpr { span, test, cons, alt, .. }) = e {
				...
				if cons.is_invalid() {
					*e = Expr::Bin(BinExpr { span: *span, op: op!("||"), left: test.take(), right: alt.take() });
				...
				if alt.is_invalid() {
					*e = Expr::Bin(BinExpr { span: *span, op: op!("&&"), left: test.take(), right: cons.take() });
		...
		if let Expr::Ident(i) = e {
			...
			if i.ctxt.outer() == self.marks.unresolved_mark {
				if self.options.side_effects || (self.options.unused && opts.drop_global_refs_if_unused) {
		...
		if self.options.side_effects || self.options.dead_code || self.options.collapse_vars || self.options.expr {
			match e {
				Expr::Unary(UnaryExpr { op: op!("!"), arg, .. }) => {
					self.ignore_return_value(
						arg,
						DropOpts { drop_str_lit: true, drop_global_refs_if_unused: true, drop_number: true, ..opts },
					);
				...
				Expr::Unary(UnaryExpr {
					op: op!("void") | op!(unary, "+") | op!(unary, "-") | op!("~"), arg, ..
				}) => {
					self.ignore_return_value(
						arg,
						DropOpts { drop_str_lit: true, drop_global_refs_if_unused: true, drop_number: true, ..opts },
				...
				Expr::Tpl(tpl) => {
					...
					if tpl.exprs.is_empty() {
						...
						if tpl.exprs.len() == 1 {
							...
							*e = SeqExpr { span: tpl.span, exprs: tpl.exprs.take() }.into();
						}
					}
		...
		match e {
			...
			Expr::Lit(Lit::Null(..) | Lit::BigInt(..) | Lit::Bool(..) | Lit::Regex(..)) | Expr::This(..) => {
			...
			Expr::Bin(
				...
				self.ignore_return_value(
					&mut bin.left,
					DropOpts { drop_number: true, drop_global_refs_if_unused: true, drop_str_lit: true, ..opts },
				);
				self.ignore_return_value(
					&mut bin.right,
					DropOpts { drop_number: true, drop_global_refs_if_unused: true, drop_str_lit: true, ..opts },
				...
				if matches!(*bin.left, Expr::Await(..) | Expr::Update(..)) {
					...
					*e = SeqExpr { span, exprs: vec![bin.left.take(), bin.right.take()] }.into();
					return;
				}
			},
			...
			Expr::Array(arr) => {
				for elem in arr.elems.iter_mut().flatten() {
					if elem.spread.is_none() {
						self.ignore_return_value(
							&mut elem.expr,
							DropOpts { drop_number: true, drop_str_lit: true, ..Default::default() },
						);
					}
				}

				arr.elems.retain(|e| e.as_ref().map_or(false, |e| !e.expr.is_invalid()));
				...
				for prop in obj.props.iter_mut() {
					match prop {
						...
						PropOrSpread::Prop(p) => match &mut **p {
							Prop::KeyValue(kv) => {
								if !kv.key.is_computed() && !kv.value.may_have_side_effects(self.expr_ctx) {
							...
							Prop::Shorthand(i) => {
								if i.ctxt.outer() != self.marks.unresolved_mark {
									*i = Ident::dummy();
									self.changed = true;
									report_change!("Dropping a shorthand property in an object literal");
								}
							},
		...
		match e {
			Expr::Lit(Lit::Str(s)) => {
				if (self.options.directives && !matches!(&*s.value, "use strict" | "use asm"))
					|| opts.drop_str_lit
					|| (s.value.starts_with("@swc/helpers") || s.value.starts_with("@babel/helpers"))
				{
					self.changed = true;
					*e = Invalid { span: DUMMY_SP }.into();
			...
			Expr::Call(CallExpr { callee: Callee::Expr(callee), .. }) if callee.is_fn_expr() => match &mut **callee {
				Expr::Fn(callee) => {
		...
		if self.options.side_effects && self.options.pristine_globals {
			match e {
				Expr::New(NewExpr { span, callee, args, .. })
					if callee.is_one_of_global_ref_to(
						self.expr_ctx,
					...
					) =>
				{
					...
					self.changed = true;
					*e = self
						.make_ignored_expr(*span, args.iter_mut().flatten().map(|arg| arg.expr.take()))
						.unwrap_or(Invalid { span: DUMMY_SP }.into());
					return;
				},

				Expr::Call(CallExpr { span, callee: Callee::Expr(callee), args, .. })
					if callee.is_one_of_global_ref_to(self.expr_ctx, &["Array", "Object", "Boolean", "Number"]) =>
				{
				...
				Expr::Array(arr) => {
					if arr.elems.iter().any(|e| match e {
						Some(ExprOrSpread { spread: Some(..), .. }) => true,
						_ => false,
					}) {
						*e = ArrayLit {
							elems: arr
								...
								.filter_map(|mut e| {
									...
									Some(ExprOrSpread { spread: None, expr: e.expr })
								})
								.map(Some)
								.collect(),
							..*arr
				...
				//
				//  foo(),basr(),foo;
				Expr::Member(MemberExpr { span, obj, prop: MemberProp::Computed(prop), .. }) => match obj.as_mut() {
					Expr::Object(object) => {
					...
					Expr::Array(..) => {
						...
						*e = self
							.make_ignored_expr(*span, vec![obj.take(), prop.expr.take()].into_iter())
							.unwrap_or(Invalid { span: DUMMY_SP }.into());
						return;
					},
					_ => {},
	...
	pub(super) fn compress_negated_bin_eq(&self, e: &mut Expr) {
		...
		match &mut *unary.arg {
			Expr::Bin(BinExpr { op: op @ op!("=="), left, right, .. })
			| Expr::Bin(BinExpr { op: op @ op!("==="), left, right, .. }) => {
				*e = BinExpr {
					span: unary.span,
					op: if *op == op!("==") { op!("!=") } else { op!("!==") },
					left: left.take(),
					right: right.take(),
				}
				.into()
	...
	pub(super) fn optimize_nullish_coalescing(&mut self, e: &mut Expr) {
		let (l, r) = match e {
			Expr::Bin(BinExpr { op: op!("??"), left, right, .. }) => (&mut **left, &mut **right),
			_ => return,
		};
	...
	pub(super) fn drop_neeedless_pat(&mut self, p: &mut Pat) {
		if let Pat::Assign(assign) = p {
			if is_pure_undefined(self.expr_ctx, &assign.right) {
				*p = *assign.left.take();
				self.changed = true;
				report_change!("Converting an assignment pattern with undefined on RHS to a normal pattern");
			}
		}
	}
	...
	pub(super) fn drop_needless_block(&mut self, s: &mut Stmt) {
		...
		if let Stmt::Block(bs) = s {
			...
			if bs.stmts.len() == 1 && !is_block_scoped_stmt(&bs.stmts[0]) && is_simple_stmt(&bs.stmts[0]) {
				*s = bs.stmts.remove(0);
				self.changed = true;
