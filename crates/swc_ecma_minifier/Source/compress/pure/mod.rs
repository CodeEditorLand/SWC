...
impl VisitMut for Pure<'_> {
	...
	fn visit_mut_assign_expr(&mut self, e: &mut AssignExpr) {
		{
			let ctx = Ctx { is_lhs_of_assign: true, ..self.ctx };
	...
	fn visit_mut_call_expr(&mut self, e: &mut CallExpr) {
		{
			let ctx = Ctx { is_callee: true, ..self.ctx };
			e.callee.visit_mut_with(&mut *self.with_ctx(ctx));
		}

		{
			let ctx = Ctx { is_callee: false, ..self.ctx };
	...
	fn visit_mut_do_while_stmt(&mut self, s: &mut DoWhileStmt) {
		...
		{
			let ctx = Ctx { preserve_block: true, ..self.ctx };
	...
	fn visit_mut_expr(&mut self, e: &mut Expr) {
		...
		match e {
			Expr::Member(..) => {
				if !(self.ctx.in_delete || self.ctx.is_update_arg || self.ctx.is_lhs_of_assign || self.ctx.in_opt_chain)
				{
					let mut changed = false;
					simplify::expr::optimize_member_expr(self.expr_ctx, e, self.ctx.is_callee, &mut changed);
		...
		if self.options.unused {
			if let Expr::Unary(UnaryExpr { span, op: op!("void"), arg }) = e {
				if !arg.is_lit() {
					self.ignore_return_value(
						arg,
						DropOpts { drop_global_refs_if_unused: true, drop_number: true, drop_str_lit: true },
					);
					if arg.is_invalid() {
	...
	fn visit_mut_expr_stmt(&mut self, s: &mut ExprStmt) {
		...
		self.ignore_return_value(
			&mut s.expr,
			DropOpts { drop_number: true, drop_global_refs_if_unused: true, drop_str_lit: false },
		);
	...
	fn visit_mut_fn_decl(&mut self, n: &mut FnDecl) {
		#[cfg(feature = "debug")]
		let _tracing = tracing::span!(Level::ERROR, "visit_mut_fn_decl", id = tracing::field::display(&n.ident)).entered();

		n.visit_mut_children_with(self);
	}

	fn visit_mut_for_head(&mut self, head: &mut ForHead) {
		let ctx = Ctx { is_update_arg: true, ..Default::default() };
	...
	fn visit_mut_function(&mut self, f: &mut Function) {
		{
			let ctx = Ctx { _in_try_block: false, ..self.ctx };
	...
	fn visit_mut_if_stmt(&mut self, s: &mut IfStmt) {
		...
		match &mut s.alt {
			Some(alt) => {
				let ctx = Ctx { preserve_block: false, ..self.ctx };
				self.with_ctx(ctx).visit_par_ref(&mut [&mut *s.cons, &mut **alt]);
			},
			None => {
				let ctx = Ctx { preserve_block: false, ..self.ctx };
	...
	fn visit_mut_labeled_stmt(&mut self, s: &mut LabeledStmt) {
		let ctx = Ctx { is_label_body: true, ..self.ctx };
		s.body.visit_mut_with(&mut *self.with_ctx(ctx));
	}

	fn visit_mut_member_expr(&mut self, e: &mut MemberExpr) {
		{
			let ctx = Ctx { is_callee: false, is_update_arg: false, ..self.ctx };
			e.obj.visit_mut_with(&mut *self.with_ctx(ctx));
		}

		if let MemberProp::Computed(c) = &mut e.prop {
			let ctx = Ctx { is_callee: false, is_update_arg: false, is_lhs_of_assign: false, ..self.ctx };
	...
	fn visit_mut_opt_chain_expr(&mut self, e: &mut OptChainExpr) {
		let ctx = Ctx { in_opt_chain: true, ..Default::default() };
	...
	fn visit_mut_seq_expr(&mut self, e: &mut SeqExpr) {
		...
		let exprs = &e.exprs;
		if maybe_par!(exprs.iter().any(|e| e.is_seq()), *crate::LIGHT_TASK_PARALLELS) {
		...
		for (idx, e) in e.exprs.iter_mut().enumerate() {
			...
			if !is_last {
				self.ignore_return_value(
					e,
					DropOpts { drop_number: true, drop_global_refs_if_unused: false, drop_str_lit: true },
				);
			}
		}
	...
	fn visit_mut_stmt(&mut self, s: &mut Stmt) {
		...
		match s {
			Stmt::Expr(ExprStmt { expr, .. }) => {
				...
				self.ignore_return_value(expr, DropOpts { drop_number: true, ..Default::default() });
		...
		if let Stmt::Block(block) = s {
			let span = block.span;
			if let [Stmt::Expr(e), Stmt::Return(ReturnStmt { arg: None, .. })] = &mut block.stmts[..] {
				...
				if !(e.expr.is_bin() || e.expr.is_assign() || e.expr.is_seq()) {
					...
					*s = Stmt::Return(ReturnStmt {
						span,
						arg: Some(Box::new(Expr::Unary(UnaryExpr { span: DUMMY_SP, op: op!("void"), arg: e }))),
					})
				}
			}
		}
	...
	fn visit_mut_try_stmt(&mut self, n: &mut TryStmt) {
		let ctx = Ctx { _in_try_block: true, ..self.ctx };
	...
	fn visit_mut_unary_expr(&mut self, e: &mut UnaryExpr) {
		{
			let ctx = Ctx { in_delete: e.op == op!("delete"), ..self.ctx };
			e.visit_mut_children_with(&mut *self.with_ctx(ctx));
		}
