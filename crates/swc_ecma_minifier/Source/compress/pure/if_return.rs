...
impl Pure<'_> {
	...
	pub(super) fn negate_if_terminate(&mut self, stmts: &mut Vec<Stmt>, handle_return: bool, handle_continue: bool) {
		if handle_return {
		...
		if stmts[pos_of_if..].iter().any(|s| match s {
			Stmt::Decl(Decl::Var(v)) if matches!(&**v, VarDecl { kind: VarDeclKind::Const | VarDeclKind::Let, .. }) => {
				true
			},
			_ => false,
		}) {
		...
		self.changed = true;
		report_change!("if_return: Negating `foo` in `if (foo) return; bar()` to make it `if (!foo) bar()`");
		...
		match if_stmt {
			Stmt::If(mut s) => {
				...
				s.cons = if cons.len() == 1 && is_fine_for_if_cons(&cons[0]) {
					...
					Box::new(BlockStmt { span: DUMMY_SP, stmts: cons, ..Default::default() }.into())
				};
	...
	pub(super) fn merge_else_if(&mut self, s: &mut IfStmt) {
		...
		})) = s.alt.as_deref_mut()
		{
			...
			match &mut **alt_of_alt {
				...
				Stmt::Expr(..) => {
					*alt_of_alt = Box::new(
						BlockStmt { span: DUMMY_SP, stmts: vec![*alt_of_alt.take()], ..Default::default() }.into(),
					);
				},
				_ => {
					return;
