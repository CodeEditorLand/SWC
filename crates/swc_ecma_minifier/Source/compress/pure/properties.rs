...
impl Pure<'_> {
	pub(super) fn optimize_property_of_member_expr(
		...
		match &*c.expr {
			Expr::Lit(Lit::Str(s))
				if s.value.is_reserved() || s.value.is_reserved_in_es3() || is_valid_identifier(&s.value, true) =>
			{
				self.changed = true;
				report_change!("properties: Computed member => member expr with identifier as a prop");

				Some(IdentName { span: s.span, sym: s.value.clone() })
			},
		...
		if let PropName::Computed(c) = p {
			match &mut *c.expr {
				Expr::Lit(Lit::Str(s)) => {
					...
					if s.value.is_reserved() || s.value.is_reserved_in_es3() || is_valid_identifier(&s.value, false) {
	...
	pub(super) fn optimize_prop_name(&mut self, name: &mut PropName) {
		if let PropName::Str(s) = name {
			if s.value.is_reserved() || s.value.is_reserved_in_es3() || is_valid_identifier(&s.value, false) {
				...
				*name = PropName::Ident(IdentName { span: s.span, sym: s.value.clone() });
				return;
			}
			...
			if (!s.value.starts_with('0') && !s.value.starts_with('+')) || s.value.len() <= 1 {
				if let Ok(v) = s.value.parse::<u32>() {
					...
					*name = PropName::Num(Number { span: s.span, value: v as _, raw: None });
				}
			}
		}
	}

	pub(super) fn handle_known_computed_member_expr(&mut self, c: &mut ComputedPropName) -> Option<IdentName> {
