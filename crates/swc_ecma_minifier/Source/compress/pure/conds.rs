...
impl Pure<'_> {
	pub(super) fn merge_nested_if(&mut self, s: &mut IfStmt) {
		...
		if let Stmt::If(IfStmt { test, cons, alt: None, .. }) = &mut *s.cons {
			self.changed = true;
			report_change!("if_return: Merging nested if statements");

			s.test = BinExpr { span: s.test.span(), op: op!("&&"), left: s.test.take(), right: test.take() }.into();
			s.cons = cons.take();
		}
	}
		...
		if p.is_pure() {
			if v {
				...
				*e = if cond.cons.directness_matters() {
					Expr::Seq(SeqExpr { span: cond.span, exprs: vec![0.into(), cond.cons.take()] })
				} else {
					*cond.cons.take()
				};
			} else {
				...
				*e = if cond.alt.directness_matters() {
					Expr::Seq(SeqExpr { span: cond.span, exprs: vec![0.into(), cond.alt.take()] })
				} else {
					*cond.alt.take()
				};
			}
	...
	pub(super) fn compress_cond_with_logical_as_logical(&mut self, e: &mut Expr) {
		...
		match (&mut *cond.cons, &mut *cond.alt) {
			(Expr::Bin(cons @ BinExpr { op: op!("||"), .. }), alt) if (*cons.right).eq_ignore_span(&*alt) => {
				...
				*e = BinExpr {
					span: cond.span,
					op: op!("||"),
					left: BinExpr { span: cons_span, op: op!("&&"), left: cond.test.take(), right: cons.left.take() }
						.into(),
	...
	pub(super) fn compress_conds_as_arithmetic(&mut self, e: &mut Expr) {
		...
		match (&mut *cond.cons, &mut *cond.alt) {
			(Expr::Lit(Lit::Num(Number { value, .. })), Expr::Lit(Lit::Num(Number { value: 0.0, .. })))
				if *value > 0.0
					&& (!cond.test.is_bin() || cond.test.get_type(self.expr_ctx) == Value::Known(Type::Bool)) =>
			{
				...
				*e = Expr::Bin(BinExpr { span, op: op!("*"), left, right })
			},
			(Expr::Lit(Lit::Num(Number { value: 0.0, .. })), Expr::Lit(Lit::Num(Number { value, .. })))
				if *value > 0.0 && (!cond.test.is_bin() || can_absorb_negate(&cond.test, self.expr_ctx)) =>
			{
