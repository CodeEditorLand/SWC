...
impl Pure<'_> {
	...
	pub(super) fn handle_instant_break(&mut self, s: &mut Stmt) {
		if let Stmt::Labeled(ls) = s {
			match &*ls.body {
				Stmt::Break(BreakStmt { label: Some(label), .. }) => {
	...
	pub(super) fn optimize_labeled_stmt(&mut self, s: &mut Stmt) -> Option<()> {
		...
		if let Stmt::Labeled(ls) = s {
			...
			if let Stmt::Block(bs) = &mut *ls.body {
				...
				if let Stmt::If(IfStmt { test, cons, alt: None, .. }) = first {
					if let Stmt::Break(BreakStmt { label: Some(label), .. }) = &**cons {
						if ls.label.sym == label.sym {
							self.changed = true;
							report_change!("Optimizing labeled stmt with a break to if statement: `{}`", label);
							...
							ls.body = Box::new(
								IfStmt { span: ls.span, test, cons: Box::new(Stmt::Block(cons)), alt: None }.into(),
							);
							return None;
						}
					}
				...
				if let Stmt::If(IfStmt { test, cons, alt: Some(alt), .. }) = first {
					if let Stmt::Break(BreakStmt { label: Some(label), .. }) = &**alt {
						if ls.label.sym == label.sym {
							self.changed = true;
							report_change!("Optimizing labeled stmt with a break in alt to if statement: {}", ls.label);
							...
							ls.body = Box::new(
								IfStmt { span: ls.span, test, cons: Box::new(Stmt::Block(new_cons)), alt: None }.into(),
							);
							return None;
						}
					}
	...
	fn drop_useless_continue_inner(&mut self, label: Option<Ident>, loop_stmt: &mut Stmt) -> Option<Stmt> {
		...
		fn drop<T: StmtLike>(stmt: &mut T, last: &Stmt, need_break: bool, ctx: ExprCtx) -> bool {
			match stmt.as_stmt_mut() {
				Some(s) if s.eq_ignore_span(last) => {
					if need_break {
						*s = BreakStmt { label: None, span: s.span() }.into();
					} else {
						s.take();
					}
					true
				...
				Some(Stmt::Try(t)) => {
					...
					if let Some(stmt) = t.block.stmts.last_mut() {
						let side_effect = match last {
							...
							Stmt::Return(ReturnStmt { arg: Some(arg), .. }) => arg.may_have_side_effects(ctx),
							Stmt::Throw(_) => true,
							_ => unreachable!(),
						};
		...
		T: StmtLike,
	{
		fn is_ok(b: &BlockStmt) -> bool {
			maybe_par!(b.stmts.iter().all(is_fine_for_if_cons), *crate::LIGHT_TASK_PARALLELS)
		}
		...
		let new: Vec<T> = if old_stmts.len() >= *crate::LIGHT_TASK_PARALLELS {
				...
				.flat_map(|stmt| match stmt.try_into_stmt() {
					Ok(v) => match v {
						Stmt::Block(v) if is_ok(&v) => {
							let stmts = v.stmts;
							maybe_par!(stmts.into_iter().map(T::from).collect(), *crate::LIGHT_TASK_PARALLELS)
						},
						_ => vec![T::from(v)],
					},
					Err(v) => vec![v],
			...
			old_stmts.into_iter().for_each(|stmt| match stmt.try_into_stmt() {
				Ok(v) => match v {
					Stmt::Block(v) if is_ok(&v) => {
	...
	pub(super) fn drop_unused_stmt_at_end_of_fn(&mut self, s: &mut Stmt) {
		if let Stmt::Return(r) = s {
			if let Some(Expr::Unary(UnaryExpr { span, op: op!("void"), arg })) = r.arg.as_deref_mut() {
				report_change!("unused: Removing `return void` in end of a function");
				self.changed = true;
				*s = ExprStmt { span: *span, expr: arg.take() }.into();
			}
		}
	}
		...
		T: StmtLike,
	{
		...
		stmts.take().into_iter().for_each(|stmt| match stmt.try_into_stmt() {
			Ok(stmt) => match stmt {
				Stmt::If(mut s) => {
					if let Value::Known(v) = s.test.cast_to_bool(self.expr_ctx).1 {
						let mut var_ids = Vec::new();
						new.push(T::from(ExprStmt { span: DUMMY_SP, expr: s.test.take() }.into()));
	...
	pub(super) fn handle_known_delete(&mut self, e: &mut Expr) {
		...
		let Expr::Unary(UnaryExpr { op: op!("delete"), arg, .. }) = e else {
			return;
		};
...
{
	let mut v = LabelFinder { label, found: false };
