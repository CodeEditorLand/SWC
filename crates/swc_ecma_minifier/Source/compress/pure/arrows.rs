...
impl Pure<'_> {
	pub(super) fn unsafe_optimize_fn_as_arrow(&mut self, e: &mut Expr) {
		...
		if let Expr::Fn(FnExpr { ident: None, function }) = e {
	...
	pub(super) fn optimize_arrow_method_prop(&mut self, p: &mut Prop) {
		...
		if let Prop::Method(m) = p {
			...
			if let Some(body) = &mut m.function.body {
				if body.stmts.len() == 1 && matches!(body.stmts[0], Stmt::Return(ReturnStmt { arg: Some(..), .. })) {
					...
					let arg = body.take().stmts.remove(0).expect_return_stmt().arg.take().unwrap();

					*p = Prop::KeyValue(KeyValueProp {
						key: m.key.take(),
						value: ArrowExpr {
							span: m_span,
							params: m.function.params.take().into_iter().map(|v| v.pat).collect(),
