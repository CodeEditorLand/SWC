...
impl Pure<'_> {
	...
	pub(super) fn eval_str_addition(&mut self, e: &mut Expr) {
		let (span, l_l, r_l, r_r) = match e {
			Expr::Bin(e @ BinExpr { op: op!(bin, "+"), .. }) => match &mut *e.right {
				Expr::Bin(r @ BinExpr { op: op!(bin, "+"), .. }) => (e.span, &mut *e.left, &mut *r.left, &mut r.right),
				_ => return,
			},
			_ => return,
		};
		...
		if let (Known(lls), Known(rls)) = (lls, rls) {
			...
			*e = BinExpr { span, op: op!(bin, "+"), left: s.into(), right: r_r.take() }.into();
		}
	}
	...
	pub(super) fn eval_tpl_as_str(&mut self, e: &mut Expr) {
			...
			&& tpl.quasis[1].raw.is_empty()
		{
			...
			*e = BinExpr {
				span: tpl.span,
				op: op!(bin, "+"),
				left: tpl.quasis[0].cooked.clone().unwrap_or_else(|| tpl.quasis[0].raw.clone()).into(),
	...
	pub(super) fn eval_nested_tpl(&mut self, e: &mut Expr) {
		...
		if !tpl.exprs.iter().any(|e| match &**e {
			Expr::Tpl(t) => t.quasis.iter().all(|q| q.cooked.is_some() && !q.raw.contains('\\')),
			_ => false,
		}) {
			return;
		}
		...
		let mut new_tpl = Tpl { span: tpl.span, quasis: Default::default(), exprs: Default::default() };
			...
			if idx % 2 == 0 {
				...
				match *e {
					...
					_ => {
						...
						new_tpl
							.quasis
							.push(TplElement { span: DUMMY_SP, tail: false, cooked: Some(cooked), raw });
			...
			Expr::Tpl(t) if t.quasis.len() == 1 && t.exprs.is_empty() => {
				if let Some(value) = &t.quasis[0].cooked {
					if value.chars().all(|c| match c {
						...
						*e = Lit::Str(Str { span: t.span, raw: None, value: value.clone() }).into();
						return;
					}
				}
					...
					&& !c.contains("\\u")
				{
					...
					*e = Lit::Str(Str { span: t.span, raw: None, value }).into();
				}
			},
			_ => {},
		}
	...
	pub(super) fn compress_tpl(&mut self, tpl: &mut Tpl) {
		...
		let has_str_lit = tpl.exprs.iter().any(|expr| matches!(&**expr, Expr::Lit(Lit::Str(..))));
		if !has_str_lit {
			return;
		}
				...
				match *e {
					Expr::Lit(Lit::Str(s)) => {
						...
						if let Some(raw) = &s.raw {
							if raw.len() >= 2 {
								// Exclude quotes
								cur_raw.push_str(&convert_str_raw_to_tpl_raw(&raw[1..raw.len() - 1]));
							}
						} else {
			...
			(Expr::Tpl(l), Expr::Lit(Lit::Str(rs))) => {
				...
				if let Some(l_last) = l.quasis.last_mut() {
					...
					if let Some(cooked) = &mut l_last.cooked {
						*cooked = format!("{}{}", cooked, convert_str_value_to_tpl_cooked(&rs.value)).into();
					}
			...
			(Expr::Lit(Lit::Str(ls)), Expr::Tpl(r)) => {
				...
				if let Some(r_first) = r.quasis.first_mut() {
					...
					if let Some(cooked) = &mut r_first.cooked {
						*cooked = format!("{}{}", convert_str_value_to_tpl_cooked(&ls.value), cooked).into()
					}
	...
	pub(super) fn concat_str(&mut self, e: &mut Expr) {
		if let Expr::Bin(bin @ BinExpr { op: op!(bin, "+"), .. }) = e {
			if let Expr::Bin(left @ BinExpr { op: op!(bin, "+"), .. }) = &mut *bin.left {
					...
					if let Value::Known(Type::Str) = type_of_third {
						if let Value::Known(second_str) = left.right.as_pure_string(self.expr_ctx) {
							if let Value::Known(third_str) = bin.right.as_pure_string(self.expr_ctx) {
								...
								self.changed = true;
								report_change!("strings: Concatting `{} + {}` to `{}`", second_str, third_str, new_str);

								*e = BinExpr {
									span: bin.span,
									op: op!(bin, "+"),
									left: left.left.take(),
									right: Lit::Str(Str { span: left_span, raw: None, value: new_str.into() }).into(),
								}
								.into();
							}
						}
	...
	pub(super) fn drop_useless_addition_of_str(&mut self, e: &mut Expr) {
		if let Expr::Bin(BinExpr { op: op!(bin, "+"), left, right, .. }) = e {
...
pub(super) fn convert_str_value_to_tpl_cooked(value: &Atom) -> Cow<str> {
	value.replace("\\\\", "\\").replace("\\`", "`").replace("\\$", "$").into()
}
