use swc_common::{DUMMY_SP, EqIgnoreSpan, util::take::Take};
use swc_ecma_ast::*;
...
impl Pure<'_> {
	pub(super) fn optimize_expr_in_num_ctx(&mut self, e: &mut Expr) {
		if let Expr::Lit(Lit::Str(Str { span, value, .. })) = e {
			...
			*e = Lit::Num(Number { span: *span, value, raw: None }).into();
		}
	}
	...
	pub(super) fn lift_minus(&mut self, e: &mut Expr) {
		...
		if let Expr::Bin(arg) = e {
			...
			match &mut *arg.right {
				Expr::Unary(UnaryExpr { op: op!(unary, "-"), arg: right_arg, .. }) => {
					self.changed = true;
					...
					*e = UnaryExpr {
						span: arg.span,
						op: op!(unary, "-"),
						arg: BinExpr { span: arg.span, op: arg.op, left: arg.left.take(), right: right_arg.take() }
							.into(),
					}
					.into();
				},
					...
					if value.is_sign_negative() {
						...
						*e = UnaryExpr {
							...
							arg: BinExpr {
								...
								op: arg.op,
								left: arg.left.take(),
								right: Box::new(Expr::Lit(Lit::Num(Number { span: *span, value: -*value, raw: None }))),
							}
							.into(),
						}
						.into();
	...
	pub(super) fn optimize_to_number(&mut self, e: &mut Expr) {
		match e {
			Expr::Bin(bin) => {
				if bin.op == op!("*") && matches!(&*bin.left, Expr::Lit(Lit::Num(Number { value: 1.0, .. }))) {
					...
					*e = Expr::Unary(UnaryExpr { span, op: op!(unary, "+"), arg: value })
				}
			},
				...
				) = (&a.left, &*a.right)
				{
					if let Expr::Ident(r_id) = &**arg {
						if l_id.id.eq_ignore_span(r_id) {
							...
							a.op = op!("*=");
							a.right = Box::new(Expr::Lit(Lit::Num(Number { span: DUMMY_SP, value: 1.0, raw: None })))
						}
					}
				}
			},
