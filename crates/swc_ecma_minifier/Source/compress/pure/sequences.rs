...
impl Pure<'_> {
	pub(super) fn drop_useless_ident_ref_in_seq(&mut self, seq: &mut SeqExpr) {
		...
		if let (Expr::Assign(assign @ AssignExpr { op: op!("="), .. }), Expr::Ident(ident)) =
			(&*seq.exprs[seq.exprs.len() - 2], &*seq.exprs[seq.exprs.len() - 1])
		{
			...
			if let AssignTarget::Simple(SimpleAssignTarget::Ident(left)) = &assign.left {
				if left.id.sym == ident.sym && left.id.ctxt == ident.ctxt {
					report_change!("drop_useless_ident_ref_in_seq: Dropping `{}` as it's useless", left.id);
					self.changed = true;
					seq.exprs.pop();
				}
			}
	...
	pub(super) fn shift_assignment(&mut self, e: &mut SeqExpr) {
		...
		if let Some(last) = e.exprs.last() {
			...
			if let Expr::Assign(assign @ AssignExpr { op: op!("="), .. }) = &*e.exprs[e.exprs.len() - 2] {
		...
		T: StmtLike,
	{
		...
		self.changed = true;
		report_change!("sequences: Splitted a sequence expression to multiple expression statements");
		*stmts = new_stmts;
	}
	...
	pub(super) fn lift_seqs_of_bin(&mut self, e: &mut Expr) {
		...
		if let Expr::Seq(left) = &mut *bin.left {
			...
			exprs.push(BinExpr { span: left.span, op: bin.op, left: left_last, right: bin.right.take() }.into());

			*e = SeqExpr { span: bin.span, exprs }.into()
		}
	}
	...
	pub(super) fn lift_seqs_of_cond_assign(&mut self, e: &mut Expr) {
		...
		if let Expr::Seq(test) = &mut *cond.test {
			...
			if test.exprs.len() >= 2 {
				...
				*e = SeqExpr { span: assign.span, exprs: new_seq }.into();
			}
		}
	}
	...
	pub(super) fn merge_seq_call(&mut self, e: &mut SeqExpr) {
		...
		for idx in 0..e.exprs.len() {
			...
			if let (
				...
				Expr::Call(CallExpr { callee: Callee::Expr(b_callee), args, .. }),
			) = (&mut *a, &mut *b)
			{
				...
				match &mut **b_callee {
					Expr::Member(MemberExpr { obj: b_callee_obj, prop, .. })
						if prop.is_ident_with("apply") || prop.is_ident_with("call") =>
					{
						...
						let new = CallExpr {
							span,
							callee: MemberExpr { span: DUMMY_SP, obj, prop: prop.take() }.as_callee(),
							args: args.take(),
							..Default::default()
						}
						.into();
						b.take();
						self.changed = true;
						report_change!("sequences: Reducing `(a = foo, a.call())` to `((a = foo).call())`");
