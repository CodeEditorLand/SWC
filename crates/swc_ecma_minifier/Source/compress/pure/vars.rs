...
use swc_ecma_ast::*;
use swc_ecma_utils::{StmtLike, prepend_stmt};
...
impl Pure<'_> {
...

		{
			...
			let can_work = stmts.windows(2).any(|stmts| match (stmts[0].as_stmt(), stmts[1].as_stmt()) {
				(Some(Stmt::Decl(Decl::Var(l))), Some(r)) => match r {
					...
					Stmt::For(ForStmt { init: Some(VarDeclOrExpr::VarDecl(v)), .. })
				...
				Ok(stmt) => {
					...
					match stmt {
						...
						Stmt::For(mut stmt) => match &mut stmt.init {
							...
							None if cur.as_ref().map(|v| v.kind == VarDeclKind::Var).unwrap_or(true) => {
								stmt.init = cur
									.take()
	...
	where
		T: StmtLike,
		Vec<T>: VisitWith<VarWithOutInitCounter> + VisitMutWith<VarMover> + VisitMutWith<VarPrepender>,
	{
			...
			let if_need_work = stmts.iter().any(|stmt| {
				match stmt.as_stmt() {
					Some(Stmt::Decl(Decl::Var(v))) if v.kind == target => {
						if !(found_other && found_vars_without_init) && v.decls.iter().all(|v| v.init.is_none()) {
							if found_other {
								need_work = true;
							}
		...
		let vars = {
			let mut v = VarMover { target, vars: Default::default(), var_decl_kind: Default::default() };
...
impl VisitMut for VarMover {
	...
	fn visit_mut_var_declarators(&mut self, d: &mut Vec<VarDeclarator>) {
		...
		if has_init {
			...
			d.take()
				.into_iter()
				.for_each(|v| if v.init.is_some() { new.push(v) } else { self.vars.push(v) });
