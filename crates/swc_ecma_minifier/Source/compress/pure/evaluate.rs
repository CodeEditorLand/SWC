...
use swc_ecma_ast::*;
use swc_ecma_utils::{ExprExt, IsEmpty, Type, Value, number::ToJsString};
...
impl Pure<'_> {
	...
	pub(super) fn optimize_lit_cmp(&mut self, n: &mut BinExpr) -> Option<Expr> {
		...
		let mut make_lit_bool = |value: bool| {
			self.changed = true;
			Some(Lit::Bool(Bool { span: n.span, value: flag ^ value }).into())
		};
		match (n.left.get_type(self.expr_ctx).opt()?, n.right.get_type(self.expr_ctx).opt()?) {
	...
	pub(super) fn eval_array_spread(&mut self, e: &mut Expr) {
		...
		if !elems.iter().any(|elem| match elem {
			Some(ExprOrSpread { spread: Some(..), expr }) => expr.is_array(),
			_ => false,
		}) {
			return;
		}
	...
	pub(super) fn eval_logical_expr(&mut self, e: &mut Expr) {
		let Expr::Bin(b @ BinExpr { op: op!("||") | op!("&&"), .. }) = e else {
			return;
		};
	...
	pub(super) fn eval_array_method_call(&mut self, e: &mut Expr) {
		...
		if let Expr::Member(MemberExpr { span, obj, prop: MemberProp::Ident(method_name) }) = callee {
			...
			let has_spread_elem = arr
				.elems
				.iter()
				.any(|s| matches!(s, Some(ExprOrSpread { spread: Some(..), .. })));
					...
					1 => {
						if let Value::Known(start) = call.args[0].expr.as_pure_number(self.expr_ctx) {
							...
							if start >= arr.elems.len() {
								*e = ArrayLit { span: *span, elems: Default::default() }.into();
								return;
							}
					...
					_ => {
						...
						if let Value::Known(start) = start {
							...
							if let Value::Known(end) = end {
								...
								self.changed = true;
								report_change!("evaluate: Reducing array.slice({}, {}) call", start, end);
								if start >= arr.elems.len() {
									*e = ArrayLit { span: *span, elems: Default::default() }.into();
									return;
								}
	...
	pub(super) fn eval_fn_method_call(&mut self, e: &mut Expr) {
		...
		if let Expr::Member(MemberExpr { obj, prop: MemberProp::Ident(method_name), .. }) = callee {
			if obj.may_have_side_effects(self.expr_ctx) {
				return;
			}
				...
				&& f.function.body.is_empty()
			{
				...
				*e = Str { span: call.span, value: "function(){}".into(), raw: None }.into();
			}
		}
	}
			...
			MemberProp::Computed(p) => {
				if let Expr::Lit(Lit::Str(s)) = &*p.expr {
					if let Ok(value) = s.value.parse::<u32>() {
						p.expr = Lit::Num(Number { span: s.span, value: value as f64, raw: None }).into();
					}
				}
			},
		}
			...
			if let Expr::Call(CallExpr { span, callee: Callee::Expr(callee), args, .. }) = e {
				if args.len() == 1 && args[0].spread.is_none() {
					if callee.is_ident_ref_to("Number") {
						self.changed = true;
						report_change!("evaluate: Reducing a call to `Number` into an unary operation");
	...
	pub(super) fn eval_number_method_call(&mut self, e: &mut Expr) {
		...
		let (num, method, args) = match e {
			Expr::Call(CallExpr { callee: Callee::Expr(callee), args, .. }) => match &mut **callee {
				Expr::Member(MemberExpr { obj, prop: MemberProp::Ident(prop), .. }) => match &mut **obj {
		...
		if args.iter().any(|arg| arg.expr.may_have_side_effects(self.expr_ctx)) {
			return;
		}
		...
		if &*method.sym == "toFixed" {
                ...
                .map_or(Some(0f64), |arg| eval_as_number(self.expr_ctx, &arg.expr))
			{
				...
				self.changed = true;
				report_change!("evaluate: Evaluating `{}.toFixed({})` as `{}`", num, precision, value);

				*e = Lit::Str(Str { span: e.span(), raw: None, value: value.into() }).into();
			}
		...
		if &*method.sym == "toPrecision" {
			...
			if args.is_empty() {
				...
				self.changed = true;
				report_change!("evaluate: Evaluating `{}.toPrecision()` as `{}`", num, value);

				*e = Lit::Str(Str { span: e.span(), raw: None, value }).into();
				return;
			}

			if let Some(precision) = args.first().and_then(|arg| eval_as_number(self.expr_ctx, &arg.expr)) {
		...
		if &*method.sym == "toExponential" {
			...
			if args.is_empty() {
				...
				self.changed = true;
				report_change!("evaluate: Evaluating `{}.toExponential()` as `{}`", num, value);

				*e = Lit::Str(Str { span: e.span(), raw: None, value }).into();
				return;
			} else if let Some(precision) = args.first().and_then(|arg| eval_as_number(self.expr_ctx, &arg.expr)) {
				let p = precision.trunc() as usize;
				// 5. If p < 1 or p > 100, throw a RangeError exception.
				if !(0..=20).contains(&p) {
					return;
				...
				self.changed = true;
				report_change!("evaluate: Evaluating `{}.toPrecision({})` as `{}`", num, precision, value);
		...
		if &*method.sym == "toString" {
			if let Some(base) = args.first().map_or(Some(10f64), |arg| eval_as_number(self.expr_ctx, &arg.expr)) {
				if base.trunc() == 10. {
					let value = num.value.to_js_string().into();
					*e = Lit::Str(Str { span: e.span(), raw: None, value }).into();
					return;
	...
	pub(super) fn eval_member_expr(&mut self, e: &mut Expr) {
		...
		if let Some(replacement) = self.optimize_member_expr(&mut member_expr.obj, &member_expr.prop) {
			*e = replacement;
			self.changed = true;
			report_change!("member_expr: Optimized member expression as {}", dump(&*e, false));
		}
	}

	fn eval_trivial_two(&mut self, a: &Expr, b: &mut Expr) {
		if let Expr::Assign(AssignExpr { left: a_left, op: op!("="), right: a_right, .. }) = a {
			match &**a_right {
				Expr::Lit(..) => {},
				_ => return,
			}
	...
	pub(super) fn eval_str_method_call(&mut self, e: &mut Expr) {
		...
		let (s, method) = match &call.callee {
			...
			Callee::Expr(callee) => match &**callee {
				Expr::Member(MemberExpr { obj, prop: MemberProp::Ident(prop), .. }) => match &**obj {
				...
				if let Expr::Lit(Lit::Num(Number { value, .. })) = &*call.args[0].expr {
					...
					match c {
						Some(v) => {
							...
							self.changed = true;
							report_change!("evaluate: Evaluated `charCodeAt` of a string literal as `{}`", v);
							*e = Lit::Num(Number { span: call.span, value: v as usize as f64, raw: None }).into()
						},
						None => {
							self.changed = true;
							report_change!("evaluate: Evaluated `charCodeAt` of a string literal as `NaN`",);
			...
			"codePointAt" => {
				...
				if let Expr::Lit(Lit::Num(Number { value, .. })) = &*call.args[0].expr {
					...
					match c {
						Some(v) => {
							self.changed = true;
							report_change!("evaluate: Evaluated `codePointAt` of a string literal as `{}`", v);
							*e = Lit::Num(Number { span: call.span, value: v as usize as f64, raw: None }).into()
						},
						None => {
							self.changed = true;
							report_change!("evaluate: Evaluated `codePointAt` of a string literal as `NaN`",);
							*e = Ident::new(
								"NaN".into(),
								e.span(),
		...
		*e = Lit::Str(Str { value: new_val.into(), raw: None, ..s }).into();
	}
}
...
fn round_to_precision(digits: &mut String, precision: usize) -> bool {
	if digits.len() > precision {
		...
		let mut digit = digits.pop().expect("already checked that length is bigger than precision") as u8;
