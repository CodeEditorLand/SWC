...
impl Pure<'_> {
	...
	fn remove_empty_switch(&mut self, s: &mut Stmt) {
		...
		if sw.cases.is_empty() {
			...
			*s = ExprStmt { span: sw.span, expr: discriminant }.into();
		}
	}
	...
	fn optimize_const_switches(&mut self, s: &mut Stmt) {
		...
		for (idx, case) in stmt.cases.iter_mut().enumerate() {
			if let Some(test) = case.test.as_ref() {
				if let Some(e) = is_primitive(self.expr_ctx, tail_expr(test)) {
					if match (e, tail) {
						(Expr::Lit(Lit::Num(e)), Expr::Lit(Lit::Num(tail))) => e.value == tail.value,
					...
					if !may_match_other_than_exact && !test.is_ident() && !idents_used_by(test).is_empty() {
		...
		let var_ids: Vec<VarDeclarator> = var_ids
			.into_iter()
			.map(|name| VarDeclarator { span: DUMMY_SP, name: name.into(), init: None, definite: Default::default() })
			.collect();

		self.changed = true;

		if cases.len() == 1 && (cases[0].test.is_none() || exact.is_some()) && !contains_nested_break(&cases[0]) {
			...
			*s = BlockStmt { stmts, ..Default::default() }.into();
			return;
		}
	...
	fn merge_cases_with_same_cons(&mut self, cases: &mut Vec<SwitchCase>) {
		...
		while i < len {
			...
			for j in (i + 1)..len {
				...
				let case_i_terminates = cases[i].cons.last().map(|s| s.terminates()).unwrap_or(false);
				...
				let found = case_i_terminates
					&& if j != len - 1 {
						...
						if let Some(Stmt::Break(BreakStmt { label: None, .. })) = cases[i].cons.last() {
							...
							SyntaxContext::within_ignored_ctxt(|| cases[i].cons.eq_ignore_span(&cases[j].cons))
						}
					};
		...
		if let Stmt::Switch(sw) = s {
			match &mut *sw.cases {
				[] => {
					self.changed = true;
					report_change!("switches: Removing empty switch");
					*s = ExprStmt { span: sw.span, expr: sw.discriminant.take() }.into()
				},
				[case] => {
					...
					if let Some(test) = case.test {
						let test = BinExpr { left: discriminant, right: test, op: op!("==="), span: DUMMY_SP }.into();
						...
						*s = BlockStmt { span: sw.span, stmts, ..Default::default() }.into()
					}
				},
				[first, second] if first.test.is_none() || second.test.is_none() => {
					...
					if terminate {
						...
						let (def, case) = if first.test.is_none() { (first, second) } else { (second, first) };
						*s = IfStmt {
							span: sw.span,
							test: BinExpr {
								span: DUMMY_SP,
							...
							.into(),
							alt: Some(
								Stmt::Block(BlockStmt { span: DUMMY_SP, stmts: def.cons.take(), ..Default::default() })
									.into(),
							),
						}
						.into()
	...
	fn optimize_switch_with_default_on_last(&mut self, stmt: &mut Stmt) {
			...
			&& !contains_nested_break(s.cases.last().unwrap())
		{
			...
			exprs.extend(s.cases.iter_mut().filter_map(|case| case.test.take()).filter_map(|mut e| {
				self.ignore_return_value(
					&mut e,
					DropOpts { drop_number: true, drop_str_lit: true, ..Default::default() },
				);
			...
			if !exprs.is_empty() {
				prepend_stmt(&mut stmts, ExprStmt { span: DUMMY_SP, expr: Expr::from_exprs(exprs) }.into());
			}

			report_change!("switches: Turn switch with default on last into block");
			self.changed = true;
			let block: Stmt = BlockStmt { span: s.span, stmts, ..Default::default() }.into();
			*stmt = block;
		}
	}
}
...
fn contains_nested_break(case: &SwitchCase) -> bool {
	...
	let mut v = BreakFinder { top_level: true, nested_unlabelled_break: false };
