...
impl Pure<'_> {
	pub(super) fn optimize_for_init(&mut self, init: &mut Option<VarDeclOrExpr>) {
		...
		if let Some(VarDeclOrExpr::Expr(e)) = init {
			self.ignore_return_value(e, DropOpts { drop_number: true, drop_str_lit: true, ..Default::default() });
	...
	pub(super) fn optimize_for_if_break(&mut self, s: &mut ForStmt) -> Option<()> {
		...
		if let Stmt::Block(body) = &mut *s.body {
			...
			if let Stmt::If(IfStmt { span, test, cons, alt: None, .. }) = first {
				if let Stmt::Break(BreakStmt { label: None, .. }) = &**cons {
					...
					match s.test.as_deref_mut() {
						Some(e) => {
							...
							*e = BinExpr { span: *span, op: op!("&&"), left: Box::new(orig_test), right: test.take() }
								.into();
						},
						None => {
			...
			if let Stmt::If(IfStmt { span, test, cons, alt: Some(alt), .. }) = first {
	...
	pub(super) fn merge_for_if_break(&mut self, s: &mut ForStmt) {
		if let Stmt::If(IfStmt { test, cons, alt: None, .. }) = &mut *s.body {
			if let Stmt::Break(BreakStmt { label: None, .. }) = &**cons {
				...
				match s.test.take() {
					Some(left) => {
						s.test = Some(BinExpr { span: s.test.span(), op: op!("&&"), left, right: test.take() }.into());
					},
					None => {
						s.test = Some(test.take());
					},
			...
			Stmt::While(stmt) => {
				...
				if let Value::Known(false) = val {
					...
					*s = var_ids
					...
					report_change!("loops: Removing a while loop with a constant false condition (no run)");
					self.changed = true;
				}
			},
			_ => {},
...
fn should_not_inline_loop_body(s: &Stmt, allow_break_continue: bool) -> bool {
	match s {
		Stmt::Block(s) => s.stmts.iter().any(|s| should_not_inline_loop_body(s, allow_break_continue)),
		...
		Stmt::Continue(ContinueStmt { label: Some(..), .. }) | Stmt::Break(BreakStmt { label: Some(..), .. }) => true,

		Stmt::Break(..) | Stmt::Continue(..) => !allow_break_continue,

		Stmt::Return(..) | Stmt::Throw(..) | Stmt::Empty(..) | Stmt::Expr(..) | Stmt::Debugger(..) => false,
