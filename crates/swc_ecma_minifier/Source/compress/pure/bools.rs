...
impl Pure<'_> {
	...
	pub(super) fn make_bool_short(&mut self, e: &mut Expr, in_bool_ctx: bool, ignore_return_value: bool) {
		match e {
			Expr::Cond(cond) => {
			...
			Expr::Bin(BinExpr { op: op @ (op!("&&") | op!("||")), left, right, .. }) => {
					...
					match *op {
						...
						op!("&&") => {
							...
							if let (_, Value::Known(false)) = left.cast_to_bool(self.expr_ctx) {
								report_change!("bools: `false && foo` => `false` (in bool context)");
								self.changed = true;
								*e = *left.take();
								return;
							}
		...
		if let Expr::Bin(BinExpr { op: op @ (op!("&&") | op!("||")), left, .. }) = e {
	...
	pub(super) fn optimize_negate_eq(&mut self, e: &mut Expr) {
		...
		let Expr::Unary(UnaryExpr { op: op!("!"), arg, .. }) = e else {
			return;
		};
		...
		match &mut **arg {
			...
			Expr::Bin(BinExpr { op: op @ (op!("&&") | op!("||")), left, right, .. }) if arg_can_negate => {
				self.changed = true;
	...
	pub(super) fn compress_cmp_with_long_op(&mut self, e: &mut BinExpr) {
		fn should_optimize(l: &Expr, r: &Expr, ctx: ExprCtx, opts: &CompressOptions) -> bool {
			match (l, r) {
				(Expr::Unary(UnaryExpr { op: op!("typeof"), .. }), Expr::Lit(..)) => true,
				_ => {
					if opts.comparisons {
	...
	pub(super) fn remove_useless_logical_rhs(&mut self, e: &mut Expr) {
		...
		match e {
			Expr::Bin(BinExpr { left, op: op @ op!("&&"), right, .. })
			| Expr::Bin(BinExpr { left, op: op @ op!("||"), right, .. }) => {
	...
	pub(super) fn compress_useless_deletes(&mut self, e: &mut Expr) {
		...
		let delete = match e {
			Expr::Unary(u @ UnaryExpr { op: op!("delete"), .. }) => u,
			_ => return,
		};

		if delete
			.arg
			.may_have_side_effects(ExprCtx { is_unresolved_ref_safe: true, ..self.expr_ctx })
		{
			return;
		}
		...
		let convert_to_true = match &*delete.arg {
			...
			Expr::Bin(BinExpr { op: op!("/"), right, .. }) => {
				...
				let v = if let Value::Known(rn) = rn { rn != 0.0 } else { false };
				...
				if v {
					...
					*e = SeqExpr { span, exprs: vec![delete.arg.take(), Box::new(make_bool(span, true))] }.into();
					return;
				}
			},
	...
	pub(super) fn handle_negated_seq(&mut self, n: &mut Expr) {
		match &mut *n {
			...
			Expr::Unary(UnaryExpr { op: op!("delete"), .. }) => {
				// TODO
			},
			_ => {},
		}
	...
	pub(super) fn optimize_expr_in_bool_ctx(&mut self, n: &mut Expr, is_return_value_ignored: bool) {
		match n {
		...
		match n {
			Expr::Unary(UnaryExpr { span, op: op!("!"), arg }) => match &mut **arg {
				Expr::Lit(Lit::Num(Number { value, .. })) => {
					report_change!("Optimizing: number => number (in bool context)");

					self.changed = true;
					*n =
						Lit::Num(Number { span: *span, value: if *value == 0.0 { 1.0 } else { 0.0 }, raw: None }).into()
				},
			...
			Expr::Unary(UnaryExpr { span, op: op!("typeof"), arg }) => {
				report_change!("Optimizing: typeof => true (in bool context)");
				self.changed = true;

				match &**arg {
					Expr::Ident(..) => *n = Lit::Num(Number { span: *span, value: 1.0, raw: None }).into(),
					_ => {
						// Return value of typeof is always truthy
						let true_expr = Lit::Num(Number { span: *span, value: 1.0, raw: None }).into();
						*n = SeqExpr { span: *span, exprs: vec![arg.take(), true_expr] }.into()
					},
				}
			},
			...
			Expr::Lit(Lit::Str(s)) => {
				if !is_return_value_ignored {
					...
					self.changed = true;
					*n =
						Lit::Num(Number { span: s.span, value: if s.value.is_empty() { 0.0 } else { 1.0 }, raw: None })
							.into();
				}
			},
			...
			Expr::Lit(Lit::Num(num)) => {
				...
				if self.options.bools {
					...
					self.changed = true;
					*n =
						Lit::Num(Number { span: num.span, value: if num.value == 0.0 { 0.0 } else { 1.0 }, raw: None })
							.into();
				}
			},

			Expr::Bin(BinExpr { op: op!("??"), left, right, .. }) => {
				// Optimize if (a ?? false); as if (a);
				if let Value::Known(false) = right.as_pure_bool(self.expr_ctx) {
					report_change!("Dropping right operand of `??` as it's always false (in bool context)");
					self.changed = true;
					*n = *left.take();
				}
			},
	...
	fn can_swap_bin_operands(&mut self, l: &Expr, r: &Expr, is_for_rel: bool) -> bool {
		match (l, r) {
			...
			(Expr::Member(..) | Expr::Call(..) | Expr::Assign(..), r) if is_pure_undefined(self.expr_ctx, r) => true,

			(Expr::Ident(l), Expr::Ident(r)) => is_for_rel && self.options.unsafe_comps && l.sym < r.sym,

			(Expr::Ident(..), Expr::Lit(..)) if is_for_rel => false,

			(Expr::Ident(..), Expr::Lit(..))
			| (Expr::Ident(..) | Expr::Member(..), Expr::Unary(UnaryExpr { op: op!("void") | op!("!"), .. }))
			| (Expr::This(..), Expr::Unary(UnaryExpr { op: op!("void"), .. }))
			| (Expr::Unary(..), Expr::Lit(..))
	...
	fn try_swap_bin(&mut self, op: BinaryOp, left: &mut Expr, right: &mut Expr) -> bool {
		...
		let can_swap = can_swap
			|| (matches!(op, op!("*") | op!("&") | op!("|") | op!("^"))
				&& right.as_bin().filter(|b| b.op.precedence() == op.precedence()).is_some()
				&& left.as_bin().filter(|b| b.op.precedence() == op.precedence()).is_none()
