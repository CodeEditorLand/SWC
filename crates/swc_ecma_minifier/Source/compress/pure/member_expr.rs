...
use swc_ecma_ast::{
	ArrayLit, Expr, ExprOrSpread, IdentName, Lit, MemberExpr, MemberProp, ObjectLit, Prop, PropOrSpread, SeqExpr, Str,
};
...
impl Pure<'_> {
	...
	pub(super) fn optimize_member_expr(&mut self, obj: &mut Expr, prop: &MemberProp) -> Option<Expr> {
				...
				Some(*self.expr_ctx.preserve_effects(
					*span,
					if is_known_symbol {
						...
						MemberExpr {
							span: *span,
							obj: ObjectLit { span: *span, props: Vec::new() }.into(),
