...
use swc_ecma_usage_analyzer::{
	...
	analyzer::{
		Ctx, ScopeKind, UsageAnalyzer, analyze_with_storage,
...
impl VarUsageInfo {
	...
	pub(crate) fn can_inline_fn_once(&self) -> bool {
		(self.callee_count > 0 || !self.executed_multiple_time && (self.is_fn_local || !self.used_in_non_child_fn))
			&& !(self.used_recursively && self.has_property_access && self.property_mutation_count != 0)
	}
...
impl Storage for ProgramData {
		...
		for (id, mut var_info) in child.vars {
			...
			match self.vars.entry(id.clone()) {
				Entry::Occupied(mut e) => {
					...
					let var_assigned =
						var_info.assign_count > 0 || (var_info.var_initialized && !e.get().var_initialized);
					...
					e.get_mut().no_side_effect_for_member_access =
						e.get_mut().no_side_effect_for_member_access && var_info.no_side_effect_for_member_access;
	...
	fn report_usage(&mut self, ctx: Ctx, i: Id) {
		...
		let e = self
			.vars
			.entry(i.clone())
			.or_insert_with(|| Box::new(VarUsageInfo { used_above_decl: true, ..Default::default() }));
	...
	fn report_assign(&mut self, ctx: Ctx, i: Id, is_op: bool, ty: Value<Type>) {
		...
		let mut to_visit: IndexSet<Id, FxBuildHasher> = IndexSet::from_iter(e.infects_to.iter().cloned().map(|i| i.0));
...
impl ProgramData {
	...
	pub(crate) fn contains_unresolved(&self, e: &Expr) -> bool {
		match e {
			...
			Expr::Bin(BinExpr { left, right, .. }) => self.contains_unresolved(left) || self.contains_unresolved(right),
			...
			Expr::Assign(AssignExpr { left, right, .. }) => {
				// TODO
				(match left {
					AssignTarget::Simple(left) => self.simple_assign_target_contains_unresolved(left),
					AssignTarget::Pat(_) => false,
				}) || self.contains_unresolved(right)
			},
			Expr::Cond(CondExpr { test, cons, alt, .. }) => {
				self.contains_unresolved(test) || self.contains_unresolved(cons) || self.contains_unresolved(alt)
			},
	...
	pub(crate) fn ident_is_unresolved(&self, i: &Ident) -> bool {
		...
		if is_global_var_with_pure_property_access(&i.sym) || matches!(&*i.sym, "arguments" | "window" | "global") {
			return false;
		}
