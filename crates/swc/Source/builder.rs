...
use swc_atoms::Atom;
use swc_common::{FileName, Mark, SourceMap, comments::Comments, errors::Handler, sync::Lrc, util::take::Take};
...
use swc_ecma_transforms::{
	Assumptions, compat,
	feature::{FeatureFlag, enable_available_feature_from_es_version},
...
impl<'a, 'b, P: Pass> PassBuilder<'a, 'b, P> {
		...
		P: 'cmt,
	{
		let (need_analyzer, import_interop, ignore_dynamic) = match module {
			...
			Some(ModuleConfig::Amd(ref c)) => (true, c.config.import_interop(), c.config.ignore_dynamic),
			Some(ModuleConfig::Umd(ref c)) => (true, c.config.import_interop(), c.config.ignore_dynamic),
			Some(ModuleConfig::SystemJs(_)) | Some(ModuleConfig::Es6(..)) | Some(ModuleConfig::NodeNext(..)) | None => {
				(false, true.into(), true)
			},
		};
		...
		let compat_pass = if let Some(env) = self.env {
			...
			Either::Right((
				...
				Optional::new(compat::es2021::es2021(), should_enable(self.target, EsVersion::Es2021)),
				Optional::new(
				...
				Optional::new(compat::es2019::es2019(), should_enable(self.target, EsVersion::Es2019)),
				Optional::new(
				...
				Optional::new(
					compat::es2015(
						...
						compat::es2015::Config {
							...
							computed_props: compat::es2015::computed_props::Config { loose: self.loose },
							for_of: compat::es2015::for_of::Config { assume_array: false, loose: self.loose },
				...
				Optional::new(compat::es3(true), cfg!(feature = "es3") && self.target == EsVersion::Es3),
			))
		};
		...
		(
			...
			ModuleConfig::build(self.cm.clone(), comments, module, self.unresolved_mark, feature_flag, resolver),
	...
	fn visit_mut_module(&mut self, m: &mut Module) {
		if let Some(options) = &self.options {
			let opts = MinifyOptions {
				...
				mangle: options.mangle.clone().unwrap_as_option(|default| match default {
					Some(true) => Some(Default::default()),
					_ => None,
				}),
				..Default::default()
