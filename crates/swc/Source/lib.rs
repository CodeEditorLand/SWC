...
pub use sourcemap;
use swc_common::{
	FileName, GLOBALS, Mark, SourceFile, SourceMap, Spanned, comments::Comments, errors::Handler, sync::Lrc,
};
...
use swc_ecma_codegen::{Node, to_code_with_comments};
use swc_ecma_loader::resolvers::{lru::CachingResolver, node::NodeModulesResolver, tsc::TsConfigResolver};
...
use crate::config::{
	BuiltInput, Config, ConfigFile, InputSourceMap, IsModule, JsMinifyCommentOption, JsMinifyOptions, Options,
	OutputCharset, Rc, RootMode, SourceMapsConfig,
};
...
type SwcImportResolver =
	Arc<NodeImportResolver<CachingResolver<TsConfigResolver<CachingResolver<NodeModulesResolver>>>>>;
...
impl Compiler {
	...
	fn get_orig_src_map(
		...
		self.run(|| -> Result<_, Error> {
			...
			let read_inline_sourcemap = |data_url: &str| -> Result<Option<sourcemap::SourceMap>, Error> {
			...
			let read_file_sourcemap = |data_url: Option<&str>| -> Result<Option<sourcemap::SourceMap>, Error> {
				match &**name {
					FileName::Real(filename) => {
						...
						let map_path = match data_url {
							Some(data_url) => {
								...
								if !map_path.exists() {
									...
									// convenience.
									let fallback_map_path = PathBuf::from(format!("{}.map", filename.display()));
						...
						match map_path {
							Some(map_path) => {
								...
								if file.as_ref().is_err_and(|err| err.kind() == ErrorKind::NotFound) {
									warn!(
	...
	pub fn parse_js(
		...
		swc_compiler_base::parse_js(self.cm.clone(), fm, handler, target, syntax, is_module, comments)
	}
...
impl Compiler {
	...
	pub fn read_config(&self, opts: &Options, name: &FileName) -> Result<Option<Config>, Error> {
		...
		self.run(|| -> Result<_, Error> {
			let Options { ref root, root_mode, swcrc, config_file, .. } = opts;
	...
	where
		F: FnOnce() -> Ret,
	{
		self.run(|| helpers::HELPERS.set(&Helpers::new(external_helpers), || HANDLER.set(handler, op)))
	}
		...
		P2: Pass,
	{
		self.run(|| -> Result<_, Error> {
			let config = self.run(|| {
				self.parse_js_as_input(fm.clone(), program, handler, opts, &fm.name, Some(&comments), |program| {
	...
	pub fn minify(
		...
		self.run(|| {
			...
			let mut min_opts = MinifyOptions {
				...
				mangle: opts.mangle.clone().unwrap_as_option(|default| match default {
					Some(true) | None => Some(Default::default()),
					_ => None,
				}),
				..Default::default()
			...
			swc_compiler_base::minify_file_comments(&comments, preserve_comments, opts.format.preserve_annotations);
			...
			let ret = self.print(
				...
				PrintArgs {
					...
					codegen_config: swc_ecma_codegen::Config::default()
						...
						.with_emit_assert_for_import_attributes(opts.format.emit_assert_for_import_attributes)
	...
	fn apply_transforms(
		...
		self.run(|| {
			...
			if config.emit_isolated_dts && !config.syntax.typescript() {
				handler.warn("jsc.experimental.emitIsolatedDts is enabled but the syntax is not TypeScript");
			}
			...
			let dts_code = if emit_dts {
				...
				let mut checker = FastDts::new(fm.name.clone(), config.unresolved_mark, Default::default());
			...
			let (program, output) = swc_transform_common::output::capture(|| {
				if let Some(dts_code) = dts_code {
					emit("__swc_isolated_declarations__".into(), serde_json::Value::String(dts_code));
				}
			...
			self.print(
				...
				PrintArgs {
					...
					codegen_config: swc_ecma_codegen::Config::default()
						...
						.with_emit_assert_for_import_attributes(config.emit_assert_for_import_attributes)
...
impl JsMinifyExtras {
	pub fn with_mangle_name_cache(mut self, mangle_name_cache: Option<Arc<dyn MangleCache>>) -> Self {
...
fn parse_swcrc(s: &str) -> Result<Rc, Error> {
	...
	serde_json::from_value(v).map(Rc::Single).map_err(convert_json_err)
}
