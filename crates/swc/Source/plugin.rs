...
impl RustPlugins {
	...
	fn apply_inner(&mut self, n: Program) -> Result<Program, anyhow::Error> {
		...
		swc_plugin_proxy::COMMENTS.set(&swc_plugin_proxy::HostCommentsStorage { inner: self.comments.clone() }, || {
			...
			if let Some(plugins) = &mut self.plugins {
				for p in plugins.drain(..) {
					...
					let span =
						tracing::span!(tracing::Level::INFO, "execute_plugin_runner", plugin_module = p.0.as_str())
							.entered();

					serialized = transform_plugin_executor
						.transform(&serialized, Some(should_enable_comments_proxy))
						.with_context(|| {
							format!("failed to invoke `{}` as js transform plugin at {}", &p.0, plugin_name)
						})?;
					drop(span);
				}
			}
