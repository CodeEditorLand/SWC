...
impl From<FxHashSet<Id>> for UsageCollect {
	fn from(id_usage: FxHashSet<Id>) -> Self {
		Self { id_usage, import_chain: Default::default() }
	}
}
...
impl Visit for DeclareCollect {
	...
	fn visit_import_decl(&mut self, n: &ImportDecl) {
		n.specifiers.iter().for_each(|import_specifier| match import_specifier {
...
impl VisitMut for StripImportExport {
	...
	fn visit_mut_module_items(&mut self, n: &mut Vec<ModuleItem>) {
		...
		n.retain_mut(|module_item| match module_item {
			ModuleItem::ModuleDecl(ModuleDecl::Import(ImportDecl { specifiers, type_only: false, .. }))
			...
			ModuleItem::ModuleDecl(ModuleDecl::ExportNamed(NamedExport {
				specifiers, src, type_only: false, ..
			})) => {
			...
			ModuleItem::ModuleDecl(ModuleDecl::ExportNamed(NamedExport { ref type_only, .. })) => !type_only,
			ModuleItem::ModuleDecl(ModuleDecl::ExportDefaultExpr(ExportDefaultExpr { ref expr, .. })) => expr
				.as_ident()
				.map(|ident| {
					let id = ident.to_id();
...
impl VisitMut for StripTsImportEquals {
	fn visit_mut_module_items(&mut self, n: &mut Vec<ModuleItem>) {
		let has_ts_import_equals = n
			.iter()
			.any(|module_item| matches!(module_item, ModuleItem::ModuleDecl(ModuleDecl::TsImportEquals(..))));

		// TS1235: A namespace declaration is only allowed at the top level of a
		// namespace or module.
		let has_ts_module = n
			.iter()
			.any(|module_item| matches!(module_item, ModuleItem::Stmt(Stmt::Decl(Decl::TsModule(..)))));
