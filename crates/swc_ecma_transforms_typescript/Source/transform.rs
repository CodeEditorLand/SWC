...
use swc_atoms::Atom;
use swc_common::{
	DUMMY_SP, Mark, Span, Spanned, SyntaxContext, errors::HANDLER, source_map::PURE_SP, util::take::Take,
};
use swc_ecma_ast::*;
use swc_ecma_utils::{
	ExprFactory, QueryRef, RefRewriter, StmtLikeInjector, alias_ident_for, constructor::inject_after_super,
	find_pat_ids, ident::IdentLike, is_literal, member_expr, private_ident, quote_ident, quote_str,
...
impl Visit for Transform {
	...
	fn visit_ts_enum_decl(&mut self, node: &TsEnumDecl) {
		...
		let TsEnumDecl { declare, is_const, id, members, .. } = node;
		...
		for m in members {
			...
			let key = TsEnumRecordKey { enum_id: id.to_id(), member_name: m.id.as_ref().clone() };
	...
	fn visit_export_decl(&mut self, node: &ExportDecl) {
		...
		match &node.decl {
			...
			Decl::TsEnum(ts_enum_decl) => {
				self.exported_binding.insert(ts_enum_decl.id.to_id(), self.namespace_id.clone());
			},
	...
	fn visit_export_named_specifier(&mut self, node: &ExportNamedSpecifier) {
		if let ModuleExportName::Ident(ident) = &node.orig {
			self.exported_binding.insert(ident.to_id(), self.namespace_id.clone());
		}
	}
	...
	fn visit_ts_import_equals_decl(&mut self, node: &TsImportEqualsDecl) {
		...
		if node.is_export {
			self.exported_binding.insert(node.id.to_id(), self.namespace_id.clone());
		}
	}
...
impl VisitMut for Transform {
	...
	fn visit_mut_program(&mut self, node: &mut Program) {
		...
		if !self.exported_binding.is_empty() {
			self.ref_rewriter = Some(RefRewriter { query: ExportQuery { export_name: self.exported_binding.clone() } });
		}
	...
	fn visit_mut_module(&mut self, node: &mut Module) {
		...
		if !self.export_var_list.is_empty() {
			let decls = self.export_var_list.take().into_iter().map(id_to_var_declarator).collect();

			node.body
				.push(ExportDecl { decl: VarDecl { decls, ..Default::default() }.into(), span: DUMMY_SP }.into())
		}
	}
	...
	fn visit_mut_module_items(&mut self, node: &mut Vec<ModuleItem>) {
		...
		if !var_list.is_empty() {
			...
			node.push(VarDecl { decls, ..Default::default() }.into())
		}
	}
		...
		node.params
			...
			.for_each(|param_or_ts_param_prop| match param_or_ts_param_prop {
				ParamOrTsParamProp::TsParamProp(ts_param_prop) => {
					let TsParamProp { span, decorators, param, .. } = ts_param_prop;
					...
					let (pat, expr, id) = match param {
						TsParamPropParam::Ident(binding_ident) => {
							...
							(binding_ident.clone().into(), assign_value_to_this_prop(prop_name, value), id)
						},
						TsParamPropParam::Assign(assign_pat) => {
							...
							(assign_pat.clone().into(), assign_value_to_this_prop(prop_name, value), id)
						},
					};
					...
					*param_or_ts_param_prop = Param { span: *span, decorators: decorators.take(), pat }.into();
				},
	...
	fn visit_mut_module_item(&mut self, node: &mut ModuleItem) {
		...
		if let Some(ExportDecl { decl, .. }) = node.as_mut_module_decl().and_then(ModuleDecl::as_mut_export_decl) {
	...
	fn visit_mut_export_default_decl(&mut self, node: &mut ExportDefaultDecl) {
		...
		if let DefaultDecl::Class(ClassExpr { ident: Some(ref ident), .. })
		| DefaultDecl::Fn(FnExpr { ident: Some(ref ident), .. }) = node.decl
		{
...
impl InitArg<'_> {
	...
	fn empty() -> ExprOrSpread {
		ExprOrSpread { spread: None, expr: ObjectLit::default().into() }
	}
	...
	fn or_empty(&self) -> ExprOrSpread {
		...
		ExprOrSpread { spread: None, expr: bin.into() }
	}
	...
	fn or_assign_empty(&self) -> ExprOrSpread {
		...
		let assign = self.namespace_id.cloned().map_or_else(
			...
			|namespace_id| {
				ObjectLit::default().make_assign_to(op!("="), namespace_id.make_member(self.id.clone().into()).into())
			},
		);

		let bin = BinExpr { op: op!("||"), left: expr.into(), right: assign.into(), ..Default::default() };
...
impl Transform {
	fn transform_ts_enum(&mut self, ts_enum: TsEnumDecl, is_first: bool, is_export: bool) -> FoldedDecl {
		let TsEnumDecl { span, declare, is_const, id, members } = ts_enum;

		debug_assert!(!declare);

		let ts_enum_safe_remove =
			!self.verbatim_module_syntax && is_const && !is_export && !self.exported_binding.contains_key(&id.to_id());
		...
		let member_list: Vec<_> = members
			...
			.map(|m| {
				...
				let key = TsEnumRecordKey { enum_id: id.to_id(), member_name: name.clone() };
		...
		let body = if !iife {
			let return_stmt: Stmt = ReturnStmt { arg: Some(id.clone().into()), ..Default::default() }.into();

			let stmts = stmts.chain(iter::once(return_stmt)).collect();

			BlockStmt { stmts, ..Default::default() }
		} else {
			BlockStmt { stmts: stmts.collect(), ..Default::default() }
		};
		...
		let init_arg = 'init_arg: {
			let init_arg = InitArg { id: &id, namespace_id: self.namespace_id.as_ref().filter(|_| is_export) };
			if !is_first {
		...
		if iife {
			FoldedDecl::Expr(ExprStmt { span, expr: expr.into() }.into())
		} else {
			let var_declarator = VarDeclarator { span, name: id.into(), init: Some(expr.into()), definite: false };

			FoldedDecl::Decl(VarDecl { span, kind: var_kind, decls: vec![var_declarator], ..Default::default() }.into())
		}
	}
	...
	fn transform_ts_enum_member(
		...
		member
			.init
			.map(|expr| EnumValueComputer { enum_id, unresolved_ctxt, record }.compute(expr))
...
impl Transform {
	fn transform_ts_module(&self, ts_module: TsModuleDecl, is_export: bool) -> FoldedDecl {
		...
		let TsModuleDecl { span, id: TsModuleName::Ident(module_ident), body: Some(body), .. } = ts_module else {
			unreachable!();
		};
	...
	fn transform_ts_namespace_body(id: Id, body: TsNamespaceBody) -> BlockStmt {
		let TsNamespaceDecl { span, declare, global, id: local_name, body } = match body {
		...
		let init_arg = InitArg { id: &local_name, namespace_id: Some(&id.to_id()) }.or_assign_empty();

		let expr = Factory::function(vec![local_name.into()], body).as_call(DUMMY_SP, vec![init_arg]);

		BlockStmt { span, stmts: vec![expr.into_stmt()], ..Default::default() }
	}
	...
	fn transform_ts_module_block(id: Id, TsModuleBlock { span, body }: TsModuleBlock) -> BlockStmt {
				...
				ModuleItem::ModuleDecl(ModuleDecl::ExportDecl(ExportDecl { decl, span, .. })) => match decl {
					...
					Decl::Var(var_decl) => {
						let mut exprs: Vec<Box<_>> = var_decl
							.decls
							.into_iter()
							.flat_map(|VarDeclarator { span, name, init, .. }| {
								let right = init?;
								let left = name.try_into().unwrap();

								Some(AssignExpr { span, left, op: op!("="), right }.into())
							})
							.collect();
						...
						stmts.push(ExprStmt { span: var_decl.span, expr }.into());
					},
					...
					match decl.module_ref {
						TsModuleRef::TsEntityName(ts_entity_name) => {
							...
							let stmt = if decl.is_export {
								...
								ExprStmt { span: decl.span, expr: expr.into() }.into()
							} else {
								// const foo = bar.baz
		...
		BlockStmt { span, stmts, ..Default::default() }
	}
}
...
impl Transform {
	fn reorder_class_prop_decls_and_inits(
		...
		for (index, member) in class_member_list.iter_mut().enumerate() {
			match member {
				ClassMember::Constructor(..) => {
					let empty = EmptyStmt { span: member.span() }.into();
					constructor = mem::replace(member, empty).constructor();
					cons_index = index;
				},
				ClassMember::ClassProp(ClassProp { key, value: value @ Some(..), is_static: false, .. }) => {
					let key = match &mut *key {
						PropName::Computed(ComputedPropName { span, expr }) if !is_literal(expr) => {
							...
							**expr = expr.take().make_assign_to(op!("="), ident.clone().into());
							...
							PropName::Computed(ComputedPropName { span: *span, expr: ident.into() })
						},
						_ => key.clone(),
					};
				...
				ClassMember::PrivateProp(PrivateProp { key, value: value @ Some(..), is_static: false, .. }) => {
					init_list.push(assign_value_to_this_private_prop(key.clone(), *value.take().unwrap()));
				},
				_ => {},
			}
		}
		...
		class_member_list.splice(
			...
			prop_list
				...
				.map(Ident::from)
				.map(PropName::from)
				.map(|key| ClassProp { key, ..Default::default() })
	...
	fn reorder_class_prop_decls(
		...
		if let Some(constructor) = class_member_list.iter_mut().find_map(|m| m.as_mut_constructor()) {
...
impl Transform {
	...
	fn assign_prop(id: &Id, prop: &Ident, span: Span) -> Stmt {
		...
		ExprStmt { span, expr: expr.into() }.into()
	}
...
impl Transform {
	fn enter_expr_for_inline_enum(&mut self, node: &mut Expr) {
		...
		if let Expr::Member(MemberExpr { obj, prop, .. }) = node {
			...
			let key = TsEnumRecordKey { enum_id, member_name };
		...
		for module_item in &mut node.body {
			match module_item {
					...
					match &mut decl.module_ref {
						...
						TsModuleRef::TsEntityName(ts_entity_name) => {
							...
							*module_item = if decl.is_export {
								ExportDecl { span: decl.span, decl: var_decl.into() }.into()
							} else {
								var_decl.span = decl.span;
								var_decl.into()
							};
						...
						TsModuleRef::TsExternalModuleRef(TsExternalModuleRef { expr, .. }) => {
							match self.import_export_assign_config {
								TsImportExportAssignConfig::Classic => {
									// require("foo");
									let mut init = cjs_require.clone().as_call(DUMMY_SP, vec![expr.take().as_arg()]);

									// exports.foo = require("foo");
									if decl.is_export {
										init = init.make_assign_to(
											op!("="),
											cjs_exports.clone().make_member(decl.id.clone().into()).into(),
										)
									}
				...
				ModuleItem::ModuleDecl(ModuleDecl::TsExportAssignment(..)) => {
					let ts_export_assign = module_item.take().module_decl().unwrap().ts_export_assignment().unwrap();
			...
			node.body.prepend_stmts([
				...
				create_require
					.as_call(
						DUMMY_SP,
						vec![
							MetaPropExpr { span: DUMMY_SP, kind: MetaPropKind::ImportMeta }
...
impl QueryRef for ExportQuery {
	...
	fn query_jsx(&self, ident: &Ident) -> Option<JSXElementName> {
		self.export_name.get(&ident.to_id())?.clone().map(|namespace_id| {
			JSXMemberExpr {
				span: DUMMY_SP,
...
impl EnumMemberItem {
	...
	fn build_assign(self, enum_id: &Id) -> Stmt {
		...
		let inner_assign =
			value.make_assign_to(op!("="), Ident::from(enum_id.clone()).computed_member(self.name.clone()).into());
		...
		let outer_assign = if is_string {
			...
			value.make_assign_to(op!("="), Ident::from(enum_id.clone()).computed_member(inner_assign).into())
		};

		ExprStmt { span: self.span, expr: outer_assign.into() }.into()
	}
}
...
impl ModuleId for TsModuleName {
	fn to_id(&self) -> Id {
		self.as_ident().expect("Only ambient modules can use quoted names.").to_id()
	}
}

fn id_to_var_declarator(id: Id) -> VarDeclarator {
	VarDeclarator { span: DUMMY_SP, name: id.into(), init: None, definite: false }
}

fn get_enum_id(e: &Expr) -> Option<Id> {
	if let Expr::Ident(ident) = e { Some(ident.to_id()) } else { None }
}
