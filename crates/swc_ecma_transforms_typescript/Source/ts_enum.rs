...
impl TsEnumRecordValue {
	...
	pub fn is_const(&self) -> bool {
		matches!(self, TsEnumRecordValue::String(..) | TsEnumRecordValue::Number(..))
	}
...
impl From<TsEnumRecordValue> for Expr {
	fn from(value: TsEnumRecordValue) -> Self {
		match value {
			...
			TsEnumRecordValue::Number(num) if num.is_nan() => {
				Ident { span: DUMMY_SP, sym: "NaN".into(), ..Default::default() }.into()
			},
			TsEnumRecordValue::Number(num) if num.is_infinite() => {
				let value: Expr = Ident { span: DUMMY_SP, sym: "Infinity".into(), ..Default::default() }.into();

				if num.is_sign_negative() {
					UnaryExpr { span: DUMMY_SP, op: op!(unary, "-"), arg: value.into() }.into()
				} else {
					value
				}
			},
			TsEnumRecordValue::Number(num) => Lit::Num(Number { span: DUMMY_SP, value: *num, raw: None }).into(),
...
impl EnumValueComputer<'_> {
	...
	fn compute_rec(&self, expr: Box<Expr>) -> TsEnumRecordValue {
		match *expr {
			...
			Expr::Ident(Ident { ctxt, sym, .. }) if &*sym == "Infinity" && ctxt == self.unresolved_ctxt => {
				TsEnumRecordValue::Number(f64::INFINITY.into())
			},
			Expr::Ident(ref ident) => self
				.record
				.get(&TsEnumRecordKey { enum_id: self.enum_id.clone(), member_name: ident.sym.clone() })
				.cloned()
				.map(|value| match value {
					TsEnumRecordValue::String(..) | TsEnumRecordValue::Number(..) => value,
					_ => TsEnumRecordValue::Opaque(self.enum_id.clone().make_member(ident.clone().into()).into()),
				})
	...
	fn compute_unary(&self, expr: UnaryExpr) -> TsEnumRecordValue {
		...
		let TsEnumRecordValue::Number(num) = inner else {
			return TsEnumRecordValue::Opaque(
				UnaryExpr { span: expr.span, op: expr.op, arg: Box::new(inner.into()) }.into(),
			);
		};
	...
	fn compute_member(&self, expr: MemberExpr) -> TsEnumRecordValue {
		...
		self.record
			.get(&TsEnumRecordKey { enum_id: ident.to_id(), member_name })
			.cloned()
...
impl VisitMut for EnumValueComputer<'_> {
	...
	fn visit_mut_expr(&mut self, expr: &mut Expr) {
		...
		if self
			.record
			.contains_key(&TsEnumRecordKey { enum_id: self.enum_id.clone(), member_name: ident.sym.clone() })
		{
			*expr = self.enum_id.clone().make_member(ident.clone().into()).into();
		}
	}
}
