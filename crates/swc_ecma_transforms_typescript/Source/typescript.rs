...
pub fn typescript(config: Config, unresolved_mark: Mark, top_level_mark: Mark) -> impl Pass {
	...
	visit_mut_pass(TypeScript { config, unresolved_mark, top_level_mark, id_usage: Default::default() })
}
...
impl TypeScript {
	fn get_last_module_span(&self, n: &Module) -> Option<Span> {
		...
		n.body.iter().rev().find(|m| m.is_es_module_decl()).map(Spanned::span)
	}
	...
	fn restore_esm_ctx(n: &mut Module, span: Span) {
		...
		n.body.push(NamedExport { span, ..NamedExport::dummy() }.into());
	}
}
...
impl EsModuleDecl for ModuleDecl {
	fn is_es_module_decl(&self) -> bool {
		...
		match self {
			...
			ModuleDecl::TsImportEquals(..) | ModuleDecl::TsExportAssignment(..) | ModuleDecl::TsNamespaceExport(..) => {
				false
			},
		}
	}
...
impl EsModuleDecl for ModuleItem {
	fn is_es_module_decl(&self) -> bool {
		self.as_module_decl().map_or(false, ModuleDecl::is_es_module_decl)
	}
}
	...
	fn visit_mut_module(&mut self, n: &mut Module) {
		...
		if !self.config.verbatim_module_syntax {
			let span = if n.shebang.is_some() {
				n.span.with_lo(n.body.first().map(|s| s.span_lo()).unwrap_or(n.span.lo))
			} else {
				n.span
			};

			let JsxDirectives { pragma, pragma_frag, .. } = self.comments.with_leading(span.lo, |comments| {
