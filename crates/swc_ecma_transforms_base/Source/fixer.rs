...
use swc_common::{DUMMY_SP, Span, Spanned, comments::Comments, util::take::Take};
use swc_ecma_ast::*;
...
impl VisitMut for Fixer<'_> {
	...
	fn visit_mut_await_expr(&mut self, expr: &mut AwaitExpr) {
		...
		match &*expr.arg {
			Expr::Cond(..) | Expr::Assign(..) | Expr::Bin(..) | Expr::Yield(..) => self.wrap(&mut expr.arg),
			_ => {},
		}
	}
	...
	fn visit_mut_bin_expr(&mut self, expr: &mut BinExpr) {
		...
		match &mut *expr.left {
			...
			Expr::Unary(UnaryExpr { op: op!("void"), .. })
				if expr.op == op!("==") || expr.op == op!("===") || expr.op == op!("!=") || expr.op == op!("!==") => {},
		...
		match &mut node.super_class {
			Some(e)
				if e.is_seq()
					|| e.is_await_expr()
					|| e.is_yield_expr()
					|| e.is_bin() || e.is_assign()
					|| e.is_cond() || e.is_unary() =>
			{
				self.wrap(e)
			},
			_ => {},
	...
	fn visit_mut_for_of_stmt(&mut self, s: &mut ForOfStmt) {
		...
		if !s.is_await {
			match &s.left {
				ForHead::Pat(p)
					if match &**p {
						Pat::Ident(BindingIdent { id: Ident { sym, .. }, .. }) => &**sym == "async",
						_ => false,
					} =>
				{
	...
	fn visit_mut_if_stmt(&mut self, node: &mut IfStmt) {
		...
		if will_eat_else_token(&node.cons) {
			node.cons = Box::new(
				BlockStmt { span: node.cons.span(), stmts: vec![*node.cons.take()], ..Default::default() }.into(),
			);
		}
	}
	...
	fn visit_mut_unary_expr(&mut self, n: &mut UnaryExpr) {
		...
		match &*n.arg {
			...
			Expr::Assign(..) | Expr::Bin(..) | Expr::Seq(..) | Expr::Cond(..) | Expr::Arrow(..) | Expr::Yield(..) => {
				self.wrap(&mut n.arg)
			},
...
impl Fixer<'_> {
	fn wrap_with_paren_if_required(&mut self, e: &mut Expr) {
		...
		match e {
			...
			Expr::Call(CallExpr { callee: Callee::Expr(callee), .. })
				if callee.is_seq() || callee.is_arrow() || callee.is_await_expr() || callee.is_assign() =>
			{
				self.wrap(callee);
			},
			...
			Expr::Call(CallExpr { callee: Callee::Expr(callee), .. }) if callee.is_fn_expr() => match self.ctx {
				Context::ForcedExpr | Context::FreeExpr => {},
	...
	fn unwrap_expr(&mut self, e: &mut Expr) {
		loop {
			match e {
				...
				Expr::Paren(ParenExpr { span: paren_span, expr, .. }) => {
	...
	fn handle_expr_stmt(&mut self, expr: &mut Expr) {
		match expr {
			...
			// ({ a } = foo)
			Expr::Assign(AssignExpr { left: AssignTarget::Pat(left), .. }) if left.is_object() => self.wrap(expr),
...
fn ignore_return_value(expr: Box<Expr>, has_padding_value: &mut bool) -> Option<Box<Expr>> {
	match *expr {
		...
		Expr::Unary(UnaryExpr { op: op!("void"), arg, .. }) => ignore_return_value(arg, has_padding_value),
		_ => Some(expr),
	}
}
...
mod tests {
	...
	identical!(regression_03, "_iteratorNormalCompletion = (_step = _iterator.next()).done");
...
const _ref = {}, { c =( _tmp = {}, d = _extends({}, _tmp), _tmp)  } = _ref;"
	...
	identical!(regression_07, "( _temp = super(), _initialize(this), _temp).method();");
...
a = (b, c, d);"
	...
	test_fixer!(fixer_07, "a => ((b, c) => ((a, b), c));", "(a)=>(b, c)=>(a, b, c);");
...
"
	...
	identical!(deno_10487_2, "class MultiVector extends (options.baseType||Float32Array) {}");

	identical!(extends_nullish_coalescing, "class Foo extends (Bar ?? class{}) {}");

	identical!(extends_assign, "class Foo extends (Bar = class{}) {}");

	identical!(extends_logical_or_assin, "class Foo extends (Bar ||= class{}) {}");

	identical!(extends_logical_and_assin, "class Foo extends (Bar &&= class{}) {}");

	identical!(extends_logical_nullish_assin, "class Foo extends (Bar ??= class{}) {}");
	...
	test_fixer!(new_member_call_2, "new (getObj().ctor)()", "new (getObj()).ctor()");
	test_fixer!(new_member_call_3, "new (x.getObj().ctor)()", "new (x.getObj()).ctor()");
	...
	identical!(new_await_1, "async function foo() { new (await getServerImpl())(options) }");
	...
	test_fixer!(paren_of_bin_left_3, "(function () {} || 2)", "(function () {}) || 2");
