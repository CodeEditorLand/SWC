...
use std::{
	...
	mem::{ManuallyDrop, take, transmute_copy},
};
...
use swc_common::{Mark, SyntaxContext, util::take::Take};
use swc_ecma_ast::*;
use tracing::debug;
...
impl Scope {
		...
		R: Renamer,
	{
		...
		self.rename_one_scope_in_normal_mode(renamer, to, previous, reverse, queue, preserved, preserved_symbols);

		for child in &mut self.children {
			child.rename_in_normal_mode(renamer, to, &Default::default(), reverse, preserved, preserved_symbols);
		}
	}
		...
		R: Renamer,
	{
		...
		for id in queue {
			if preserved.contains(&id) || to.get(&id).is_some() || previous.get(&id).is_some() || id.0 == "eval" {
				continue;
			}
