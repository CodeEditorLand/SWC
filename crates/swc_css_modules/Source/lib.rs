...
use swc_css_ast::{
	ComplexSelector, ComplexSelectorChildren, ComponentValue, Declaration, DeclarationName, Delimiter, DelimiterValue,
	FunctionName, Ident, KeyframesName, PseudoClassSelectorChildren, QualifiedRule, QualifiedRulePrelude, Stylesheet,
	SubclassSelector,
};
...
pub fn compile<'a>(ss: &mut Stylesheet, config: impl 'a + TransformConfig) -> TransformResult {
	let mut compiler = Compiler {
		...
		result: TransformResult { renamed: Default::default() },
	};
	...
	C: TransformConfig,
{
	...
	fn visit_mut_keyframes_name(&mut self, n: &mut KeyframesName) {
		match n {
			...
			KeyframesName::PseudoFunction(pseudo_function) if pseudo_function.pseudo.value == "local" => {
			...
			KeyframesName::PseudoFunction(pseudo_function) if pseudo_function.pseudo.value == "global" => {
				match &pseudo_function.name {
					KeyframesName::CustomIdent(custom_ident) => {
						*n = KeyframesName::CustomIdent(custom_ident.clone());
					},
			...
			KeyframesName::PseudoPrefix(pseudo_prefix) if pseudo_prefix.pseudo.value == "global" => {
				match &pseudo_prefix.name {
					KeyframesName::CustomIdent(custom_ident) => {
						*n = KeyframesName::CustomIdent(custom_ident.clone());
					},
					...
					for subclass_sel in &sel.subclass_selectors {
						if let SubclassSelector::Class(class_sel) = &subclass_sel {
							if let Some(composes) = &composes {
								let key = self.data.renamed_to_orig.get(&class_sel.text.value).cloned();
					...
					if n.value.len() >= 3 {
						match (&n.value[n.value.len() - 2], &n.value[n.value.len() - 1]) {
								...
								if from.value == "from" && global.value == "global" =>
							{
								for class_name in n.value.iter().take(n.value.len() - 2) {
									if let ComponentValue::Ident(value) = class_name {
										composes_for_current.push(CssClassName::Global { name: *value.clone() });
									}
								}
								return;
							},
						...
						match &*class_sel.name.value {
							"local" => {
								if let Some(children) = &mut class_sel.children {
									if let Some(PseudoClassSelectorChildren::ComplexSelector(complex_selector)) =
										children.get_mut(0)
									{
										let old_is_global_mode = self.data.is_global_mode;
										...
										let mut complex_selector_children = complex_selector.children.clone();
