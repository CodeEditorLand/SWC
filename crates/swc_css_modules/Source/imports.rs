...
use swc_atoms::Atom;
use swc_css_ast::{ComponentValue, Declaration, DeclarationName, ImportHref, ImportPrelude, Stylesheet, UrlValue};
use swc_css_visit::{Visit, VisitWith};

pub fn analyze_imports(ss: &Stylesheet) -> Vec<Atom> {
	let mut v = Analyzer { imports: Default::default() };
	ss.visit_with(&mut v);
	v.imports.sort();
	v.imports.dedup();
	v.imports
			...
			if &*name.value == "composes" {
				...
				if d.value.len() >= 3 {
					match (&d.value[d.value.len() - 2], &d.value[d.value.len() - 1]) {
						(ComponentValue::Ident(ident), ComponentValue::Str(s)) if ident.value == "from" => {
							self.imports.push(s.value.clone());
						},
						_ => (),
					}
