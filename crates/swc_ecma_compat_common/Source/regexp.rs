	...
	fn visit_mut_expr(&mut self, expr: &mut Expr) {
		...
		if let Expr::Lit(Lit::Regex(regex)) = expr {
			if (self.config.dot_all_regex && regex.flags.contains('s'))
				...
				|| (self.config.lookbehind_assertion && regex.exp.contains("(?<=") || regex.exp.contains("(?<!"))
				|| (self.config.unicode_property_regex && (regex.exp.contains("\\p{") || regex.exp.contains("\\P{")))
			{
				...
				*expr = CallExpr { span, callee: quote_ident!("RegExp").as_callee(), args, ..Default::default() }.into()
			}
		}
	}
}
