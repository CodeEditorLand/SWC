...
use swc_ecma_utils::{
	ExprFactory, IdentRenamer, alias_ident_for, constructor::inject_after_super, default_constructor_with_span,
...
pub(crate) fn decorator_impl(version: DecoratorVersion) -> impl Pass {
	visit_mut_pass(DecoratorPass { version, ..Default::default() })
}
...
impl DecoratorPass {
	fn preserve_side_effect_of_decorators(&mut self, decorators: Vec<Decorator>) -> Vec<Option<ExprOrSpread>> {
		decorators
			.into_iter()
	...
	fn preserve_side_effect_of_decorator(&mut self, dec: Box<Expr>) -> Box<Expr> {
		...
		self.extra_vars
			.push(VarDeclarator { span: DUMMY_SP, name: ident.clone().into(), init: None, definite: false });
		self.pre_class_inits
			.push(AssignExpr { span: DUMMY_SP, op: op!("="), left: ident.clone().into(), right: dec }.into());
	...
	fn consume_inits(&mut self) {
		...
		for id in self.state.static_lhs.drain(..).chain(self.state.proto_lhs.drain(..)) {
		...
		combined_args.push(ArrayLit { span: DUMMY_SP, elems: self.state.class_decorators.take() }.as_arg());
		...
		let e_pat = if e_lhs.is_empty() {
			...
			Some(ObjectPatProp::KeyValue(KeyValuePatProp {
				...
				value: ArrayPat { span: DUMMY_SP, elems: e_lhs, type_ann: Default::default(), optional: false }.into(),
			}))
		};
		...
		self.state.extra_stmts.push(ExprStmt { span: DUMMY_SP, expr }.into());
	...
	fn initializer_name(&mut self, name: &mut PropName, prefix: &str) -> (Box<Expr>, Ident) {
		match name {
			PropName::Ident(i) => (
				Lit::Str(Str { span: i.span, value: i.sym.clone(), raw: None }).into(),
				Ident::new(
			...
			_ => {
				...
				*name = PropName::Computed(ComputedPropName { span: DUMMY_SP, expr: key_ident.clone().into() });
	...
	fn ensure_identity_constructor<'a>(&mut self, c: &'a mut Class) -> &'a mut Constructor {
		...
		c.body.insert(
			...
			ClassMember::Constructor(Constructor {
				...
				params: Vec::new(),
				body: Some(BlockStmt { span: DUMMY_SP, stmts: Vec::new(), ..Default::default() }),
				..Default::default()
			}),
		);
	...
	fn handle_class_expr(&mut self, class: &mut Class, ident: Option<&Ident>) -> Ident {
		...
		let new_class_name = ident
			.as_ref()
			.map_or_else(|| private_ident!("_class"), |i| private_ident!(format!("_{}", i.sym)));

		if let Some(ident) = ident {
			replace_ident(&mut class.body, ident.to_id(), &new_class_name);
		}

		self.state.class_lhs.push(Some(new_class_name.clone().into()));
...

		{
			...
			class.body.push(ClassMember::StaticBlock(StaticBlock {
				span: DUMMY_SP,
				body: BlockStmt { span: DUMMY_SP, stmts: vec![call_stmt], ..Default::default() },
			}));
		}
	...
	fn handle_class_decl(&mut self, c: &mut ClassDecl) -> Stmt {
		...
		self.rename_map.insert(c.ident.to_id(), new_class_name.to_id());
		...
		if has_static_member {
			...
			for m in body.iter_mut() {
				match m {
					ClassMember::ClassProp(..) | ClassMember::PrivateProp(..) | ClassMember::AutoAccessor(..) => {
...

			{
				...
				constructor.body.as_mut().unwrap().stmts.insert(
					...
					SeqExpr {
						span: DUMMY_SP,
						exprs: once(super_call).chain(static_call).chain(once(init_class_call)).collect(),
					}
					.into_stmt(),
				);
			}
	...
	fn process_prop_name(&mut self, name: &mut PropName) {
		match name {
			...
			_ => {
				...
				*name = PropName::Computed(ComputedPropName { span: DUMMY_SP, expr: ident.into() });
			},
		}
	}
...
impl VisitMut for DecoratorPass {
	...
	fn visit_mut_class(&mut self, n: &mut Class) {
			...
			for member in n.body.iter_mut() {
				if let ClassMember::ClassProp(prop) = member {
					...
					if let Some(value) = prop.value.clone() {
						prop.value = Some(Expr::from_exprs(vec![init_proto_expr.clone().into(), value]));
		...
		if !self.state.extra_stmts.is_empty() {
			n.body.insert(
				...
				ClassMember::StaticBlock(StaticBlock {
					span: DUMMY_SP,
					body: BlockStmt { span: DUMMY_SP, stmts: self.state.extra_stmts.take(), ..Default::default() },
				}),
			);
		}
	...
	fn visit_mut_class_member(&mut self, n: &mut ClassMember) {
		...
		if let ClassMember::PrivateMethod(p) = n {
			...
			if p.is_static {
				self.state.init_static.get_or_insert_with(|| private_ident!("_initStatic"));
			} else {
				self.state.init_proto.get_or_insert_with(|| private_ident!("_initProto"));
			}

			let caller = FnExpr { ident: None, function: p.function.clone() };
			...
			match p.kind {
				MethodKind::Method => {
					let call_stmt = ReturnStmt { span: DUMMY_SP, arg: Some(init.into()) }.into();

					p.kind = MethodKind::Getter;
					p.function.body = Some(BlockStmt { span: DUMMY_SP, stmts: vec![call_stmt], ..Default::default() });
				},
				MethodKind::Getter => {
	...
	fn visit_mut_class_members(&mut self, members: &mut Vec<ClassMember>) {
				...
				ClassMember::AutoAccessor(mut accessor) => {
					...
					let private_field = PrivateProp {
						...
						key: match &mut accessor.key {
							Key::Private(k) => {
								name = Lit::Str(Str { span: DUMMY_SP, value: k.name.clone(), raw: None }).into();
								...
								PrivateName {
									span: k.span,
									name: format!("__{}_{}", k.name, self.state.private_id_index).into(),
								}
							},
							Key::Public(k) => {
								(name, init) = self.initializer_name(k, "init");
								field_name_like = format!("__{}", init.sym).replacen("init", "private", 1).into();

								self.state.private_id_index += 1;

								PrivateName {
									span: init.span,
									name: format!("{field_name_like}_{}", self.state.private_id_index).into(),
								}
							},
						},
					...
					if !accessor.decorators.is_empty() {
							...
							ArrayLit {
								...
								elems: match &accessor.key {
									Key::Private(_) => {
										let data = vec![
											dec,
											Some(if accessor.is_static { 6.as_arg() } else { 1.as_arg() }),
											Some(name.as_arg()),
											Some(FnExpr { ident: None, function: getter_function }.as_arg()),
											Some(FnExpr { ident: None, function: setter_function }.as_arg()),
										];
						...
						if accessor.is_static {
							...
							self.state.static_lhs.extend(getter_var.into_iter().chain(setter_var));
						} else {
							self.state.proto_lhs.push(init);
							self.state.init_proto_args.push(Some(initialize_init));
							self.state.proto_lhs.extend(getter_var.into_iter().chain(setter_var));
						}
		...
		for mut m in new.take() {
			match m {
				ClassMember::Method(..) | ClassMember::PrivateMethod(..) | ClassMember::AutoAccessor(..) => {},
	...
	fn visit_mut_class_prop(&mut self, p: &mut ClassProp) {
		...
		self.extra_vars
			.push(VarDeclarator { span: p.span, name: init.clone().into(), init: None, definite: false });
	...
	fn visit_mut_expr(&mut self, e: &mut Expr) {
		if let Expr::Class(c) = e {
			if !c.class.decorators.is_empty() {
				...
				*e = SeqExpr { span: DUMMY_SP, exprs: vec![Box::new(e.take()), Box::new(Expr::Ident(new))] }.into();
	...
	fn visit_mut_module_item(&mut self, s: &mut ModuleItem) {
		match s {
			ModuleItem::ModuleDecl(ModuleDecl::ExportDecl(ExportDecl { span, decl: Decl::Class(c) }))
				if !c.class.decorators.is_empty() =>
			{
				...
				*s = new_stmt.into();
				self.extra_exports.push(ExportSpecifier::Named(ExportNamedSpecifier {
					span,
			...
			ModuleItem::ModuleDecl(ModuleDecl::ExportDefaultDecl(ExportDefaultDecl {
				...
				let ident = c.ident.get_or_insert_with(|| private_ident!("_default")).clone();
	...
	fn visit_mut_module_items(&mut self, n: &mut Vec<ModuleItem>) {
		...
		for (index, n) in n.iter_mut().enumerate() {
			...
			if !self.pre_class_inits.is_empty() {
				insert_builder.push_back(
					index,
					ExprStmt { span: DUMMY_SP, expr: Expr::from_exprs(self.pre_class_inits.take()) }.into(),
				);
			}
		}
		...
		let initialize_init = {
			...
			ArrayLit {
				...
				elems: vec![
					...
					Some(FnExpr { ident: None, function: getter }.as_arg()),
					Some(FnExpr { ident: None, function: setter }.as_arg()),
				],
			}
			.as_arg()
		};
	...
	fn visit_mut_stmts(&mut self, n: &mut Vec<Stmt>) {
		...
		if !self.extra_vars.is_empty() {
			let insert_pos = n.iter().position(|stmt| !is_maybe_branch_directive(stmt)).unwrap_or(0);
			insert_builder.push_front(
				insert_pos,
				VarDecl {
					span: DUMMY_SP,
...
fn merge_decorators(decorators: Vec<Option<ExprOrSpread>>) -> Option<ExprOrSpread> {
	...
	Some(ArrayLit { span: DUMMY_SP, elems: decorators }.as_arg())
}
