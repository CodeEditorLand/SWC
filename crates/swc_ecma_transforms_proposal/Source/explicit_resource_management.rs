...
use swc_ecma_utils::{
	ExprFactory, ModuleItemLike, StmtLike, private_ident, quote_ident, stack_size::maybe_grow_default,
};
...
impl Default for State {
	fn default() -> Self {
		Self { env: private_ident!("env"), has_await: false, vars: Vec::new() }
	}
}
...
impl ExplicitResourceManagement {
		...
		T: StmtLike + ModuleItemLike,
	{
		...
		let mut try_block = BlockStmt { stmts: vec![], ..Default::default() };
			...
			body: BlockStmt {
				...
				stmts: vec![
					...
					AssignExpr {
						...
						.into(),
						op: op!("="),
						right: Box::new(Expr::Lit(Lit::Bool(Bool { span: DUMMY_SP, value: true }))),
					}
					.into_stmt(),
				],
				..Default::default()
		...
		let finally_block = if is_async {
			...
			let if_stmt = Stmt::If(IfStmt {
				...
				cons: Stmt::Expr(ExprStmt {
					expr: Box::new(Expr::Await(AwaitExpr { arg: result.clone().into(), ..Default::default() })),
					..Default::default()
				})
				.into(),
				..Default::default()
		...
		let try_stmt = TryStmt {
			...
			finalizer: Some(BlockStmt { stmts: finally_block, ..Default::default() }),
		};
...
impl VisitMut for ExplicitResourceManagement {
	...
	fn visit_mut_for_of_stmt(&mut self, n: &mut ForOfStmt) {
		...
		if let ForHead::UsingDecl(using) = &mut n.left {
			...
			let mut inner_var_decl = VarDecl { kind: VarDeclKind::Const, ..Default::default() };
			...
			for decl in &mut using.decls {
				...
				inner_var_decl.decls.push(VarDeclarator {
					...
					init: Some(
						CallExpr {
							callee: helper!(ts, ts_add_disposable_resource),
							args: vec![state.env.clone().as_arg(), new_var.clone().as_arg(), using.is_await.as_arg()],
							..Default::default()
						}
						.into(),
					),
			...
			let mut body = vec![Stmt::Decl(Decl::Var(Box::new(inner_var_decl))), *n.body.take()];
			self.wrap_with_try(state, &mut body);

			n.left = ForHead::VarDecl(var_decl);
			n.body = Box::new(BlockStmt { span: DUMMY_SP, stmts: body, ..Default::default() }.into())
		}
	}
