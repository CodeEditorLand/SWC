...
impl TscDecorator {
	...
	fn key(&mut self, k: &mut PropName) -> Expr {
		match k {
			...
			PropName::Ident(i) => return Lit::Str(Str { span: DUMMY_SP, raw: None, value: i.sym.clone() }).into(),
			_ => {},
		}
...
impl Visit for TscDecorator {
	fn visit_ts_enum_decl(&mut self, e: &TsEnumDecl) {
		let enum_kind = e
			...
			.map(|init| match init {
				Some(e) => match &**e {
					Expr::Unary(UnaryExpr { op: op!(unary, "-"), .. }) => EnumKind::Num,
...
impl VisitMut for TscDecorator {
	fn visit_mut_class(&mut self, n: &mut Class) {
		...
		let appended_private = mem::replace(&mut self.appended_private_access_exprs, appended_private);
		...
		if let Some(class_name) = self.class_name.clone() {
			if !n.decorators.is_empty() {
				...
				let decorated = CallExpr {
					span: DUMMY_SP,
					callee: helper!(ts, ts_decorate),
					args: vec![decorators, class_name.clone().with_pos(BytePos::DUMMY, BytePos::DUMMY).as_arg()],
					..Default::default()
				}
				.into();
