...
impl ParamMetadata {
	fn create_param_decorator(&self, param_index: usize, mut decorator_expr: Box<Expr>) -> Decorator {
...
impl VisitMut for Metadata<'_> {
	fn visit_mut_class(&mut self, c: &mut Class) {
		...
		{
			let dec = self.create_metadata_design_decorator("design:type", quote_ident!("Function").as_arg());
	...
	fn visit_mut_class_method(&mut self, m: &mut ClassMethod) {
...

		{
			let type_arg = match m.kind {
				...
				MethodKind::Setter => {
					serialize_type(self.class_name, get_type_ann_of_pat(&m.function.params[0].pat)).as_arg()
				},
			};
...
impl<'a> Metadata<'a> {
	pub(super) fn new(enums: &'a EnumMapType, class_name: Option<&'a Ident>) -> Self {
		Self { enums: EnumMap(enums), class_name }
	}
...
fn serialize_type(class_name: Option<&Ident>, param: Option<&TsTypeAnn>) -> Expr {
	fn check_object_existed(expr: Box<Expr>) -> Box<Expr> {
		match *expr {
			Expr::Member(ref member_expr) => {
				...
				BinExpr {
					...
					right: Box::new(
						BinExpr {
							span: DUMMY_SP,
							left: Box::new(Expr::Unary(UnaryExpr { span: DUMMY_SP, op: op!("typeof"), arg: expr })),
							op: op!("==="),
			...
			_ => BinExpr {
				span: DUMMY_SP,
				left: Box::new(UnaryExpr { span: DUMMY_SP, op: op!("typeof"), arg: expr }.into()),
				op: op!("==="),
				right: Box::new(Lit::Str(Str { span: DUMMY_SP, value: "undefined".into(), raw: None }).into()),
			}
			.into(),
		}
	}
	...
	fn serialize_type_list(class_name: &str, types: &[Box<TsType>]) -> Expr {
		...
		for ty in types {
			...
			match &**ty {
				...
				TsType::TsKeywordType(TsKeywordType { kind: TsKeywordTypeKind::TsNeverKeyword, .. }) => {
					continue;
				},
	...
	fn serialize_type_node(class_name: &str, ty: &TsType) -> Expr {
		let span = ty.span();
		match ty {
			TsType::TsKeywordType(TsKeywordType { kind: TsKeywordTypeKind::TsVoidKeyword, .. })
			| TsType::TsKeywordType(TsKeywordType { kind: TsKeywordTypeKind::TsUndefinedKeyword, .. })
			| TsType::TsKeywordType(TsKeywordType { kind: TsKeywordTypeKind::TsNullKeyword, .. })
			| TsType::TsKeywordType(TsKeywordType { kind: TsKeywordTypeKind::TsNeverKeyword, .. }) => *Expr::undefined(span),
			...
			TsType::TsLitType(TsLitType { lit: TsLit::Bool(..), .. })
			...
			TsType::TsKeywordType(TsKeywordType { kind: TsKeywordTypeKind::TsObjectKeyword, .. }) => {
				quote_ident!("Object").into()
			},

			TsType::TsLitType(TsLitType { lit: TsLit::Number(..), .. })
			| TsType::TsKeywordType(TsKeywordType { kind: TsKeywordTypeKind::TsNumberKeyword, .. }) => {
				quote_ident!("Number").into()
			},

			TsType::TsKeywordType(TsKeywordType { kind: TsKeywordTypeKind::TsBigIntKeyword, .. }) => CondExpr {
				span: DUMMY_SP,
			...
			TsType::TsKeywordType(TsKeywordType { kind: TsKeywordTypeKind::TsSymbolKeyword, .. }) => {
			...
			| TsType::TsKeywordType(TsKeywordType { kind: TsKeywordTypeKind::TsUnknownKeyword, .. })
			...
			TsType::TsUnionOrIntersectionType(ty) => match ty {
				...
				TsUnionOrIntersectionType::TsIntersectionType(ty) => serialize_type_list(class_name, &ty.types),
			},
...
fn ts_entity_to_member_expr(type_name: &TsEntityName) -> Expr {
	match type_name {
		TsEntityName::TsQualifiedName(q) => {
			...
			MemberExpr { span: DUMMY_SP, obj: obj.into(), prop: MemberProp::Ident(q.right.clone()) }.into()
		},
...
fn is_str(ty: &TsType) -> bool {
	match ty {
		TsType::TsLitType(TsLitType { lit: TsLit::Str(..) | TsLit::Tpl(..), .. })
		| TsType::TsKeywordType(TsKeywordType { kind: TsKeywordTypeKind::TsStringKeyword, .. }) => true,
