	...
	fn exec(&self) -> QueryResult {
			...
			T: AsRef<str>,
		{
			let distribs = browserslist::resolve(
				s,
				&browserslist::Opts { mobile_to_desktop: true, ignore_unknown_versions: true, ..Default::default() },
			)
			.with_context(|| {
				format!(
			...
			let versions = BrowserData::parse_versions(distribs).expect("failed to parse browser version");

			Ok(Arc::new(versions))
		}

		static CACHE: Lazy<DashMap<Query, Arc<Versions>, FxBuildHasher>> = Lazy::new(Default::default);
...
pub fn targets_to_versions(v: Option<Targets>) -> Result<Arc<Versions>, Error> {
	match v {
		...
		Some(Targets::Query(q)) => q.exec().context("failed to convert target query to version data"),
