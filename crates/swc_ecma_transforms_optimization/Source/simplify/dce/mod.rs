...
use petgraph::{Direction::Incoming, algo::tarjan_scc, prelude::DiGraphMap};
...
use swc_ecma_ast::*;
use swc_ecma_transforms_base::perf::{Parallel, cpu_count};
use swc_ecma_utils::{
...
impl Data {
	...
	fn add_dep_edge(&mut self, from: &Id, to: &Id, assign: bool) {
		...
		match self.graph.edge_weight_mut(from, to) {
			...
			None => {
				self.graph
					.add_edge(from, to, VarInfo { usage: u32::from(!assign), assign: u32::from(assign) });
			},
		};
	}
...
impl TreeShaker {
	...
	fn can_drop_assignment_to(&self, name: Id, is_var: bool) -> bool {
		...
		self.bindings.contains(&name) && self.data.used_names.get(&name).map(|v| v.usage == 0).unwrap_or_default()
	}
...
impl VisitMut for TreeShaker {
	...
	fn visit_mut_assign_expr(&mut self, n: &mut AssignExpr) {
		...
		if let Some(id) = n.left.as_ident() {
			// TODO: `var`
			if self.can_drop_assignment_to(id.to_id(), false) && !n.right.may_have_side_effects(self.expr_ctx) {
				self.changed = true;
					...
					&& c.class.body.iter().all(|m| match m {
						...
						ClassMember::ClassProp(m) => {
							!matches!(m.key, PropName::Computed(..))
								&& !m.value.as_deref().map_or(false, |e| e.may_have_side_effects(self.expr_ctx))
						},
			...
			if args.is_empty() {
				match &mut **callee {
					Expr::Fn(FnExpr { ident: None, function: f })
						if matches!(&**f, Function { is_async: false, is_generator: false, body: Some(..), .. }) =>
					{
	...
	fn visit_mut_module_item(&mut self, n: &mut ModuleItem) {
		match n {
			ModuleItem::ModuleDecl(ModuleDecl::Import(i)) => {
				...
				if !self.config.preserve_imports_with_side_effects && !is_for_side_effect && i.specifiers.is_empty() {
	...
	fn visit_mut_var_declarator(&mut self, v: &mut VarDeclarator) {
		...
		if let Pat::Ident(i) = &v.name {
			...
			if can_drop && self.can_drop_binding(i.to_id(), self.var_decl_kind == Some(VarDeclKind::Var)) {
				self.changed = true;
