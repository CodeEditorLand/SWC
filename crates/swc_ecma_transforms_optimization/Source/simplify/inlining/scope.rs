...
impl Inlining<'_> {
		...
		F: for<'any> FnOnce(&mut Inlining<'any>),
	{
		...
		if !matches!(kind, ScopeKind::Fn { .. }) {
			...
			for (id, v) in
				v.into_iter().filter_map(
					|(id, v)| {
						if v.kind == VarType::Var(VarDeclKind::Var) { Some((id, v)) } else { None }
					},
				) {
				let v: VarInfo = v;
	...
	pub(super) fn declare(&mut self, id: Id, init: Option<Cow<Expr>>, is_change: bool, kind: VarType) {
		tracing::trace!("({}, {:?}) declare({})", self.scope.depth(), self.phase, id.0);
		...
		let is_undefined =
			self.var_decl_kind == VarDeclKind::Var && !is_change && init.is_none() && self.phase == Phase::Inlining;
...
impl<'a> Scope<'a> {
	pub fn new(parent: Option<&'a Scope<'a>>, kind: ScopeKind) -> Self {
		Self { parent, kind, ..Default::default() }
	}
	...
	pub fn add_write(&mut self, id: &Id, force_no_inline: bool) {
		...
		if let Some(var_info) = scope.find_binding_from_current(id) {
			...
			tracing::trace!("({}): Unresolved. (scope = ({})): {:?}", self.depth(), scope.depth(), id);
	...
	fn idx_val(&self, id: &Id) -> Option<(usize, &VarInfo)> {
		self.bindings
			.iter()
			.enumerate()
			.find_map(|(idx, (k, v))| if k == id { Some((idx, v)) } else { None })
	}
	...
	fn has_constant(&self, id: &Id) -> bool {
		...
		self.parent.map(|parent| parent.has_constant(id)).unwrap_or(false)
	}
	...
	pub fn is_inline_prevented(&self, e: &Expr) -> bool {
		match e {
			...
			Expr::Member(MemberExpr { obj: right_expr, .. }) if right_expr.is_ident() => {
