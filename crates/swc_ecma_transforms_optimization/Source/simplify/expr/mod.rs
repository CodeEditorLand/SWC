...
use swc_ecma_utils::{
	BoolType, ExprCtx, ExprExt, NullType, NumberType, ObjectType, StringType, SymbolType, UndefinedType, Value,
	is_literal, number::JsNumber, prop_name_eq, to_int32,
};
...
impl VisitMut for SimplifyExpr {
			...
			Callee::Expr(e) => {
				...
				match &mut **e {
					Expr::Seq(seq) => {
						if seq.exprs.len() == 1 {
							...
							*e = expr;
						} else if seq.exprs.last().map(|v| &**v).map_or(false, Expr::directness_matters) {
				...
				if may_inject_zero && need_zero_for_this(e) {
					match &mut **e {
						...
						_ => {
							let seq = SeqExpr { span: DUMMY_SP, exprs: vec![0.0.into(), e.take()] };
							**e = seq.into();
						},
					}
				}
	...
	fn visit_mut_expr(&mut self, expr: &mut Expr) {
		if let Expr::Unary(UnaryExpr { op: op!("delete"), .. }) = expr {
			return;
		}
		...
		match expr {
				...
				if let (p, Known(val)) = test.cast_to_bool(self.expr_ctx) {
					...
					*expr = if p.is_pure() {
						if expr_value.directness_matters() {
							SeqExpr { span: *span, exprs: vec![0.into(), expr_value.take()] }.into()
						} else {
							*expr_value.take()
						}
					} else {
						SeqExpr { span: *span, exprs: vec![test.take(), expr_value.take()] }.into()
					}
				}
			},
			...
			Expr::Array(ArrayLit { elems, .. }) => {
				...
				for elem in elems.take() {
					match elem {
						Some(ExprOrSpread { spread: Some(..), expr }) if expr.is_array() => {
							self.changed = true;

							e.extend(expr.array().unwrap().elems.into_iter().map(|elem| {
								Some(
									elem.unwrap_or_else(|| ExprOrSpread {
										spread: None,
			...
			Expr::Object(ObjectLit { props, .. }) => {
				...
				for p in props.take() {
					match p {
						PropOrSpread::Spread(SpreadElement { dot3_token, expr, .. }) if expr.is_object() => {
							if let Expr::Object(obj) = &*expr {
								if obj.props.iter().any(|p| match p {
									PropOrSpread::Spread(..) => true,
									PropOrSpread::Prop(p) => {
										!matches!(&**p, Prop::Shorthand(_) | Prop::KeyValue(_) | Prop::Method(_))
									},
								}) {
	...
	fn visit_mut_seq_expr(&mut self, e: &mut SeqExpr) {
		...
		for expr in e.exprs.take() {
			match *expr {
				...
				Expr::Lit(..) | Expr::Ident(..) if self.in_callee && !expr.may_have_side_effects(self.expr_ctx) => {
					if exprs.is_empty() {
						self.changed = true;
...
pub fn optimize_member_expr(expr_ctx: ExprCtx, expr: &mut Expr, is_callee: bool, changed: &mut bool) {
	...
	let op = match prop {
		MemberProp::Ident(IdentName { sym, .. }) if &**sym == "length" && !obj.is_object() => KnownOp::Len,
	...
	match &mut **obj {
		Expr::Lit(Lit::Str(Str { value, span, .. })) => match op {
			...
			KnownOp::Index(idx) => {
				...
				*expr = Lit::Str(Str { raw: None, value: value.into(), span: *span }).into()
			},
		...
		Expr::Array(ArrayLit { elems, span }) => {
			...
			let has_spread = elems
				.iter()
				.any(|elem| elem.as_ref().map(|elem| elem.spread.is_some()).unwrap_or(false));
			...
			match op {
				KnownOp::Len => {
					...
					*expr = Lit::Num(Number { value: elems.len() as _, span: *span, raw: None }).into();
				},
				...
				KnownOp::Index(idx) => {
					...
					let after_has_side_effect = elems.iter().skip((idx as usize + 1) as _).any(|elem| match elem {
		...
		Expr::Object(ObjectLit { props, span }) => {
			...
			*expr = *expr_ctx.preserve_effects(*span, v, once(ObjectLit { props: props.take(), span: *span }.into()));
		},
...
pub fn optimize_bin_expr(expr_ctx: ExprCtx, expr: &mut Expr, changed: &mut bool) {
	let BinExpr { left, op, right, span } = match expr {
		Expr::Bin(bin) => bin,
		_ => return,
	};
	let op = *op;
				...
				Known(v) => {
					*changed = true;
					...
					let value_expr = if !v.is_nan() {
						Expr::Lit(Lit::Num(Number { value: v, span: *span, raw: None }))
					} else {
	...
	match op {
		op!(bin, "+") => {
			...
			if left.is_str() || left.is_array_lit() || right.is_str() || right.is_array_lit() {
				if let (Known(l), Known(r)) = (left.as_pure_string(expr_ctx), right.as_pure_string(expr_ctx)) {
					...
					*expr = Lit::Str(Str { raw: None, value: l.into(), span: *span }).into();
					return;
				}
			}
			...
			match expr.get_type(expr_ctx) {
				...
				Known(StringType) => match expr {
					Expr::Bin(BinExpr { left, right, span, .. }) => {
						if !left.may_have_side_effects(expr_ctx) && !right.may_have_side_effects(expr_ctx) {
							if let (Known(l), Known(r)) =
								(left.as_pure_string(expr_ctx), right.as_pure_string(expr_ctx))
							{
								*changed = true;
						...
						Expr::Bin(BinExpr { left, right, span, .. }) => {
							if let Known(v) = perform_arithmetic_op(expr_ctx, op, left, right) {
								*changed = true;
								...
								let value_expr = if !v.is_nan() {
									Lit::Num(Number { value: v, span, raw: None }).into()
								} else {
				...
				if !left.may_have_side_effects(expr_ctx) {
					*changed = true;
					...
					if node.directness_matters() {
						*expr = SeqExpr { span: node.span(), exprs: vec![0.into(), node.take()] }.into();
					} else {
						*expr = *node.take();
					}
				} else {
					*changed = true;
					...
					let seq = SeqExpr { span: *span, exprs: vec![left.take(), node.take()] };
		...
		op!("instanceof") => {
			fn is_non_obj(e: &Expr) -> bool {
				match e {
					...
					| Expr::Unary(UnaryExpr { op: op!(unary, "-"), ref arg, .. })
					| Expr::Unary(UnaryExpr { op: op!("void"), ref arg, .. }) => is_non_obj(arg),
					_ => false,
				}
			}
		...
		op!("*") | op!("&") | op!("|") | op!("^") => {
			...
			// Try left.rhs * right
			if let Expr::Bin(BinExpr { span: _, left: left_lhs, op: left_op, right: left_rhs }) = &mut **left {
				if *left_op == op {
					if let Known(value) = perform_arithmetic_op(expr_ctx, op, left_rhs, right) {
						let value_expr = if !value.is_nan() {
							Lit::Num(Number { value, span: *span, raw: None }).into()
						} else {
							Ident::new("NaN".into(), *span, expr_ctx.unresolved_ctxt).into()
						};
	...
	match op {
		...
		op!(unary, "+") => {
			if let Known(v) = arg.as_pure_number(expr_ctx) {
				...
				*expr = *expr_ctx.preserve_effects(
					*span,
					Lit::Num(Number { value: v, span: *span, raw: None }).into(),
					iter::once(arg.take()),
				);
			}
		},
		op!(unary, "-") => match &**arg {
			...
			Expr::Lit(Lit::Num(Number { value: f, .. })) => {
				*changed = true;
				*expr = Lit::Num(Number { value: -f, span: *span, raw: None }).into();
			},
			_ => {
		...
		op!("void") if !may_have_side_effects => {
			...
			*arg = Lit::Num(Number { value: 0.0, span: arg.span(), raw: None }).into();
		},
...
fn try_fold_typeof(_expr_ctx: ExprCtx, expr: &mut Expr, changed: &mut bool) {
	...
	let val = match &**arg {
		...
		Expr::Unary(UnaryExpr { op: op!("void"), .. }) => "undefined",
	...
	*expr = Lit::Str(Str { span: *span, raw: None, value: val.into() }).into();
}
...
fn perform_arithmetic_op(expr_ctx: ExprCtx, op: BinaryOp, left: &Expr, right: &Expr) -> Value<f64> {
		...
		($value:expr) => {{
			...
			if ls.is_dummy() || rs.is_dummy() {
				...
				if right.span().hi() > left.span().lo() {
					let orig_len = right.span().hi() - right.span().lo() + left.span().hi() - left.span().lo();
					if new_len <= orig_len.0 as usize + 1 { Known($value) } else { Unknown }
				} else {
					Known($value)
				}
			}
	...
	let (lv, rv) = (left.as_pure_number(expr_ctx), right.as_pure_number(expr_ctx));
...
fn perform_abstract_rel_cmp(expr_ctx: ExprCtx, left: &Expr, right: &Expr, will_negate: bool) -> Value<bool> {
	match (left, right) {
		// Special case: `x < x` is always false.
		(
			&Expr::Ident(Ident { sym: ref li, ctxt: l_ctxt, .. }, ..),
			&Expr::Ident(Ident { sym: ref ri, ctxt: r_ctxt, .. }),
		) if !will_negate && li == ri && l_ctxt == r_ctxt => {
			return Known(false);
		},
		// Special case: `typeof a < typeof a` is always false.
		(
			&Expr::Unary(UnaryExpr { op: op!("typeof"), arg: ref la, .. }),
			&Expr::Unary(UnaryExpr { op: op!("typeof"), arg: ref ra, .. }),
		) if la.as_ident().is_some() && la.as_ident().map(|i| i.to_id()) == ra.as_ident().map(|i| i.to_id()) => {
			return Known(false);
		},
		_ => {},
	}
	...
	if let (Known(StringType), Known(StringType)) = (lt, rt) {
		if let (Known(lv), Known(rv)) = (left.as_pure_string(expr_ctx), right.as_pure_string(expr_ctx)) {
...
fn perform_abstract_eq_cmp(expr_ctx: ExprCtx, span: Span, left: &Expr, right: &Expr) -> Value<bool> {
	let (lt, rt) = (try_val!(left.get_type(expr_ctx)), try_val!(right.get_type(expr_ctx)));
	...
	match (lt, rt) {
		...
		(NumberType, StringType) | (_, BoolType) => {
			...
			perform_abstract_eq_cmp(expr_ctx, span, left, &Lit::Num(Number { value: rv, span, raw: None }).into())
		},

		(StringType, NumberType) | (BoolType, _) => {
			let lv = try_val!(left.as_pure_number(expr_ctx));
			perform_abstract_eq_cmp(expr_ctx, span, &Lit::Num(Number { value: lv, span, raw: None }).into(), right)
		},

		(StringType, ObjectType) | (NumberType, ObjectType) | (ObjectType, StringType) | (ObjectType, NumberType) => {
			Unknown
		},
...
fn perform_strict_eq_cmp(expr_ctx: ExprCtx, left: &Expr, right: &Expr) -> Value<bool> {
	...
	match lt {
		...
		NumberType => Known(try_val!(left.as_pure_number(expr_ctx)) == try_val!(right.as_pure_number(expr_ctx))),
		StringType => {
			let (lv, rv) = (
