...
use swc_ecma_utils::{
	ExprCtx, ExprExt, ExprFactory, Hoister, IsEmpty, StmtExt, StmtLike, Value::Known, extract_var_ids, is_literal,
	prepend_stmt,
};
...
pub fn dead_branch_remover(unresolved_mark: Mark) -> impl Repeated + Pass + CompilerPass + VisitMut + 'static {
...
impl VisitMut for Remover {
	...
	fn visit_mut_expr(&mut self, e: &mut Expr) {
		...
		match e {
			...
			Expr::Assign(AssignExpr { op: op!("="), left: AssignTarget::Simple(l), right: r, .. })
				if match &*l {
			...
			Expr::Assign(AssignExpr { op: op!("="), left: AssignTarget::Pat(left), right, .. })
				if match &*left {
					AssignTargetPat::Array(arr) => arr.elems.is_empty() || arr.elems.iter().all(|v| v.is_none()),
					_ => false,
				} =>
			{
				if cfg!(feature = "debug") {
	...
	fn visit_mut_for_stmt(&mut self, s: &mut ForStmt) {
		...
		s.init = s.init.take().and_then(|e| match e {
			VarDeclOrExpr::Expr(e) => ignore_result(e, true, self.expr_ctx).map(VarDeclOrExpr::from),
			_ => Some(e),
		});

		s.update = s.update.take().and_then(|e| ignore_result(e, true, self.expr_ctx));

		s.test = s.test.take().and_then(|e| {
			let span = e.span();
			if let Known(value) = e.as_pure_bool(self.expr_ctx) {
				return if value { None } else { Some(Lit::Bool(Bool { span, value: false }).into()) };
			}
	...
	fn visit_mut_object_pat_prop(&mut self, p: &mut ObjectPatProp) {
		...
		match p {
			ObjectPatProp::Assign(AssignPatProp { span, key, value: Some(expr) })
				if expr.is_undefined(self.expr_ctx)
					|| match **expr {
						Expr::Unary(UnaryExpr { op: op!("void"), ref arg, .. }) => is_literal(&**arg),
						_ => false,
					} =>
			{
				*p = ObjectPatProp::Assign(AssignPatProp { span: *span, key: key.take(), value: None });
			},
	...
	fn visit_mut_seq_expr(&mut self, e: &mut SeqExpr) {
		...
		let mut exprs = if should_preserved_this {
			...
			e.exprs.take().move_flat_map(|e| ignore_result(e, false, self.expr_ctx))
		};
	...
	fn visit_mut_stmt(&mut self, stmt: &mut Stmt) {
		...
		stmt.map_with_mut(|stmt| {
			match stmt {
				Stmt::If(IfStmt { span, test, cons, alt }) => {
					if let Stmt::If(IfStmt { alt: Some(..), .. }) = *cons {
						return IfStmt {
							test,
							cons: Box::new(BlockStmt { stmts: vec![*cons], ..Default::default() }.into()),
							alt,
							span,
						}
						.into();
					...
					if let (p, Known(v)) = test.cast_to_bool(self.expr_ctx) {
						...
						let mut block: Stmt = BlockStmt { span, stmts, ..Default::default() }.into();
					...
					IfStmt { span, test, cons, alt }.into()
				},
				...
				Stmt::Labeled(LabeledStmt { label, span, body, .. }) if body.is_empty() => {
					debug!("Dropping an empty label statement: `{}`", label);
					EmptyStmt { span }.into()
				},

				Stmt::Labeled(LabeledStmt { span, body, ref label, .. })
					if match &*body {
				...
				Stmt::Block(BlockStmt { span, stmts, ctxt }) => {
					if stmts.is_empty() {
						...
						EmptyStmt { span }.into()
					} else if stmts.len() == 1 && !is_block_scoped_stuff(&stmts[0]) && stmt_depth(&stmts[0]) <= 1 {
						if cfg!(feature = "debug") {
				...
				Stmt::Try(s) => {
					let TryStmt { span, block, handler, finalizer } = *s;
				...
				Stmt::Switch(mut s) => {
										...
										if matches!(&*var, VarDecl { kind: VarDeclKind::Var, .. }) =>
									{
										return Some(
											VarDecl {
												...
												decls: var.decls.move_map(|decl| VarDeclarator { init: None, ..decl }),
												..Default::default()
											}
											.into(),
										);
					...
					if s.cases.len() == 1 && s.cases[0].test.is_none() && !has_conditional_stopper(&s.cases[0].cons) {
						if cfg!(feature = "debug") {
						...
						let mut block: Stmt = BlockStmt { span: s.span, stmts, ..Default::default() }.into();
						block.visit_mut_with(self);
						return block;
					}
						...
						s.cases.iter().position(|case| {
							...
							if let Some(ref test) = case.test {
								let v = match (&**test, &*s.discriminant) {
									(
										&Expr::Lit(Lit::Str(Str { value: ref test, .. })),
					...
					if let Some(i) = selected {
						if !has_conditional_stopper(&s.cases[i].cons) {
							...
							let decls = cases
								.flat_map(|case| {
									exprs.extend(case.test.and_then(|e| ignore_result(e, true, self.expr_ctx)));
									case.cons
								})
								.flat_map(|stmt| stmt.extract_var_ids())
								.map(|i| VarDeclarator { span: DUMMY_SP, name: i.into(), init: None, definite: false })
								.collect::<Vec<_>>();
							...
							if let Some(i) = idx {
								for case in &s.cases[..i] {
									for cons in &case.cons {
										vars.extend(cons.extract_var_ids().into_iter().map(|name| VarDeclarator {
											span: DUMMY_SP,
											name: name.into(),
											init: None,
								...
								if !has_conditional_stopper(&s.cases[i].cons) {
									...
									let mut block: Stmt =
										BlockStmt { span: s.span, stmts, ..Default::default() }.into();
					...
					if is_matching_literal {
						...
						s.cases = s.cases.move_flat_map(|case| {
							...
							let res = match case.test {
								...
								{
									case.cons.into_iter().for_each(|stmt| var_ids.extend(stmt.extract_var_ids()));
...

					{
						...
						let is_all_case_empty = s.cases.iter().all(|case| case.test.is_none() || case.cons.is_empty());
						...
						if is_default_last
							&& is_all_case_empty && is_all_case_side_effect_free
					...
					if is_matching_literal
						...
						if s.cases.iter().all(|case| !has_conditional_stopper(&case.cons)) {
							if cfg!(feature = "debug") {
					...
					} =>
				{
					...
					if s.init.is_some() {
						ForStmt { body: Box::new(body), update: None, ..s }.into()
					} else {
						body
					}
				},
				...
				Stmt::While(s) => {
					if let (purity, Known(v)) = s.test.cast_to_bool(self.expr_ctx) {
						if v {
							if purity.is_pure() {
								WhileStmt { test: Lit::Bool(Bool { span: s.test.span(), value: true }).into(), ..s }
									.into()
							} else {
								s.into()
							}
							...
							if purity.is_pure() {
								...
								WhileStmt { body: Box::new(body), ..s }.into()
							}
						}
					} else {
						s.into()
				...
				Stmt::DoWhile(s) => {
					...
					if let Known(v) = s.test.as_pure_bool(self.expr_ctx) {
						if v {
							...
							ForStmt { span: s.span, init: None, test: None, update: None, body: s.body }.into()
						} else {
							let mut body = prepare_loop_body_for_inlining(*s.body);
							body.visit_mut_with(self);

							if let Some(test) = ignore_result(s.test, true, self.expr_ctx) {
								BlockStmt { span: s.span, stmts: vec![body, test.into_stmt()], ..Default::default() }
									.into()
							} else {
								body
							}
			...
			let stmt_like = match stmt_like.try_into_stmt() {
				Ok(stmt) => {
					let stmt = match stmt {
						...
						Stmt::Block(BlockStmt { span, mut stmts, ctxt, .. }) => {
							if stmts.is_empty() {
								continue;
							}
							...
							if !is_ok_to_inline_block(&stmts) {
								...
								new_stmts
									.extend(stmts.into_iter().filter(|s| !matches!(s, Stmt::Empty(..))).map(T::from));
								continue;
							}
						},

						// Optimize if statement.
						Stmt::If(IfStmt { test, cons, alt, span }) => {
							// check if
							match test.cast_to_bool(self.expr_ctx) {
								(purity, Known(val)) => {
									...
									if !purity.is_pure() {
										...
										if let Some(expr) = expr {
											new_stmts.push(T::from(ExprStmt { span: DUMMY_SP, expr }.into()));
										}
									}
								...
								_ => IfStmt { test, cons, alt, span }.into(),
							}
						},
...
fn ignore_result(e: Box<Expr>, drop_str_lit: bool, ctx: ExprCtx) -> Option<Box<Expr>> {
	match *e {
		Expr::Lit(Lit::Num(..)) | Expr::Lit(Lit::Bool(..)) | Expr::Lit(Lit::Null(..)) | Expr::Lit(Lit::Regex(..)) => {
			None
		},
		...
		Expr::Assign(AssignExpr { op: op!("="), left: AssignTarget::Simple(left), right, .. })
			if match &left {
		...
		Expr::Bin(BinExpr { span, left, op, right }) if !op.may_short_circuit() => {
			let left = ignore_result(left, true, ctx);
			let right = ignore_result(right, true, ctx);

			match (left, right) {
				(Some(l), Some(r)) => {
					ignore_result(ctx.preserve_effects(span, Expr::undefined(span), vec![l, r]), true, ctx)
				},
				(Some(l), None) => Some(l),
				(None, Some(r)) => Some(r),
				(None, None) => None,
		...
		Expr::Bin(BinExpr { span, left, op, right }) => {
			if op == op!("&&") {
				...
				if let Known(l) = l {
					...
					Some(BinExpr { span, left, op, right }.into())
				}
			} else {
				...
				if let Known(l) = l {
					if l { None } else { ignore_result(right, true, ctx) }
				} else {
					let right = ignore_result(right, true, ctx);
		...
		Expr::Unary(UnaryExpr { span, op, arg }) => match op {
			...
			op!("void") | op!(unary, "+") | op!(unary, "-") | op!("!") | op!("~") => ignore_result(arg, true, ctx),
		...
		Expr::Array(ArrayLit { span, elems, .. }) => {
			...
			let elems = elems.move_flat_map(|v| match v {
				Some(ExprOrSpread { spread: Some(..), .. }) => {
					has_spread = true;
					Some(v)
				},
				None => None,
				Some(ExprOrSpread { spread: None, expr }) => {
					ignore_result(expr, true, ctx).map(|expr| Some(ExprOrSpread { spread: None, expr }))
				},
			});
			...
			if elems.is_empty() {
				...
				ignore_result(
					ctx.preserve_effects(span, Expr::undefined(span), elems.into_iter().map(|v| v.unwrap().expr)),
					true,
					ctx,
				)
			}
		...
		Expr::Object(ObjectLit { span, props, .. }) => {
			...
			if props.is_empty() {
				...
				ignore_result(
					ctx.preserve_effects(span, Expr::undefined(DUMMY_SP), once(ObjectLit { span, props }.into())),
					true,
					ctx,
				)
			}
		...
		Expr::New(NewExpr { span, ref callee, args, .. }) if callee.is_pure_callee(ctx) => ignore_result(
			ArrayLit {
				span,
				elems: args
		...
		Expr::Call(CallExpr { span, callee: Callee::Expr(ref callee), args, .. }) if callee.is_pure_callee(ctx) => {
			ignore_result(ArrayLit { span, elems: args.into_iter().map(Some).collect() }.into(), true, ctx)
		},

		Expr::Tpl(Tpl { span, exprs, .. }) => {
			ignore_result(ctx.preserve_effects(span, Expr::undefined(span), exprs), true, ctx)
		},

		Expr::TaggedTpl(TaggedTpl { span, tag, tpl, .. }) if tag.is_pure_callee(ctx) => {
			ignore_result(ctx.preserve_effects(span, Expr::undefined(span), tpl.exprs), true, ctx)
		},
		...
		Expr::Seq(SeqExpr { span, mut exprs, .. }) => {
			if exprs.is_empty() {
				return None;
			}
		...
		Expr::Cond(CondExpr { span, test, cons, alt }) => {
			let alt = if let Some(alt) = ignore_result(alt, true, ctx) {
				...
				return ignore_result(BinExpr { span, left: test, op: op!("&&"), right: cons }.into(), true, ctx);
			};

			let cons = if let Some(cons) = ignore_result(cons, true, ctx) {
				cons
			} else {
				return ignore_result(BinExpr { span, left: test, op: op!("||"), right: alt }.into(), true, ctx);
			};

			Some(CondExpr { span, test, cons, alt }.into())
		},
...
fn is_ok_to_inline_block(s: &[Stmt]) -> bool {
	...
	let last_stopper = s
		.iter()
		.rposition(|s| matches!(s, Stmt::Return(..) | Stmt::Throw(..) | Stmt::Break(..) | Stmt::Continue(..)));
...
fn prepare_loop_body_for_inlining(stmt: Stmt) -> Stmt {
	...
	stmts.retain(|stmt| {
		...
		match stmt {
			Stmt::Break(BreakStmt { label: None, .. }) | Stmt::Continue(ContinueStmt { label: None, .. }) => {
				done = true;
				false
			},
	...
	BlockStmt { span, stmts, ..Default::default() }.into()
}
...
fn check_for_stopper(s: &[Stmt], only_conditional: bool) -> bool {
	...
	let mut v = Visitor { in_cond: !only_conditional, found: false };
	v.visit_stmts(s);
	v.found
}
