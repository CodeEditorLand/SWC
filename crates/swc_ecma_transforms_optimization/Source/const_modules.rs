...
pub fn const_modules(cm: Lrc<SourceMap>, globals: FxHashMap<Atom, FxHashMap<Atom, String>>) -> impl Pass {
...
fn parse_option(cm: &SourceMap, name: &str, src: String) -> Arc<Expr> {
	...
	let fm = cm.new_source_file(FileName::Internal(format!("<const-module-{}.js>", name)).into(), src);
	if let Some(expr) = CACHE.get(&**fm.src) {
		return expr.clone();
	}

	let expr = parse_file_as_expr(&fm, Default::default(), Default::default(), None, &mut Vec::new())
		.map_err(|e| {
			if HANDLER.is_set() {
		...
		.map(drop_span)
		.unwrap_or_else(|()| panic!("failed to parse jsx option {}: '{}' is not an expression", name, fm.src,));
		...
		match n {
			Expr::Ident(ref id @ Ident { ref sym, .. }) => {
				...
				if self.scope.namespace.contains(&id.to_id()) {
					panic!("The const_module namespace `{}` cannot be used without member accessor", sym)
				}
			},
		...
		match n {
			Prop::Shorthand(id) => {
				if let Some(value) = self.scope.imported.get(&id.sym) {
					*n = Prop::KeyValue(KeyValueProp { key: id.take().into(), value: Box::new((**value).clone()) });
					return;
				}

				if self.scope.namespace.contains(&id.to_id()) {
					panic!("The const_module namespace `{}` cannot be used without member accessor", id.sym)
				}
			},
