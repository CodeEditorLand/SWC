...
pub fn inline_globals2(
	...
	visit_mut_pass(InlineGlobals { envs, globals, global_exprs, typeofs, bindings: Default::default() })
}
...
impl VisitMut for InlineGlobals {
	...
	fn visit_mut_expr(&mut self, expr: &mut Expr) {
		...
		if let Some(value) = Ident::within_ignored_ctxt(|| self.global_exprs.get(&NodeIgnoringSpan::borrowed(expr))) {
			*expr = value.clone();
		...
		match expr {
			...
			Expr::Unary(UnaryExpr { span, op: op!("typeof"), arg, .. }) => {
				if let Expr::Ident(Ident { ref sym, ctxt: arg_ctxt, .. }) = &**arg {
					...
					if let Some(value) = self.typeofs.get(sym).cloned() {
						*expr = Lit::Str(Str { span: *span, raw: None, value }).into();
					}
				}
			},

			Expr::Member(MemberExpr { obj, prop, .. }) => match &**obj {
				Expr::Member(MemberExpr { obj: first_obj, prop: inner_prop, .. })
...
mod tests {
	...
	fn mk_map(tester: &mut Tester<'_>, values: &[(&str, &str)], is_env: bool) -> FxHashMap<Atom, Expr> {
	...
	test!(
		::swc_ecma_parser::Syntax::default(),
		|tester| inline_globals(envs(tester, &[]), globals(tester, &[("__DEBUG__", "true")]), Default::default(),),
		globals_simple,
		r#"if (__DEBUG__) {}"#
	);
	...
	test!(
		::swc_ecma_parser::Syntax::default(),
		|tester| inline_globals(envs(tester, &[]), globals(tester, &[("debug", "true")]), Default::default(),),
		non_global,
		r#"if (foo.debug) {}"#
	);
	...
	test!(
		Default::default(),
		|tester| inline_globals(envs(tester, &[("x", "FOO")]), globals(tester, &[]), Default::default(),),
		issue_417_2,
		"const test = process.env['x']"
	);
	...
	test!(
		Default::default(),
		|tester| inline_globals(envs(tester, &[("x", "BAR")]), globals(tester, &[]), Default::default(),),
		issue_2499_1,
		"process.env.x = 'foo'"
	);
}
