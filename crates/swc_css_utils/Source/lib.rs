...
impl VisitMut for PseudoElementOnPseudoClassReplacer<'_> {
	fn visit_mut_subclass_selector(&mut self, n: &mut SubclassSelector) {
		...
		match n {
			SubclassSelector::PseudoElement(PseudoElementSelector { name, span, .. }) if &*name.value == self.from => {
...
pub fn replace_pseudo_class_selector_on_pseudo_element_selector<N>(node: &mut N, from: &str, to: &str)
where
...
fn is_escape_not_required(value: &str) -> bool {
	...
	value.chars().all(|c| {
		match c {
			...
			'\x01'..='\x1f' | '\x7F' => false,
			'-' | '_' => true,
			_ if !c.is_ascii() || c.is_ascii_digit() || c.is_ascii_uppercase() || c.is_ascii_lowercase() => true,
...
fn hex_escape(ascii_byte: u8, _minify: bool) -> String {
	...
	if ascii_byte > 0x0f {
		...
		unsafe { str::from_utf8_unchecked(&[b'\\', HEX_DIGITS[high], HEX_DIGITS[low], b' ']) }.to_string()
	} else {
		unsafe { str::from_utf8_unchecked(&[b'\\', HEX_DIGITS[ascii_byte as usize], b' ']) }.to_string()
	}
}
