	...
	pub fn msg(&self) -> Cow<'static, str> {
		match self {
			...
			SyntaxError::InvalidIdentInStrict(identifier_name) => {
				format!("`{}` cannot be used as an identifier in strict mode", identifier_name).into()
			},
			SyntaxError::InvalidIdentInAsync => "`await` cannot be used as an identifier in an async context".into(),
			SyntaxError::EvalAndArgumentsInStrict => "'eval' and 'arguments' cannot be used as a \
                                                      binding identifier in strict mode"
				.into(),
			SyntaxError::ArgumentsInClassField => "'arguments' is only allowed in functions and class methods".into(),
			...
			SyntaxError::LineBreakBeforeArrow => "Unexpected line break between arrow head and arrow".into(),
			SyntaxError::Unexpected { ref got, ref expected } => {
			...
			SyntaxError::Expected(token, ref got) => format!("Expected '{:?}', got '{}'", token, got).into(),
			...
			SyntaxError::ReservedWordInObjShorthandOrPat => "Cannot use a reserved word as a shorthand property".into(),

			SyntaxError::MultipleDefault { .. } => "A switch block cannot have multiple defaults".into(),
			SyntaxError::CommaAfterRestElement => "Trailing comma isn't permitted after a rest element".into(),
			SyntaxError::NonLastRestParam => "Rest element must be final element".into(),
			SyntaxError::SpreadInParenExpr => "Parenthesized expression cannot contain spread operator".into(),
			...
			SyntaxError::DuplicateLabel(ref label) => format!("Label {} is already declared", label).into(),
			SyntaxError::AsyncGenerator => "An async function cannot be generator".into(),
			SyntaxError::NonTopLevelImportExport => "'import', and 'export' are not permitted here".into(),
			SyntaxError::ImportExportInScript => "'import', and 'export' cannot be used outside of module code".into(),
			...
			SyntaxError::LabelledGeneratorOrAsync => "Generator or async function cannot be labelled".into(),
			SyntaxError::LabelledFunctionInStrict => "Function cannot be labelled in strict mode".into(),
			SyntaxError::YieldParamInGen => "'yield' cannot be used as a parameter within generator".into(),
			SyntaxError::AwaitParamInAsync => "`await` expressions cannot be used in a parameter initializer.".into(),
			...
			SyntaxError::EmptyJSXAttr => "JSX attributes must only be assigned a non-empty expression".into(),
			SyntaxError::InvalidJSXValue => "JSX value should be either an expression or a quoted JSX text".into(),
			SyntaxError::JSXExpectedClosingTagForLtGt => "Expected corresponding JSX closing tag for <>".into(),
			SyntaxError::JSXExpectedClosingTag { ref tag } => {
				format!("Expected corresponding JSX closing tag for <{}>", tag).into()
			},
			SyntaxError::InvalidLeadingDecorator => "Leading decorators must be attached to a class declaration".into(),
			SyntaxError::DecoratorOnExport => "Using the export keyword between a decorator and a \
                                               ...
                                               class` instead."
				.into(),
			SyntaxError::TsRequiredAfterOptional => "A required element cannot follow an optional element.".into(),
			...
			SyntaxError::TaggedTplInOptChain => "Tagged template literal is not allowed in optional chain.".into(),
			...
			SyntaxError::PrivateConstructor => "Classes can't have a private field named '#constructor'.".into(),
			...
			SyntaxError::ExportDefaultWithOutFrom => "export default statements required from '...';".into(),

			SyntaxError::ExportExpectFrom(s) => format!("`{}` cannot be used without `from` clause", s).into(),
			...
			SyntaxError::TS2703 => "The operand of a delete operator must be a property reference.".into(),
			...
			SyntaxError::UsingDeclNotAllowedForForInLoop => "Using declaration is not allowed in for-in loop".into(),
			...
			SyntaxError::GetterSetterCannotBeOptional => "A getter or a setter cannot be optional".into(),
			...
			SyntaxError::ConstDeclarationsRequireInitialization => "'const' declarations must be initialized".into(),
			...
			SyntaxError::TS1038 => "`declare` modifier not allowed for code already in an ambient context".into(),
			...
			SyntaxError::TS1089(word) => {
				format!("'{}' modifier cannot appear on a constructor declaration", word).into()
			},
			...
			SyntaxError::TS1105 => "A 'break' statement can only be used within an enclosing \
				...
				.into(),
			SyntaxError::TS1106 => "The left-hand side of a `for...of` statement may not be `async`".into(),
			...
			SyntaxError::TS1115 => "A 'continue' statement can only jump to a label of an \
				...
				.into(),
			SyntaxError::TS1116 => "A 'break' statement can only jump to a label of an enclosing statement".into(),
			...
			SyntaxError::TS1242 => "`abstract` modifier can only appear on a class or method declaration".into(),
			SyntaxError::TS1244 => "Abstract methods can only appear within an abstract class.".into(),
			SyntaxError::TS1243(left, right) => {
				format!("'{}' modifier cannot be used with '{}' modifier.", left, right).into()
			},
			...
			SyntaxError::TS1273(word) => format!("'{}' modifier cannot appear on a type parameter", word).into(),
			...
			SyntaxError::TS2207 => "The 'type' modifier cannot be used on a named export when \
				...
				.into(),
			SyntaxError::TS2369 => "A parameter property is only allowed in a constructor implementation".into(),
			...
			SyntaxError::TS2483 => "The left-hand side of a 'for...of' statement cannot use a type annotation".into(),
			...
			SyntaxError::TS8038 => "Decorators may not appear after `export` or `export default` \
				...
				.into(),
			SyntaxError::TS18010 => "An accessibility modifier cannot be used with a private identifier.".into(),
			SyntaxError::TSTypeAnnotationAfterAssign => "Type annotations must come before default assignments".into(),
			SyntaxError::TsNonNullAssertionNotAllowed(word) => {
				format!("Typescript non-null assertion operator is not allowed with '{}'", word).into()
			},
			SyntaxError::SetterParamRequired => "Setter should have exactly one parameter".into(),
			SyntaxError::UnexpectedTokenWithSuggestions { candidate_list: token_list } => {
	...
	pub fn into_diagnostic(self, handler: &Handler) -> DiagnosticBuilder {
		...
		match kind {
			SyntaxError::ExpectedSemiForExprStmt { expr } => {
				db.span_label(expr, "This is the expression part of an expression statement");
			},
