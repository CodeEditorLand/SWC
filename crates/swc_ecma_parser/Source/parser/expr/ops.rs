...
impl<I: Tokens> Parser<I> {
	...
	pub(in crate::parser) fn parse_bin_op_recursively(
		...
		loop {
			...
			match &*next_left {
				Expr::Bin(BinExpr { span, left, op: op!("&&"), .. })
				| Expr::Bin(BinExpr { span, left, op: op!("||"), .. }) => {
	...
	fn parse_bin_op_recursively_inner(&mut self, left: Box<Expr>, min_prec: u8) -> PResult<(Box<Expr>, Option<u8>)> {
			...
			&& is!(self, "as")
		{
			...
			let node = if peeked_is!(self, "const") {
				...
				bump!(self); // const
				TsConstAssertion { span: span!(self, start), expr }.into()
			} else {
				let type_ann = self.next_then_parse_ts_type()?;
				TsAsExpr { span: span!(self, start), expr, type_ann }.into()
			};

			return self.parse_bin_op_recursively_inner(node, min_prec);
		}
		if self.input.syntax().typescript() && !self.input.had_line_break_before_cur() && is!(self, "satisfies") {
			let start = left.span_lo();
			let expr = left;
			let node = {
				let type_ann = self.next_then_parse_ts_type()?;
				TsSatisfiesExpr { span: span!(self, start), expr, type_ann }.into()
			};
		...
		bump!(self);
		if cfg!(feature = "debug") {
			trace!("parsing binary op {:?} min_prec={}, prec={}", op, min_prec, op.precedence());
		}
		match *left {
		...
		let node = BinExpr { span: Span::new(left.span_lo(), right.span_hi()), op, left, right }.into();
	...
	pub(in crate::parser) fn parse_unary_expr(&mut self) -> PResult<Box<Expr>> {
		...
		if !self.input.syntax().jsx() && self.input.syntax().typescript() && eat!(self, '<') {
			if eat!(self, "const") {
				...
				return Ok(TsConstAssertion { span: span!(self, start), expr }.into());
			}

			return self.parse_ts_type_assertion(start).map(Expr::from).map(Box::new);
		}

		// Parse update expression
		if is!(self, "++") || is!(self, "--") {
			let op = if bump!(self) == tok!("++") { op!("++") } else { op!("--") };

			let arg = self.parse_unary_expr()?;
			let span = Span::new(start, arg.span_hi());
			self.check_assign_target(&arg, false);

			return Ok(UpdateExpr { span, prefix: true, op, arg }.into());
		}
		...
		if is_one_of!(self, "delete", "void", "typeof", '+', '-', '~', '!') {
			...
			let arg = match self.parse_unary_expr() {
				...
				Err(err) => {
					self.emit_error(err);
					Invalid { span: Span::new(arg_start, arg_start) }.into()
				},
			};
			...
			if self.input.syntax().typescript() && op == op!("delete") {
				match arg.unwrap_parens() {
					Expr::Member(..) => {},
					Expr::OptChain(OptChainExpr { base, .. }) if matches!(&**base, OptChainBase::Member(..)) => {},
			...
			return Ok(UnaryExpr { span: Span::new(start, arg.span_hi()), op, arg }.into());
		}
		...
		if is_one_of!(self, "++", "--") {
			...
			return Ok(UpdateExpr { span: span!(self, expr.span_lo()), prefix: false, op, arg: expr }.into());
		}
		Ok(expr)
	}

	pub(crate) fn parse_await_expr(&mut self, start_of_await_token: Option<BytePos>) -> PResult<Box<Expr>> {
		...
		let arg = self.parse_unary_expr()?;
		Ok(AwaitExpr { span: span!(self, start), arg }.into())
	}
}
