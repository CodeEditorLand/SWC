...
fn array_lit() {
	assert_eq_ignore_span!(
		...
		Box::new(Expr::Array(ArrayLit {
			span,
			elems: vec![
				Some(ExprOrSpread { spread: None, expr: Box::new(Expr::Ident(Ident::new_no_ctxt("a".into(), span))) }),
				None,
				None,
				None,
				None,
				...
				None,
				Some(ExprOrSpread { spread: None, expr: Box::new(Expr::Ident(Ident::new_no_ctxt("e".into(), span))) }),
			]
		}))
	);
}
...
fn issue_328() {
	assert_eq_ignore_span!(
		test_parser("import('test')", Syntax::Es(Default::default()), |p| { p.parse_stmt() }),
		Stmt::Expr(ExprStmt {
			span,
		...
		Box::new(Expr::Assign(AssignExpr {
			...
			left: SuperPropExpr {
				...
				prop: SuperProp::Computed(ComputedPropName {
					span,
					expr: Box::new(Expr::Ident(Ident { span, sym: "a".into(), ..Default::default() })),
				})
			}
			.into(),
			right: Box::new(Expr::Lit(Lit::Num(Number { span, value: 123f64, raw: Some("123".into()) })))
		}))
	);
}
