...
impl<I: Tokens> Parser<I> {
	fn parse_import(&mut self) -> PResult<ModuleItem> {
		...
		if !self.ctx().module {
			...
			let ctx = Context { module: true, strict: true, ..self.ctx() };
			self.set_ctx(ctx);
		}
		...
		if let Ok(&Token::Str { .. }) = cur!(self, false) {
			let src = match bump!(self) {
				Token::Str { value, raw, .. } => Box::new(Str { span: span!(self, str_start), value, raw: Some(raw) }),
				_ => unreachable!(),
			};
		...
		'import_maybe_ident: {
			if is!(self, BindingIdent) {
				...
				specifiers.push(ImportSpecifier::Default(ImportDefaultSpecifier { span: local.span, local }));
			}
		}
		...
		let src = {
			...
			match *cur!(self, true) {
				Token::Str { .. } => match bump!(self) {
					Token::Str { value, raw, .. } => {
						Box::new(Str { span: span!(self, str_start), value, raw: Some(raw) })
					},
					_ => unreachable!(),
				},
		...
		Ok(ImportDecl { span: span!(self, start), specifiers, src, type_only, with, phase }.into())
	}
		...
		match self.parse_module_export_name()? {
					...
					if possibly_orig_name.sym == "as" {
						...
						if !is!(self, IdentName) {
							if self.ctx().is_reserved_word(&possibly_orig_name.sym) {
								syntax_error!(self, possibly_orig_name.span, SyntaxError::ReservedWordInImport)
							}
			...
			ModuleExportName::Str(orig_str) => {
				if eat!(self, "as") {
					...
					syntax_error!(self, orig_str.span, SyntaxError::ImportBindingIsString(orig_str.value))
				}
			},
		}
	}
	...
	fn parse_imported_binding(&mut self) -> PResult<Ident> {
		let ctx = Context { in_async: false, in_generator: false, ..self.ctx() };
	...
	fn parse_export(&mut self, mut decorators: Vec<Decorator>) -> PResult<ModuleDecl> {
		...
		if declare {
			...
			if let Some(decl) = self.try_parse_ts_declare(after_export_start, decorators.clone())? {
				return Ok(ExportDecl { span: span!(self, start), decl }.into());
			}
		}
		...
		if self.input.syntax().typescript() {
			if eat!(self, "import") {
				...
				// export import A = B
				return self
					.parse_ts_import_equals_decl(start, id.into(), /* is_export */ true, is_type_only)
					.map(From::from);
			}
			...
			if eat!(self, '=') {
				...
				let expr = self.parse_expr()?;
				expect!(self, ';');
				return Ok(TsExportAssignment { span: span!(self, start), expr }.into());
			}
			...
			if eat!(self, "as") {
				...
				expect!(self, ';');
				return Ok(TsNamespaceExportDecl { span: span!(self, start), id }.into());
			}
		}
		...
		if !type_only && eat!(self, "default") {
			...
			if self.input.syntax().typescript() {
				...
				if is!(self, "interface") {
					...
					let decl = self.parse_ts_interface_decl(interface_start).map(DefaultDecl::from)?;
					return Ok(ExportDefaultDecl { span: span!(self, start), decl }.into());
				}
			}
				...
				&& !self.input.has_linebreak_between_cur_and_peeked()
			{
				...
				&& (is!(self, "from") || (is!(self, ',') && (peeked_is!(self, '{') || peeked_is!(self, '*'))))
			{
				export_default = Some(Ident::new_no_ctxt("default".into(), self.input.prev_span()))
			} else {
				let expr = self.include_in_expr(true).parse_assignment_expr()?;
				expect!(self, ';');
				return Ok(ExportDefaultExpr { span: span!(self, start), expr }.into());
			}
		}
			...
			&& !self.input.has_linebreak_between_cur_and_peeked()
		{
		...
		} else if !type_only && self.input.syntax().typescript() && is!(self, "const") && peeked_is!(self, "enum") {
			...
			return self
				...
				.map(Decl::from)
				.map(|decl| ExportDecl { span: span!(self, start), decl }.into());
		} else if !type_only
			&& (is!(self, "var")
				|| is!(self, "const")
				|| (is!(self, "let"))
						...
						.unwrap_or(false))
		{
			...
			if default.is_none() && is!(self, '*') && !peeked_is!(self, "as") {
				...
				return Ok(ExportAll { span: span!(self, start), src, type_only, with }.into());
			}
			...
			if let Some(default) = default {
				...
				specifiers.push(ExportSpecifier::Default(ExportDefaultSpecifier { exported: default }))
			}
			...
			if has_default || has_ns {
				if is!(self, "from") {
					let (src, with) = self.parse_from_clause_and_semi()?;
					return Ok(
						NamedExport { span: span!(self, start), specifiers, src: Some(src), type_only, with }.into(),
					);
				} else if !self.input.syntax().export_default_from() {
					// emit error
					expect!(self, "from");
			...
			return Ok(NamedExport { span: span!(self, start), specifiers, src, type_only, with }.into());
		};

		Ok(ExportDecl { span: span!(self, start), decl }.into())
	}
	...
	fn parse_named_export_specifier(&mut self, type_only: bool) -> PResult<ExportNamedSpecifier> {
		...
		let exported = if eat!(self, "as") { Some(self.parse_module_export_name()?) } else { None };

		Ok(ExportNamedSpecifier { span: span!(self, start), orig, exported, is_type_only })
	}
...
export default class Foo {
  bar() {
    class Baz {}
			...
			Syntax::Es(EsSyntax { decorators: true, decorators_before_export: true, ..Default::default() }),
			|p| p.parse_module(),
		);
	}
}
