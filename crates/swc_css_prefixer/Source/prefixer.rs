...
static PREFIXES_AND_BROWSERS: Lazy<FxHashMap<String, [BrowserData<Option<Version>>; 2]>> = Lazy::new(|| {
	let map: FxHashMap<String, [BrowserData<Option<Version>>; 2]> =
		serde_json::from_str(include_str!("../data/prefixes_and_browsers.json")).expect("failed to parse json");
...
fn should_enable(target: &Versions, low_versions: &Versions, high_versions: &Versions, default: bool) -> bool {
	let mut iter = target.iter().zip(low_versions.iter().zip(high_versions.iter()));

	if iter.all(|((_, target_version), ((_, l), (_, h)))| target_version.is_none() && l.is_none() && h.is_none()) {
		return default;
	}
	...
	iter.any(
		|((target_name, maybe_target_version), ((_, maybe_low_version), (_, maybe_high_version)))| {
		...
		if n.name == *self.from {
			...
			let mut new_value: Vec<ComponentValue> = n
				...
				.filter(|n| {
					!matches!(
						n,
						ComponentValue::Percentage(_) | ComponentValue::Number(_) | ComponentValue::Integer(_)
					)
				})
				.cloned()
				.collect();
			...
			new_value.extend(vec![
				ComponentValue::Delimiter(Box::new(Delimiter { span: DUMMY_SP, value: DelimiterValue::Comma })),
				ComponentValue::Number(Box::new(Number { span: DUMMY_SP, value: transparency_value, raw: None })),
			]);
	...
	fn visit_mut_component_value(&mut self, n: &mut ComponentValue) {
		...
		if let ComponentValue::Str(node) = n {
			*n = ComponentValue::Url(Box::new(Url {
				span: node.span,
				name: Ident { span: DUMMY_SP, value: "url".into(), raw: None },
...
{
	node.visit_mut_with(&mut ImageSetFunctionReplacerOnLegacyVariant { from, to, in_function: false });
}
...
impl VisitMut for LinearGradientFunctionReplacerOnLegacyVariant<'_> {
	fn visit_mut_function(&mut self, n: &mut Function) {
		...
		if n.name == *self.from {
			...
			match first {
				Some(ComponentValue::Ident(ident)) if ident.value.as_ref().eq_ignore_ascii_case("to") => {
					...
					match (n.value.get(1), n.value.get(2)) {
						(Some(ComponentValue::Ident(first)), Some(ComponentValue::Ident(second))) => {
							let Ident { value: first_value, span: first_span, .. } = &**first;
							let Ident { value: second_value, span: second_span, .. } = &**second;
							if let (Some(new_first_direction), Some(new_second_direction)) =
								(get_old_direction(first_value), get_old_direction(second_value))
							{
								let new_value = vec![
				...
				Some(ComponentValue::Dimension(dimension)) => {
					if let Dimension::Angle(Angle { value, unit, span, .. }) = &**dimension {
						...
						if angle == 0.0 {
							...
							n.value[0] =
								ComponentValue::Ident(Box::new(Ident { span: *span, value: "left".into(), raw: None }));
						} else if angle == 180.0 {
							n.value[0] =
								ComponentValue::Ident(Box::new(Ident { span: *span, value: "top".into(), raw: None }));
						} else if angle == 270.0 {
							n.value[0] = ComponentValue::Ident(Box::new(Ident {
								span: *span,
								value: "right".into(),
							...
							n.value[0] = ComponentValue::Dimension(Box::new(Dimension::Angle(Angle {
								span: *span,
								value: Number { span: value.span, value: new_value, raw: None },
								unit: Ident { span: unit.span, value: "deg".into(), raw: None },
							})));
						}
					}
				},
			...
			if matches!(self.from, "radial-gradient" | "repeating-radial-gradient") {
				let at_index = n.value.iter().position(
					|n| matches!(n, ComponentValue::Ident(ident) if ident.value.as_ref().eq_ignore_ascii_case("at")),
				);
...
impl VisitMut for MediaFeatureResolutionReplacerOnLegacyVariant<'_> {
	fn visit_mut_media_feature_plain(&mut self, n: &mut MediaFeaturePlain) {
		...
		{
			let MediaFeatureName::Ident(Ident { value: feature_name_value, span: feature_name_span, .. }) = &n.name
			else {
				return;
			};

			if feature_name_value == self.from {
				n.name = MediaFeatureName::Ident(Ident { span: *feature_name_span, value: self.to.into(), raw: None });
...
impl VisitMut for CalcReplacer<'_> {
	fn visit_mut_function(&mut self, n: &mut Function) {
		...
		if (is_webkit_calc && self.to == Some("-moz-calc")) || (is_moz_calc && self.to == Some("-webkit-calc")) {
			return;
		}
	...
	fn visit_mut_calc_value(&mut self, n: &mut CalcValue) {
		...
		if let CalcValue::Function(function) = n {
			if matches_eq!(function.name, "calc", "-webkit-calc", "-moz-calc") {
				...
				*n = CalcValue::Sum(CalcSum { span: function.span, expressions: calc_sum.expressions });
			}
		}
	}
}
...
{
	node.visit_mut_with(&mut CalcReplacer { inside_calc: false, to });
}
	...
	fn visit_mut_function(&mut self, n: &mut Function) {
		...
		if let Some(ComponentValue::Ident(ident)) = n.value.first() {
			...
			let new_value = match &*new_value {
				"woff" | "truetype" | "opentype" | "woff2" | "embedded-opentype" | "collection" | "svg" => new_value,
				_ => {
					return;
				},
			};

			let new_value = Str { value: new_value, span: ident.span, raw: None };
...
impl VisitMut for ClampReplacer {
	fn visit_mut_function(&mut self, n: &mut Function) {
		...
		let min_function =
			if let (Some(middle @ ComponentValue::CalcSum(_)), Some(comma), Some(right @ ComponentValue::CalcSum(_))) =
				(n.value.get(2), n.value.get(3), n.value.get(4))
			{
				Function {
					span: n.span,
					name: FunctionName::Ident(Ident { span: n.span, value: "min".into(), raw: None }),
		...
		if let (Some(left), Some(comma)) = (n.value.first(), n.value.get(1)) {
			*n = Function {
				span: n.span,
				name: FunctionName::Ident(Ident { span: n.span, value: "max".into(), raw: None }),
				value: vec![
					left.clone(),
					comma.clone(),
					ComponentValue::CalcSum(Box::new(CalcSum {
						...
						expressions: vec![CalcProductOrOperator::Product(CalcProduct {
							span: DUMMY_SP,
							expressions: vec![CalcValueOrOperator::Value(CalcValue::Function(min_function))],
						})],
					})),
				],
			};
...
macro_rules! to_ident {
	($val:expr) => {{ ComponentValue::Ident(Box::new(Ident { span: DUMMY_SP, value: $val.into(), raw: None })) }};
}

macro_rules! to_integer {
	($val:expr) => {{ ComponentValue::Integer(Box::new(Integer { span: DUMMY_SP, value: $val, raw: None })) }};
}

macro_rules! to_number {
	($val:expr) => {{ ComponentValue::Number(Box::new(Number { span: DUMMY_SP, value: $val, raw: None })) }};
}
...
impl Prefixer {
	fn add_at_rule(&mut self, prefix: Prefix, at_rule: &AtRule) {
		if self.simple_block.is_none() {
			self.added_top_rules.push((prefix, Rule::AtRule(Box::new(at_rule.clone()))));
		} else {
			self.added_at_rules.push((prefix, Box::new(at_rule.clone())));
		}
	}
	...
	fn add_declaration2<'a>(
		...
		if should_prefix(property, &self.env, true) && !self.is_duplicate(property) {
			let name = DeclarationName::Ident(Ident { span: DUMMY_SP, value: property.into(), raw: None });

			if let Some(value) = value {
				let mut declaration =
					Declaration { span: n.span, name, value: value(), important: n.important.clone() };
				...
				let mut declaration =
					Declaration { span: n.span, name, value: n.value.clone(), important: n.important.clone() };
	...
	fn visit_mut_stylesheet(&mut self, stylesheet: &mut Stylesheet) {
		...
		for mut rule in take(&mut stylesheet.rules) {
			...
			for mut added_rule in take(&mut self.added_top_rules) {
				let need_skip = new_rules.iter().any(|existing_rule| added_rule.1.eq_ignore_span(existing_rule));
			...
			AtRuleName::Ident(Ident { span, value, .. }) if value == "viewport" => {
				if should_prefix("@-o-viewport", &self.env, false) {
					self.add_at_rule(
						...
						&AtRule {
							span: at_rule.span,
							name: AtRuleName::Ident(Ident { span: *span, value: "-ms-viewport".into(), raw: None }),
				...
				if should_prefix("@-ms-viewport", &self.env, false) {
					self.add_at_rule(
						...
						&AtRule {
							span: at_rule.span,
							name: AtRuleName::Ident(Ident { span: *span, value: "-o-viewport".into(), raw: None }),
							prelude: at_rule.prelude.clone(),
			...
			AtRuleName::Ident(Ident { span, value, .. }) if value == "keyframes" => {
				...
				if should_prefix("@-moz-keyframes", &self.env, false) {
					self.add_at_rule(
						...
						&AtRule {
							span: at_rule.span,
							name: AtRuleName::Ident(Ident { span: *span, value: "-moz-keyframes".into(), raw: None }),
							prelude: at_rule.prelude.clone(),
							block: original_simple_block.clone(),
						},
					);
				...
				if should_prefix("@-o-keyframes", &self.env, false) {
					self.add_at_rule(
						...
						&AtRule {
							span: at_rule.span,
							name: AtRuleName::Ident(Ident { span: DUMMY_SP, value: "-o-keyframes".into(), raw: None }),
							prelude: at_rule.prelude.clone(),
							block: original_simple_block,
						},
					);
		...
		if !self.added_declarations.is_empty() {
			if let Some(supports) = import_conditions.supports.take() {
				...
				if let Some(ComponentValue::Declaration(declaration)) = supports.value.first() {
					conditions.push(SupportsConditionType::SupportsInParens(SupportsInParens::Feature(
						SupportsFeature::Declaration(declaration.clone()),
					)));
				}
				...
				for n in take(&mut self.added_declarations) {
					let supports_condition_type = SupportsConditionType::Or(SupportsOr {
						span: DUMMY_SP,
						keyword: None,
						condition: Box::new(SupportsInParens::Feature(SupportsFeature::Declaration(n))),
					});
				...
				import_conditions.supports = Some(Box::new(Function {
					span: supports.span,
					name: supports.name,
					value: vec![ComponentValue::SupportsCondition(Box::new(SupportsCondition {
						span: DUMMY_SP,
						conditions,
					}))],
				}));
		...
		if let Some(supports_condition) = &self.supports_condition {
			match supports_in_parens {
				SupportsInParens::Feature(_) if !self.added_declarations.is_empty() => {
					...
					conditions.push(swc_css_ast::SupportsConditionType::SupportsInParens(supports_in_parens.clone()));
					...
					for n in take(&mut self.added_declarations) {
						...
						let need_skip = supports_condition.conditions.iter().any(|existing_condition_type| {
					...
					if conditions.len() > 1 {
						*supports_in_parens =
							SupportsInParens::SupportsCondition(SupportsCondition { span: DUMMY_SP, conditions });
					}
				},
				_ => {},
			}
	...
	fn visit_mut_media_query_list(&mut self, media_query_list: &mut MediaQueryList) {
		...
		for n in &media_query_list.queries {
			if should_prefix("-webkit-min-device-pixel-ratio", &self.env, false) {
				...
				let need_skip = media_query_list
					.queries
					.iter()
					.any(|existing_media_query| new_media_query.eq_ignore_span(existing_media_query));
	...
	fn visit_mut_qualified_rule(&mut self, n: &mut QualifiedRule) {
		...
		if self.rule_prefix == Some(Prefix::Webkit) || self.rule_prefix.is_none() {
			...
			if should_prefix(":-webkit-full-screen", &self.env, false) {
				replace_pseudo_class_selector_name(&mut new_webkit_prelude, "fullscreen", "-webkit-full-screen");
			}
			...
			if should_prefix("::-webkit-backdrop", &self.env, false) {
				replace_pseudo_element_selector_name(&mut new_webkit_prelude, "backdrop", "-webkit-backdrop");
			}
			...
			if !n.prelude.eq_ignore_span(&new_webkit_prelude) {
				...
				if self.simple_block.is_none() {
					self.added_top_rules.push((Prefix::Webkit, Rule::QualifiedRule(qualified_rule)));
				} else {
					self.added_qualified_rules.push((Prefix::Webkit, qualified_rule));
				}
			}
		}
		...
		if self.rule_prefix == Some(Prefix::Moz) || self.rule_prefix.is_none() {
			...
			if should_prefix(":-moz-placeholder", &self.env, false) {
				...
				if !new_moz_prelude_with_previous.eq_ignore_span(&new_moz_prelude) {
					...
					if self.simple_block.is_none() {
						self.added_top_rules.push((Prefix::Moz, Rule::QualifiedRule(qualified_rule)));
					} else {
						self.added_qualified_rules.push((Prefix::Moz, qualified_rule));
					}
				}
			}

			if should_prefix("::-moz-placeholder", &self.env, false) {
				replace_pseudo_element_selector_name(&mut new_moz_prelude, "placeholder", "-moz-placeholder");
			}

			if !n.prelude.eq_ignore_span(&new_moz_prelude) {
				let qualified_rule =
					QualifiedRule { span: DUMMY_SP, prelude: new_moz_prelude, block: original_simple_block.clone() };

				if self.simple_block.is_none() {
					self.added_top_rules
						.push((Prefix::Moz, Rule::QualifiedRule(Box::new(qualified_rule))));
				} else {
					self.added_qualified_rules.push((Prefix::Moz, Box::new(qualified_rule)));
				}
			}
		}
		...
		if self.rule_prefix == Some(Prefix::Ms) || self.rule_prefix.is_none() {
			...
			if should_prefix(":-ms-input-placeholder", &self.env, false) {
				replace_pseudo_class_selector_name(&mut new_ms_prelude, "placeholder-shown", "-ms-input-placeholder");
			}

			if should_prefix("::-ms-browse", &self.env, false) {
				replace_pseudo_element_selector_name(&mut new_ms_prelude, "file-selector-button", "-ms-browse");
			}
			...
			if should_prefix(":-ms-input-placeholder", &self.env, false) {
				...
				if !new_ms_prelude_with_previous.eq_ignore_span(&new_ms_prelude) {
					...
					if self.simple_block.is_none() {
						self.added_top_rules.push((Prefix::Ms, Rule::QualifiedRule(qualified_rule)));
					} else {
						self.added_qualified_rules.push((Prefix::Ms, qualified_rule));
					}
				}
			}

			if should_prefix("::-ms-input-placeholder", &self.env, false) {
				replace_pseudo_element_selector_name(&mut new_ms_prelude, "placeholder", "-ms-input-placeholder");
			}

			if !n.prelude.eq_ignore_span(&new_ms_prelude) {
				let qualified_rule =
					Box::new(QualifiedRule { span: DUMMY_SP, prelude: new_ms_prelude, block: original_simple_block });
		...
		for mut n in take(&mut simple_block.value) {
			...
			match n {
				ComponentValue::Declaration(_) => {
					new.extend(self.added_declarations.drain(..).map(ComponentValue::Declaration));
	...
	fn visit_mut_declaration(&mut self, n: &mut Declaration) {
		...
		if self.rule_prefix == Some(Prefix::Webkit) || self.rule_prefix.is_none() {
			...
			if should_prefix("-webkit-image-set()", &self.env, false) {
				replace_image_set_function_on_legacy_variant(&mut webkit_value, "image-set", "-webkit-image-set");
			}
			...
			if should_prefix("-webkit-cross-fade()", &self.env, false) {
				replace_cross_fade_function_on_legacy_variant(&mut webkit_value, "cross-fade", "-webkit-cross-fade");
			}
		...
		if self.rule_prefix == Some(Prefix::Moz) || self.rule_prefix.is_none() {
			...
			if should_prefix("-moz-linear-gradient()", &self.env, false) {
				replace_gradient_function_on_legacy_variant(&mut moz_value, "linear-gradient", "-moz-linear-gradient");
			}
			...
			if should_prefix("-moz-radial-gradient()", &self.env, false) {
				replace_gradient_function_on_legacy_variant(&mut moz_value, "radial-gradient", "-moz-radial-gradient");
			}
		...
		if self.rule_prefix == Some(Prefix::O) || self.rule_prefix.is_none() {
			if should_prefix("-o-repeating-linear-gradient()", &self.env, false) {
				replace_gradient_function_on_legacy_variant(&mut o_value, "linear-gradient", "-o-linear-gradient");
			}
			...
			if should_prefix("-o-radial-gradient()", &self.env, false) {
				replace_gradient_function_on_legacy_variant(&mut o_value, "radial-gradient", "-o-radial-gradient");
			}
		...
		match &**name {
			...
			"animation" => {
				let need_prefix = n.value.iter().all(|n| match n {
					ComponentValue::Ident(ident) => {
						!matches!(&*ident.value.to_ascii_lowercase(), "reverse" | "alternate-reverse")
					},
					_ => true,
				});
			...
			"display" => {
				if n.value.len() == 1 {
					if self.rule_prefix == Some(Prefix::Webkit) || self.rule_prefix.is_none() {
						...
						if should_prefix("-webkit-inline-box", &self.env, false) {
							replace_ident(&mut old_spec_webkit_value, "inline-flex", "-webkit-inline-box");
						}
				...
				} else if n.value.len() == 2 && should_prefix("display:multi-keyword-values", &self.env, false) {
				...
				} else if n.value.len() == 3 && should_prefix("display:multi-keyword-values", &self.env, false) {
					if let (
			...
			"flex" => {
				let spec_2009_value = match n.value.first() {
					...
					Some(ComponentValue::Ident(ident)) if ident.value.eq_ignore_ascii_case("auto") => {
						Some(ComponentValue::Integer(Box::new(Integer {
							span: ident.span,
							value: 1,
							raw: None,
				...
				if let Some(spec_2009_value) = &spec_2009_value {
					add_declaration!(Prefix::Moz, "-moz-box-flex", Box::new(|| { vec![spec_2009_value.clone()] }));
				} else {
				...
				if n.value.len() == 3 {
					add_declaration!(
						...
						Box::new(|| {
							...
							{
								value[2] = ComponentValue::Dimension(Box::new(Dimension::Length(Length {
									span,
									value: Number { span: DUMMY_SP, value: 0.0, raw: None },
									unit: Ident { span: DUMMY_SP, value: "px".into(), raw: None },
								})));
							}
			...
			"flex-direction" => {
				let old_values = match n.value.first() {
					Some(ComponentValue::Ident(ident)) if ident.value.eq_ignore_ascii_case("row") => {
						Some(("horizontal", "normal"))
					},
					Some(ComponentValue::Ident(ident)) if ident.value.eq_ignore_ascii_case("column") => {
						Some(("vertical", "normal"))
					},
					Some(ComponentValue::Ident(ident)) if ident.value.eq_ignore_ascii_case("row-reverse") => {
						Some(("horizontal", "reverse"))
					},
					Some(ComponentValue::Ident(ident)) if ident.value.eq_ignore_ascii_case("column-reverse") => {
				...
				if let Some((orient, direction)) = old_values {
					add_declaration!(Prefix::Webkit, "-webkit-box-orient", Box::new(|| { vec![to_ident!(orient)] }));
					add_declaration!(
						Prefix::Webkit,
				...
				if let Some((orient, direction)) = old_values {
					add_declaration!(Prefix::Moz, "-moz-box-orient", Box::new(|| { vec![to_ident!(orient)] }));
					add_declaration!(
						Prefix::Webkit,
						"-moz-box-direction",
						Box::new(|| { vec![to_ident!(direction)] })
			...
			"flex-flow" => {
				...
				if let Some((orient, direction)) = old_values {
					add_declaration!(Prefix::Moz, "-moz-box-orient", Box::new(|| { vec![to_ident!(orient)] }));
					add_declaration!(Prefix::Moz, "-moz-box-direction", Box::new(|| { vec![to_ident!(direction)] }));
				}
			...
			"image-rendering" => {
				if self.rule_prefix == Some(Prefix::Webkit) || self.rule_prefix.is_none() {
					...
					if should_prefix("-webkit-optimize-contrast", &self.env, false) {
						replace_ident(&mut webkit_value, "crisp-edges", "-webkit-optimize-contrast");
					}
				}
			...
			"text-decoration-skip-ink" => {
				if let ComponentValue::Ident(ident) = &n.value[0] {
					match &*ident.value.to_ascii_lowercase() {
						...
						_ => {
							add_declaration!(Prefix::Webkit, "-webkit-text-decoration-skip-ink", None);
						},
					}
				}
			},
			...
			"writing-mode" if n.value.len() == 1 => {
				if let Some(simple_block) = &self.simple_block {
					let direction = match simple_block.value.iter().rev().find(|declaration| {
						...
						Some(ComponentValue::Declaration(declaration)) => match declaration.value.first() {
							Some(ComponentValue::Ident(ident)) if ident.value.eq_ignore_ascii_case("rtl") => {
								Some("rtl")
							},
							_ => Some("ltr"),
						},
			...
			| "grid-auto-rows" => {
				...
				if self.rule_prefix == Some(Prefix::Webkit) || self.rule_prefix.is_none() {
					...
					if should_prefix("-webkit-fill-available", &self.env, false) {
						replace_ident(&mut webkit_value, "fill-available", "-webkit-fill-available");
						replace_ident(&mut webkit_value, "fill", "-webkit-fill-available");
						replace_ident(&mut webkit_value, "stretch", "-webkit-fill-available");
					}
				}

				if !is_grid_property && (self.rule_prefix == Some(Prefix::Moz) || self.rule_prefix.is_none()) {
			...
			"unicode-bidi" => {
				if self.rule_prefix == Some(Prefix::Webkit) || self.rule_prefix.is_none() {
					...
					if should_prefix("-webpack-isolate-override", &self.env, false) {
						replace_ident(&mut webkit_value, "isolate-override", "-webpack-isolate-override");
					}
			...
			"overflow" if should_prefix("overflow", &self.env, false) && n.value.len() == 2 => {
				if let (Some(left @ ComponentValue::Ident(first)), Some(right @ ComponentValue::Ident(second))) =
					(n.value.first(), n.value.get(1))
				{
			...
			"break-before" => {
				if let ComponentValue::Ident(ident) = &n.value[0] {
					match &*ident.value.to_ascii_lowercase() {
						...
						"page" => {
							add_declaration!("page-break-before", Some(Box::new(|| { vec![to_ident!("always")] })));
						},
						"column" => {
							add_declaration!(
								Prefix::Webkit,
			...
			"break-after" => {
				if let ComponentValue::Ident(ident) = &n.value[0] {
					match &*ident.value.to_ascii_lowercase() {
						...
						"page" => {
							add_declaration!("page-break-after", Some(Box::new(|| { vec![to_ident!("always")] })));
						},
						"column" => {
							add_declaration!(
								Prefix::Webkit,
			...
			"place-content" if should_prefix("place-content", &self.env, false) => {
				match (n.value.first(), n.value.get(1)) {
					(Some(left), Some(right)) => {
						add_declaration!("align-content", Some(Box::new(|| { vec![left.clone()] })));
						add_declaration!("justify-content", Some(Box::new(|| { vec![right.clone()] })));
					},
					(Some(left), None) => {
						add_declaration!("align-content", Some(Box::new(|| { vec![left.clone()] })));
						add_declaration!("justify-content", Some(Box::new(|| { vec![left.clone()] })));
					},
					_ => {},
				}
			},
			...
			"place-items" if should_prefix("place-items", &self.env, false) => {
				match (n.value.first(), n.value.get(1)) {
					(Some(left), Some(right)) => {
						...
						add_declaration!("justify-items", Some(Box::new(|| { vec![right.clone()] })));
					},
					(Some(left), None) => {
						add_declaration!("align-items", Some(Box::new(|| { vec![left.clone()] })));
						add_declaration!("justify-items", Some(Box::new(|| { vec![left.clone()] })));
					},
					_ => {},
				}
			},

			"place-self" if should_prefix("place-self", &self.env, false) => match (n.value.first(), n.value.get(1)) {
				(Some(left), Some(right)) => {
					add_declaration!("align-self", Some(Box::new(|| { vec![left.clone()] })));
					add_declaration!("justify-self", Some(Box::new(|| { vec![right.clone()] })));
				},
				(Some(left), None) => {
					add_declaration!("align-self", Some(Box::new(|| { vec![left.clone()] })));
