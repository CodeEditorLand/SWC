...
pub fn bench_files(c: &mut Criterion) {
	...
	let mut bench_file = |name: &str| {
		group.bench_function(format!("es/lints/libs/{}", name), |b| {
			let src = read_to_string(format!("../swc_ecma_minifier/benches/full/{}.js", name)).unwrap();

			let globals = Globals::default();
			GLOBALS.set(&globals, || {
				let cm = Lrc::new(SourceMap::default());
				let handler =
					Handler::with_tty_emitter(swc_common::errors::ColorConfig::Always, true, false, Some(cm.clone()));
				...
				let program = parse_file_as_module(&fm, Default::default(), Default::default(), None, &mut Vec::new())
					.map_err(|err| {
				...
				b.iter(|| {
					GLOBALS.set(&globals, || {
						HANDLER.set(&handler, || {
							run(cm.clone(), &mut program.clone(), unresolved_mark, top_level_mark)
						});
					});
				});
			});
