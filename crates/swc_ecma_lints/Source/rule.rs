...
fn join_lint_rules<N: LintNode<R>, R: Rule>(rules: &mut [R], program: &N) {
	...
	GLOBALS.with(|globals| {
		HANDLER.with(|handler| {
			join(
				|| GLOBALS.set(globals, || HANDLER.set(handler, || join_lint_rules(ra, program))),
				|| GLOBALS.set(globals, || HANDLER.set(handler, || join_lint_rules(rb, program))),
			)
		})
	});
}
...
fn lint_rules<N: LintNode<R>, R: Rule>(rules: &mut Vec<R>, program: &N) {
	...
	if cfg!(target_arch = "wasm32") {
		...
		{
			HANDLER.set(&Handler::with_emitter(true, false, Box::new(capturing.clone())), || {
