	...
	fn visit_mut_expr(&mut self, n: &mut Expr) {
		match n {
			Expr::This(ThisExpr { span }) if self.in_nested_scope => {
				*n = quote_ident!(
					SyntaxContext::empty()
						.apply_mark(*self.this_alias_mark.get_or_insert_with(|| Mark::fresh(Mark::root()))),
					*span,
					"_this"
				)
				.into();
			...
			// calls.
			Expr::Call(CallExpr { callee: Callee::Expr(expr), .. }) if expr.is_ident_ref_to("_define_property") => {
			...
			Expr::Update(UpdateExpr { arg, .. }) if arg.is_super_prop() => {
				if let Expr::SuperProp(SuperPropExpr { obj: Super { span: super_token, .. }, prop, .. }) = &**arg {
					*arg = self.super_to_update_call(*super_token, prop.clone()).into();
				}
			},

			Expr::Assign(AssignExpr { ref left, op: op!("="), right, .. }) if is_assign_to_super_prop(left) => {
			...
			Expr::Call(CallExpr { callee: Callee::Expr(callee_expr), args, .. }) if callee_expr.is_super_prop() => {
...
impl SuperFieldAccessFolder<'_> {
	...
	fn visit_mut_super_member_call(&mut self, n: &mut Expr) {
		if let Expr::Call(CallExpr { callee: Callee::Expr(callee_expr), args, .. }) = n {
			if let Expr::SuperProp(SuperPropExpr { obj: Super { span: super_token, .. }, prop, .. }) = &**callee_expr {
				let this = match self.this_alias_mark.or(self.constructor_this_mark) {
					Some(mark) => {
						let ident = quote_ident!(SyntaxContext::empty().apply_mark(mark), "_this").as_arg();
	...
	fn visit_mut_super_member_update(&mut self, n: &mut Expr) {
		if let Expr::Assign(AssignExpr { left, op, .. }) = n {
			...
			if let AssignTarget::Simple(expr) = left {
				if let SimpleAssignTarget::SuperProp(SuperPropExpr { obj: Super { span: super_token }, prop, .. }) =
					expr.take()
				{
			...
			MemberExpr {
				...
				obj: Box::new({
					let name = self
						.super_class
						.clone()
						.unwrap_or_else(|| quote_ident!(if self.is_static { "Function" } else { "Object" }).into());
	...
	fn super_to_set_call(&mut self, super_token: Span, prop: SuperProp, op: AssignOp, rhs: Box<Expr>) -> Expr {
		debug_assert_eq!(op, op!("="));

		let this_expr = Box::new(match self.constructor_this_mark {
			Some(mark) => quote_ident!(SyntaxContext::empty().apply_mark(mark), super_token, "_this").into(),
		...
		if self.constant_super {
			...
			AssignExpr { span: super_token, left: left.into(), op, right: rhs }.into()
		} else {
			let proto_arg = self.proto_arg();
	...
	fn proto_arg(&mut self) -> Box<Expr> {
		let expr = if self.is_static {
			...
			// Foo.prototype
			self.class_name.clone().make_member(quote_ident!("prototype")).into()
		};
...
fn prop_arg(prop: SuperProp) -> Expr {
	match prop {
		SuperProp::Ident(IdentName { sym: value, span, .. }) => Lit::Str(Str { span, raw: None, value }).into(),
