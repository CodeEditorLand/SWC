...
impl<'a> Parser<'a> {
	...
	pub(crate) fn parse_function_expression(&mut self, is_async: bool, is_generator: bool) -> Result<ast::Expr> {
	...
	fn try_parse_arrow_function_from_ident(&mut self, ident: ast::Ident, is_async: bool) -> Result<Option<ast::Expr>> {
		...
		let binding_ident = ast::BindingIdent { id: ident.clone(), type_ann: None };

		let params = vec![ast::Param { span: ident.span, decorators: Vec::new(), pat: ast::Pat::Ident(binding_ident) }];
