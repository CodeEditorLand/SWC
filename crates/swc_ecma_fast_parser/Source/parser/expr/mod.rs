...
impl<'a> Parser<'a> {
	...
	pub(crate) fn parse_expression(&mut self) -> Result<ast::Expr> {
		...
		if exprs.len() == 1 {
			...
			let span = exprs.first().unwrap().span().merge_with(exprs.last().unwrap().span());
	...
	pub(crate) fn parse_assignment_expression(&mut self) -> Result<ast::Expr> {
		...
		if self.is_token_type(TokenType::Async) && !self.cur_token.had_line_break {
			...
			// Check for async arrow function with single parameter
				...
				if self.is_token_type(TokenType::Arrow) {
					...
					let params = vec![ast::Pat::Ident(ast::BindingIdent { id: ident, type_ann: None })];
			...
			|| self.is_token_type(TokenType::OrAssign)
		{
			...
			let left = match expr.as_pat() {
				...
				Err(_) => {
					return Err(
						self.error(ErrorKind::General { message: "Invalid left-hand side in assignment".into() })
					);
				},
			};
			...
			return Ok(ast::Expr::Assign(ast::AssignExpr { span, op, left, right: Box::new(right) }));
		}
	...
	pub(crate) fn parse_yield_expression(&mut self) -> Result<ast::Expr> {
		...
		if !self.in_generator {
			return Err(
				self.error(ErrorKind::General { message: "'yield' is only allowed in generator functions".into() })
			);
		}
		...
		let span = start_span.merge_with(if let Some(ref arg) = arg { arg.span() } else { start_span });

		Ok(ast::Expr::Yield(ast::YieldExpr { span, arg, delegate }))
	}
	...
	pub(crate) fn parse_arrow_function_expression(
		...
		let start_span = params.first().map(|p| p.span()).unwrap_or_else(|| self.prev_token.span);
	...
	pub(crate) fn parse_jsx_expression(&mut self) -> Result<ast::Expr> {
		...
		if !self.syntax.jsx {
			return Err(self.error(ErrorKind::General { message: "JSX syntax is not enabled".into() }));
		}

		Err(self.error(ErrorKind::General { message: "JSX parsing is not fully implemented".into() }))
	}
	...
	pub(crate) fn parse_ts_as_expression(&mut self, expr: ast::Expr) -> Result<ast::Expr> {
		if !self.syntax.typescript {
			return Err(self.error(ErrorKind::General { message: "TypeScript syntax is not enabled".into() }));
		}
	...
	pub(crate) fn parse_ts_non_null_expression(&mut self, expr: ast::Expr) -> Result<ast::Expr> {
		...
		Ok(ast::Expr::TsNonNull(ast::TsNonNullExpr { span, expr: Box::new(expr) }))
	}
