...
impl<'a> Parser<'a> {
		...
		match self.cur_token.token_type {
			...
			TokenType::Async if !self.cur_token.had_line_break && self.is_async_function() => {
				...
				if self.is_token_type(TokenType::Function) {
					...
					Ok(ast::Expr::Arrow(self.parse_arrow_function_expression(true)?))
				}
			},
	...
	pub(crate) fn parse_literal(&mut self) -> Result<ast::Expr> {
		...
		let expr = match self.cur_token.token_type {
			...
			TokenType::Str => {
				...
				ast::Expr::Lit(ast::Lit::Str(ast::Str { span, value, raw: Some(raw) }))
			},
			...
			TokenType::Num => {
				...
				ast::Expr::Lit(ast::Lit::Num(ast::Number { span, value, raw: Some(raw) }))
			},
			...
			TokenType::BigInt => {
				...
				ast::Expr::Lit(ast::Lit::BigInt(ast::BigInt { span, value, raw: Some(raw) }))
			},
			...
			TokenType::RegExp => {
				...
				ast::Expr::Lit(ast::Lit::Regex(ast::Regex { span, exp: pattern, flags }))
			},
	...
	pub(crate) fn parse_parenthesized_expression(&mut self) -> Result<ast::Expr> {
		...
		if self.is_token_type(TokenType::RParen) {
			return Err(self.error(ErrorKind::General { message: "Empty parentheses are not allowed".into() }));
		}
	...
	pub(crate) fn parse_template_literal(&mut self, tag: Option<Box<ast::Expr>>) -> Result<ast::Expr> {
		let start_span = self.cur_token.span;
		...
		if !self.cur_token.template_has_substitutions() {
			...
			quasis.push(ast::TplElement { span: self.cur_token.span, tail: true, cooked: Some(cooked), raw });
			...
			while !self.is_token_type(TokenType::EOF) {
				...
				// Create the template element
				quasis.push(ast::TplElement { span: self.cur_token.span, tail: is_tail, cooked: Some(cooked), raw });
				...
				if !self.is_token_type(TokenType::Template) && !self.is_token_type(TokenType::TemplateMiddle) {
