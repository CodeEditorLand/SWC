...
impl<'a> Parser<'a> {
	...
	pub(crate) fn parse_var_declarator(&mut self, is_const: bool) -> Result<ast::VarDeclarator> {
		...
		let init = if self.is_token_type(TokenType::Assign) {
			...
			if is_const {
				return Err(
					self.error(ErrorKind::General { message: "Missing initializer in const declaration".into() })
				);
			}
		...
		Ok(ast::VarDeclarator {
			span: name_span.merge_with(if let Some(ref init) = init { init.span() } else { name_span }),
			name,
			init,
			definite: false,
		})
	...
	pub(crate) fn parse_function_declaration(&mut self, is_async: bool, is_generator: bool) -> Result<ast::FnDecl> {
		let start_span = self.cur_token.span;
...
impl<'a> Parser<'a> {
	/// Parse function parameters and body
	pub(crate) fn parse_function_params_and_body(&mut self) -> Result<(Vec<ast::Param>, ast::BlockStmt)> {
			...
			loop {
				...
				let param = if is_rest {
					ast::Param {
						span: pat.span(),
						decorators: Vec::new(),
						pat: ast::Pat::Rest(ast::RestPat { span: pat.span(), arg: Box::new(pat), type_ann: None }),
					}
				} else {
					ast::Param { span: pat.span(), decorators: Vec::new(), pat }
				};
		...
		while !self.is_token_type(TokenType::RBracket) {
			...
			if is_rest {
				if !self.is_token_type(TokenType::RBracket) {
					return Err(
						self.error(ErrorKind::General { message: "Rest element must be the last element".into() })
					);
				}
				break;
			}
		...
		while !self.is_token_type(TokenType::RBrace) {
			...
			if self.is_token_type(TokenType::Ellipsis) {
				...
				if !self.is_token_type(TokenType::RBrace) {
					return Err(
						self.error(ErrorKind::General { message: "Rest element must be the last property".into() })
					);
				}
				break;
			}
		...
		match self.cur_token.token_type {
			...
			TokenType::Str => {
				let str_lit = match &self.cur_token.value {
					TokenValue::String(s) => ast::Str { span: self.cur_token.span, value: s.clone().into(), raw: None },
			...
			TokenType::Num => {
				let num_lit = match &self.cur_token.value {
					TokenValue::Number(n) => ast::Number { span: self.cur_token.span, value: *n, raw: None },
	...
	pub(crate) fn parse_class_body(&mut self) -> Result<ast::ClassBody> {
		...
		while !self.is_token_type(TokenType::RBrace) && !self.is_token_type(TokenType::EOF) {
					...
					|| self.is_token_identifier_eq("protected"))
			{
				let modifier = match self.cur_token.value {
					...
					TokenValue::String(ref s) if s == "private" => Some(ast::Accessibility::Private),
					TokenValue::String(ref s) if s == "protected" => Some(ast::Accessibility::Protected),
					_ => None,
				};
		...
		Ok(ast::ClassBody { span: start_span.merge_with(end_span), body })
	}
	...
	pub(crate) fn parse_class_element(
		...
		let kind =
			if self.is_token_identifier_eq("get") && !self.peek_token().had_line_break && !is_async && !is_generator {
			...
			Ok(ast::ClassMember::ClassProp(ast::ClassProp {
				...
				key: match key {
					...
					ast::PropName::Computed(c) => {
						Box::new(ast::Expr::Paren(ast::ParenExpr { span: c.span, expr: c.expr }))
					},
