...
impl<'a> Parser<'a> {
		...
		while !self.is_token_type(TokenType::RBrace) && !self.is_token_type(TokenType::EOF) {
			...
			// Parse a default case
				...
				if default_case.is_some() {
					return Err(self
						.error(ErrorKind::General { message: "Multiple default clauses in switch statement".into() }));
				}
	...
	pub(crate) fn parse_for_statement(&mut self) -> Result<ast::Stmt> {
		...
		let await_token = if self.is_token_type(TokenType::Await) {
			if !self.in_async {
				return Err(self.error(ErrorKind::General {
					message: "'for await' is only allowed within async functions and modules".into(),
				}));
			}
		...
		let init = if self.is_token_type(TokenType::Semi) {
			...
			if self.is_token_type(TokenType::In) || (self.is_token_identifier_eq("of") && !await_token) {
		...
		if let Some(ast::VarDeclOrExpr::VarDecl(var_decl)) = &init {
			if var_decl.decls.len() == 1 && self.is_token_type(TokenType::In) {
				...
				// For-of loop
				return self.parse_for_in_of_statement_var(start_span, var_decl.clone(), await_token);
			}
		}
	...
	pub(crate) fn parse_try_statement(&mut self) -> Result<ast::TryStmt> {
		...
		let handler = if self.is_token_type(TokenType::Catch) {
			...
			Some(ast::CatchClause { span: catch_start.merge_with(body.span), param, body })
		} else {
			None
		};
		...
		if handler.is_none() && finalizer.is_none() {
			return Err(self.error(ErrorKind::General { message: "Missing catch or finally after try".into() }));
		}
		...
		Ok(ast::TryStmt { span: start_span.merge_with(end_span), block, handler, finalizer })
	}
	...
	pub(crate) fn parse_with_statement(&mut self) -> Result<ast::WithStmt> {
		...
		if self.strict_mode {
			return Err(
				self.error(ErrorKind::General { message: "'with' statements are not allowed in strict mode".into() })
			);
		}
	...
	pub(crate) fn parse_break_statement(&mut self) -> Result<ast::BreakStmt> {
		...
		if !self.in_iteration && !self.in_switch {
			return Err(self
				.error(ErrorKind::General { message: "Illegal break statement outside of a loop or switch".into() }));
		}
		...
		let label = if !self.can_insert_semicolon() && self.is_token_identifier() {
			...
			if !self.has_label(&label.sym) {
				return Err(self.error(ErrorKind::General { message: format!("Undefined label '{}'", label.sym) }));
			}
		...
		Ok(ast::BreakStmt { span: start_span.merge_with(self.prev_token.span), label })
	}
	...
	pub(crate) fn parse_continue_statement(&mut self) -> Result<ast::ContinueStmt> {
		...
		if !self.in_iteration {
			return Err(
				self.error(ErrorKind::General { message: "Illegal continue statement outside of a loop".into() })
			);
		}
		...
		Ok(ast::ContinueStmt { span: start_span.merge_with(self.prev_token.span), label })
	}
	...
	pub(crate) fn parse_return_statement(&mut self) -> Result<ast::ReturnStmt> {
		...
		if !self.in_function {
			return Err(
				self.error(ErrorKind::General { message: "Illegal return statement outside of a function".into() })
			);
		}
		...
		Ok(ast::ReturnStmt { span: start_span.merge_with(self.prev_token.span), arg })
	}
	...
	pub(crate) fn parse_throw_statement(&mut self) -> Result<ast::ThrowStmt> {
		...
		if self.cur_token.had_line_break {
			return Err(self.error(ErrorKind::General { message: "Illegal newline after throw".into() }));
		}
		...
		Ok(ast::ThrowStmt { span: start_span.merge_with(self.prev_token.span), arg: Box::new(arg) })
	}
}
...
impl<'a> Parser<'a> {
	...
	pub(crate) fn parse_for_in_of_statement(
		...
		let left = match left.as_pat() {
			...
			Err(_) => {
				return Err(
					self.error(ErrorKind::General { message: "Invalid left-hand side in for-in/for-of loop".into() })
				);
			},
		};
