...
use swc_common::{DUMMY_SP, Mark, Spanned, SyntaxContext, source_map::PURE_SP, util::take::Take};
use swc_ecma_ast::*;
use swc_ecma_transforms_base::{helper, helper_expr, perf::Check};
use swc_ecma_transforms_macros::fast_path;
use swc_ecma_utils::{
	ExprFactory, Remapper, StmtLike, find_pat_ids,
	function::{FnEnvHoister, FnWrapperResult, FunctionWrapper, init_this},
...
impl VisitMut for Actual {
	...
	fn visit_mut_class_method(&mut self, m: &mut ClassMethod) {
		...
		let expr = make_fn_ref(FnExpr { ident: None, function: m.function.take() });
			...
			ModuleItem::ModuleDecl(ModuleDecl::ExportDefaultDecl(export_default)) => {
				if let DefaultDecl::Fn(expr) = &mut export_default.decl {
					if expr.function.is_async {
						...
						*item = ExportDefaultDecl { span: export_default.span, decl: name_fn.into() }.into();
						self.extra_stmts.push(ref_fn.into());
					};
				} else {
	...
	fn visit_mut_method_prop(&mut self, prop: &mut MethodProp) {
		...
		let Function { span, params, body: Some(body), .. } = &mut *prop.function else {
			return;
		};
		...
		let expr = make_fn_ref(FnExpr { ident: None, function: prop.function.take() });
			...
			body: Some(BlockStmt {
				stmts: visitor
					...
					.chain(iter::once(
						ReturnStmt {
							span: DUMMY_SP,
							arg: Some(CallExpr { callee: expr.as_callee(), ..Default::default() }.into()),
						}
						.into(),
					))
					.collect(),
	...
	fn visit_mut_var_declarator(&mut self, var: &mut VarDeclarator) {
		if let VarDeclarator { name: Pat::Ident(id), init: Some(init), .. } = var {
			match init.as_ref() {
				Expr::Fn(FnExpr { ident: None, ref function }) if function.is_async || function.is_generator => {
...
#[swc_trace]
impl Actual {
	fn visit_mut_expr_with_binding(&mut self, expr: &mut Expr, binding_ident: Option<Ident>, in_iife: bool) {
...
fn make_fn_ref(mut expr: FnExpr) -> Expr {
	...
	expr.function
		.body
		.visit_mut_with(&mut AsyncFnBodyHandler { is_async_generator: expr.function.is_generator });
	...
	CallExpr { span, callee: helper, args: vec![expr.as_arg()], ..Default::default() }.into()
}
...
impl VisitMut for AsyncFnBodyHandler {
	...
	fn visit_mut_expr(&mut self, expr: &mut Expr) {
		...
		match expr {
			Expr::Yield(YieldExpr { span, arg: Some(arg), delegate: true }) => {
				...
				.into();
				*expr = YieldExpr { span: *span, delegate: true, arg: Some(arg) }.into()
			},
			...
			Expr::Await(AwaitExpr { span, arg }) => {
				if self.is_async_generator {
					...
					let arg =
						CallExpr { span: *span, callee, args: vec![arg.take().as_arg()], ..Default::default() }.into();
					*expr = YieldExpr { span: *span, delegate: false, arg: Some(arg) }.into()
				} else {
					*expr = YieldExpr { span: *span, delegate: false, arg: Some(arg.take()) }.into()
				}
			},
			_ => {},
		}
...
fn handle_await_for(stmt: &mut Stmt, is_async_generator: bool) {
	...
	let try_body = {
		...
		match s.left {
			ForHead::VarDecl(v) => {
				...
				let var_decl =
					VarDeclarator { span: DUMMY_SP, name: var.name, init: Some(value.into()), definite: false };
				for_loop_body.push(
					VarDecl {
						span: DUMMY_SP,
		...
		let for_loop_body = BlockStmt { span: body_span, stmts: for_loop_body, ..Default::default() };
		...
		init_var_decls.push(VarDeclarator {
			...
			init: {
				...
				Some(CallExpr { span: DUMMY_SP, callee, args: vec![s.right.as_arg()], ..Default::default() }.into())
			},
			definite: false,
		});
		init_var_decls.push(VarDeclarator { span: DUMMY_SP, name: step.clone().into(), init: None, definite: false });
		...
		let for_stmt = ForStmt {
			...
			test: {
				...
				let assign_to_step: Expr = AssignExpr {
					...
					op: op!("="),
					left: step.into(),
					right: YieldExpr { span: DUMMY_SP, arg: Some(yield_arg), delegate: false }.into(),
				}
				.into();
				...
				Some(AssignExpr { span: DUMMY_SP, op: op!("="), left, right }.into())
			},
		...
		BlockStmt { span: body_span, stmts: vec![for_stmt], ..Default::default() }
	};
	...
	let catch_clause = {
		...
		CatchClause {
			...
			body: BlockStmt { stmts: vec![mark_as_errorred, store_error], ..Default::default() },
		}
	};

	let finally_block = {
		let throw_iterator_error = ThrowStmt { span: DUMMY_SP, arg: iterator_error.clone().into() }.into();
		...
		let iterator_return: Expr = CallExpr {
			span: DUMMY_SP,
			callee: iterator.clone().make_member(quote_ident!("return")).as_callee(),
			args: Vec::new(),
			..Default::default()
		}
		.into();
		...
		let conditional_yield = IfStmt {
			...
			.into(),
			cons: Box::new(Stmt::Block(BlockStmt { stmts: vec![yield_stmt], ..Default::default() })),
			alt: None,
		}
		.into();
		let body = BlockStmt { stmts: vec![conditional_yield], ..Default::default() };

		let inner_try = TryStmt {
			span: DUMMY_SP,
			block: body,
			handler: None,
			finalizer: Some(BlockStmt { stmts: vec![throw_iterator_error], ..Default::default() }),
		}
		.into();
		BlockStmt { stmts: vec![inner_try], ..Default::default() }
	};
	...
	let stmts = vec![
		VarDecl {
			...
			decls: vec![
				...
				// var _iteratorError;
				VarDeclarator { span: DUMMY_SP, name: iterator_error.into(), init: None, definite: false },
			],
			..Default::default()
		}
		.into(),
	...
	*stmt = BlockStmt { span: s.span, stmts, ..Default::default() }.into()
}
