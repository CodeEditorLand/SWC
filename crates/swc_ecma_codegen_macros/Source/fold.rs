...
impl Fold for InjectSelf {
	fn fold_signature(&mut self, i: Signature) -> Signature {
		self.parser = i.inputs.first().cloned().and_then(|arg| match arg {
			FnArg::Receiver(Receiver { self_token, mutability: Some(..), .. })
	...
	fn fold_macro(&mut self, i: Macro) -> Macro {
		...
		match &*name {
			...
			| "debug_assert_eq" | "debug_assert_ne" | "dbg" => {
				...
				args = args.into_pairs().map(|el| el.map_item(|expr| self.fold_expr(expr))).collect();
				Macro { tokens: args.into_token_stream(), ..i }
			},
			...
			"keyword" | "emit" | "punct" | "semi" | "formatting_semi" | "space" | "formatting_space" | "operator"
			| "opt" | "opt_leading_space" | "srcmap" => {
				let tokens = if i.tokens.is_empty() {
					...
					quote_spanned!(span => #parser,).into_iter().chain(args).collect()
				};
