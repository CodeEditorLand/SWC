...
pub fn template_literal(c: Config) -> impl Pass {
	visit_mut_pass(TemplateLiteral { c, ..Default::default() })
}
...
impl VisitMut for TemplateLiteral {
	...
	fn visit_mut_expr(&mut self, e: &mut Expr) {
		...
		match e {
			Expr::Tpl(Tpl { span, exprs, quasis, .. }) => {
				assert_eq!(quasis.len(), exprs.len() + 1);

				// This makes result of addition string
				let mut obj: Box<Expr> = Box::new(
					Lit::Str({
						let s = quasis[0].cooked.clone().unwrap_or_else(|| quasis[0].raw.clone());

						Str { span: quasis[0].span, value: Atom::from(&*s), raw: None }
					})
					.into(),
				);
				...
				for i in 0..len {
					...
					let expr = if i % 2 == 0 {
						// Quasis
						match quasis.next() {
							Some(TplElement { span, cooked, raw, .. }) => {
								let s = cooked.clone().unwrap_or_else(|| raw.clone());

								Box::new(Lit::Str(Str { span: *span, value: (&*s).into(), raw: None }).into())
							},
							_ => unreachable!(),
						}
					} else {
					...
					if is_lit {
						...
						if !is_empty && args.is_empty() {
							if let Expr::Lit(Lit::Str(Str { span, value, raw })) = *obj {
								match *expr {
									Expr::Lit(Lit::Str(Str { span: r_span, value: r_value, .. })) => {
										obj = Lit::Str(Str {
							...
							obj = if self.c.ignore_to_primitive {
								...
								CallExpr {
									...
									callee: MemberExpr {
										span: DUMMY_SP,
										obj,
										prop: MemberProp::Ident(IdentName::new("concat".into(), expr_span)),
									}
									.as_callee(),
									args: mem::take(&mut args).into_iter().map(|expr| expr.as_arg()).collect(),
									..Default::default()
								}
								.into()
							}
			...
			Expr::TaggedTpl(TaggedTpl { tag, tpl, .. }) => {
				...
				self.added
					.push(FnDecl { declare: false, ident: fn_ident.clone(), function: f.into() }.into());
