...
use swc_ecma_utils::{ExprFactory, alias_if_required, member_expr, prepend_stmt, private_ident, quote_ident};
...
pub fn for_of(c: Config) -> impl Pass {
	visit_mut_pass(ForOf { c, top_level_vars: Default::default() })
}
...
#[swc_trace]
impl ForOf {
	fn fold_for_stmt(&mut self, label: Option<Ident>, ForOfStmt { span, left, right, body, .. }: ForOfStmt) -> Stmt {
		if right.is_array() || (self.c.assume_array && !self.c.loose) {
			...
			let update = Some(
				UpdateExpr { span: DUMMY_SP, prefix: false, op: op!("++"), arg: Box::new(i.clone().into()) }.into(),
			);

			let mut decls = Vec::with_capacity(2);

			decls.push(VarDeclarator { span: DUMMY_SP, name: i.clone().into(), init: Some(0.into()), definite: false });
			...
			let mut body = match *body {
				Stmt::Block(b) => b,
				_ => BlockStmt { span: DUMMY_SP, stmts: vec![*body], ..Default::default() },
			};
			...
			return match label {
				Some(label) => LabeledStmt { span, label, body: Box::new(stmt) }.into(),
				_ => stmt,
			};
		}
		...
		if self.c.loose {
			...
			let mut body = match *body {
				Stmt::Block(b) => b,
				_ => BlockStmt { stmts: vec![*body], ..Default::default() },
			};
			...
			let stmt = ForStmt {
				span,
				init: Some(VarDecl { kind: VarDeclKind::Var, decls, ..Default::default() }.into()),
				test: Some(test),
				update: None,
				body: Box::new(Stmt::Block(body)),
			}
		...
		let mut body = match *body {
			...
			body => BlockStmt { span: DUMMY_SP, stmts: vec![body], ..Default::default() },
		};
			...
			match left {
				ForHead::VarDecl(mut var) => {
					...
					VarDecl {
						span: var.span,
						kind: var.kind,
						decls: vec![VarDeclarator { init: Some(step_value.into()), ..var.decls.pop().unwrap() }],
						declare: false,
						..Default::default()
					}
					.into()
		...
		let normal_completion_ident = Ident::new("_iteratorNormalCompletion".into(), var_span, var_ctxt);
			...
			init: Some(
				VarDecl {
					...
					decls: vec![
						...
						VarDeclarator { span: DUMMY_SP, name: step.clone().into(), init: None, definite: false },
					],
					..Default::default()
				}
				.into(),
		...
		let for_stmt = match label {
			Some(label) => LabeledStmt { span, label, body: Box::new(for_stmt) }.into(),
			None => for_stmt,
		};

		TryStmt {
			span: DUMMY_SP,
			block: BlockStmt { span: DUMMY_SP, stmts: vec![for_stmt], ..Default::default() },
...
impl Parallel for ForOf {
	fn create(&self) -> Self {
		ForOf { c: self.c, top_level_vars: Default::default() }
	}
