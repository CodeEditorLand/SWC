	...
	fn visit_mut_prop(&mut self, prop: &mut Prop) {
		match prop {
			Prop::Shorthand(ident) => {
				//
				if !self.getter_props.insert(ident.sym.clone()) || !self.setter_props.insert(ident.sym.clone()) {
			...
			Prop::Getter(..) => prop.visit_mut_children_with(&mut PropNameFolder { props: &mut self.getter_props }),
			Prop::Setter(..) => prop.visit_mut_children_with(&mut PropNameFolder { props: &mut self.setter_props }),
			_ => {
				prop.visit_mut_children_with(&mut PropNameFolder { props: &mut self.getter_props });
				prop.visit_mut_children_with(&mut PropNameFolder { props: &mut self.setter_props })
			},
		}
	}
}
			...
			PropName::Ident(ident) => {
				if !self.props.insert(ident.sym.clone()) {
					*name = PropName::Computed(ComputedPropName {
						span,
						expr: Lit::Str(Str { span, raw: None, value: ident.sym.clone() }).into(),
					})
				}
			},
			PropName::Str(s) => {
				if !self.props.insert(s.value.clone()) {
					*name = PropName::Computed(ComputedPropName { span: s.span, expr: s.clone().into() })
				}
			},
