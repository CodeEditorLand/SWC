...
use swc_atoms::Atom;
use swc_common::{
	BytePos, DUMMY_SP, EqIgnoreSpan, Mark, Span, Spanned, SyntaxContext, comments::Comments, util::take::Take,
};
use swc_ecma_ast::*;
use swc_ecma_transforms_base::helper;
use swc_ecma_utils::{ExprFactory, function::FnEnvHoister, private_ident, prop_name_to_expr_value, quote_ident};
...
where
	C: Comments,
{
	visit_mut_pass(Wrapper { unresolved_ctxt: SyntaxContext::empty().apply_mark(unresolved_mark) })
}
...
impl VisitMut for Wrapper {
	...
	fn visit_mut_function(&mut self, f: &mut Function) {
		...
		if f.is_generator {
			...
			stmts.visit_mut_with(&mut InvalidToLit { map: v.label_exprs.as_deref() });
			let inner_fn = Box::new(Function {
				...
				decorators: Default::default(),
				body: Some(BlockStmt { stmts, ..Default::default() }),
				is_generator: false,
				is_async: false,
				..Default::default()
			});
			let generator_object = CallExpr {
				...
				args: vec![
					...
					FnExpr { ident: None, function: inner_fn }.as_arg(),
				],
				..Default::default()
			}
			.into();
			...
			stmts.push(ReturnStmt { span: DUMMY_SP, arg: Some(generator_object) }.into());
		...
		match e {
			...
			Expr::Cond(node) => {
				...
				if contains_yield(&node.cons) || contains_yield(&node.alt) {
					...
					self.emit_assignment(result_local.clone().into(), node.cons.take(), Some(cons_span));
								...
								Some(OpArgs::Stmt(Box::new(
									ExprStmt {
										...
										expr: if pending_expressions.len() == 1 {
											...
											SeqExpr { span: DUMMY_SP, exprs: pending_expressions.take() }.into()
										},
									}
									.into(),
								))),
			...
			Expr::Member(MemberExpr { obj, prop: MemberProp::Computed(prop), .. }) => {
					...
					Some(SimpleAssignTarget::Member(left)) => {
						match &mut left.prop {
							...
							MemberProp::Computed(prop) => {
								...
								left.obj = obj.into();
								left.prop =
									MemberProp::Computed(ComputedPropName { span: prop_span, expr: prop.into() });
							},
						}
						// [source]
					},
			...
			Expr::Object(node) if node.props.iter().any(contains_yield) => {
				...
				node.props.iter_mut().take(num_initial_properties).for_each(|p| {
				...
				self.emit_assignment(
					...
					ObjectLit {
						span: DUMMY_SP,
						props: node.props.iter_mut().take(num_initial_properties).map(|v| v.take()).collect(),
					}
					.into(),
					None,
				);
				...
				let mut expressions = node
					...
					.fold(Vec::<CompiledProp>::new(), |mut props, p| {
						match p {
							...
							PropOrSpread::Prop(p) => match *p {
								Prop::Getter(p) => {
									if let Some(CompiledProp::Accessor(g, _)) = props.iter_mut().find(|prev| match prev
									{
								...
								Prop::Setter(p) => {
									if let Some(CompiledProp::Accessor(_, s)) = props.iter_mut().find(|prev| match prev
									{
					...
					.into_iter()
					.fold(Vec::new(), |exprs, property| self.reduce_property(exprs, property, &mut temp));
		...
		if contains_yield(&node.args) {
			...
			let mut arg = if let Some(args) = node.args.take() {
				...
				Some(self.visit_elements(
					&mut args,
					Some(ExprOrSpread { spread: None, expr: Expr::undefined(DUMMY_SP) }),
					None,
				))
			} else {
				None
			...
			let apply = callee.apply(node.span, this_arg, arg.take().map(|v| v.as_arg()).into_iter().collect());

			*node = NewExpr { span: node.span, callee: Box::new(apply), args: None, ..Default::default() };
			return;
		}
		...
		if let Some(VarDeclOrExpr::VarDecl(initializer)) = &mut node.init {
			...
			node.init = if exprs.is_empty() {
				...
				Some(VarDeclOrExpr::Expr(if exprs.len() == 1 {
					...
					SeqExpr { span: DUMMY_SP, exprs }.into()
				}))
			};
			node.test.visit_mut_with(self);
...
impl Generator {
			...
			self.emit_assignment(
				...
				ArrayLit {
					...
					elems: leading_element
						...
						.into_iter()
						.map(Some)
						.chain(elements.iter_mut().take(num_initial_elements).map(|e| e.take()))
						.collect(),
				}
				.into(),
				None,
		...
		if let Some(temp) = temp {
			CallExpr {
				...
				args: vec![ExprOrSpread {
					spread: None,
					expr: Box::new(Expr::Array(ArrayLit { span: DUMMY_SP, elems: expressions })),
				}],
				..Default::default()
			}
			.into()
			...
			ArrayLit {
				span: DUMMY_SP,
				elems: leading_element.take().into_iter().map(Some).chain(expressions).collect(),
			}
			.into()
		}
	}
		...
		if contains_yield(&element) && !expressions.is_empty() {
			...
			self.emit_assignment(
				...
				if has_assigned_temp {
					CallExpr {
						span: DUMMY_SP,
						callee: temp.clone().unwrap().make_member(quote_ident!("concat")).as_callee(),
						args: vec![
							Box::new(Expr::Array(ArrayLit { span: DUMMY_SP, elems: expressions.take() })).as_arg(),
						],
						..Default::default()
					}
					.into()
					...
					Box::new(
						ArrayLit {
							span: DUMMY_SP,
							elems: leading_element.take().into_iter().map(Some).chain(expressions.take()).collect(),
						}
						.into(),
					)
				},
	...
	fn reduce_property(
		...
		{
			self.emit_stmt(ExprStmt { span: DUMMY_SP, expr: Expr::from_exprs(expressions.take()) }.into());
		}
		...
		let mut expression: Expr = match property {
			CompiledProp::Prop(p) => match p {
				...
				Prop::KeyValue(p) => AssignExpr {
					span: DUMMY_SP,
					op: op!("="),
					left: MemberExpr { span: DUMMY_SP, obj: temp.clone().into(), prop: p.key.into() }.into(),
					right: p.value,
				}
				.into(),
				Prop::Assign(_) => {
			...
			CompiledProp::Accessor(getter, setter) => {
				...
				CallExpr {
					...
					args: vec![temp.clone().as_arg(), prop_name_to_expr_value(key).as_arg(), desc.as_arg()],
					..Default::default()
				}
				.into()
			},
	...
	fn visit_logical_bin_expr(&mut self, node: &mut BinExpr) -> Expr {
		...
		node.left.visit_mut_with(self);
		self.emit_assignment(result_local.clone().into(), node.left.take(), Some(left_span));

		if node.op == op!("&&") {
			// Logical `&&` shortcuts when the left-hand operand is falsey.
			self.emit_break_when_false(result_label, Box::new(result_local.clone().into()), Some(left_span))
		} else {
			// Logical `||` shortcuts when the left-hand operand is truthy.
			self.emit_break_when_true(result_label, Box::new(result_local.clone().into()), Some(left_span))
		}

		let right_span = node.right.span();
		node.right.visit_mut_with(self);
		self.emit_assignment(result_local.clone().into(), node.right.take(), Some(right_span));
		self.mark_label(result_label);
				...
				self.emit_stmt(
					ExprStmt {
						...
						expr: if pending_expressions.len() == 1 {
							...
							SeqExpr { span: DUMMY_SP, exprs: take(&mut pending_expressions) }.into()
						},
					}
					.into(),
				)
	...
	fn transform_and_emit_for_stmt(&mut self, mut node: ForStmt) {
		if contains_yield(&node) {
			...
			if let Some(init) = node.init {
				match init {
					...
					VarDeclOrExpr::Expr(mut init) => {
						...
						self.emit_stmt(ExprStmt { span: init.span(), expr: init }.into());
					},
				}
			}
			...
			if let Some(mut incrementor) = node.update {
				...
				self.emit_stmt(ExprStmt { span: incrementor.span(), expr: incrementor }.into());
			}
	...
	fn transform_and_emit_for_in_stmt(&mut self, mut node: ForInStmt) {
		if contains_yield(&node) {
			...
			self.emit_assignment(keys_array.clone().into(), Box::new(ArrayLit::dummy().into()), None);
			...
			self.emit_stmt(
				ForInStmt {
					...
					body: Box::new(Stmt::Expr(ExprStmt {
						...
						expr: CallExpr {
							span: DUMMY_SP,
							callee: keys_array.clone().make_member(quote_ident!("push")).as_callee(),
							args: vec![key.as_arg()],
							..Default::default()
						}
						.into(),
			...
			self.emit_break_when_false(
				...
				Box::new(
					keys_index
						.clone()
						.make_bin(op!("<"), keys_array.clone().make_member(quote_ident!("length"))),
				),
				None,
			);
				...
				for (i, clause) in node.cases.iter_mut().enumerate().skip(clauses_written) {
					if clause.test.is_some() {
						...
						pending_clauses.push(SwitchCase {
							...
							cons: vec![self.create_inline_break(clause_labels[i], Some(span)).into()],
						})
					} else {
	...
	fn transform_and_emit_try_stmt(&mut self, mut node: TryStmt) {
		...
		if contains_yield(&node) {
			...
			if let Some(catch) = node.handler {
				self.begin_catch_block(VarDeclarator { name: catch.param.clone().unwrap(), ..Take::dummy() });
	...
	fn declare_local(&mut self, name: Option<Atom>) -> Ident {
		let temp = name.map(|name| private_ident!(name)).unwrap_or_else(|| private_ident!("_tmp"));
	...
	fn define_label(&mut self) -> Label {
		...
		if label.0 as usize >= self.label_offsets.as_ref().unwrap().len() {
			self.label_offsets.as_mut().unwrap().resize(label.0 as usize + 1, 0);
		}
	...
	fn mark_label(&mut self, label: Label) {
		...
		self.label_offsets.as_mut().unwrap()[label.0 as usize] = self.operations.as_deref().map_or(0, |v| v.len() as _);
	...
	fn end_block(&mut self) -> Ptr<CodeBlock> {
		...
		self.block_actions.as_mut().unwrap().push(BlockAction::Close);
		self.block_offsets
			.as_mut()
			.unwrap()
			.push(self.operations.as_ref().map_or(0, |v| v.len()));
	...
	fn begin_with_block(&mut self, expr: Ident) {
		...
		self.begin_block(CodeBlock::With(WithBlock { expression: expr, start_label, end_label }));
	}
	...
	fn begin_catch_block(&mut self, variable: VarDeclarator) {
		...
		self.emit_assignment(
			...
			CallExpr {
				span: DUMMY_SP,
				callee: self.state.clone().make_member(quote_ident!("sent")).as_callee(),
				args: Vec::new(),
				..Default::default()
			}
			.into(),
	...
	fn begin_loop_block(&mut self, continue_label: Label) -> Label {
		...
		self.begin_block(CodeBlock::Loop(LoopBlock { is_script: false, break_label, continue_label }));
		break_label
	}
	...
	fn begin_script_switch_block(&mut self) {
		self.begin_block(CodeBlock::Switch(SwitchBlock { is_script: true, break_label: Label(-1) }));
	}
	...
	fn begin_switch_block(&mut self) -> Label {
		let break_label = self.define_label();
		self.begin_block(CodeBlock::Switch(SwitchBlock { is_script: false, break_label }));
		break_label
	}
	...
	fn begin_labeled_block(&mut self, label_text: Atom) {
		let break_label = self.define_label();
		self.begin_block(CodeBlock::Labeled(LabeledBlock { is_script: false, label_text, break_label }));
	}
	...
	fn create_label(&mut self, label: Option<Label>) -> Box<Expr> {
		if let Some(label) = label {
			if label.0 > 0 {
				...
				let expr = Loc { pos: BytePos(label.0 as _), value: -1 };
				if label_expressions.get(label.0 as usize).is_none() {
					...
					label_expressions.get_mut(label.0 as usize).unwrap().push(expr);
				}
				return Invalid { span: Span::new(BytePos(label.0 as _), BytePos(label.0 as _)) }.into();
			}
		}
	...
	fn create_instruction(&mut self, instruction: Instruction) -> Number {
		...
		// );
		Number { span: DUMMY_SP, value: instruction as u16 as _, raw: None }
	}
	...
	fn create_inline_break(&mut self, label: Label, span: Option<Span>) -> ReturnStmt {
		...
		ReturnStmt {
			...
			arg: Some(ArrayLit { span: DUMMY_SP, elems: args }.into()),
		}
	}
		...
		ReturnStmt {
			...
			arg: Some(
				ArrayLit {
					...
					elems: match expr {
						Some(expr) => {
							vec![Some(self.create_instruction(Instruction::Return).as_arg()), Some(expr.as_arg())]
						},
	...
	fn emit_break_when_true(&mut self, label: Label, condition: Box<Expr>, loc: Option<Span>) {
		self.emit_worker(OpCode::BreakWhenTrue, Some(OpArgs::LabelExpr(label, condition)), loc);
	}
	...
	fn emit_break_when_false(&mut self, label: Label, condition: Box<Expr>, loc: Option<Span>) {
		self.emit_worker(OpCode::BreakWhenFalse, Some(OpArgs::LabelExpr(label, condition)), loc);
	}
	...
	fn emit_worker(&mut self, code: OpCode, args: Option<OpArgs>, loc: Option<Span>) {
		...
		self.operation_locs.as_mut().unwrap().push(loc.unwrap_or(DUMMY_SP));
	}
	...
	fn build_stmts(&mut self) -> Vec<Stmt> {
		...
		if let Some(clauses) = self.clauses.take() {
			...
			let switch_stmt = SwitchStmt { span: DUMMY_SP, discriminant: label_expr.into(), cases: clauses };
	...
	fn is_final_label_reachable(&self, op_index: usize) -> bool {
		...
		// is reachable.
		for (label, label_offset) in self.label_offsets.as_ref().unwrap().iter().copied().enumerate() {
			if label_offset as usize == op_index && self.label_exprs.as_ref().unwrap().get(label).is_some() {
				return true;
			}
		}
	...
	fn append_label(&mut self, mark_label_end: bool) {
		...
		let stmts = if let Some(mut stmts) = self.stmts.take() {
			if self.with_block_stack.is_some() {
				...
				for (_i, with_block) in self.with_block_stack.as_ref().unwrap().iter().enumerate().rev() {
					...
					stmts = vec![Stmt::With(WithStmt {
						...
						body: Box::new(Stmt::Block(BlockStmt { span: DUMMY_SP, stmts, ..Default::default() })),
					})];
				}
			}

			if cfg!(debug_assertions) {
				debug!("current_exception_block = {:?}", self.current_exception_block);
			}

			if let Some(current_exception_block) = self.current_exception_block.take() {
				let b = current_exception_block.borrow();
				let ExceptionBlock { start_label, catch_label, finally_label, end_label, .. } = match &*b {
					CodeBlock::Exception(v) => v,
					_ => {
						unreachable!()
					},
	...
	fn try_enter_label(&mut self, op_index: usize) {
		...
		for (label, label_offset) in self.label_offsets.clone().unwrap().into_iter().enumerate() {
			if label_offset as usize == op_index {
				...
				if let Some(v) = self.label_numbers.as_mut().unwrap().get_mut(self.label_number) {
					v.push(label);
				} else {
					if self.label_number >= self.label_numbers.as_ref().unwrap().len() {
						self.label_numbers.as_mut().unwrap().resize(self.label_number + 1, Vec::new());
					}
				...
				match &*b {
					CodeBlock::Exception(_) => {
						if block_action == BlockAction::Open {
							...
							#[cfg(debug_assertions)]
							debug!("Current exception block: close = {:?}", self.current_exception_block);
						}
					},
	...
	fn write_assign(&mut self, left: AssignTarget, right: Box<Expr>, op_loc: Option<Span>) {
		self.write_stmt(
			ExprStmt {
				...
				expr: AssignExpr { span: DUMMY_SP, op: op!("="), left, right }.into(),
			}
			.into(),
		)
	}
	...
	fn write_throw(&mut self, expr: Box<Expr>, op_loc: Option<Span>) {
		...
		self.write_stmt(ThrowStmt { span: op_loc.unwrap_or(DUMMY_SP), arg: expr }.into())
	}
	...
	fn write_break(&mut self, label: Label, op_loc: Option<Span>) {
		...
		self.write_stmt(
			ReturnStmt {
				span: op_loc.unwrap_or(DUMMY_SP),
				arg: Some(ArrayLit { span: DUMMY_SP, elems: vec![Some(inst.as_arg()), Some(label.as_arg())] }.into()),
			}
			.into(),
		)
	}
		...
		self.write_stmt(
			IfStmt {
				...
				cons: Box::new(Stmt::Return(ReturnStmt {
					span: op_loc.unwrap_or(DUMMY_SP),
					arg: Some(
						ArrayLit { span: DUMMY_SP, elems: vec![Some(inst.as_arg()), Some(label.as_arg())] }.into(),
					),
				})),
				alt: None,
			}
	...
	fn write_break_when_false(&mut self, label: Label, cond: Box<Expr>, op_loc: Option<Span>) {
		...
		self.write_stmt(
			IfStmt {
				span: DUMMY_SP,
				test: UnaryExpr { span: DUMMY_SP, op: op!("!"), arg: cond }.into(),
				cons: Box::new(Stmt::Return(ReturnStmt {
					span: op_loc.unwrap_or(DUMMY_SP),
					arg: Some(
						ArrayLit { span: DUMMY_SP, elems: vec![Some(inst.as_arg()), Some(label.as_arg())] }.into(),
	...
	fn write_yield(&mut self, expr: Option<Box<Expr>>, op_loc: Option<Span>) {
		...
		self.write_stmt(
			ReturnStmt {
				span: op_loc.unwrap_or(DUMMY_SP),
				arg: Some(ArrayLit { span: DUMMY_SP, elems }.into()),
			}
			.into(),
		);
	}
	...
	fn write_yield_star(&mut self, expr: Box<Expr>, op_loc: Option<Span>) {
		...
		self.write_stmt(
			ReturnStmt {
				span: op_loc.unwrap_or(DUMMY_SP),
				arg: Some(ArrayLit { span: DUMMY_SP, elems: vec![Some(arg1.as_arg()), Some(expr.as_arg())] }.into()),
			}
			.into(),
		)
	}
	...
	fn write_end_finally(&mut self) {
		...
		self.write_stmt(
			ReturnStmt {
				span: DUMMY_SP,
				arg: Some(ArrayLit { span: DUMMY_SP, elems: vec![Some(arg.as_arg())] }.into()),
			}
			.into(),
		)
	}
	...
	fn get_initialized_variables<'a>(&self, initializer: &'a mut VarDecl) -> Vec<&'a mut VarDeclarator> {
		initializer.decls.iter_mut().filter(|v| v.init.is_some()).collect()
	}
	...
	fn create_call_binding(&mut self, expr: Box<Expr>, is_new_call: bool) -> (Box<Expr>, Box<Expr>) {
		let mut callee = expr;

		match &mut *callee {
			Expr::Ident(..) => (callee.clone(), if is_new_call { callee } else { Expr::undefined(DUMMY_SP) }),
		...
		if let Expr::Invalid(Invalid { span }) = e {
			if span.lo != BytePos(0) && span.lo == span.hi {
				if let Some(Loc { value, .. }) =
					self.map.iter().flat_map(|v| v.iter()).flatten().find(|loc| loc.pos == span.lo)
				{
