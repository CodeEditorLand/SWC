	...
	W: CssWriter,
{
	...
	fn emit_rule(&mut self, n: &Rule) -> Result {
		match n {
			...
			Rule::ListOfComponentValues(n) => {
				emit!(&mut *self.with_ctx(Ctx { in_list_of_component_values: true, ..self.ctx }), n)
			},
		}
	}
	...
	fn emit_at_rule(&mut self, n: &AtRule) -> Result {
		...
		emit!(&mut *self.with_ctx(Ctx { allow_to_lowercase: true, ..self.ctx }), n.name);

		if let Some(prelude) = &n.prelude {
			emit!(&mut *self.with_ctx(Ctx { in_single_line_selectors: true, ..self.ctx }), prelude);
		}
		...
		match n {
			...
			AtRulePrelude::MediaPrelude(n) => {
				let need_space = match n.queries.first() {
					Some(media_query) if media_query.modifier.is_none() && media_query.media_type.is_none() => {
						match media_query.condition.as_deref() {
							Some(MediaConditionType::All(media_condition)) => !matches!(
								media_condition.conditions.first(),
								Some(MediaConditionAllType::MediaInParens(MediaInParens::MediaCondition(_)))
									| Some(MediaConditionAllType::MediaInParens(MediaInParens::Feature(_)))
									| Some(MediaConditionAllType::MediaInParens(MediaInParens::GeneralEnclosed(
										GeneralEnclosed::SimpleBlock(_)
									)))
							),
							_ => true,
						}
			...
			AtRulePrelude::SupportsPrelude(n) => {
				let need_space = !matches!(
					n.conditions.first(),
					Some(SupportsConditionType::SupportsInParens(SupportsInParens::SupportsCondition(_)))
						| Some(SupportsConditionType::SupportsInParens(SupportsInParens::Feature(
							SupportsFeature::Declaration(_)
						))) | Some(SupportsConditionType::SupportsInParens(SupportsInParens::GeneralEnclosed(
						GeneralEnclosed::SimpleBlock(_)
					),))
				);
			...
			AtRulePrelude::ContainerPrelude(n) => {
				let need_space = match n.name {
					...
					_ => !matches!(
						...
						Some(ContainerQueryType::QueryInParens(QueryInParens::ContainerQuery(_,)))
							| Some(ContainerQueryType::QueryInParens(QueryInParens::SizeFeature(_)))
							| Some(ContainerQueryType::QueryInParens(QueryInParens::GeneralEnclosed(
								GeneralEnclosed::SimpleBlock(_)
							),))
					),
				};
	...
	fn emit_list_of_component_values(&mut self, n: &ListOfComponentValues) -> Result {
		self.emit_list_of_component_values_inner(&n.children, ListFormat::SpaceDelimited | ListFormat::SingleLine)?;
	}
	...
	fn emit_import_layer_name(&mut self, n: &ImportLayerName) -> Result {
		match n {
			...
			ImportLayerName::Function(n) if n.value.is_empty() => {
				...
				emit!(
					self,
					AtRuleName::Ident(swc_css_ast::Ident { span: n.span, value: "layer".into(), raw: None })
				)
			},
	...
	fn emit_keyframe_selector(&mut self, n: &KeyframeSelector) -> Result {
		match n {
			KeyframeSelector::Ident(n) => {
				emit!(&mut *self.with_ctx(Ctx { allow_to_lowercase: true, ..self.ctx }), n)
			},
	...
	fn emit_media_query(&mut self, n: &MediaQuery) -> Result {
		if n.modifier.is_some() {
			emit!(&mut *self.with_ctx(Ctx { allow_to_lowercase: true, ..self.ctx }), n.modifier);

			space!(self);
		}

		if n.media_type.is_some() {
			emit!(&mut *self.with_ctx(Ctx { allow_to_lowercase: true, ..self.ctx }), n.media_type);
	...
	fn emit_page_selector_pseudo(&mut self, n: &PageSelectorPseudo) -> Result {
		...
		emit!(&mut *self.with_ctx(Ctx { allow_to_lowercase: true, ..self.ctx }), n.value);
	}
	...
	#[emitter]
	fn emit_document_prelude_matching_function(&mut self, n: &DocumentPreludeMatchingFunction) -> Result {
		match n {
	...
	fn emit_list_of_component_values_inner(&mut self, nodes: &[ComponentValue], format: ListFormat) -> Result {
		let iter = nodes.iter();
				...
				let need_delim = match node {
					...
					| ComponentValue::TimePercentage(_) => match next {
						Some(ComponentValue::Delimiter(delimiter))
							if matches!(**delimiter, Delimiter { value: DelimiterValue::Comma, .. }) =>
						{
							false
						},
					...
					ComponentValue::Color(color)
						if matches!(
							**color,
							Color::AbsoluteColorBase(AbsoluteColorBase::Function(_)) | Color::Function(_)
						) =>
					{
						match next {
							Some(ComponentValue::Delimiter(delimiter))
					...
					ComponentValue::Ident(_) | ComponentValue::DashedIdent(_) => match next {
						...
						Some(ComponentValue::Color(color))
							if matches!(**color, Color::AbsoluteColorBase(AbsoluteColorBase::HexColor(_),)) =>
						{
							!self.config.minify
						},
						...
						Some(component_value) if self.config.minify => {
							if let Some(minified) = match component_value {
								ComponentValue::LengthPercentage(p) => p.as_length().map(|l| l.value.value),

								ComponentValue::FrequencyPercentage(p) => p.as_frequency().map(|f| f.value.value),

								ComponentValue::AnglePercentage(p) => p.as_angle().map(|a| a.value.value),

								ComponentValue::TimePercentage(p) => p.as_time().map(|t| t.value.value),
	...
	fn emit_function(&mut self, n: &Function) -> Result {
		...
		self.emit_list_of_component_values_inner(&n.value, ListFormat::SpaceDelimited | ListFormat::SingleLine)?;
	...
	fn emit_simple_block(&mut self, n: &SimpleBlock) -> Result {
		...
		for (idx, node) in n.value.iter().enumerate() {
			match node {
				...
				ComponentValue::AtRule(_) | ComponentValue::QualifiedRule(_) | ComponentValue::KeyframeBlock(_) => {
					formatting_newline!(self);
			...
			match node {
				ComponentValue::ListOfComponentValues(node) => {
					emit!(&mut *self.with_ctx(Ctx { in_list_of_component_values: true, ..self.ctx }), node);
				},
	...
	fn emit_declaration(&mut self, n: &Declaration) -> Result {
		...
		if is_custom_property {
			self.with_ctx(Ctx { in_list_of_component_values: true, ..self.ctx })
	...
	fn emit_length(&mut self, n: &Length) -> Result {
		...
		emit!(
			&mut *self.with_ctx(Ctx { is_dimension_unit: true, allow_to_lowercase: true, ..self.ctx }),
			n.unit
		);
	}
	...
	fn emit_token_and_span(&mut self, n: &TokenAndSpan) -> Result {
		...
		match &n.token {
			...
			Token::Dimension(token) => {
				let mut dimension = String::with_capacity(token.raw_value.len() + token.raw_unit.len());
	...
	fn emit_unicode_range(&mut self, n: &UnicodeRange) -> Result {
		let mut value =
			String::with_capacity(n.start.len() + if let Some(end) = &n.end { end.len() + 1 } else { 0 } + 2);
	...
	#[emitter]
	fn emit_forgiving_relative_selector_list(&mut self, n: &ForgivingRelativeSelectorList) -> Result {
		for (idx, node) in n.children.iter().enumerate() {
			if idx != 0 {
				write_raw!(self, ",");
	...
	fn emit_complex_selector(&mut self, n: &ComplexSelector) -> Result {
		for (idx, node) in n.children.iter().enumerate() {
			...
			match node {
				ComplexSelectorChildren::Combinator(Combinator { value: CombinatorValue::Descendant, .. }) => {},
	...
	fn emit_pseudo_class_selector_children(&mut self, n: &PseudoClassSelectorChildren) -> Result {
		match n {
			...
			PseudoClassSelectorChildren::SelectorList(n) => {
				emit!(&mut *self.with_ctx(Ctx { in_single_line_selectors: true, ..self.ctx }), n)
			},
			PseudoClassSelectorChildren::ForgivingSelectorList(n) => {
				emit!(&mut *self.with_ctx(Ctx { in_single_line_selectors: true, ..self.ctx }), n)
			},
	...
	fn emit_list_pseudo_class_selector_children(&mut self, nodes: &[PseudoClassSelectorChildren]) -> Result {
		let len = nodes.len();
	...
	#[emitter]
	fn emit_pseudo_element_selector_children(&mut self, n: &PseudoElementSelectorChildren) -> Result {
		match n {
	...
	fn emit_list_pseudo_element_selector_children(&mut self, nodes: &[PseudoElementSelectorChildren]) -> Result {
		let len = nodes.len();
...
fn serialize_dimension_unit(value: &str) -> Cow<'_, str> {
	// Fast-path
	let need_escape = (value.len() >= 2 && value.as_bytes()[0] == b'e' && value.as_bytes()[1].is_ascii_digit())
