...
pub fn is_global_var_with_pure_property_access(s: &str) -> bool {
	match s {
		"JSON" | "Array" | "Set" | "Map" | "String" | "Object" | "Number" | "Date" | "BigInt" | "Boolean" | "Math"
		| "Error" | "Reflect" => return true,
		_ => {},
	}
...
pub fn can_end_conditionally(s: &Stmt) -> bool {
	...
	fn can_end(s: &Stmt, ignore_always: bool) -> bool {
		match s {
			Stmt::If(s) => can_end(&s.cons, false) || s.alt.as_deref().map(|s| can_end(s, false)).unwrap_or_default(),

			Stmt::Switch(s) => s.cases.iter().any(|case| case.cons.iter().any(|s| can_end(s, false))),
