...
use swc_core::{
	...
	common::{GLOBALS, Globals, Span},
	ecma::{
		ast::{
			Bool, Expr, IdentName, KeyValueProp, Lit, MemberExpr, MemberProp, MetaPropExpr, MetaPropKind, PropName, Str,
		},
		loader::{NODE_BUILTINS, TargetEnv},
	},
...
impl Task for BundleTask {
	...
	fn compute(&mut self) -> napi::Result<Self::Output> {
		let builtins = if let TargetEnv::Node = self.config.static_items.config.target {
			NODE_BUILTINS.iter().copied().map(Atom::from).collect::<Vec<_>>()
		} else {
			Vec::new()
		};
		...
		GLOBALS.set(&globals, || {
			...
			if let Some(s) = err.downcast_ref::<String>() {
				return Err(napi::Error::new(Status::GenericFailure, format!("panic detected: {}", s)));
			}
...
pub(crate) fn bundle(conf_items: Buffer, signal: Option<AbortSignal>) -> napi::Result<AsyncTask<BundleTask>> {
	...
	let loader = Box::new(swc_core::bundler::node::loaders::swc::SwcLoader::new(
		...
		static_items
			...
			.as_ref()
			.cloned()
			.unwrap_or_else(|| serde_json::from_value(serde_json::Value::Object(Default::default())).unwrap()),
	));
...
impl swc_core::bundler::Hook for Hook {
	fn get_import_meta_props(&self, span: Span, module_record: &ModuleRecord) -> Result<Vec<KeyValueProp>, Error> {
			...
			KeyValueProp {
				...
				value: Box::new(if module_record.is_entry {
					Expr::Member(MemberExpr {
						span,
						obj: Box::new(Expr::MetaProp(MetaPropExpr { span, kind: MetaPropKind::ImportMeta })),
