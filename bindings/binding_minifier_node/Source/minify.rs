...
use swc_compiler_base::{IdentCollector, PrintArgs, SourceMapsConfig, TransformOutput, minify_file_comments, parse_js};
	...
	fn to_file(&self, cm: Lrc<SourceMap>) -> Lrc<SourceFile> {
		match self {
			...
			MinifyTarget::Map(codes) => {
				assert_eq!(codes.len(), 1, "swc.minify does not support concatting multiple files yet");
...
fn do_work(input: MinifyTarget, options: JsMinifyOptions, extras: JsMinifyExtras) -> napi::Result<TransformOutput> {
	...
	try_with(cm.clone(), false, |handler| {
		...
		let source_map_names = if source_map.enabled() {
			let mut v = IdentCollector { names: Default::default() };
		...
		minify_file_comments(&comments, preserve_comments, options.format.preserve_annotations);
		...
		swc_compiler_base::print(
			...
			PrintArgs {
				...
				codegen_config: swc_core::ecma::codegen::Config::default()
					...
					.with_emit_assert_for_import_attributes(options.format.emit_assert_for_import_attributes),
				..Default::default()
			},
		)
	})
...
#[napi]
fn minify(code: Buffer, opts: Buffer, extras: NapiMinifyExtra, signal: Option<AbortSignal>) -> AsyncTask<MinifyTask> {
	crate::util::init_default_trace_subscriber();
	let code = String::from_utf8_lossy(code.as_ref()).to_string();
	let options = String::from_utf8_lossy(opts.as_ref()).to_string();
	let extras = JsMinifyExtras::default().with_mangle_name_cache(extras.mangle_name_cache.as_deref().cloned());

	let task = MinifyTask { code, options, extras };

	AsyncTask::with_optional_signal(task, signal)
}

#[napi]
pub fn minify_sync(code: Buffer, opts: Buffer, extras: NapiMinifyExtra) -> napi::Result<TransformOutput> {
	crate::util::init_default_trace_subscriber();
