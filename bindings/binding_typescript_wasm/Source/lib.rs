...
use swc_error_reporters::{PrettyEmitterConfig, to_miette_source_code, to_miette_span};
	...
	fn emit(&mut self, db: &DiagnosticBuilder) {
		...
		let snippet = d.span.primary_span().and_then(|span| {
			...
			match self.reporter.render_report(
				&mut snippet,
				&Snippet {
					source_code: &to_miette_source_code(&self.cm, &PrettyEmitterConfig { skip_filename: true }),
					span,
				},
			) {
				Ok(()) => Some({
...
impl miette::Diagnostic for Snippet<'_> {
	...
	fn labels(&self) -> Option<Box<dyn Iterator<Item = miette::LabeledSpan> + '_>> {
		Some(Box::new(once(LabeledSpan::new_with_span(None, to_miette_span(self.span)))))
	}
}
	...
	for line in lines {
		...
		if let Some(pos) = line.find('[') {
			if let Some(end_pos) = line[pos..].find(']') {
				...
				if bracket_content.chars().all(|c| c.is_ascii_digit() || c == ':') {
		...
		if !trimmed.is_empty() && trimmed.chars().next().unwrap().is_ascii_digit() {
			...
			if digit_end < trimmed.len() && trimmed[digit_end..].starts_with(|c: char| c.is_whitespace()) {
				// Count trailing whitespace
				let trailing_space_end = trimmed[digit_end..].chars().take_while(|&c| c.is_whitespace()).count();
